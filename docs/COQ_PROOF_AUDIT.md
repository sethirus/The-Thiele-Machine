...
- Promoted `thielemachine/coqproofs/HyperThiele_Halting.v` into `_CoqProject` so every `make core` run compiles the concrete oracle witness, and extended `Subsumption.v` with `thiele_formally_subsumes_turing_with_hyperoracle` to keep the containment/separation theorem aligned with the halting trace equivalence under `H_correct`.【F:coq/_CoqProject†L1-L70】【F:coq/thielemachine/coqproofs/Subsumption.v†L1-L86】
