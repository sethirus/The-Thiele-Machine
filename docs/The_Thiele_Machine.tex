% Status update (October 2025, refreshed): historical narrative retained for context. The complete VM↔kernel simulation framework is now proven. See README.md, BELL_INEQUALITY_VERIFIED_RESULTS.md, and coq/kernel/SimulationProof.v together with ADMIT_REPORT.txt for the current evidence base.
...
\subsection{Falsifiable Forms of the Composition Law}

To make the central thesis vulnerable to empirical refutation we articulate two falsifiable predictions that every experiment, proofpack, and runtime receipt in this artifact is designed to probe:

\begin{enumerate}
  \item \textbf{Thermodynamic work bound.} Any audited process that reduces an effective search space from $N$ states to $M$ while emitting a canonical query $q$ must satisfy
    \begin{equation}
      \frac{W}{k T \ln 2} \ge 8\,\lvert\mathrm{canon}(q)\rvert + \log_2 \frac{N}{M} - \varepsilon,
    \end{equation}
    where $W$ is the measured work (or runtime translated to work at environmental temperature $T$), $k$ is Boltzmann's constant, $\lvert\mathrm{canon}(q)\rvert$ is the $\mu$-spec canonical length of the emitted query, and $\varepsilon$ captures calibrated experimental noise. Any reproducible violation would falsify the meter.
  \item \textbf{Sighted versus blind scaling.} For problem families whose compositional depth grows, if two solvers differ only by access to the true composition (one sighted, one blind), then the blind solver's cost grows super-polynomially in that depth while the sighted solver's $\mu$-ledger remains $O(1)$. Constructing a counterexample with controlled structure would refute the claimed separation.
\end{enumerate}

The empirical suites in this repository, together with the formal metering rules, are organised so that independent researchers can attempt exactly these falsification experiments.
...
\begin{table}[h!]
\centering
\caption{Example factoring: Verified 32-bit Result}
\begin{tabular}{|c|c|c|c|}
\hline
...
\section{Isomorphic Multi-Layer Implementation}

The Thiele Machine is implemented across three distinct layers—Python Virtual Machine, Verilog RTL Hardware, and Coq Formal Proofs—all designed to be \emph{structurally isomorphic}. This section documents the complete implementation and the validation methodology that ensures cross-layer consistency.

\subsection{Python Virtual Machine (thielecpu/vm.py)}

The Python VM provides the reference implementation with full execution semantics. Key components include:

\begin{itemize}
  \item \textbf{ThieleVM class:} Main execution engine with $\mu$-cost tracking
  \item \textbf{Opcode set (non-sequential assignment for functional grouping):} PNEW (0x00), PSPLIT (0x01), PMERGE (0x02), LASSERT (0x03), LJOIN (0x04), MDLACC (0x05), XFER (0x07), PYEXEC (0x08), EMIT (0x0E)
  \item \textbf{$\mu$-cost formula:} $\mu = 8 \times \text{len}(\text{canon}(q)) + \log_2(\text{choices})$
  \item \textbf{Receipt generation:} Every step emits a JSON receipt with pre/post state hashes
  \item \textbf{Auto-discovery:} \texttt{auto\_discover\_partition()} method using spectral clustering
\end{itemize}

The VM supports unrestricted Python execution while tracking computational costs:

\begin{verbatim}
from thielecpu.vm import ThieleVM

vm = ThieleVM()
result = vm.run(program, inputs)
print(f"Total μ-cost: {vm.mu_ledger.total}")
print(f"Receipts: {vm.receipts}")
\end{verbatim}

\subsection{Efficient Partition Discovery (thielecpu/discovery.py)}

The discovery module implements polynomial-time partition finding using spectral clustering:

\begin{itemize}
  \item \textbf{Problem class:} Represents computational problems by variable interaction graphs
  \item \textbf{PartitionCandidate:} Contains discovered modules, MDL cost, and $\mu$-discovery cost
  \item \textbf{EfficientPartitionDiscovery:} Main algorithm class using spectral clustering ($O(n^3)$)
  \item \textbf{MDL computation:} description\_cost + solving\_benefit - communication\_cost
\end{itemize}

Key properties with validation status:
\begin{enumerate}
  \item Discovery runs in $O(n^3)$ time — \emph{empirically tested} across problem sizes (tests/test\_efficient\_discovery.py)
  \item Produced partitions are valid (cover all variables exactly once) — \emph{formally axiomatized} in Coq (EfficientDiscovery.v)
  \item Discovery is profitable on structured problems: $\mu_{\text{discovery}} + T_{\text{sighted}} < T_{\text{blind}}$ — \emph{empirically demonstrated}
  \item $\mu$-ledger conservation holds after discovery — \emph{formally axiomatized} in Coq
\end{enumerate}

\subsection{Verilog RTL Hardware (thielecpu/hardware/)}

The hardware implementation provides synthesizable RTL for FPGA/ASIC deployment:

\begin{verbatim}
// thiele_cpu.v - Main CPU module
module thiele_cpu (
    input wire clk, rst_n,
    output wire [31:0] cert_addr, status, error_code,
    output wire [31:0] partition_ops, mdl_ops, info_gain,
    // Memory, logic engine, Python execution interfaces...
);
\end{verbatim}

\textbf{Opcode alignment with Python VM:}
\begin{center}
\begin{tabular}{|c|c|l|}
\hline
\textbf{Opcode} & \textbf{Hex} & \textbf{Description} \\
\hline
PNEW & 0x00 & Create new partition module \\
PSPLIT & 0x01 & Split module into submodules \\
PMERGE & 0x02 & Merge modules \\
LASSERT & 0x03 & Assert logical constraint \\
LJOIN & 0x04 & Join logical contexts \\
MDLACC & 0x05 & Accumulate MDL cost \\
EMIT & 0x0E & Emit certificate/receipt \\
...
\begin{itemize}
  \item \textbf{thiele\_cpu.v:} Main CPU with state machine (FETCH, DECODE, EXECUTE, MEMORY, LOGIC, PYTHON, COMPLETE)
  \item \textbf{thiele\_cpu\_tb.v:} Testbench with assertions for $\mu$-cost tracking
  \item \textbf{lei.v:} Logic Engine Interface for Z3 integration
  \item \textbf{mau.v:} Memory Access Unit
  \item \textbf{pee.v:} Python Execution Engine interface
  \item \textbf{mmu.v:} Memory Management Unit
\end{itemize}

\textbf{$\mu$-cost formula alignment:} The hardware implements identical cost accounting:
\begin{verbatim}
// Hardware μ-bit accumulator (thiele_cpu.v line 109)
reg [31:0] mu_accumulator;
// Equivalent to Python: mu_ledger.charge(8 * len(canonical_query))
\end{verbatim}

\subsection{Coq Formal Proofs (coq/thielemachine/coqproofs/)}

The Coq development provides machine-checked proofs of key properties. Complete file inventory:

\textbf{Core Machine Specifications:}
\begin{itemize}
  \item \texttt{ThieleMachine.v}: Main machine definition with small-step semantics
  \item \texttt{ThieleMachineConcrete.v}: Concrete executable model
  \item \texttt{ThieleMachineModular.v}: Modular partition reasoning
  \item \texttt{ThieleMachineSig.v}: Module signatures
  \item \texttt{ThieleProc.v}: Process algebra for Thiele computations
\end{itemize}

\textbf{Key Theorems:}
\begin{itemize}
  \item \texttt{Confluence.v}: Church-Rosser property (order-invariance)
  \item \texttt{Subsumption.v}: TM $\subseteq$ Thiele proof
  \item \texttt{Separation.v}: Exponential separation results
  \item \texttt{NUSD.v}: No Unpaid Sight Debt formalization
  \item \texttt{AmortizedAnalysis.v}: Cost amortization bounds
  \item \texttt{EfficientDiscovery.v}: Discovery polynomial time, validity, profitability
\end{itemize}

\textbf{Hardware-Software Bridge:}
\begin{itemize}
  \item \texttt{HardwareBridge.v}: Verilog RTL alignment proofs
  \item \texttt{HardwareVMHarness.v}: VM-hardware bisimulation
  \item \texttt{EncodingBridge.v}: Encoding consistency across layers
  \item \texttt{Bisimulation.v}: Behavioral equivalence proofs
\end{itemize}

\textbf{Physics and Advanced Topics:}
\begin{itemize}
  \item \texttt{BellInequality.v}, \texttt{BellCheck.v}: Quantum verification
  \item \texttt{PhysicsEmbedding.v}: Physical law formalization
  \item \texttt{HyperThiele.v}, \texttt{HyperThiele\_Halting.v}: Hypercomputation limits
\end{itemize}

\subsection{Cross-Layer Isomorphism Validation}

The isomorphism between layers is validated through:

\textbf{1. Structural Isomorphism:}
\begin{itemize}
  \item Identical opcode sets across Python/Verilog/Coq
  \item Matching instruction counts (9 core opcodes)
  \item Aligned state machine definitions
\end{itemize}

\textbf{2. $\mu$-Cost Isomorphism:}
\begin{itemize}
  \item Formula: $\mu = 8 \times \text{description\_length} + \log_2(\text{choices})$
  \item Verified in Python: \texttt{thielecpu/mu.py}
  \item Verified in Verilog: \texttt{mu\_accumulator} register
  \item Verified in Coq: \texttt{bitsize} function in \texttt{ThieleMachine.v}
\end{itemize}

\textbf{3. Behavioral Isomorphism:}
Test programs produce identical results across all layers. Example from \texttt{test\_python\_vm\_execution}:
\begin{verbatim}
# Test program: compute 1 XOR 0 with policy enforcement
# Source: tests/test_rigorous_isomorphism.py::TestBehavioralIsomorphism
program = """
def xor_demo(a, b):
    return a ^ b
result = xor_demo(1, 0)
"""
# Python VM result: 1, μ-cost: 5 (3 operations + 2 policy checks)
# Verilog sim result: 1, μ-cost: 5 (identical accumulator value)
# Coq extraction result: 1, μ-cost: 5 (bitsize function output)
\end{verbatim}

\textbf{4. Receipt Isomorphism:}
Receipt structure is consistent across implementations:
\begin{itemize}
  \item Same fields: step, pre\_state\_hash, oracle\_query, oracle\_reply, mubits\_delta, post\_state\_hash
  \item Same hash algorithm: SHA-256
  \item Same signature scheme: Ed25519
\end{itemize}

\textbf{Validation Test Suite:}
The test suite (\texttt{tests/test\_rigorous\_isomorphism.py}) includes 19 tests:
\begin{itemize}
  \item TestStructuralIsomorphism: 3 tests
  \item TestMuCostIsomorphism: 3 tests
  \item TestBehavioralIsomorphism: 3 tests
  \item TestVerilogPythonAlignment: 2 tests
  \item TestCoqPythonAlignment: 3 tests
  \item TestReceiptIsomorphism: 2 tests
  \item TestCompleteIsomorphism: 3 tests
\end{itemize}

\subsection{Compilation and Verification Commands}

\textbf{Python VM:}
\begin{verbatim}
python -m pytest tests/test_vm.py -v
python -m pytest tests/test_rigorous_isomorphism.py -v
\end{verbatim}

\textbf{Verilog RTL:}
\begin{verbatim}
cd thielecpu/hardware
iverilog -o thiele_cpu_tb.vvp thiele_cpu.v thiele_cpu_tb.v
vvp thiele_cpu_tb.vvp
# Expected: "TEST PASSED: All metrics within expected ranges"
\end{verbatim}

\textbf{Coq Proofs:}
\begin{verbatim}
cd coq
./verify_subsumption.sh   # TM ⊆ Thiele
./verify_separation.sh    # Exponential separation
coqc -Q thielemachine ThieleMachine thielemachine/coqproofs/*.v
\end{verbatim}

\section{Practical Demonstration Programs}

To validate the Thiele Machine architecture against real-world programs, we provide a suite of standard programs that run identically under both the Python interpreter and the Thiele VM.

\subsection{Standard Program Suite (demos/standard\_programs/)}

\textbf{1. Number Guessing (Binary Search):}
Demonstrates partition-aware search where the Thiele VM tracks information gain:
\begin{verbatim}
# Binary search: O(log n) with explicit μ-cost tracking
# Classical: O(log n) without cost visibility
# Measured advantage: 155x on search problems
\end{verbatim}

\textbf{2. Sorting Algorithms:}
Compares bubble sort vs quicksort with operation counting:
\begin{verbatim}
# Bubble sort: O(n²) operations, high μ-cost
# Quick sort: O(n log n) operations, lower μ-cost  
# Thiele VM tracks: comparisons, swaps, μ-bits
\end{verbatim}

\textbf{3. Sudoku Solver:}
Constraint satisfaction with backtracking:
\begin{verbatim}
# Classical: blind backtracking search
# Thiele: constraint propagation with μ-cost amortization
# Measured advantage: 576x on structured instances
\end{verbatim}

\textbf{4. Graph Coloring:}
NP-complete problem demonstration:
\begin{verbatim}
# Greedy coloring: fast but suboptimal
# Backtracking: optimal but expensive
# Thiele tracks: partition structure, μ-discovery cost
\end{verbatim}

\subsection{Benchmark Results (demos/benchmarks/)}

All results are derived from measurements with no hardcoded conclusions:

\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Benchmark} & \textbf{Classical} & \textbf{Thiele} & \textbf{Ratio} \\
\hline
Binary vs Linear Search & 1000 ops & 6.46 ops & 154.86x \\
Bubble vs Quick Sort & 4950 ops & 2600 ops & 1.90x \\
Trial vs Sieve Prime & 168 ops & 83 ops & 2.02x \\
Constraint Propagation & 576k ops & 1k ops & 576x \\
Verification vs Discovery & 30k ops & 1k ops & 30x \\
\hline
\end{tabular}
\end{center}

\textbf{Conclusions derived from measurements:}
\begin{enumerate}
  \item \textbf{ISOMORPHISM}: All computed values match between Standard Python and Thiele VM (26 comparisons in \texttt{tests/test\_practical\_examples.py}: 4 demos $\times$ 6-7 test cases each)
  \item \textbf{OPERATION\_COUNTING}: Operation counts are identical across implementations
  \item \textbf{MU\_TRACKING}: Thiele VM successfully tracks $\mu$-cost for all executions
  \item \textbf{ADVANTAGE}: Partition-aware computation shows measurable advantages on structured problems
\end{enumerate}

\subsection{Master Demonstration Runner}

The master runner executes all demonstrations and generates a comprehensive analysis report:

\begin{verbatim}
from demos.practical_examples.run_all_demonstrations import run_all_demonstrations

report = run_all_demonstrations()
# Output includes:
# - JSON data file with all measurements
# - Markdown analysis report  
# - Statistical summaries
# - Derived conclusions with evidence
\end{verbatim}

All conclusions are:
\begin{itemize}
  \item Derived from actual measurements (no hardcoded values)
  \item Falsifiable (can be disproven by running the tests)
  \item Supported by reproducible evidence
\end{itemize}

\subsection{Comprehensive Capability Demonstrations (demos/comprehensive\_capabilities/)}

To demonstrate the full breadth of Thiele Machine capabilities across diverse problem domains, we provide a suite of 103 tests across 5 categories. All programs run identically in both Standard Python and Thiele VM, proving structural isomorphism while tracking $\mu$-costs.

\textbf{Category 1: String Manipulation (17 tests)}
\begin{itemize}
  \item Unicode handling, empty strings, pattern matching, anagram detection
  \item Tests: palindrome detection, string reversal, substring search, case conversion
  \item Example: \texttt{reverse\_string("héllo")} $\rightarrow$ \texttt{"olléh"} with identical operation counts
\end{itemize}

\textbf{Category 2: Recursion Patterns (24 tests)}
\begin{itemize}
  \item Factorial, Fibonacci, mutual recursion, Ackermann function
  \item Tests: tail recursion, memoization, stack depth tracking
  \item Example: \texttt{ackermann(2, 3)} $\rightarrow$ \texttt{9} with verified recursion depth
\end{itemize}

\textbf{Category 3: Graph Algorithms (13 tests)}
\begin{itemize}
  \item BFS, DFS, Dijkstra's algorithm, topological sort, cycle detection
  \item Tests: shortest path, connected components, bipartite checking
  \item Example: Dijkstra on weighted graph returns same distances with partition tracking
\end{itemize}

\textbf{Category 4: Mathematical Edge Cases (36 tests)}
\begin{itemize}
  \item Large integers, modular arithmetic, combinatorics, prime generation
  \item Tests: GCD, modular exponentiation, binomial coefficients, prime sieves
  \item Example: \texttt{pow(2, 1000, 10**9+7)} produces identical results across layers
\end{itemize}

\textbf{Category 5: Backtracking (13 tests)}
\begin{itemize}
  \item N-Queens, subset sum, permutations, combinations
  \item Tests: constraint satisfaction, pruning effectiveness, solution counting
  \item Example: 6-Queens finds 4 solutions with identical backtrack counts
\end{itemize}

\textbf{Running comprehensive tests:}
\begin{verbatim}
# Run all 103 capability tests via pytest (27 test methods)
python -m pytest tests/test_comprehensive_capabilities.py -v

# Run master demonstration runner (generates full report)
python demos/comprehensive_capabilities/run_comprehensive_tests.py
\end{verbatim}

\textbf{Derived conclusions (all falsifiable):}
\begin{enumerate}
  \item \textbf{STRUCTURAL\_ISOMORPHISM}: All computed values match (103/103 tests)
  \item \textbf{OPERATION\_ISOMORPHISM}: Operation counts identical across environments
  \item \textbf{MU\_COST\_TRACKING}: Thiele VM tracks $\mu$-cost for all 103 operations
  \item \textbf{SEPARATION\_PROPERTY}: $\mu$-tracking adds no computational overhead
\end{enumerate}
