...
- The repository does not ship private keys. On first use the tooling generates a local keypair in `kernel_secret.key` (private) and `kernel_public.key` (public) with restrictive filesystem permissions. If you need to supply your own keys, place them at those paths or set the `THIELE_KERNEL_SIGNING_KEY`/`THIELE_KERNEL_VERIFY_KEY` environment variables.
- Signatures created **before commit 2f783ee** used a private key that was bundled with the repository. They should be treated as demonstrative only and must not be relied on for authenticity.
