...
- `make -C coq -j2` still times out in `thielemachine/verification/ThieleUniversalBridge.v` when replaying the archived trace, though the FindRule transition lemmas themselves are now proved using the program-prefix rewrites and fast steppers. The verification target remains quarantined behind `make bridge`/`make verification`, and the default goal for `make -C coq` is the audited `core` tier to keep CI green while the archive replay is optimised.【35dec9†L1-L38】
...
4. **Plan the bridge repairs** – although the FindRule transition lemmas are now proved, the archived trace replay in `thielemachine/verification/ThieleUniversalBridge.v` still times out. Capture the remaining invariants and execution traces before attempting further feature work, and use `make -C coq bridge-timed BRIDGE_TIMEOUT=<seconds>` to bound wall-clock time while iterating on the proof scripts.【0249db†L47-L63】【0cb618†L1-L3】 The new program-prefix lookup lemmas and `state_eqb`/`check_transition` reflection helpers allow heavy `run_n` traces to be discharged with computation rather than fragile symbolic expansion.
