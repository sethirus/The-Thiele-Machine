...
\begin{enumerate}
    ...
    The theory claims that partition geometry alone, without explicit revelation, remains local. CHSH experiments verify that the manifold projection mechanism respects the classical bound ($S \le 2$) in the absence of signaling, which means causal isolation is preserved.
    
    \item \textbf{Is the implementation practical?} \\
    A beautiful theory that runs too slowly is useless. Performance and resource utilization are benchmarked to assess practicality and check whether the hardware fits within standard FPGA constraints (Lattice ECP5).
...
The validation suite enforces strict causal isolation, so geometric projections do not produce signaling in the tested regime.
