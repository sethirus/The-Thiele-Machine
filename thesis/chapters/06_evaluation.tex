...
\begin{itemize}
    ...
    \item \textbf{"mem": [<4096 integers>]} --- All 4,096 memory words. Element-by-element comparison.
...
\textbf{What is this test?} This is a \textbf{randomized property test} that verifies the \textbf{$\mu$-ledger monotonicity property}: the $\mu$ value never decreases during VM execution. The actual test suite contains \texttt{test\_mu\_never\_decreases} (in \path{tests/test_mu.py}) and the Hypothesis-based property fuzzer \texttt{test\_mu\_non\_negative} (in \path{tests/test_hypothesis_cross_layer.py}). Both test the operational implementation of the formal theorem \texttt{mu\_conservation\_kernel} from Chapter 5.
...
\textbf{MuLedger implementation:} In the Python VM, the ledger is split into two components (see \texttt{MuLedger} in \path{thielecpu/vm.py}):
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Cross-layer bisimulation}: end-to-end validation spanning Python VM and Verilog RTL across all 31 opcodes, plus OCaml-vs-Python state identity, I/O ports, heap ops, Bianchi freeze, and trap routing (\path{tests/test_cross_layer_bisimulation.py}, 47 tests).
...
In total the test suite contains 35 test files collecting 478 tests. Running the full suite:
...
\paragraph{Structural heat.} Generate the artifact JSON and the scaling sweep (script archived at \path{archive/scripts\_unused/structural\_heat\_experiment.py}):
\begin{lstlisting}
...
\begin{itemize}
    ...
    \item Output: a JSON file containing $n$, $\log_2(n!)$, charged $\mu$, slack, and verification status.
\end{itemize}
...
\paragraph{Time dilation.} Generate the artifact JSON (script archived at \path{archive/scripts\_unused/time\_dilation\_experiment.py}):
\begin{lstlisting}
...
\begin{enumerate}
    ...
    \item \textbf{Test Coverage}: 35 test files collect 478 tests, spanning cross-layer, co-simulation, falsifiability, and adversarial suites
...
The empirical results validate the theoretical claims: the Thiele Machine enforces structural accounting as a machine invariant, verified across all three implementation layers.
