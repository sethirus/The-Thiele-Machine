...
\begin{enumerate}
    ...
    A beautiful theory that runs too slowly is useless. Performance and resource utilization are benchmarked to assess practicality, verifying that the hardware places-and-routes cleanly on a commodity open-source FPGA target (Xilinx Artix-7 / Arty A7-35T via the openXC7 toolchain).
...
\begin{itemize}
    ...
    \item \textbf{"regs": [<16 integers>]} — All 16 general-purpose registers. The isomorphism test compares these element-by-element.
    \item \textbf{"mem": [<128 integers>]} --- All $128$ memory words. Element-by-element comparison.
...
\textbf{Gate-key enforcement:} The first test confirms that CHSH\_TRIAL without the logic gate key (\texttt{0xCAFEEACE}) is rejected with \texttt{ERR\_LOGIC\_VAL} (\texttt{0xC43471A1} in \texttt{coq/kami\_hw/ThieleTypes.v}). The second confirms that with the gate key, the trial succeeds and charges the declared $\mu$.
...
The four $\Omega \to \Omega'$ traces were executed with the bridge harness, exporting a JSON artifact. The runs charge $\mu$ via partition discovery only (explicit \texttt{MDLACC} omitted to mirror the hardware harness) and capture normalization flags plus the strict-evidence acceptance condition for $\mu$ propagation across layers. Each scenario fails fast if the requested region is not representable by the hardware encoding. These runs are intended to validate that the ledger and trace machinery produce consistent, reproducible $\mu$ values that a future physical experiment can bind to energy.
...
\]
The implementation enforces the revelation rule by charging an explicit information cost (rounded up to the next integer bit) at the partition-discovery step:
\[
...
\begin{itemize}
    \item \textbf{Logic cells (LUT1--LUT6 + INV): $\sim$13{,}648} on the
          canonical RTL with REG\_COUNT=16, MEM\_SIZE=128, PTableSz=64.
          The partition graph, ALU, instruction decoder, $\mu$-ledger,
          and locality logic all fit inside that budget.
    \item \textbf{Flip-Flops: $\sim$3{,}580} (FDRE/FDSE).
    \item \textbf{Distributed RAM: 64 RAM64M cells} (partition / descriptor tables).
    \item \textbf{Block RAM: 2 RAMB36E1} (instruction / data memories).
    \item \textbf{Target: Xilinx Artix-7 (Digilent Arty A7-35T,
          \texttt{xc7a35tcsg324-1})}. Total device capacity:
          $\sim$20{,}800 LUTs, $\sim$41{,}600 FFs, $90$ BRAMs, $90$ DSPs.
          The synthesized design uses $\sim 66\%$ of the LUT budget,
          leaving the rest of the chip free for board peripherals,
          debug logic, or scaling the kernel constants up.
\end{itemize}
...
\begin{itemize}
    \item Logic cells: $\sim$2{,}500
    \item Flip-Flops: $\sim$1{,}200
    \item Target: Xilinx Artix-7 (Arty A7-35T) via openXC7
\end{itemize}
...
\textbf{Parameters:}
\begin{itemize}
    \item \textbf{NUM\_MODULES = 64} — Maximum number of partition modules in the Kami specification. With 64 modules, the on-chip module-ID field uses 6 bits and module wraparound (\texttt{m mod 64}) keeps the live ID space bounded. The Python wrapper carries no parallel bitmask field; cross-layer comparison happens at the runner-output level, not at this hardware-internal encoding.
...
\begin{itemize}
    ...
    \item \textbf{Target: Xilinx Artix-7 (large) — \texttt{xc7a100t}} ($\sim$63{,}400 LUTs) or \texttt{xc7a200t} ($\sim$215{,}000 LUTs) via the same \texttt{nextpnr-xilinx} backend; the chip-database step is the only thing that retargets when scaling to a bigger device.
\end{itemize}
...
\begin{itemize}
    ...
    \item Target: Xilinx Artix-7 (large, e.g., \texttt{xc7a100t} / \texttt{xc7a200t}) via openXC7
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Verilog co-simulation}: direct Python-to-RTL comparison via Icarus Verilog testbenches, covering compute, partition, and error-handling paths (\path{tests/test_verilog_cosim.py}, 38 tests as of this writing).
