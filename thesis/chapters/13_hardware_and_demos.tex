...
The $\mu$-ALU is a specialized arithmetic unit for cost accounting. \textbf{Note:} The module listing below is illustrative/conceptual. No standalone \texttt{mu\_alu.v} file exists; the actual $\mu$-accounting logic is integrated within the Kami-generated RTL (\texttt{thiele\_cpu\_kami.v}).
\begin{lstlisting}
module mu_alu (
    input wire clk,
    input wire rst_n,
...
Key property: \textbf{$\mu$ only increases} at the ledger boundary. The $\mu$-ALU logic is integrated within the Kami-generated RTL (\texttt{thiele\_cpu\_kami.v}), while the $\mu$-core enforces the monotonicity policy by gating ledger updates so that any decreasing update is rejected. (Note: There is no standalone \texttt{mu\_alu.v} file; the description above is conceptual.)
...
\begin{itemize}
    ...
    \item \textbf{mem:} Memory contents from RTL (\texttt{cpu.mem.memory[0:65535]} array, 65,536 words $\times$ 32 bits each).
