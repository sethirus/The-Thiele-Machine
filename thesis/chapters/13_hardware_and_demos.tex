...
\begin{quote}
\textit{Author's Note (Devon): I cannot tell you how satisfying it was to see the Verilog simulation output match the Python VM match the Coq extraction. Three implementations---with the Python layer delegating to the extracted OCaml runner rather than reimplementing semantics---written across three different languages, producing the \textbf{same answer}. That's not luck. That's not coincidence. That's what happens when your theory is actually correct. Or at least, correct enough to survive three different ``mechanics'' checking the same engine.}
\end{quote}
