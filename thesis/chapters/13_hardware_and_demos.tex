...
The key insight is that the $\mu$-ledger's monotonicity is not just a theorem---it is \textit{physically enforced} by the hardware. The $\mu$-core gates ledger updates and rejects any proposed cost update that would decrease the accumulated value (see \texttt{thielecpu/hardware/rtl/mu\_core.v}). This makes $\mu$-decreasing transitions architecturally invalid rather than merely discouraged by software.
...
The complete RTL consists of 7 synthesizable source files and 8 testbench files (15 Verilog files total):
...
\hline
\texttt{thiele\_cpu\_unified.v} & 1,413 & Unified fetch/decode/execute pipeline \\
\texttt{partition\_core.v} & 251 & PNEW/PSPLIT/PMERGE partition operations \\
\texttt{chsh\_partition.v} & 330 & CHSH supra-quantum distribution engine \\
\texttt{mu\_core.v} & 188 & $\mu$-ledger enforcement and monotonicity gate \\
\texttt{mu\_alu.v} & 167 & Q16.16 fixed-point cost arithmetic \\
\texttt{receipt\_integrity\_checker.v} & 120 & Hardware receipt chain validation \\
\texttt{synth\_lite\_out.v} & --- & Yosys gate-level synthesis output \\
\hline
\end{tabular}}
\end{center}

Each accelerator module (\texttt{partition\_core}, \texttt{mu\_alu}, \texttt{receipt\_integrity\_checker}, \texttt{chsh\_partition}) has a standalone co-simulation harness in \path{thielecpu/hardware/accel\_cosim.py} that generates testbenches, runs them through iverilog, and parses JSON output for cross-layer comparison. This means every accelerator block is independently testable against Python reference implementations---you don't need to run the full CPU to verify that the $\mu$-ALU computes $1.5 + 2.25 = 3.75$ correctly in Q16.16.
