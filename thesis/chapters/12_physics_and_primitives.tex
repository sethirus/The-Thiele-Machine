...
The wave model lives in \texttt{coq/physics/WaveModel.v}, and its embedding into the Thiele Machine is proven in \texttt{coq/physics\_exploration/WaveEmbedding.v}. The lattice and dissipative models follow the same pattern: define a state and step function, then prove conservation or monotonicity lemmas that can be linked back to kernel invariants.
...
The Thiele Machine constructs a formal arrow of time from the kernel's accounting. The derivation chain, formalized in \texttt{coq/kernel/ArrowOfTimeSynthesis.v}, is:
\[
...
The dissipative model also offers a framework for the quantum measurement problem. In the Thiele Machine, measurement isn't treated as a mysterious ``collapse''---it's modeled as a specific physical process: the REVEAL instruction extracts information from a partition module at $\mu$-cost. The formal bridge to quantum POVMs (Positive Operator-Valued Measures) is proven in \texttt{coq/kernel/POVMBridge.v}:
\begin{itemize}
...
The formal development proves that this structure satisfies the axioms of a \textbf{causal set} in the Bombelli--Lee--Meyer--Sorkin (1987) sense. The proof lives in \texttt{CausalSetAxioms.v} and establishes:
\begin{itemize}
...
The formal development includes an exploration of whether fundamental physical constants can be derived from the $\mu$-theory. These proofs live in \path{coq/physics_exploration/} and are maintained \textit{separately} from the zero-axiom kernel. This section documents the successes, failures, and lessons learned.

\subsection{The Planck Constant: A Successful Derivation}

\textbf{Result: [STRUCTURAL] RELATIONSHIP ESTABLISHED, NOT A DERIVATION}

The relationship between Planck's constant $h$ and Landauer's principle establishes a structural connection between information theory and quantum mechanics. The formal proof is in \path{coq/physics_exploration/PlanckDerivation.v} (115 lines, compiles).
...
The speed of light derivation establishes structural relationships but cannot predict the numerical value. The formal proof is in \path{coq/physics_exploration/EmergentSpacetime.v} (31 lines).
...
The gravitational constant $G$ resists derivation. The formal analysis is in \path{coq/physics_exploration/HolographicGravity.v} (23 lines).
...
The attempt to derive particle masses failed completely. The formal analysis is in \path{coq/physics_exploration/ParticleMasses.v} (45 lines).

\paragraph{Tested Patterns:}

The Python experiment \path{archive/experiments_unused/experiments/derive\_masses\_couplings.py} tested for:
\begin{enumerate}
...
\textbf{Key point:} The physics\_exploration directory is \textit{isolated} from the zero-axiom kernel. The kernel proofs (\path{coq/kernel/}) remain completely axiom-free.
...
The formal boundary between derivable and empirical constants is made precise in \texttt{coq/kernel/ConstantDerivationStrength.v}. The key contributions are:
...
The file \path{coq/bridge/BoxWorld_to_Kernel.v} (7.2KB) embeds finite box-world predictions into kernel receipts:
...
The file \path{coq/bridge/FiniteQuantum_to_Kernel.v} (8.9KB) extends the box-world bridge to quantum-admissible correlations:
...
\begin{enumerate}
    \item \textbf{Physics models}: Wave, dissipative, and discrete dynamics with conservation laws, including a derived arrow of time (\texttt{coq/kernel/ArrowOfTimeSynthesis.v}) and a formal POVM-to-REVEAL measurement bridge (\texttt{coq/kernel/POVMBridge.v})
    ...
    \item \textbf{Physical constants}: Relational identities with a precise derivable/empirical classification (\texttt{coq/kernel/ConstantDerivationStrength.v})
