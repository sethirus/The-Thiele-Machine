...
The Thiele Machine derives the arrow of time as a theorem rather than assuming it as a postulate. The complete derivation chain, formalized in \texttt{coq/kernel/ArrowOfTimeSynthesis.v}, is:
\[
\text{vm\_step costs} \;\Rightarrow\; \mu\text{-monotonicity} \;\Rightarrow\; \text{irreversibility} \;\Rightarrow\; \text{second law} \;\Rightarrow\; \text{temporal ordering} \;\Rightarrow\; \text{arrow of time}
\]
The key insight is that $\mu$-monotonicity defines a total preorder on states: if $\mu(s') \geq \mu(s)$, then $s'$ is ``later'' than $s$. When $\mu$ strictly increases, this ordering is asymmetric---time cannot flow backwards. This connects directly to Landauer's principle: each irreversible bit operation contributes $k_B T \ln 2$ of dissipation, and $\mu$ counts exactly these operations. The formal proof (\texttt{arrow\_of\_time\_derived}) collects all levels into a single constructive record with zero admits.
...
\end{itemize}
The Born rule emerges from information-theoretic accounting (\texttt{BornRule.v}) and is independently derived from tensor product consistency alone in \texttt{BornRuleFromSymmetry.v}, eliminating any circularity concern. Post-measurement state is uniquely determined by maximum information extraction (\texttt{CollapseDetermination.v}), and measurement irreversibility follows from $\mu$-monotonicity (\texttt{ObservationIrreversibility.v}). Together, these proofs suggest that ``collapse'' may be what information extraction looks like when structural cost is tracked---not a resolution, but a concrete formal framework that makes the question amenable to mechanized reasoning.
...
\textbf{This is NOT a derivation of $h$ from first principles.} The Coq proof establishes that the relationship $h = 4 E_{\text{landauer}} \tau_\mu$ is algebraically consistent, but $\tau_\mu$ is \textit{computed from known $h$}, not independently derived. We are working backwards, not forwards.

\paragraph{The Core Relationship:}

Starting from Landauer's principle $E_{\text{landauer}} = k_B T \ln 2$, we \textit{define} the fundamental $\mu$-time scale as:
\[
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Circular: $G$ appears in the formula we're trying to derive
    \end{itemize}
