...
\begin{itemize}
    ...
    \item \textbf{Stage 2 (Period Finding, blue box):} The \textit{core quantum/partition subroutine}: find the \textbf{period $r$} of the function $f(k) = a^k \bmod N$, i.e., the smallest positive integer $r$ such that $a^r \equiv 1 \pmod{N}$. This is the \textit{hard step}: classically requires $O(\sqrt{N})$ time (exponential in bit-length $\log_2 N$), and quantum computers achieve $O((\log N)^3)$ time via quantum Fourier transform (Shor's breakthrough). \textbf{CLARIFICATION:} The Thiele Machine's current classical implementation remains $O(\sqrt{N})$ via trial division and does \textit{not} achieve quantum-like speedup. The \texttt{shor\_reduction} theorem proves the mathematical correctness of the reduction (given $r$, extract factors) but does not provide a polynomial-time period-finding algorithm without quantum hardware. See \texttt{coq/shor\_primitives/PolylogConjecture.v} which explicitly states previous polylog claims were incorrect. Example: $N=21, a=2$ requires computing $2^1=2, 2^2=4, 2^3=8, 2^4=16\equiv-5, 2^5\equiv-10, 2^6\equiv-20\equiv1 \pmod{21}$, so period $r=6$.
