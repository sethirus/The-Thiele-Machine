...
The wave model lives in \texttt{WaveModel.v}, and its embedding into the Thiele Machine is proven in \texttt{WaveEmbedding.v} (developed during the physics exploration phase; no longer part of the active build tree). The lattice and dissipative models follow the same pattern: define a state and step function, then prove conservation or monotonicity lemmas that can be linked back to kernel invariants.
...
The Thiele Machine constructs a formal arrow of time from the kernel's accounting. The derivation chain, formalized in \texttt{ArrowOfTimeSynthesis.v} (no longer in the active build tree), is:
\[
...
The dissipative model also offers a framework for the quantum measurement problem. In the Thiele Machine, measurement isn't treated as a mysterious ``collapse''---it's modeled as a specific physical process: the REVEAL instruction extracts information from a partition module at $\mu$-cost. The formal bridge to quantum POVMs (Positive Operator-Valued Measures) is proven in \texttt{POVMBridge.v} (no longer in the active build tree):
\begin{itemize}
...
The formal development proves that this structure satisfies the axioms of a \textbf{causal set} in the Bombelli--Lee--Meyer--Sorkin (1987) sense. The proof in \texttt{CausalSetAxioms.v} (no longer in the active build tree) establishes:
\begin{itemize}
...
The relationship between Planck's constant $h$ and Landauer's principle establishes a structural connection between information theory and quantum mechanics. The formal proof is in \texttt{PlanckDerivation.v} (115 lines, compiles; no longer in the active build tree).
...
Given this definition, the relationship $h = 4 E_{\text{landauer}} \tau_\mu$ is trivially true by algebra.
...
The speed of light derivation establishes structural relationships but cannot predict the numerical value. The formal proof is in \texttt{EmergentSpacetime.v} (31 lines; no longer in the active build tree).
...
The gravitational constant $G$ resists derivation. The formal analysis is in \texttt{HolographicGravity.v} (23 lines; no longer in the active build tree).
...
The attempt to derive particle masses failed completely. The formal analysis is in \texttt{ParticleMasses.v} (45 lines; no longer in the active build tree).
...
\textbf{Key point:} The physics\_exploration directory is \textit{isolated} from the kernel which has zero project-local axioms (no Admitted). The kernel proofs (\path{coq/kernel/}) contain no Admitted.
...
The formal boundary between derivable and empirical constants is made precise in \texttt{ConstantDerivationStrength.v} (no longer in the active build tree). The key contributions are:
...
The formal statement above appears in \texttt{Randomness\_to\_Kernel.v} (no longer in the active build tree). It is the connective tissue between high-level randomness claims and the kernel trace semantics, ensuring that a ``randomness proof'' is literally a filtered view of receipted steps.
...
The file \texttt{BoxWorld\_to\_Kernel.v} (7.2KB; no longer in the active build tree) embeds finite box-world predictions into kernel receipts:
...
The file \texttt{FiniteQuantum\_to\_Kernel.v} (8.9KB; no longer in the active build tree) extends the box-world bridge to quantum-admissible correlations:
