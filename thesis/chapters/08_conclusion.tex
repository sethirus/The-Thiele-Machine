...
\node[draw, rounded corners, dashed, inner sep=4pt, fit=(five)(mu)(nfi)(nosig), label={[font=\tiny]above:Theoretical Contributions}] {};
\draw[arr] (five) -- (mu);
\draw[arr] (five) -- (nfi);
\draw[arr] (mu) -- (nosig);
\draw[arr] (nfi) -- (nosig);
\end{tikzpicture}
\caption{Theoretical contribution dependencies. The 5-tuple formalization grounds the $\mu$-bit and No Free Insight theorem, which together enable the no-signaling proof.}
...
\end{enumerate}
The theorem names in the table correspond to statements in the Coq kernel (for example, \texttt{observational\_no\_signaling} in \path{KernelPhysics.v} and \texttt{nonlocal\_correlation\_requires\_revelation} in \path{RevelationRequirement.v}). This explicit mapping is what makes the verification story reproducible.
