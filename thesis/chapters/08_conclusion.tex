...
\begin{enumerate}
    ...
    \item \textbf{47-Opcode ISA}: The instruction set covers partition-native computation. Every opcode has a clear semantic role: structure creation, structure modification, certification, computation, categorical relational tracking, and control.
    \begin{itemize}
...
\begin{enumerate}
    \item \textbf{Zero-Admit Campaign}: The Coq formalization contains a complete active proof tree with no admits and no project-local axioms beyond imported foundational library principles. This is enforced by the verification tooling and guarantees that every theorem is fully discharged within the formal system.
...
\begin{enumerate}
    ...
    \item \textbf{Validated}: active tests confirm CHSH-related certification behavior, $\mu$-ledger monotonicity, receipt integrity, and a broad cross-layer regression surface. The QM-divergent predictions remain explicit falsifiable proposals rather than experimentally settled facts.
\end{enumerate}
...
Are the 40 active opcodes sufficient for all partition-native computation? Is there a normal form theorem?
...
\begin{itemize}
    \item \textbf{The Coq Kernel}: Full active proof corpus, $\sim$75,000 lines of verified specification that can be extended to new instruction sets.
    \item \textbf{The Python VM}: A thin wrapper delegating to the Coq-extracted OCaml runner, backed by 566 passing tests.
