...
\begin{enumerate}
    ...
    \item \textbf{Oracle Impossibility}: Three theorems (\texttt{halting\_undecidable}, \texttt{oracle\_halts\_costs\_mu}, \texttt{hypercomputation\_bounded}) in \path{OracleImpossibility.v} establish that no finite-$\mu$ trace can exceed Turing computability. Any partial halting oracle must pay $\mu$-cost proportional to its input. This caps the model from above.
...
\begin{itemize}
    \item \textbf{The Coq Kernel}: 285 files, $\sim$77,000 lines of verified specification that can be extended to new instruction sets.
    ...
    \item \textbf{The Inquisitor}: A 2,851-line discipline enforcer that scans every Coq file for admits, axioms, and unproven claims.
