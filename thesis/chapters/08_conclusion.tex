...
\begin{enumerate}
    ...
    \item \textbf{31-Instruction ISA}: The instruction set covers partition-native computation. Every opcode has a clear semantic role: structure creation, structure modification, certification, computation, and control.
    \begin{itemize}
...
\begin{enumerate}
    ...
    \end{center}
    All six hard-math facts (correlation bound, algebraic CHSH, classical CHSH, no-signaling, symmetric coherence bound, and Tsirelson from coherence) are mechanically proven from first principles in \path{coq/kernel/HardMathFactsProven.v} with zero axioms and zero admits. These are the foundational bounds that the Bell/CHSH framework depends on.
...
\begin{enumerate}
    ...
    \item \textbf{Validated}: 469 tests across 33 test files confirm CHSH supra-quantum certification requires revelation, the $\mu$-ledger is monotonic, and six QM-divergent predictions (QD1--QD6) produce concrete numerical values that differ from standard quantum mechanics---each naming the observable, the predicted value, and the experiment that would falsify it.
\end{enumerate}
...
Are the 31 instructions sufficient for all partition-native computation? Is there a normal form theorem?
...
\begin{itemize}
    \item \textbf{The Coq Kernel}: Full active proof corpus, $\sim$99,000 lines of verified specification that can be extended to new instruction sets.
    \item \textbf{The Python VM}: An executable reference for rapid prototyping, backed by 469 passing tests.
    \item \textbf{The Verilog RTL}: A hardware template for physical realization, with co-simulation gates.
    \item \textbf{The Inquisitor}: A ${\sim}6{,}700$-line discipline enforcer that scans every Coq file for admits, axioms, and unproven claims.
