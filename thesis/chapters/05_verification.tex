...
\begin{quote}
\textit{Author's Note (Devon): Okay, confession time. When I first heard about ``formal verification'' I thought it was some academic flex---people writing math to prove their code works instead of, you know, actually running it. Sounds backwards, right? But here's what I learned: testing can lie to you. Your tests pass, you feel great, then some edge case appears and everything collapses. Formal verification is different. It's not about ``this worked 1000 times.'' It's about ``this works. Period. Forever. Math says so.'' And let me tell you---when Coq accepted the proofs as complete, it hit different than any green test suite ever did.}
\end{quote}
...
\begin{enumerate}
    ...
    \item \textbf{Certificate checkers}: SAT model validator and countermodel validator in \path{coq/kernel/CertCheck.v}. (\texttt{check\_lrat} also exists in \texttt{CertCheck.v} but is not invoked by the current LASSERT step semantics; the UNSAT path always fails at the kernel level.)
    \item \textbf{Hash primitives}: SHA-256 implementation for receipt digests (assumed collision-resistant)
...
\begin{itemize}
    ...
    \item \textbf{No vacuous statements}: Theorems prove properties with genuine logical content, not tautologies. (Some results are deeper than others---see the proof depth discussion at the end of this chapter.)
\end{itemize}
...
\textbf{Inquisitor Quality Assessment:} The enforcement mechanism is \path{scripts/inquisitor.py}, which scans the active repository Coq tree. The current status is \textbf{HIGH: 0, MEDIUM: 0, LOW: 0} with:
\begin{itemize}
    ...
    \item \textbf{0 global axioms}: All assumptions are explicit \texttt{Context} parameters within labeled \texttt{Section} blocks, so nothing leaks into the global namespace.
...
\begin{enumerate}
    \item \textbf{Correlation Bound (T1-1)}: For any normalized probability distribution, correlations satisfy $|E(x,y)| \leq 1$ (\path{coq/kernel/BoxCHSH.v})
    \item \textbf{Algebraic CHSH Bound (T1-2)}: For any valid box (non-negative, normalized, no-signaling), the CHSH statistic satisfies $|S| \leq 4$ (\path{coq/kernel/AlgebraicCoherence.v})
    ...
    \item \textbf{Oracle Impossibility}: No decidable function solves the halting problem (standard diagonal argument); oracle queries cost $\mu \geq 1$ by definition
    \item \textbf{Turing Completeness}: The Thiele Machine is Turing-complete, proven via Minsky machine embedding
\end{enumerate}

\textbf{Bell Inequality Foundation:} Theorems 1 and 2 establish the mathematical foundation for all Bell-type inequalities using pure probability theory. Both are proven from first principles with no project-local axioms beyond Coq's standard library, verified via \texttt{Print Assumptions normalized\_E\_bound} (in \path{BoxCHSH.v}) and \texttt{Print Assumptions valid\_box\_S\_le\_4} (in \path{BoxCHSH.v}) (both return ``Closed under the global context''). These proofs establish that the algebraic ceiling for CHSH correlations is 4---any theory (classical, quantum, or hypothetical supra-quantum) cannot exceed this bound without violating basic probability.

Each of these theorems has a concrete home in the Coq tree: Bell bounds are in \path{BoxCHSH.v}, \path{ClassicalBound.v}, and \path{AlgebraicCoherence.v}, observational no-signaling is proven in \path{KernelPhysics.v}, $\mu$-conservation is proven in \path{KernelPhysics.v} and \path{MuLedgerConservation.v}, and No Free Insight appears in \path{NoFreeInsight.v} and \path{MuNoFreeInsightQuantitative.v}. The names matter because they pin the prose to specific proof artifacts a reader can inspect.

\subsection{Quantum Axioms from $\mu$-Accounting}

The kernel also includes machine-verified proofs connecting fundamental quantum axioms to conservation constraints. A word of honesty is needed here: these proofs are \textit{conditional} results. Each one says ``IF you accept a particular conservation constraint motivated by $\mu$-accounting, THEN the corresponding quantum axiom follows as arithmetic.'' The physics enters through the hypotheses---the proofs show these assumptions logically entail the conclusions, but the assumptions themselves encode the physical content. The value is structural: these results demonstrate that quantum axioms are not independent postulates but are connected through a common conservation principle. Whether $\mu$-conservation is the \textit{right} conservation principle is a physical claim the formalism cannot settle.

\begin{enumerate}
    \item \textbf{No-Cloning} (\path{coq/kernel/NoCloning.v}): Perfect cloning at zero cost yields a contradiction. The theorem \texttt{no\_cloning\_from\_conservation} models a cloning operation as four real numbers: input information, two output copies, and $\mu$-cost. Conservation requires \texttt{out1 + out2 $\leq$ in + cost}. Perfect cloning sets both outputs equal to the input, giving $2I \leq I + \mu$. With $\mu = 0$ and $I > 0$, you get $2I \leq I$---contradiction via \texttt{lra}. It is real arithmetic, not deep, but it captures the structural reason no-cloning works: it is an accounting constraint. Approximate cloning costs are bounded by \texttt{approximate\_cloning\_bound}.
    
    \item \textbf{Unitarity} (\path{coq/kernel/Unitarity.v}): The theorem \texttt{nonunitary\_requires\_mu} proves: if information loss is bounded by $\mu$-cost (the hypothesis \texttt{respects\_info\_conservation}), and some evolution loses information (\texttt{info\_loss > 0}), then $\mu$-cost must be positive. This is one step of real arithmetic: $x > 0 \wedge x \leq y \Rightarrow y > 0$. The actual physics---that non-unitary evolution loses information---is the \textit{hypothesis}, not the conclusion. The proof just chains the inequality. CPTP map characterization and Lindblad bounds follow the same pattern.
    
    \item \textbf{Born Rule} (\path{coq/kernel/BornRule.v}): The theorem \texttt{valid\_linear\_rule\_is\_born\_with\_cost\_side\_condition} (and the simpler \texttt{linear\_implies\_born} that does the actual work) proves that any probability rule satisfying normalization, eigenstate conditions ($P(0,0,1,0)=1$, $P(0,0,-1,0)=0$), and linearity in the Bloch $z$-component must equal $(1+z)/2$. That is the Born rule for qubit measurements along $z$. The key assumption doing all the work is \texttt{is\_linear\_in\_z}---linearity of probabilities in the Bloch parameter. The \texttt{measurement\_cost\_nonnegative} hypothesis is present but unused in the proof; it reduces to the Bloch ball constraint already given. This is less ``deriving the Born rule from accounting'' and more ``the Born rule is the unique linear rule consistent with eigenstates.''
    
    \item \textbf{Purification} (\path{coq/kernel/Purification.v}): The theorem \texttt{purification\_principle} proves that for any Bloch sphere point with $x^2 + y^2 + z^2 < 1$ (a mixed state), there exists a reference system such that the combined state is pure. The purification deficit equals $1 - \gamma$ where $\gamma$ is the purity.
    
    \item \textbf{Tsirelson Bound} (\path{coq/kernel/TsirelsonGeneral.v}): The theorem \texttt{tsirelson\_from\_row\_bounds} proves: if four correlation values satisfy row-norm constraints ($e_{00}^2 + e_{01}^2 \leq 1$ and $e_{10}^2 + e_{11}^2 \leq 1$), then $S^2 \leq 8$. This is legitimate algebra, proven via a sum-of-squares decomposition identity. The result is correct and the proof is real. The row-norm constraints themselves come from the NPA-1 moment matrix (a quantum-mechanical object), so the physics is in the hypotheses, but the algebraic bound is genuinely derived.
    
    \item \textbf{Born Rule from No-Signaling} (\path{coq/kernel/BornRuleLinearity.v}): The theorem \texttt{born\_rule\_from\_no\_signaling} derives $P(z) = (1+z)/2$ as the unique affine rule consistent with no-signaling (mixture compatibility) and boundary conditions ($P(1)=1$, $P(-1)=0$).

    \item \textbf{Machine-Native No-Cloning} (\path{coq/kernel/NoCloning.v}): The theorem \texttt{no\_cloning\_from\_conservation} proves no-cloning from unitarity.
    
    \item \textbf{Algebraic Tsirelson} (\path{coq/kernel/TsirelsonFromAlgebra.v}): A standalone algebraic derivation of the Tsirelson bound with achievability witness. The theorem \texttt{tsirelson\_squared} proves $S^2 \leq 8$ from $A^2 = B^2 = I$ via a sum-of-squares identity.
\end{enumerate}

\begin{quote}
\textit{Author's Note (Devon): I want to be straight with you about these proofs. Some of them---no-cloning, unitarity---are basically arithmetic dressed up in physics language. The conservation constraint is the hypothesis, and the ``quantum law'' drops out as a consequence. That is not nothing. It shows these axioms are not independent---they are connected through a common accounting principle. But I am not going to pretend that Coq derived quantum mechanics from scratch. The physics is in the assumptions. What the proofs establish is that IF you buy the conservation framing, the rest follows mechanically. The Tsirelson bound is probably the most genuinely interesting one---real algebra, real proof, real result. The others are more like consistency checks than derivations. That is still useful. It tells you the framework is not obviously broken.}
\end{quote}

\begin{tcolorbox}[thesisbox,colback=green!5!white,colframe=green!75!black,title=Quantum Axiom Verification Summary]
{\tiny
\begin{tabular}{@{}lll@{}}
\textbf{File} & \textbf{Key Theorem} & \textbf{Proof Character} \\
\hline
NoCloning.v & \texttt{no\_cloning\_from\_conservation} \checkmark & Conservation hypothesis $\Rightarrow$ arithmetic \\
Unitarity.v & \texttt{nonunitary\_requires\_mu} \checkmark & Info-loss hypothesis $\Rightarrow$ arithmetic \\
BornRule.v & \texttt{linear\_implies\_born} \checkmark & Linearity + boundary $\Rightarrow$ unique rule \\
BornRuleLinearity.v & \texttt{born\_rule\_from\_no\_signaling} \checkmark & No-signaling + boundary $\Rightarrow$ affine rule \\
Purification.v & \texttt{purification\_principle} \checkmark & Bloch sphere geometry \\
TsirelsonGeneral.v & \texttt{tsirelson\_from\_row\_bounds} \checkmark & Genuine algebraic bound \\
TsirelsonFromAlgebra.v & \texttt{tsirelson\_squared} \checkmark & Sum-of-squares identity \\
\end{tabular}}

\vspace{2pt}
{\scriptsize All zero Admitted. Physics enters via hypotheses; proofs verify logical entailment.}
\end{tcolorbox}
...
All proofs are verified by Coq 8.18.x. The Inquisitor enforces this invariant: any commit introducing an admit or undocumented axiom fails CI. The static analysis also detects vacuous statements, trivial tautologies, and hidden assumptions. See \path{scripts/inquisitor.py} and \path{scripts/inquisitor\_rules.py} for complete documentation of the 100 scan rules and enforcement policies.
...
\begin{itemize}
    ...
    \item \textbf{Oracle and computability}: Undecidability of halting, oracle cost bounds, and Turing subsumption.
\end{itemize}
...
\textbf{Twelve immutable fields:}
\begin{itemize}
    \item \textbf{vm\_graph : PartitionGraph} — The complete partition structure (modules, regions, axioms, and categorical morphisms between modules). Every locality theorem quantifies over this graph.
    ...
    \item \textbf{vm\_mu\_tensor : list nat} — Bianchi tensor. The invariant $\sum\text{tensor}[i] \le \mu$ is checked per step.
    \item \textbf{vm\_err : bool} — Error latch. Once set, the VM halts. Proofs about error propagation reference this flag.
    \item \textbf{vm\_logic\_acc : nat} — Logic engine accumulator. Tracks accumulated structural evidence.
    \item \textbf{vm\_mstatus : nat} — Machine status register. Encodes execution state metadata.
    \item \textbf{vm\_witness : WitnessCounts} — CHSH trial witness counters. Eight buckets tracking same/different agreement outcomes across four measurement-setting pairs $(a,b)$. Proofs about quantum admissibility reference these counters.
    \item \textbf{vm\_certified : bool} — State-based certification flag. Set by the CERTIFY instruction; proofs about certification status reference this field.
\end{itemize}
...
\textbf{What is this predicate?} The well-formedness invariant for partition graphs has three conjuncts: (1) every module ID is below the module ID counter, (2) every morphism ID is below the morphism ID counter, and (3) every morphism's source and target are real modules. All three must hold simultaneously.

\textbf{The three conditions:}
\begin{itemize}
    \item \textbf{all\_ids\_below} — Every \texttt{ModuleID} in \texttt{pg\_modules} is strictly less than \texttt{pg\_next\_id}. Ensures the module ID counter is genuinely fresh.
    \item \textbf{all\_morph\_ids\_below} — Every \texttt{MorphismID} in \texttt{pg\_morphisms} is strictly less than \texttt{pg\_next\_morph\_id}. Same freshness guarantee for morphism IDs.
    \item \textbf{all\_morph\_endpoints\_valid} — Every morphism's \texttt{morph\_source} and \texttt{morph\_target} exist in \texttt{pg\_modules}. Prevents dangling arrows after module deletion.
\end{itemize}

\textbf{Why three conditions?} Adding the MORPH family (categorical morphism instructions) required tracking arrows between modules. A single ID-counter condition is no longer sufficient: you also need to guarantee that morphisms don't refer to modules that were deleted, which requires the endpoint-validity check.

\textbf{Preservation under operations:} Every graph operation that touches modules or morphisms has a corresponding preservation lemma. This means that once you start with a well-formed graph (the empty graph is well-formed by \texttt{empty\_graph\_well\_formed}), all reachable graphs remain well-formed.

\textbf{Physical interpretation:} Well-formedness is the ``identity discipline'' of the kernel. Just as physical systems require distinct particle labels and interaction endpoints, the kernel requires distinct IDs and valid arrow endpoints. The invariant enforces this at the mathematical level.
...
\textbf{Inductive type:} In Coq, an \texttt{Inductive} type defines a set of constructors. \texttt{vm\_instruction} has 46 constructors, each representing one instruction. No other instructions exist---the type is closed.
...
\begin{itemize}
    ...
    \item \textbf{Logical operations:} \texttt{instr\_lassert}, \texttt{instr\_ljoin} — Assert formulas with SAT witness packages, join certificate chains.
    \item \textbf{Discovery:} \texttt{instr\_pdiscover}, \texttt{instr\_mdlacc} — Declare axioms, compute logarithmic model size.
    \item \textbf{Data transfer:} \texttt{instr\_xfer}, \texttt{instr\_load\_imm}, \texttt{instr\_lui}, \texttt{instr\_xor\_*} — Register transfer, immediate loads, upper-immediate loads, bitwise XOR operations.
    \item \textbf{Memory \& ALU:} \texttt{instr\_load}, \texttt{instr\_store}, \texttt{instr\_add}, \texttt{instr\_sub}, \texttt{instr\_mul}, \texttt{instr\_and}, \texttt{instr\_or}, \texttt{instr\_shl}, \texttt{instr\_shr} — Memory access and arithmetic/bitwise operations.
    \item \textbf{Certification:} \texttt{instr\_chsh\_trial}, \texttt{instr\_emit}, \texttt{instr\_reveal}, \texttt{instr\_certify} — CHSH trials, receipt emission, state revelation, state-based certification.
    \item \textbf{Control:} \texttt{instr\_jump}, \texttt{instr\_jnez}, \texttt{instr\_call}, \texttt{instr\_ret}, \texttt{instr\_halt} — Branches, subroutines, termination.
    \item \textbf{Morphisms:} \texttt{instr\_morph}, \texttt{instr\_compose}, \texttt{instr\_morph\_id}, \texttt{instr\_morph\_delete}, \texttt{instr\_morph\_assert}, \texttt{instr\_morph\_tensor}, \texttt{instr\_morph\_get} — Categorical morphism operations.
    \item \textbf{I/O and Heap:} \texttt{instr\_checkpoint}, \texttt{instr\_read\_port}, \texttt{instr\_write\_port}, \texttt{instr\_heap\_load}, \texttt{instr\_heap\_store} — Port I/O, heap memory, checkpointing.
    \item \textbf{Tensor:} \texttt{instr\_tensor\_set}, \texttt{instr\_tensor\_get} — Per-module $\mu$-tensor read/write.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{:= ...} — The body (not shown) contains 58 step rules, one or more per instruction constructor (success and failure branches), defining exactly how each instruction transforms state.
\end{itemize}
...
% Noether box
\node[draw, rounded corners=2pt, fill=yellow!15, minimum width=4.5cm, font=\tiny, align=center, inner sep=3pt] at (0,-4.3) {\textbf{Structural analogy:} $\mu$-shift leaves partition unchanged (fields independent)};
\end{tikzpicture}
...
\textbf{What this proves:} Partition structure is unchanged when the $\mu$-ledger is shifted by a constant. This is true by construction: \texttt{mu\_gauge\_shift} only modifies \texttt{vm\_mu}, leaving \texttt{vm\_graph} untouched. The proof is structural---it unfolds the definitions and confirms the graph field is identical.

\textbf{Why this matters:} The result formalizes an important design property of the state record: the absolute $\mu$ value carries no semantic content for the partition structure. Only $\mu$ \textit{differences} matter. This is the sense in which I call it ``gauge invariance''---the zero point of $\mu$ is arbitrary, analogous to the arbitrary zero of potential energy in physics. Stating it as a theorem makes it available as a lemma for downstream reasoning, including the cross-layer comparison gates that compare $\mu$-deltas rather than absolute values.

\textbf{Noether analogy:} I draw a parallel to Noether's theorem: gauge symmetry ($\mu$-shift freedom) ``corresponds to'' conservation of partition structure. The analogy is deliberate but should not be over-read. In physics, Noether's theorem is a deep result connecting continuous symmetries to conservation laws. Here, it is a definitional property: the graph does not depend on $\mu$ because it is stored in a separate field. The value of stating it as a theorem is that it becomes available for mechanized reasoning in downstream proofs.
...
\end{tikzpicture}
\caption{$\mu$-conservation: the ledger accumulates instruction costs monotonically. No instruction can decrease $\mu$---a design invariant analogous to the Second Law.}
...
\textbf{What does this prove?} This proves that the $\mu$ ledger never decreases. Every instruction either increases $\mu$ or leaves it unchanged.
...
\textbf{Why is this true?} Every step rule calls \texttt{apply\_cost s instr}, which computes \texttt{s.vm\_mu + instruction\_cost(instr)}. Since \texttt{instruction\_cost} returns a \texttt{nat} (natural number, always $\geq 0$), the result is always $\geq$ the original \texttt{vm\_mu}. This monotonicity is true \textit{by construction}: the cost type is unsigned, so adding it cannot decrease the total. The proof examines each step rule and confirms this arithmetic pattern holds uniformly.

\textbf{Why does this matter?} Even though monotonicity is a consequence of the design (costs are nats), making it explicit as a Coq theorem is valuable: it becomes a lemma that later proofs depend on. The irreversibility bound, the ledger conservation theorem, and parts of the NoFI argument all chain through this result. The design choice that makes this true by construction---unsigned costs added to a monotonic counter---is intentional. I engineered the step semantics so that this property falls out automatically, precisely so it would be available for downstream reasoning.

\textbf{Physical analogy:} This is the kernel's analog of the Second Law of Thermodynamics: entropy (here, $\mu$) never decreases. The analogy is intentional but should be read carefully. In physics, the Second Law is a deep empirical fact about the universe. Here, it is a definitional consequence of how I structured the cost accounting. The machine does not \textit{discover} that $\mu$ is monotonic---it is \textit{built} so that $\mu$ is monotonic.
...
\begin{itemize}
    ...
    \item \textbf{| Some instr => run\_vm fuel' trace (vm\_apply s instr)} — If instruction found, execute it via \texttt{vm\_apply} (the executable function in \texttt{coq/kernel/SimulationProof.v} that matches the \texttt{vm\_step} relation), then recurse with decremented fuel.
\end{itemize}
...
\textbf{What is \texttt{vm\_apply}?} This is the executable, deterministic counterpart of the \texttt{vm\_step} inductive relation: given \texttt{(s, instr)}, it returns the unique \texttt{s'} such that \texttt{vm\_step s instr s'}, or returns \texttt{s} unchanged if the step is undefined. The agreement between the two is established by \texttt{vm\_step\_vm\_apply} in \texttt{SimulationProof.v}.

\textbf{Halting conditions:}
\begin{itemize}
    \item Fuel exhausted: \texttt{fuel = O}.
    \item PC out of bounds: \texttt{nth\_error trace s.vm\_pc = None}.
    \item Implicit: If an instruction sets \texttt{vm\_err = true}, subsequent steps likely become no-ops (depends on \texttt{vm\_apply} implementation).
\end{itemize}
...
\textbf{Structure mirrors run\_vm:} The recursion structure is identical to \texttt{run\_vm}, so the ledger corresponds exactly to the executed trace. If \texttt{run\_vm} executes $n$ instructions, \texttt{ledger\_entries} returns a list of length $n$.
...
\textbf{What this proves:} The count of ``irreversible'' instructions is bounded by total $\mu$ growth. The counting function \texttt{irreversible\_count} uses a simple proxy: each instruction with non-zero cost counts as 1 (otherwise 0). Since every non-zero-cost instruction contributes at least 1 to $\mu$ growth, the count never exceeds the total $\mu$ increase. This is a conservative bound---the actual information-theoretic irreversibility could be larger, but this proxy gives a clean lower bound.

\textbf{Landauer connection:} I draw an analogy to Landauer's principle (erasing one bit costs at least $k_B T \ln 2$), but the analogy is loose. The formal result counts non-zero-cost instructions, not actual bits of information erased. A stronger connection would require defining irreversibility in information-theoretic terms (e.g., Shannon entropy change), which is outside the current formalization scope.
...
\textbf{What does this define?} The kernel now splits certification into two layers. \texttt{CertifiedObs} is the observation layer: execution finished without error and the predicate accepts the decoded receipts. \texttt{CertifiedWithSupra} adds the cert-channel requirement \texttt{has\_supra\_cert}. The legacy name \texttt{Certified} is an alias for that stronger second layer.
...
\begin{itemize}
  \item \textbf{Observation-only witnesses are real:} a trace can establish posterior admissibility as \texttt{CertifiedObs} without yet closing the stronger bridge.
  \item \textbf{Full certification is stricter:} to reach \texttt{Certified}, the final state must also carry \texttt{has\_supra\_cert}.
  \item \textbf{The boundary is explicit:} the repository now proves both the observation-only side and the fully bridged side on nearby concrete witnesses.
\end{itemize}

\textbf{What each layer rules out:}
\begin{itemize}
    \item Without \texttt{vm\_err = false}, a crashed execution could spuriously satisfy the predicate.
  \item Without \texttt{P(...) = true}, the receipts might not match the claim.
  \item Without \texttt{has\_supra\_cert}, the run stays observational and does not close the full structural bridge.
\end{itemize}
...
Theorem no_free_insight_general :
  ...
  (exists n fa ca k fl mu,
    nth_error trace n =
      Some (instr_lassert fa ca k fl mu)) \/
  (exists n mid p c mu,
    nth_error trace n =
      Some (instr_morph_assert mid p c mu)).
\end{lstlisting}
\end{theorem}

\paragraph{Understanding no\_free\_insight\_general (early reference):}

\textbf{What this proves:} If you gain supra-certification (go from no certificate to has\_supra\_cert), the trace must cross a positive-cost certification boundary. The theorem above is the conservative outer envelope: REVEAL, EMIT, LJOIN, LASSERT, or MORPH\_ASSERT must appear. The sharper current-semantics shortcut frontier is narrower still: the repository now proves that a trace with no executed MORPH\_ASSERT bridge cannot finish fully certified.
...
\begin{itemize}
    ...
    \item \textbf{has\_structure\_addition fuel trace s\_init} — Conclusion: the trace contains an executed cert-address bridge event.
\end{itemize}

  	extbf{Why ``structure addition''?} The predicate \texttt{has\_structure\_addition} is execution-semantic: it checks that some executed step changed \texttt{csr\_cert\_addr} from 0 to non-zero. In the current hardware-aligned structural shortcut path, that bridge is a successful MORPH\_ASSERT with a non-zero property checksum.
...
\textbf{Concrete example:} Suppose \texttt{P\_weak} accepts any non-empty receipt list, and \texttt{P\_strong} accepts only $[42]$. If you start with no certificate and end with full certification of \texttt{P\_strong}, the run must cross the cert-address bridge. In the current hardware-aligned structural shortcut path, that bridge is a successful \texttt{MORPH\_ASSERT} on a real morphism, with a non-zero property checksum. You cannot magically certify $[42]$ without explicitly earning that bridge.
...
\textbf{What does this prove?} This proves that \textbf{supra-quantum correlations} (correlations stronger than quantum mechanics allows, achieved via partition-native computing) require an explicit positive-cost certification boundary. You cannot produce nonlocal correlations (e.g., CHSH violation > 2$\sqrt{2}$) without paying $\mu$ cost.

\textbf{Theorem statement:} This is \textit{identical} to \texttt{no\_free\_insight\_general}. The difference is \textit{interpretation}: here, the theorem is framed in terms of physical correlations (CHSH experiments, Bell tests) rather than abstract predicate strengthening. For the structural shortcut boundary specifically, the repository now also proves the sharper MORPH\_ASSERT bridge frontier.
...
\begin{itemize}
    ...
    \item To produce such a certificate, the VM must cross the positive-cost certification boundary. In the current shortcut path that concrete cert-address bridge is MORPH\_ASSERT; the broader revelation-class theorem remains the conservative outer envelope.
\end{itemize}
...
\end{enumerate}
The theorem guarantees you can't skip the positive-cost bridge and still certify the correlation.

\textbf{Interpretation}: To achieve supra-quantum certification, you must explicitly pay for it through the certification boundary. There is no backdoor.
...
This functor wraps the interface obligation: the proof is literally \texttt{eapply X.no\_free\_insight\_contract; eauto}. The generic theorem contains no proof content of its own---it delegates entirely to the interface contract. The value of the functor is \textit{modularity}: any system satisfying the interface inherits the theorem, without re-proving it.
...
\begin{enumerate}
    ...
    \item \textbf{Trace property:} The underlying result---certification requires structure events---is genuine and machine-checked
\end{enumerate}
...
\section{Computability and Bell Foundations}
...
\subsection{Bell Inequality Foundations}

The Bell-inequality bounds are mechanically proven from first principles in the active kernel:

\begin{itemize}
    \item \texttt{normalized\_E\_bound}: $|E(x,y)| \leq 1$ from probability axioms (\path{coq/kernel/BoxCHSH.v})
    \item \texttt{chsh\_bound\_4}: $|S| \leq 4$ from the triangle inequality (\path{coq/kernel/AlgebraicCoherence.v})
    \item \texttt{local\_S\_2\_deterministic}: $|S| \leq 2$ for local deterministic theories, by exhaustive case analysis (\path{coq/kernel/BoxCHSH.v})
    \item \texttt{tsirelson\_from\_algebraic\_coherence}: Tsirelson's bound $2\sqrt{2}$ via algebraic coherence (\path{coq/kernel/AlgebraicCoherence.v})
\end{itemize}

Run \texttt{Print Assumptions} on these theorems and Coq reports only standard library axioms. The entire Bell-inequality foundation is self-contained.
...
\begin{itemize}
    ...
    \item \textbf{No revelation events in the trace} — The trace must \textit{not} contain REVEAL, EMIT, LJOIN, LASSERT, or MORPH\_ASSERT instructions.
\end{itemize}
...
\begin{enumerate}
    ...
    \item A Thiele Machine trace that starts with \texttt{csr\_cert\_addr = 0}, ends with \texttt{Certified(..., P\_strong, ...)}, but contains \textit{none} of REVEAL, EMIT, LJOIN, LASSERT, or MORPH\_ASSERT.
\end{enumerate}
...
% 3x2 grid of theorem boxes
\node[thm, fill=blue!15] (ns) at (-1.4,3.5) {No-Signaling};
\node[thm, fill=green!15] (gi) at (1.4,3.5) {Gauge Invariance};
\node[thm, fill=orange!15] (mc) at (-1.4,2.6) {$\mu$-Conservation};
\node[thm, fill=red!12] (nfi) at (1.4,2.6) {No Free Insight};
\node[thm, fill=purple!12] (oi) at (-1.4,1.7) {Oracle Impossibility};
\node[thm, fill=cyan!15] (ts) at (1.4,1.7) {Turing Subsumption};
% Zero-Admit Standard
\node[std] (std) at (0,0.6) {\textbf{Zero-Admit:} No Admitted, No Axiom};
% Inquisitor
\node[inq] (inq) at (0,-0.4) {\textbf{Inquisitor} (100 rules) --- 0 HIGH};
% Arrows
\draw[arr] (ns.south) -- (mc.north);
\draw[arr] (gi.south) -- (nfi.north);
\draw[arr] (mc.south) -- (oi.north);
\draw[arr] (nfi.south) -- (ts.north);
\draw[arr] (oi.south) -- (oi.south |- std.north);
\draw[arr] (ts.south) -- (ts.south |- std.north);
% Arrow from standard to inquisitor
\draw[arr] (std.south) -- (inq.north);
\end{tikzpicture}
\caption{Chapter 5 summary. Six core results---locality, gauge invariance, conservation, impossibility, oracle bounds, and Turing subsumption---all proven under the zero-admit standard, enforced by the Inquisitor.}
...
\begin{enumerate}
    ...
    \item \textbf{Conservation}: The $\mu$-ledger is monotonic (by construction: costs are unsigned) and bounds irreversible operation counts
    \item \textbf{Impossibility}: Strengthening certification requires explicit, charged structure addition
    \item \textbf{Quantum Axiom Conditionals}: No-cloning, unitarity, Born rule, purification, and Tsirelson bounds follow from conservation hypotheses---the physics is in the assumptions, the proofs verify logical entailment (zero Admitted across seven files)
    \item \textbf{Oracle Impossibility}: Halting is undecidable (genuine diagonal argument); oracle queries are priced at $\mu \geq 1$ by definition, and the pricing is self-consistent (no project-local axioms)
    \item \textbf{Turing Subsumption}: Any Turing machine can be simulated via TM $\to$ Minsky $\to$ Thiele compilation with step-by-step correspondence (no project-local axioms)
    \item \textbf{Hard Math Facts}: All 4 Bell-inequality foundations mechanically proven from first principles (no project-local axioms)
    \item \textbf{Completeness}: Zero admits, no project-local axioms---all proofs are machine-checked
\end{enumerate}

\textbf{A note on proof character:} These results have different shapes, and the difference is intentional. The no-signaling theorem is genuine case analysis across 58 step rules covering all 46 opcodes. The Tsirelson algebraic bound is a real sum-of-squares identity in $\mathbb{R}$. The Turing subsumption chain is a substantive compilation-correctness proof. The conservation and gauge results---$\mu$-monotonicity, gauge invariance, oracle cost bounds---are true by construction: the semantics was engineered so cost monotonicity falls out of the unsigned-\texttt{nat} type, partition structure is independent of the $\mu$ field by record layout, and oracle queries carry $\mu \ge 1$ in their declared cost. The proofs make those design invariants machine-checkable rather than aspirational. Engineering a system so its key properties hold by construction is the point, not a shortcut.
