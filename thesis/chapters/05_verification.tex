...
\textbf{Inquisitor Quality Assessment:} The enforcement mechanism is \path{scripts/inquisitor.py}, which scans the full repository Coq corpus across 87 scan rules (current report: 324 files scanned). The current status is \textbf{HIGH: 0, MEDIUM: 0, LOW: 0} with:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Oracle Impossibility}: No decidable function solves the halting problem (genuine diagonal argument); oracle queries are defined to cost $\mu \geq 1$, and the formalization verifies this pricing is self-consistent; $n$ independent halting queries cost $\geq n$ (\path{coq/kernel/OracleImpossibility.v})
    \item \textbf{Turing Subsumption}: The Thiele Machine simulates any Turing machine via a verified TM $\to$ Minsky $\to$ Thiele compilation chain (\path{coq/modular_proofs/TM\_to\_Minsky.v}, \path{coq/modular_proofs/ThieleInstance.v})
\end{enumerate}
...
Each of these theorems has a concrete home in the Coq tree: Bell bounds are in \path{Tier1Proofs.v}, observational no-signaling is proven in \path{KernelPhysics.v}, $\mu$-conservation is proven in \path{KernelPhysics.v} and \path{MuLedgerConservation.v}, No Free Insight appears in \path{NoFreeInsight.v} and \path{MuNoFreeInsightQuantitative.v}, oracle impossibility is proven in \path{OracleImpossibility.v}, and Turing subsumption spans \path{TM\_to\_Minsky.v} and \path{ThieleInstance.v}. The names matter because they pin the prose to specific proof artifacts a reader can inspect.
...
\begin{enumerate}
    ...
    \item \textbf{Born Rule from Symmetry} (\path{coq/kernel/BornRuleFromSymmetry.v}): An existence proof showing the identity function $g(x) = x$ satisfies eigenstate consistency, normalization, tensor product consistency, and range constraints. The lemmas \texttt{born\_identity\_eigenstate}, \texttt{born\_identity\_normalization}, \texttt{born\_identity\_tensor}, and \texttt{born\_identity\_range} verify these properties. The uniqueness proof (that $g(x) = x$ is the \emph{only} such function) was removed because it relied on Section-scoped Context assumptions that violated the zero-axiom policy.
    
    \item \textbf{Machine-Native No-Cloning} (\path{coq/kernel/NoCloningFromMuMonotonicity.v}): A self-contained no-cloning proof using only natural-number arithmetic (\texttt{lia}). The theorem \texttt{no\_cloning\_mu\_monotonicity} shows perfect cloning requires $\mu \geq n$ for $n$-bit states.
...
BornRuleFromSymmetry.v & \texttt{born\_identity\_eigenstate} \checkmark & Existence proof (identity satisfies axioms) \\
NoCloningFromMuMono.v & \texttt{no\_cloning\_mu\_monotonicity} \checkmark & Natural-number accounting \\
Purification.v & \texttt{purification\_principle} \checkmark & Bloch sphere geometry \\
TsirelsonGen.v & \texttt{tsirelson\_from\_minors} \checkmark & Genuine algebraic bound \\
...
All proofs are verified by Coq 8.18.x. The Inquisitor enforces this invariant: any commit introducing an admit or undocumented axiom fails CI. The comprehensive static analysis also detects vacuous statements, trivial tautologies, and hidden assumptions. See \path{scripts/inquisitor.py} and \path{scripts/inquisitor\_rules.py} for complete documentation of the 87 scan rules and enforcement policies.
...
\textbf{Inductive type:} In Coq, an \texttt{Inductive} type defines a set of constructors. \texttt{vm\_instruction} has 31 constructors, each representing one instruction. No other instructions exist---the type is closed.
...
\begin{itemize}
    ...
    \item \textbf{Data transfer:} \texttt{instr\_xfer}, \texttt{instr\_load\_imm}, \texttt{instr\_xor\_*} — Register transfer, immediate loads, bitwise XOR operations.
    \item \textbf{Memory \& ALU:} \texttt{instr\_load}, \texttt{instr\_store}, \texttt{instr\_add}, \texttt{instr\_sub} — Memory access and arithmetic.
    \item \textbf{Certification:} \texttt{instr\_chsh\_trial}, \texttt{instr\_emit}, \texttt{instr\_reveal} — CHSH trials, receipt emission, state revelation.
...
\begin{itemize}
    ...
    \item \textbf{:= ...} — The body (not shown) contains 39 inference rules, one or more per instruction constructor, defining exactly how each instruction transforms state.
\end{itemize}
...
\section{Oracle Impossibility and Turing Subsumption}

The verification campaign also settles two computability-theoretic questions: what the machine \textit{cannot} do for free, and what it \textit{can} simulate. Together, these results position the Thiele Machine precisely in the computational hierarchy---Turing-complete, but honest about what costs $\mu$.

\subsection{Oracle Impossibility}

\path{coq/kernel/OracleImpossibility.v} (375 lines, zero axioms, zero admits) proves three results:

\begin{enumerate}
    \item \textbf{Halting undecidability} (\texttt{halting\_undecidable}): No total computable function decides halting. The proof uses the standard diagonal argument---assume a decider exists, construct a program that does the opposite of what the decider predicts, and derive a contradiction. This is a genuine, self-contained proof with no assumed results.
    
    \item \textbf{Oracle cost bound} (\texttt{oracle\_halts\_costs\_mu}): The formalization defines a \textit{sound} oracle as one charging at least 1 unit of $\mu$ per query (\texttt{sound\_oracle\_cost}), then proves that a zero-cost oracle violates this definition. This is a design consistency check: I \textit{defined} oracle soundness to require positive cost, and the proof confirms that zero cost contradicts the definition. The result is arithmetic ($0 \geq 1 \Rightarrow \bot$), not an information-theoretic derivation. The physical justification for \textit{why} oracle queries should cost $\mu > 0$ comes from the NoFI framework (gaining information about undecidable propositions should cost something), but the Coq file proves the narrower claim that the pricing is self-consistent.
    
    \item \textbf{Hypercomputation bound} (\texttt{hypercomputation\_bounded}): Given the per-query minimum cost of 1, $n$ independent halting queries cost at least $n$. This follows arithmetically from the per-query bound. Again, the Coq proof is clean and correct---it just proves a consequence of the cost definition rather than deriving the cost from first principles.
\end{enumerate}

\begin{quote}
\textit{Author's Note (Devon): This is the one that makes me proudest, honestly. The kernel has an \texttt{oracle\_halts} instruction---I put it there on purpose. The halting undecidability proof is the real deal: standard diagonal argument, self-contained, no assumptions. The oracle cost bound is simpler---I defined soundness to require cost $\geq 1$, and the proof confirms zero cost breaks that. That is a design consistency check, not a derivation from first principles. The physical argument for WHY oracle queries should cost something is the NoFI principle, but the Coq proof just verifies the pricing is self-consistent. The machine is not pretending to be a hypercomputer. It acknowledges undecidability and prices it accordingly.}
\end{quote}

\subsection{Turing Subsumption}

The converse question: can the Thiele Machine simulate any Turing machine? Two files answer yes:

\begin{enumerate}
    \item \textbf{TM $\to$ Minsky compilation} (\path{coq/modular_proofs/TM\_to\_Minsky.v}, 351 lines): Defines a compilation from Turing machines to Minsky counter machines using standard unary encoding of tape plus head position into counter registers. The key theorem \texttt{tm\_to\_minsky\_exists} proves that for any TM, there exists a compiled Minsky program whose execution faithfully simulates the original, step by step.
    
    \item \textbf{Minsky $\to$ Thiele instantiation} (\path{coq/modular_proofs/ThieleInstance.v}, 456 lines): Defines the Thiele Machine's simulation of Minsky programs (and therefore arbitrary TMs). The main result \texttt{thiele\_subsumes\_tm\_complete} proves semantic preservation across the full chain: TM $\to$ Minsky $\to$ Thiele, with step-by-step correspondence. Concrete examples (\texttt{identity\_tm\_thiele\_simulation}, \texttt{flip\_tm\_bounded}) verify the simulation on specific machines.
\end{enumerate}

Both files carry zero axioms and zero admits. The compilation chain establishes \textbf{Turing completeness}: every computable function can be computed by the Thiele Machine (at appropriate $\mu$-cost). Combined with oracle impossibility, the picture is complete---the machine computes exactly the computable functions, charges honestly for oracle queries, and cannot solve the halting problem for free.

\subsection{Hard Math Facts: Mechanized Proofs}

The 6 hard mathematical facts formalized in \path{coq/kernel/AssumptionBundle.v}---bounds on correlations, monogamy constraints, coherence relations that underpin all Bell-type reasoning---are mechanically proven from first principles in \path{coq/kernel/HardMathFactsProven.v} (690 lines, zero axioms, zero admits):

\begin{itemize}
    \item \texttt{norm\_E\_bound}: $|E(x,y)| \leq 1$ from probability axioms
    \item \texttt{valid\_S\_4}: $|S| \leq 4$ from the triangle inequality
    \item \texttt{local\_S\_2}: $|S| \leq 2$ for local theories, by exhaustive 16-case enumeration
    \item \texttt{pr\_no\_ext}: PR-box monogamy (no extension to third party)
    \item \texttt{symm\_coh\_bound}: Symmetric coherence bound
    \item \texttt{tsir\_from\_coh}: Tsirelson's bound $2\sqrt{2}$ via algebraic coherence (mechanized using integer-scaled arithmetic with bound $= 28284272$ representing $\sqrt{2} \times 10^7$ to 7 decimal places)
\end{itemize}

Run \texttt{Print Assumptions} on the proven bundle and Coq reports only standard library axioms. The entire Bell-inequality foundation is self-contained.
...
% Inquisitor
\node[inq] (inq) at (0,-0.4) {\textbf{Inquisitor} (87 rules) --- 0 HIGH};
% Arrows
...
\begin{enumerate}
    ...
    \item \textbf{Quantum Axiom Conditionals}: No-cloning, unitarity, Born rule, purification, and Tsirelson bounds follow from conservation hypotheses---the physics is in the assumptions, the proofs verify logical entailment (zero Admitted across eight files)
    \item \textbf{Oracle Impossibility}: Halting is undecidable (genuine diagonal argument); oracle queries are priced at $\mu \geq 1$ by definition, and the pricing is self-consistent (zero axioms)
    \item \textbf{Turing Subsumption}: Any Turing machine can be simulated via TM $\to$ Minsky $\to$ Thiele compilation with step-by-step correspondence (zero axioms)
    \item \textbf{Hard Math Facts}: All 6 Bell-inequality foundations mechanically proven from first principles (zero axioms)
    \item \textbf{Completeness}: Zero admits, zero axioms---all proofs are machine-checked
\end{enumerate}

\textbf{A note on proof depth:} Some of these results are deeper than others. The no-signaling theorem requires genuine case analysis across 39 step constructors. The Tsirelson algebraic bound uses a real sum-of-squares identity. The Turing subsumption chain is a substantive compilation correctness proof. Other results---$\mu$-conservation, gauge invariance, oracle cost bounds---are closer to design consistency checks: they verify that properties the system was \textit{built to have} are indeed present. This is not a criticism. Engineering a system so that its key properties are trivially true is good design. But the thesis should be honest about which results are deep and which are definitional.
