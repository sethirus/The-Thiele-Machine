...
\begin{enumerate}
    ...
    \item \textbf{Oracle Impossibility}: No decidable function solves the halting problem; any oracle instruction must charge $\mu > 0$; the cost of $n$ independent halting queries is $\Omega(n)$ (\path{coq/kernel/OracleImpossibility.v})
...
\begin{enumerate}
    ...
    \item \textbf{Unitarity} (\path{coq/kernel/Unitarity.v}, 583 lines): Zero-cost evolution must be unitary. The theorem \texttt{nonunitary\_requires\_mu} proves that trace-preserving but non-unitary evolution requires positive $\mu$-cost. CPTP maps are characterized via \texttt{physical\_evolution\_is\_CPTP}, and Lindblad dissipation is bounded via \texttt{lindblad\_requires\_mu}.
    
    \item \textbf{Born Rule} (\path{coq/kernel/BornRule.v}, 321 lines): The probability rule $P = |a|^2$ is the unique rule consistent with linearity and $\mu$-conservation. The theorem \texttt{born\_rule\_from\_accounting} proves that any linear probability rule with zero extraction cost satisfies the Born rule constraints.
    
    \item \textbf{Purification} (\path{coq/kernel/Purification.v}, 280 lines): Every mixed state has a purification. The theorem \texttt{purification\_principle} proves that for any Bloch sphere point with $x^2 + y^2 + z^2 < 1$ (mixed), there exists a reference system such that the combined state is pure. The purification deficit equals $1 - \gamma$ where $\gamma$ is the purity.
    
    \item \textbf{Tsirelson Bound} (\path{coq/kernel/TsirelsonGeneral.v}, 315 lines): The bound $S \le 2\sqrt{2}$ follows from algebraic coherence. The theorem \texttt{tsirelson\_from\_minors} proves that any correlations satisfying a sum-of-squares constraint are bounded by $2\sqrt{2}$.
    
    \item \textbf{Born Rule from Tensor Consistency} (\path{coq/kernel/BornRuleFromSymmetry.v}, 939 lines): A non-circular derivation proving the Born rule from tensor product consistency alone, eliminating the linearity premise. The theorem \texttt{born\_rule\_from\_tensor\_consistency} establishes that any probability assignment compatible with composite systems must be the squared-amplitude rule.
    
    \item \textbf{Machine-Native No-Cloning} (\path{coq/kernel/NoCloningFromMuMonotonicity.v}, 260 lines): A self-contained no-cloning proof using only natural-number arithmetic (\texttt{lia}). The theorem \texttt{no\_cloning\_from\_mu\_monotonicity} shows perfect cloning requires $\mu \geq n$ for $n$-bit states.
    
    \item \textbf{Algebraic Tsirelson} (\path{coq/kernel/TsirelsonFromAlgebra.v}, 327 lines): A standalone algebraic derivation of the Tsirelson bound with achievability witness. The theorem \texttt{tsirelson\_bound\_proof} proves $S^2 \leq 8$ from $A^2 = B^2 = I$.
\end{enumerate}

\textbf{Total: 3,961 lines of Coq across eight files with zero Admitted statements.} These proofs establish that quantum mechanics isn't a collection of independent postulates---it's the unique physics consistent with information conservation.
...
Unitarity.v & 583 & \texttt{nonunitary\_requires\_mu} \checkmark \\
BornRule.v & 321 & \texttt{born\_rule\_from\_accounting} \checkmark \\
BornRuleFromSymmetry.v & 939 & \texttt{born\_rule\_from\_tensor\_consistency} \checkmark \\
NoCloningFromMuMono.v & 260 & \texttt{no\_cloning\_from\_mu\_monotonicity} \checkmark \\
Purification.v & 280 & \texttt{purification\_principle} \checkmark \\
TsirelsonGen.v & 315 & \texttt{tsirelson\_from\_minors} \checkmark \\
TsirelsonFromAlgebra.v & 327 & \texttt{tsirelson\_bound\_proof} \checkmark \\
\end{tabular}}
...
\begin{enumerate}
    \item \textbf{Halting undecidability} (\texttt{halting\_undecidable}): No total computable function decides halting. The proof uses the standard diagonal argument---assume a decider exists, construct a program that does the opposite of what the decider predicts, and derive a contradiction.
    
    \item \textbf{Oracle cost bound} (\texttt{oracle\_halts\_costs\_mu}): Any instruction that resolves an undecidable question must charge $\mu > 0$. A zero-cost oracle would violate No Free Insight, because it would allow the machine to gain information (decide halting) without paying structural cost.
    
    \item \textbf{Hypercomputation bound} (\texttt{hypercomputation\_bounded}): Even with oracle access, the cost of answering $n$ independent halting queries is $\Omega(n)$. You cannot amortize the cost away.
\end{enumerate}

\begin{quote}
\textit{Author's Note (Devon): This is the one that makes me proudest, honestly. The kernel has an \texttt{oracle\_halts} instruction---I put it there on purpose. But the proofs show that instruction can never be free. The machine acknowledges undecidability and prices it accordingly. It is not pretending to be a hypercomputer. It is a thermodynamic computer that charges you real $\mu$ for real answers.}
\end{quote}
...
\begin{enumerate}
    ...
    \item \textbf{Quantum Axioms}: No-cloning, unitarity, Born rule, purification, and Tsirelson bounds emerge from $\mu$-conservation (3,961 lines across eight files, zero Admitted)
    \item \textbf{Oracle Impossibility}: Halting is undecidable, oracle queries must charge $\mu > 0$, and $n$ queries cost $\Omega(n)$ (367 lines, zero axioms)
    \item \textbf{Turing Subsumption}: Any Turing machine can be simulated via TM $\to$ Minsky $\to$ Thiele compilation with step-by-step correspondence (776 lines, zero axioms)
    \item \textbf{Hard Math Facts}: All 6 Bell-inequality foundations mechanically proven from first principles (686 lines, zero axioms)
