...
\textbf{Formal verification} proves properties hold for \textit{all} inputs. When we prove "$\mu$ is monotonically non-decreasing," we don't test it on examples---we prove it mathematically.
...
Focus on understanding the \textit{statements} (what we prove), not the proof details.

\section{The Formal Verification Campaign}

The credibility of the Thiele Machine rests on machine-checked proofs. This chapter documents the formal verification campaign that culminated in December 2025 with "Campaign Zero Admits: Complete"---the elimination of all \texttt{Admitted}, \texttt{admit.}, and \texttt{Axiom} declarations from the active Coq tree.
...
The formalization resides in \texttt{coq/kernel/} with 34 verified modules. Core files:
...
\section{VMState.v: Foundation Definitions}
...
\begin{proof}
By \texttt{nodup\_nodup\_iff}: applying \texttt{nodup} twice yields the same result.
\end{proof}
...
\begin{verbatim}
Lemma graph_remove_preserves_wf : forall g mid,
  well_formed_graph g ->
  well_formed_graph (graph_remove g mid).
\end{verbatim}
\end{theorem}

\section{VMStep.v: Operational Semantics}
...
\begin{itemize}
    \item Deterministic: Each (state, instruction) pair has at most one successor
    \item Total on valid inputs: Well-formed states have defined transitions
    \item Cost-charging: Every rule updates \texttt{vm\_mu}
\end{itemize}

\section{KernelPhysics.v: Conservation Laws}
...
Note: Axioms are \textbf{not} observable—they are internal implementation details.
...
\section{MuLedgerConservation.v: Multi-Step Conservation}
...
\section{NoFreeInsight.v: The Impossibility Theorem}
...
\begin{proof}
By \texttt{nonlocal\_correlation\_requires\_revelation} from \texttt{RevelationRequirement.v}. The structure-addition analysis shows that if \texttt{csr\_cert\_addr} starts at 0 and ends non-zero (\texttt{has\_supra\_cert}), some instruction in the trace must have set it.
\end{proof}
...
\section{RevelationRequirement.v: Supra-Quantum Certification}
...
\subsection{Campaign Zero Admits}
...
\subsection{Final State (December 2025)}

\begin{itemize}
    \item \textbf{Files}: 34 kernel files
    \item \textbf{Total Theorems/Lemmas}: 229 proven theorems and lemmas
    \item \textbf{Admitted}: 0
    \item \textbf{Axioms}: 0
    \item \textbf{Lines of Coq}: $\sim$8,000
\end{itemize}
...
\hline
\texttt{observational\_no\_signaling} & KernelPhysics.v & PROVEN \\
\texttt{mu\_conservation\_kernel} & KernelPhysics.v & PROVEN \\
\texttt{kernel\_noether\_mu\_gauge} & KernelPhysics.v & PROVEN \\
\texttt{run\_vm\_mu\_conservation} & MuLedgerConservation.v & PROVEN \\
\texttt{vm\_irreversible\_bits\_lower\_bound} & MuLedgerConservation.v & PROVEN \\
\texttt{no\_free\_insight\_general} & NoFreeInsight.v & PROVEN \\
\texttt{strengthening\_requires\_structure\_addition} & NoFreeInsight.v & PROVEN \\
\texttt{nonlocal\_correlation\_requires\_revelation} & RevelationRequirement.v & PROVEN \\
\texttt{normalize\_region\_idempotent} & VMState.v & PROVEN \\
\texttt{graph\_add\_module\_preserves\_wf} & VMState.v & PROVEN \\
\hline
\end{tabular}
\end{center}
