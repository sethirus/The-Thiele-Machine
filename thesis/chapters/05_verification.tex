...
\begin{enumerate}
    ...
    \item \textbf{Born Rule from Tensor Consistency} (\path{coq/kernel/BornRuleFromSymmetry.v}): An alternative derivation proving the Born rule from tensor product consistency, replacing the linearity premise with a composite-system constraint. The theorem \texttt{born\_rule\_from\_tensor\_consistency} establishes that any probability assignment compatible with composite systems must be the squared-amplitude rule.
    
    \item \textbf{Machine-Native No-Cloning} (\path{coq/kernel/NoCloningFromMuMonotonicity.v}): A self-contained no-cloning proof using only natural-number arithmetic (\texttt{lia}). The theorem \texttt{no\_cloning\_from\_mu\_monotonicity} shows perfect cloning requires $\mu \geq n$ for $n$-bit states.
    
    \item \textbf{Algebraic Tsirelson} (\path{coq/kernel/TsirelsonFromAlgebra.v}): A standalone algebraic derivation of the Tsirelson bound with achievability witness. The theorem \texttt{tsirelson\_bound\_proof} proves $S^2 \leq 8$ from $A^2 = B^2 = I$ via a sum-of-squares identity.
\end{enumerate}
...
BornRuleFromSymmetry.v & \texttt{born\_rule\_from\_tensor\_consistency} \checkmark & Tensor consistency $\Rightarrow$ Born rule \\
NoCloningFromMuMono.v & \texttt{no\_cloning\_from\_mu\_monotonicity} \checkmark & Natural-number accounting \\
...
TsirelsonFromAlgebra.v & \texttt{tsirelson\_bound\_proof} \checkmark & Sum-of-squares identity \\
\end{tabular}}
...
\path{coq/kernel/OracleImpossibility.v} (367 lines, zero axioms, zero admits) proves three results:
