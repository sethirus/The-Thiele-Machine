...
\end{itemize}
The guarantees come from the small, trusted kernel of Coq. Every lemma in the thesis is checked against that kernel, and extraction produces executable code whose behavior is connected to the same definitions. This matters because the extracted runner is used as an oracle in isomorphism tests; the proof context and the executable context are tied to one semantics, not two ad hoc models.
...
\begin{enumerate}
    ...
    \item \textbf{Certificate checkers}: LRAT proof verifier plus SAT model and countermodel validators in \path{coq/kernel/CertCheck.v}
...
\end{quote} This matters because it supports a concrete claim: the active Coq tree compiles without \texttt{Admitted}/\texttt{admit.}, and assumptions are explicit in theorem contexts instead of hidden as global shortcuts.
...
\begin{itemize}
    ...
    \item \textbf{0 global axioms}: All assumptions are explicit \texttt{Context} parameters within labeled \texttt{Section} blocks, so assumptions do not leak into the global namespace.
...
The strictness is not ceremonial: it makes each theorem reusable as a checked dependency in later proofs. It does not remove all trust assumptions; it localizes them. Assumptions still exist, but they are attached to specific sections and theorem statements where they can be inspected, challenged, and replaced.
...
\begin{enumerate}
    ...
    \item \textbf{Born Rule} (\path{coq/kernel/BornRule.v}): The theorem \texttt{born\_rule\_from\_accounting} proves that any probability rule satisfying normalization, eigenstate conditions ($P(0,0,1,0)=1$, $P(0,0,-1,0)=0$), and linearity in the Bloch $z$-component must equal $(1+z)/2$. That is the Born rule for qubit measurements along $z$. The key assumption doing all the work is \texttt{is\_linear\_in\_z}---linearity of probabilities in the Bloch parameter. The \texttt{mu\_consistent} hypothesis is present but unused in the proof; it reduces to the Bloch ball constraint already given. This is less ``deriving the Born rule from accounting'' and more ``the Born rule is the unique linear rule consistent with eigenstates.''
    ...
    \item \textbf{Tsirelson Bound} (\path{coq/kernel/TsirelsonGeneral.v}): The theorem \texttt{tsirelson\_from\_minors} proves: if four correlation values satisfy row-norm constraints ($e_{00}^2 + e_{01}^2 \leq 1$ and $e_{10}^2 + e_{11}^2 \leq 1$), then $S^2 \leq 8$. This is legitimate algebra, proven via a sum-of-squares decomposition identity. The result is correct and the proof is real. The row-norm constraints themselves come from the NPA-1 moment matrix (a quantum-mechanical object), so the physics is in the hypotheses, but the algebraic bound is genuinely derived.
...
BornRule.v & \texttt{born\_rule\_from\_accounting} \checkmark & Linearity + boundary $\Rightarrow$ unique rule \\
...
TsirelsonGeneral.v & \texttt{tsirelson\_from\_minors} \checkmark & Genuine algebraic bound \\
...
All proofs are verified by Coq 8.18.x. The Inquisitor enforces this invariant: any commit introducing an admit or undocumented axiom fails CI. The static analysis also detects vacuous statements, tautological claims, and hidden assumptions. See \path{scripts/inquisitor.py} and \path{scripts/inquisitor\_rules.py} for complete documentation of the 100 scan rules and enforcement policies.
...
\textbf{What does this do?} This function removes duplicate bit indices from a region list and returns the canonical (deduplicated) form. If a region is $[3, 7, 3, 5]$, normalization yields $[3, 7, 5]$ (exact order may vary by \texttt{nodup} implementation, but duplicates are proven removed).
...
\textbf{Why is this important?} Graph operations may compose: you might split a module, then merge two modules, then split again. Each operation normalizes regions internally. Without idempotence, repeated normalization could change the canonical form unpredictably. Idempotence proves stability: once a region is normalized, further normalization is a no-op.
...
\end{proof}
This lemma is more than a tidying step. Observational equality depends on normalized regions; idempotence proves that repeated normalization does not change what an observer sees, which is vital when a proof chains multiple graph operations together.
...
\textbf{Why is this important?} Graph operations (PNEW, PSPLIT, PMERGE) all rely on unique module IDs. If a module could have an ID out of bounds, lookups would fail unpredictably. The well-formedness invariant proves that every module ID is valid.
...
\textbf{Why is this important?} The PNEW instruction (partition new) creates a fresh module by calling \texttt{graph\_add\_module}. If this operation could violate well-formedness, the entire graph would become corrupted. This lemma proves that PNEW is safe: starting from a well-formed graph, PNEW produces a well-formed graph.
...
\textbf{Why is this important?} The PMERGE instruction removes two modules and creates a merged module. If removal could violate well-formedness, PMERGE would be unsafe. This lemma proves that removal is safe: all remaining modules still have valid IDs.
...
\textbf{Inductive type:} In Coq, an \texttt{Inductive} type defines a set of constructors. \texttt{vm\_instruction} has 40 constructors, each representing one instruction. No other instructions exist---the type is closed.
...
\begin{itemize}
    ...
    \item \textbf{Oracle:} \texttt{instr\_oracle\_halts} — Halting oracle queries.
...
\textbf{Comparison to Chapter 3:} This is the exact same type, but Chapter 5 emphasizes the \textit{proof} structure: how theorems quantify over instructions, how case analysis works in Coq, and how the closed type proves exhaustiveness.
...
\begin{itemize}
    ...
    \item \textbf{:= ...} — The body (not shown) contains 50 step rules, one or more per instruction constructor (success and failure branches), defining exactly how each instruction transforms state.
\end{itemize}
...
\textbf{Physical interpretation:} This theorem enforces \textbf{causal structure}. Just as special relativity forbids faster-than-light signaling, the Thiele Machine forbids action-at-a-distance in the partition graph. The partition structure defines a ``space,'' and this theorem proves spatial locality.
...
\textbf{Why this matters:} The result is definitional in isolation, but still important for composition. It pins down that absolute $\mu$ is an offset, while partition structure depends on graph fields. That separation is what later proofs use when they reason about equivalence classes of states under ledger shifts.
...
\textbf{Concrete example:} If \texttt{s.vm\_mu = 50} and you execute PNEW with \texttt{mu\_delta = 10}, then \texttt{s'.vm\_mu = 60}. The theorem proves $60 \geq 50$. If you execute 5 instructions with costs $[10, 15, 20, 5, 8]$, the final $\mu$ is $50 + 10 + 15 + 20 + 5 + 8 = 108$, and the theorem proves $108 \geq 50$ after each step.
...
\end{itemize}
The corollary proves this exact accounting.
...
\end{enumerate}
The theorem proves you can't skip the positive-cost bridge and still certify the correlation.
...
\textbf{A note on proof depth:} Some of these results are deeper than others. The no-signaling theorem requires genuine case analysis across 40 step constructors. The Tsirelson algebraic bound uses a real sum-of-squares identity. The Turing subsumption chain is a substantive compilation correctness proof. Other results---$\mu$-conservation, gauge invariance, oracle cost bounds---are closer to design consistency checks: they verify that properties the system was \textit{built to have} are indeed present. This is not a criticism. Engineering a system so that its key properties hold by construction is good design. But the thesis should be honest about which results are deep and which are definitional.
