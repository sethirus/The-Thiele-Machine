...
\begin{tikzpicture}[scale=1.8, 
    node distance=3.0cm,
    block/.style={rectangle, draw, fill=blue!10, minimum width=5.4cm, minimum height=2.0cm, align=center},
    theorem/.style={rectangle, draw, fill=green!12, minimum width=5.0cm, minimum height=1.2cm, align=center, rounded corners=3pt},
    arrow/.style={->, >=Stealth, thick}
]
    % Central node
    \node[block, fill=red!12, minimum width=7.2cm, align=center, text width=4.5cm] (zero) at (0,0) {\textbf{Zero-Admit Standard}\\No Admitted, No admit., No Axiom};
    
    % Top layer: State definitions
    \node[block, align=center, text width=4.5cm] (state) at (-4, 2) {State Definitions\\VMState, PartitionGraph};
    \node[block, align=center, text width=4.5cm] (step) at (0, 2) {Step Semantics\\vm\_step relation};
    \node[block, align=center, text width=4.5cm] (obs) at (4, 2) {Observables\\Observable, ObservableRegion};
    
    % Bottom layer: Theorems
    \node[theorem, align=center, text width=4.5cm] (nosig) at (-4.5, -2) {No-Signaling\\(\S5.4)};
    \node[theorem, align=center, text width=4.5cm] (mucons) at (-1.5, -2) {$\mu$-Conservation\\(\S5.4)};
    \node[theorem, align=center, text width=4.5cm] (nofi) at (1.5, -2) {No Free Insight\\(\S5.6)};
    \node[theorem, align=center, text width=4.5cm] (gauge) at (4.5, -2) {Gauge Invariance\\(\S5.4)};
...
\centering
\begin{tikzpicture}[scale=1.8, 
    stage/.style={rectangle, draw, fill=blue!12, minimum width=5.5cm, minimum height=2.1cm, align=center, rounded corners=3pt},
    check/.style={rectangle, draw, fill=green!12, minimum width=4.5cm, minimum height=2.0cm, align=center, font=\footnotesize},
    arrow/.style={->, >=Stealth, thick}
, node distance=3.0cm]
    % Pipeline stages
    \node[stage, align=center, text width=4.5cm] (def) at (0, 0) {Definitions\\VMState, vm\_step};
    \node[stage, align=center, text width=4.5cm] (spec) at (4, 0) {Specification\\Theorem stmt};
    \node[stage, align=center, text width=4.5cm] (proof) at (8, 0) {Proof\\Tactics};
    \node[stage, fill=green!30, align=center, text width=4.5cm] (qed) at (12, 0) {\textbf{Qed.}\\Verified};
    ...
    % Curry-Howard note
    \node[draw, fill=yellow!15, rounded corners=3pt, text width=10cm, align=center] at (6, -3) {
...
\centering
\begin{tikzpicture}[scale=1.8, 
    module/.style={rectangle, draw, fill=blue!12, minimum width=4.2cm, minimum height=2.2cm, align=center},
    region/.style={rectangle, draw, dashed, fill=gray!10, minimum width=5.0cm, minimum height=2.9cm},
    arrow/.style={->, >=Stealth, thick}
, node distance=3cm]
    ...
    \node[module, align=center, text width=4.5cm, font=
ormalsize] (m1) at (-5.5, 0.5) {Module\\$mid_1$};
    \node at (-5.5, -0.8) {\footnotesize Region $R_1$};
    
    \node[region] (r2) at (-2.5, 0.5) {};
    \node[module, fill=green!30, align=center, text width=4.5cm, font=
ormalsize] (m2) at (-2.5, 0.5) {Module\\$mid_2$};
    ...
    \draw[->, >=Stealth, line width=1.5pt, red] (-0.8, 0.5) -- (0.8, 0.5);
...
ormalsize] (m1p) at (2.5, 0.5) {Module\\$mid_1$};
    ...
    \node[module, fill=orange!30, align=center, text width=4.5cm, font=
ormalsize] (m2p) at (5.5, 0.5) {Module\\$mid_2'$};
    ...
    \node[draw, fill=yellow!15, rounded corners=3pt, text width=10cm, align=center] at (0, -3) {
...
\begin{tikzpicture}[scale=1.8, 
    ...
    field/.style={rectangle, draw, fill=white, minimum width=4.7cm, minimum height=1.3cm, align=center},
    arrow/.style={->, >=Stealth, thick}
, node distance=3cm]
    % Left state
    \node[state, font=
ormalsize] (s1) at (-3.5, 0) {};
    ...
    \node[field, fill=red!12] at (-3.5, -0.2) {\texttt{vm\_mu} = $\mu$};
    \node[field] at (-3.5, -0.9) {\texttt{vm\_regs, ...}};
    
    % Arrow with shift
    \draw[->, >=Stealth, line width=1.5pt, blue] (-1.5, 0) -- (1.5, 0);
...
ormalsize] (s2) at (3.5, 0) {};
    \node at (3.5, 1.4) {\textbf{State $s'$}};
    \node[field] at (3.5, 0.5) {\texttt{vm\_graph} = $G$};
    \node[field, fill=green!12] at (3.5, -0.2) {\texttt{vm\_mu} = $\mu + k$};
    \node[field] at (3.5, -0.9) {\texttt{vm\_regs, ...}};
    
    % Invariance annotation
    \draw[<->, >=Stealth, line width=1.2pt, dashed, red] (-3.5, -2) -- (3.5, -2);
    \node at (0, -2.5) {\texttt{conserved\_partition\_structure}$(s) =$ \texttt{conserved\_partition\_structure}$(s')$};
    
    % Physical analog
    \node[draw, fill=yellow!15, rounded corners=3pt, text width=8cm, align=center] at (0, -4) {
...
\centering
\begin{tikzpicture}[scale=1.8, 
    state/.style={circle, draw, fill=blue!12, minimum size=1cm},
    arrow/.style={->, >=Stealth, thick}
, node distance=3cm]
    % States in sequence
    \node[state, font=
...
ormalsize] (s3) at (9, 0) {$s_3$};
    ...
    \draw[arrow] (s0) -- node[above, font=\small] {\footnotesize $+\mu_2$} (s2);
    \draw[arrow] (s2) -- node[above, font=\small] {\footnotesize $+\mu_3$} (s3);
    ...
    % Monotonicity arrow
    \draw[->, >=Stealth, line width=1.5pt, red, dashed] (0, -1.8) -- (11, -1.8);
    \node at (5.5, -2.3) {\color{red}\textbf{Monotonically Non-Decreasing}};
    
    % Conservation equation
    \node[draw, fill=green!12, rounded corners=3pt, text width=8cm, align=center] at (5, -3.8) {
...
, node distance=3.0cm]
    ...
    \node[set, fill=green!12, minimum width=5.5cm, minimum height=2.5cm] (strong) at (3, 0) {};
    \node at (3, 0) {$P_{\text{strong}}$};
    \node at (3, -1.5) {\footnotesize Accepts fewer traces};
    
    % Arrow showing strengthening
    \draw[->, >=Stealth, line width=1.5pt, red] (-0.8, 0) -- (0.8, 0);
    \node at (0, 0.6) {\textbf{Strengthen}};
    \node at (0, -0.5) {\footnotesize $P_{\text{strong}} < P_{\text{weak}}$};
    
    % Cost requirement
    \node[draw, fill=red!12, rounded corners=3pt] at (0, -2.5) {\textbf{Requires: structure addition ($\mu > 0$)}};
    
    % Trace box
    \node[draw, fill=yellow!15, rounded corners=3pt, text width=10cm, align=center] at (0, -4.2) {
...
\centering
\begin{tikzpicture}[scale=1.8, 
    theorem/.style={rectangle, draw, fill=green!12, minimum width=7.2cm, minimum height=2.2cm, align=center, rounded corners=3pt},
    property/.style={rectangle, draw, fill=blue!10, minimum width=6.2cm, minimum height=2.0cm, align=center},
    arrow/.style={->, >=Stealth, thick}
, node distance=3.0cm]
    % Central: Zero-Admit Standard
    \node[draw, fill=red!30, minimum width=9.0cm, minimum height=2.1cm, rounded corners=3pt, align=center, text width=4.5cm] (zero) at (0, 0) {\textbf{Zero-Admit Standard}\\206 Proofs, 0 Axioms};
    
    % Four theorem boxes around it
    \node[theorem, align=center, text width=4.5cm] (nosig) at (-4, 2.5) {\textbf{No-Signaling}\\Bell locality analog};
    \node[theorem, align=center, text width=4.5cm] (mucons) at (4, 2.5) {\textbf{$\mu$-Conservation}\\Monotonic ledger};
    \node[theorem, align=center, text width=4.5cm] (nofi) at (-4, -2.5) {\textbf{No Free Insight}\\Impossibility theorem};
    \node[theorem, align=center, text width=4.5cm] (gauge) at (4, -2.5) {\textbf{Gauge Invariance}\\Noether's theorem};
    ...
    % Bottom: Inquisitor
    \node[draw, fill=purple!20, minimum width=14.4cm, minimum height=2.0cm, rounded corners=3pt] at (0, -4.5) {\textbf{Inquisitor} enforces standard via CI (25+ rule categories)};
