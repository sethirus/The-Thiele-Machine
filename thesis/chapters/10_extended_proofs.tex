...
\textbf{Application-Level Proofs} (in \path{coq/thielemachine/coqproofs/})\textbf{:}
\begin{itemize}
...
All in \path{coq/thielemachine/coqproofs/}:
\begin{itemize}
...
The file \path{coq/kernel/FiniteInformation.v} (20KB, 18 theorems) proves information-theoretic properties without axioms:
...
The file \path{coq/kernel/Locality.v} (17KB, 13 lemmas) proves locality for every instruction:
...
The file \path{coq/kernel/LocalInfoLoss.v} (17KB, 8 theorems) connects information theory to $\mu$-cost:
...
The file \path{coq/kernel/HardAssumptions.v} (9KB) provides explicit documentation of all non-trivial assumptions:
...
\textbf{Why this matters:} Rather than hiding assumptions as global axioms, this file makes every non-trivial assumption explicit and documented. The Inquisitor can verify that no undocumented assumptions exist.

\subsection{The $\mu$-Initiality Theorem}

The file \path{coq/kernel/MuInitiality.v} (14KB, 13 theorems) proves the strongest possible statement about the $\mu$-ledger: it is not merely \textit{a} monotone cost accumulator, but \textit{the} canonical one—the initial object in the category of instruction-consistent cost functionals.
...
\textbf{Proof status:} Zero axioms, zero admits. Both \texttt{mu\_is\_initial\_monotone} and \texttt{mu\_initiality} are closed under the global context.

\subsection{The $\mu$-Landauer Validity Theorem}

The file \path{coq/kernel/MuNecessity.v} proves that $\mu$ satisfies Landauer's erasure bound—the physical constraint that erasing distinguishability costs at least the information destroyed.
...
\textbf{Proof status:} Zero axioms, zero admits. \texttt{mu\_is\_landauer\_valid} and \texttt{landauer\_valid\_bounds\_total\_loss} are closed under the global context.
...
The quantum axiom proofs live in eight files totaling 3,961 lines of Coq with zero \texttt{Admitted} statements:
...
\midrule
\texttt{NoCloning.v} & 936 & 20 & No-cloning from conservation \\
\texttt{NoCloningFromMuMonotonicity.v} & 260 & 11 & Machine-native no-cloning (\texttt{lia}) \\
\texttt{Unitarity.v} & 583 & 25 & CPTP from irreversibility \\
\texttt{BornRule.v} & 321 & 20 & Born rule from linearity \\
\texttt{BornRuleFromSymmetry.v} & 939 & 33 & Born rule from tensor consistency \\
\texttt{Purification.v} & 280 & 11 & Purification principle \\
\texttt{TsirelsonGeneral.v} & 315 & 19 & Tsirelson bound from algebra \\
\texttt{TsirelsonFromAlgebra.v} & 327 & 13 & Self-contained Tsirelson with witness \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{Machine-native proof (\texttt{NoCloningFromMuMonotonicity.v}).}
A second, self-contained no-cloning proof works entirely in natural-number arithmetic, avoiding any real-analysis imports. The key theorem \texttt{no\_cloning\_from\_mu\_monotonicity} shows that perfect cloning would require at least $\mu = n$ additional bits for an $n$-bit state, contradicting the zero-cost hypothesis. All proof obligations close with \texttt{lia} (linear integer arithmetic), making the argument fully decidable.

The file also proves:
\begin{itemize}
    \item \texttt{approximate\_cloning\_bound}: Approximate cloning fidelity is bounded by $\mu$ budget
    \item \texttt{no\_deletion\_without\_cost}: Quantum deletion also requires $\mu$ expenditure
    \item \texttt{broadcasting\_bound}: Broadcasting mixed states has $\mu$-dependent limits
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{depolarization\_cost}: Specific cost formula for depolarizing channels
\end{itemize}
...
\paragraph{Non-circular derivation (\texttt{BornRuleFromSymmetry.v}).}
The original \texttt{BornRule.v} takes linearity as a premise. A separate 939-line development derives the Born rule from \textbf{tensor product consistency} alone, eliminating the circularity concern. The key theorem \texttt{born\_rule\_from\_tensor\_consistency} establishes that any probability assignment compatible with tensor products on a composite Hilbert space must be the squared-amplitude rule. The proof proceeds by: (1)~defining separable states and their tensor structure, (2)~proving that consistency under partial trace forces multiplicativity of probabilities across subsystems, (3)~showing that multiplicativity plus normalization uniquely determines the Born rule. All obligations discharge via \texttt{lra}/\texttt{nra}/\texttt{field}; the file contains zero \texttt{Admitted} statements.
...
\begin{itemize}
    ...
    \item \texttt{measurement\_disturbance\_bound}: How much measurement disturbs the state
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{purification\_uniqueness\_up\_to\_isometry}: Purifications are unique up to isometries on the ancilla
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{chsh\_achieved\_by\_maximally\_entangled}: The bound is tight
    \item \texttt{supra\_tsirelson\_requires\_mu}: Exceeding $2\sqrt{2}$ requires $\mu$ expenditure
\end{itemize}

\paragraph{Self-contained algebraic proof (\texttt{TsirelsonFromAlgebra.v}).}
A standalone 327-line development derives the Tsirelson bound from first principles without importing \texttt{TsirelsonGeneral.v}. The proof defines a \texttt{CHSHSetup} with four observables satisfying $A^2 = B^2 = I$, constructs the CHSH operator $S = AB + AB' + A'B - A'B'$, and proves $S^2 \leq 8I$ by direct algebraic expansion. The square root gives $S \leq 2\sqrt{2}$. The file also includes an achievability witness (\texttt{tsirelson\_achieved}) and a rational bound $S \leq 5657/2000$ (\texttt{tsirelson\_rational\_bound}).
...
\textbf{What does this theorem prove?} This is the \textbf{definitive Theory of Everything (TOE) no-go theorem}. It establishes exactly which physical structures are \textit{forced by the kernel semantics} and which are \textit{not forced}. It answers the question: ``Can I derive all of physics from the kernel alone?'' The answer is: \textit{No. The kernel forces locality and causality, but not probability or geometry.}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Non-cert-setter instructions preserve \texttt{csr\_cert\_addr} (proven by case analysis on all 18 instructions).
...
\begin{enumerate}
    ...
    \item \textbf{Quantum axioms from $\mu$-accounting}: No-cloning, unitarity, Born rule, purification, and Tsirelson bound all follow from conservation-shaped hypotheses (3,961 lines, 152 definitions/theorems across eight files). The physics is in the assumptions; the proofs verify logical entailment. Three additional proofs address circularity: \texttt{BornRuleFromSymmetry.v} derives the Born rule from tensor consistency alone, \texttt{NoCloningFromMuMonotonicity.v} proves no-cloning in pure integer arithmetic, and \texttt{TsirelsonFromAlgebra.v} gives a self-contained algebraic Tsirelson derivation with achievability witness.
