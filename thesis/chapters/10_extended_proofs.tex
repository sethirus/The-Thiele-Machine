...
\textbf{Why this matters more than paper proofs:} A human could write ``By induction on $n$, trivially.'' That \textit{looks} correct, but glosses over whether each step actually follows from the definitions. I've seen plenty of ``obvious'' claims in papers that turn out to be wrong. Coq eliminates that failure mode---it forces every step to be justified, and if any step is wrong, it refuses to say \texttt{Qed.}
