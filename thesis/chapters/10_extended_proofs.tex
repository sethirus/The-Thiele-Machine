...
\textbf{Physical interpretation:} This theorem elevates $\mu$ from ``a sound lower bound'' to ``the canonical physical cost.'' Any instruction-consistent accounting of irreversibility \textit{is} $\mu$ by mathematical necessity. This is why we claim ``$\mu$ is not metaphor''—it is the unique object satisfying the axioms.
...
\textbf{Epistemic honesty:} We do NOT prove ``any Landauer-valid cost $\geq \mu$'' because Landauer only constrains information-destroying operations. For non-erasing operations, Landauer permits $C(i) = 0$ while $\mu$ may charge $> 0$. What we prove is that $\mu$ itself is Landauer-valid and tight for structural operations.
...
The kernel extension on quantum axioms proves that the ``axioms'' of quantum mechanics---properties usually taken as foundational postulates---emerge from pure $\mu$-accounting. These aren't approximations or analogies. They're machine-checked derivations showing that if you enforce conservation of structural ignorance, quantum mechanics \textit{falls out}.
...
\textbf{What this proves:} If you want a ``cloner'' function that takes any quantum state $|\psi\rangle$ and produces two independent copies---both equal to the original, both at zero $\mu$-cost---then you're asking for the impossible. The theorem derives \texttt{False}, meaning such a cloner cannot exist.

\textbf{Why it works:} Cloning requires creating new distinguishability (two systems that respond identically to all measurements). Creating distinguishability \textit{is} structural information. The $\mu$-ledger tracks this. Zero-cost cloning would violate conservation.
...
\textbf{What this proves:} Any evolution that isn't unitary but \textit{is} physical (maps valid states to valid states) must cost $\mu > 0$. Unitary evolution is the \textit{only} free operation on isolated quantum systems.

\textbf{Why it works:} Non-unitary evolution shrinks the Bloch sphere (takes pure states to mixed states). This is decoherence---the system becomes more entangled with its environment. Entanglement creates new structural relationships that cost $\mu$ to establish.
...
\textbf{What this proves:} If your probability rule (1) gives normalized probabilities and (2) is linear in the density matrix, then it \textit{must} be the Born rule. There's no freedom here---linearity plus normalization pins down $P = |\langle\phi|\psi\rangle|^2$ uniquely.

\textbf{Why it works:} The Bloch sphere representation makes this transparent. A linear functional on Bloch vectors that sums to 1 over orthogonal bases has exactly one form: $P = \frac{1}{2}(1 + \vec{r} \cdot \hat{n})$ where $\vec{r}$ is the Bloch vector and $\hat{n}$ is the measurement direction. This \textit{is} the Born rule.
...
\textbf{What this proves:} Every mixed state can be ``purified''---viewed as the partial trace of some pure state on a larger system. The mixedness isn't fundamental; it's ignorance about correlations with an environment.

\textbf{Why it works:} A Bloch vector inside the sphere (mixed) can always be written as a convex combination of surface points (pure). In the density matrix picture, this convex decomposition corresponds to tracing out an ancilla. The construction is explicit.
...
\textbf{What this proves:} The maximum CHSH value for any quantum-realizable correlation matrix is $2\sqrt{2} \approx 2.828$. This is the Tsirelson bound, derived here from algebraic constraints on correlation matrices.
...
These proofs establish that quantum mechanics isn't a collection of arbitrary postulates. The rules emerge from a single principle: \textbf{structural information is conserved}. You can't clone because cloning creates information. Evolution is unitary because non-unitary evolution destroys information (or creates entanglement, which costs $\mu$). The Born rule is forced by tensor product consistency (\texttt{BornRuleFromSymmetry.v})---any probability assignment compatible with composite systems must be the squared-amplitude rule. Tsirelson bounds correlations because stronger correlations would require revealing partition structure.

This is the kernel-level foundation for all the quantum physics in this thesis. When we claim the Thiele Machine can achieve supra-quantum correlations, we mean: it can pay the $\mu$ cost that the proofs show is required.
...
\begin{enumerate}
    ...
    \item \textbf{Quantum axioms from $\mu$-accounting}: No-cloning, unitarity, Born rule, purification, and Tsirelson bound all derived from conservation of structural information (3,961 lines, 152 definitions/theorems across eight files). Three additional proofs close circularity gaps: \texttt{BornRuleFromSymmetry.v} derives the Born rule from tensor consistency alone, \texttt{NoCloningFromMuMonotonicity.v} proves no-cloning in pure integer arithmetic, and \texttt{TsirelsonFromAlgebra.v} gives a self-contained algebraic Tsirelson derivation with achievability witness.
