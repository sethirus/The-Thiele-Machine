...
\end{itemize}
The corpus is split between the kernel (\texttt{coq/kernel/}), which remains active, and the extended proofs developed before the kernel crystallized. Many of those files have since been superseded by the kernel's consolidated proof surface and are no longer part of the active build tree. This chapter documents those formalizations for completeness.
...
\end{itemize}
For readers navigating the code, the ``kernel semantics'' block corresponds to files such as \texttt{VMState.v} and \texttt{VMStep.v} under \texttt{coq/kernel/}. The ``extended machine proofs'' (e.g.\ \texttt{PartitionLogic.v}, \texttt{Subsumption.v}) were developed before the kernel layer existed and are no longer part of the active build tree. The structure is intentionally layered so that higher-level proofs explicitly import the kernel rather than re-deriving it.
...
\texttt{NoCloningFromMuMonotonicity.v} & 281 & 14 & Machine-native no-cloning (\texttt{lia}), superseded \\
\texttt{Unitarity.v} & 592 & 25 & CPTP from irreversibility \\
\texttt{BornRule.v} & 342 & 21 & Born rule from linearity \\
\texttt{BornRuleFromSymmetry.v} & 144 & 5 & Born rule from tensor consistency, superseded \\
...
\paragraph{Machine-native proof (\texttt{NoCloningFromMuMonotonicity.v}, superseded).}
This file has been superseded by the active no-cloning proof in \texttt{kernel/NoCloning.v}, which derives no-cloning from unitarity.

The superseded version contains a self-contained no-cloning proof working entirely in natural-number arithmetic, avoiding any real-analysis imports. The key theorem \texttt{no\_cloning\_mu\_monotonicity} shows that perfect cloning would require at least $\mu = n$ additional bits for an $n$-bit state, contradicting the zero-cost hypothesis. All proof obligations close with \texttt{lia} (linear integer arithmetic), making the argument fully decidable.
...
\paragraph{Existence proof (\texttt{BornRuleFromSymmetry.v}, superseded).}
...
The following files were developed during the modular simulation proof campaign. They are no longer part of the active build tree but established the TM $\to$ Minsky $\to$ Thiele reduction chain:
