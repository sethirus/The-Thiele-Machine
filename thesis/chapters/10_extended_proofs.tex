...
\section{The ThieleMachine Proof Suite (71 Files)}
...
(* ... similar lemmas for all 31 instructions ... *)
\end{lstlisting}
...
The file \path{coq/kernel/HardAssumptions.v} (~8KB) provides explicit documentation of all non-trivial mathematical facts used by the Bell-inequality reasoning:
...
\texttt{NoCloningFromMuMonotonicity.v} & 281 & 14 & Machine-native no-cloning (\texttt{lia}) \\
\texttt{Unitarity.v} & 592 & 25 & CPTP from irreversibility \\
\texttt{BornRule.v} & 342 & 21 & Born rule from linearity \\
\texttt{BornRuleFromSymmetry.v} & 144 & 5 & Born rule from tensor consistency \\
...
A second, self-contained no-cloning proof works entirely in natural-number arithmetic, avoiding any real-analysis imports. The key theorem \texttt{no\_cloning\_mu\_monotonicity} shows that perfect cloning would require at least $\mu = n$ additional bits for an $n$-bit state, contradicting the zero-cost hypothesis. All proof obligations close with \texttt{lia} (linear integer arithmetic), making the argument fully decidable.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Non-cert-setter instructions preserve \texttt{csr\_cert\_addr} (proven by case analysis on all 31 instructions).
