...
\begin{quote}
\textit{Author's Note (Devon): Alright, this is the deep end. If you're reading this chapter, you're either really curious or really masochistic. Either way, I respect it. These are the proofs that took me months---sometimes I'd spend a whole week on a single lemma. But every time Coq said ``Qed,'' it meant that lemma was \textbf{done}. Not ``probably true.'' Not ``seems right.'' Done. Forever. That's the payoff for all the suffering.}
\end{quote}
...
\begin{itemize}
    \item \textbf{262 proof files}: 55,097 lines of Coq across kernel, proofs, and bridges
    \item \textbf{1,637 theorems}: Including theorems, lemmas, propositions, and corollaries
    \item \textbf{Zero admits}: No proof is left incomplete (kernel only; physics\_exploration has documented axioms)
    \item \textbf{Zero axioms}: No unproven assumptions in kernel (beyond foundational logic)
    \item \textbf{Full extraction}: Proofs can be compiled to executable code
\end{itemize}
The corpus is split between the kernel (\texttt{coq/kernel/}, zero axioms), extended proofs (\texttt{coq/thielemachine/coqproofs/}), and physics exploration (\texttt{coq/physics\_exploration/}, 4 files with 11 physical axioms). This division mirrors the conceptual separation between the core semantics, the larger ecosystem of applications, and speculative physical derivations.
...
\section{The ThieleMachine Proof Suite}
...
The file \path{coq/kernel/FiniteInformation.v} (21KB) proves information-theoretic properties without axioms:
...
The file \path{coq/kernel/ProperSubsumption.v} (13KB) proves Turing $\subsetneq$ Thiele with a \textbf{non-circular} definition of Turing machines:
...
The file \path{coq/kernel/LocalInfoLoss.v} (17KB) connects information theory to $\mu$-cost:
...
The file \path{coq/kernel/MuInitiality.v} (15KB) proves the strongest possible statement about the $\mu$-ledger: it is not merely \textit{a} monotone cost accumulator, but \textit{the} canonical one—the initial object in the category of instruction-consistent cost functionals.
...
The quantum axiom proofs live in five files totaling 1,174 lines of Coq with zero \texttt{Admitted} statements:
...
\midrule
\texttt{NoCloning.v} & 244 & 18 & No-cloning from conservation \\
\texttt{Unitarity.v} & 257 & 20 & CPTP from irreversibility \\
\texttt{BornRule.v} & 288 & 19 & Born rule from linearity \\
\texttt{Purification.v} & 102 & 8 & Purification principle \\
\texttt{TsirelsonGeneral.v} & 301 & 9 & Tsirelson bound from algebra \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
    ...
    \item \textbf{Quantum axioms from $\mu$-accounting}: No-cloning, unitarity, Born rule, purification, and Tsirelson bound all derived from conservation of structural information (1,174 lines, 19 theorems).
