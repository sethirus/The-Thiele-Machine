...
\end{itemize}
The corpus is split between the kernel (\texttt{coq/kernel/}), which remains active, and the extended proofs that were developed during the project's exploration phase. Many extended proof files have since been superseded by the kernel's consolidated proof surface and are no longer part of the active build tree. This chapter documents those extended formalizations for historical completeness.
...
\end{itemize}
For readers navigating the code, the ``kernel semantics'' block corresponds to files such as \texttt{VMState.v} and \texttt{VMStep.v} under \texttt{coq/kernel/}. The ``extended machine proofs'' (e.g.\ \texttt{PartitionLogic.v}, \texttt{Subsumption.v}) were developed during the exploration phase and are no longer part of the active build tree. The structure is intentionally layered so that higher-level proofs explicitly import the kernel rather than re-deriving it.
