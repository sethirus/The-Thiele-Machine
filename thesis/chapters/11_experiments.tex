...
\begin{enumerate}
    \item \textbf{Direct CSR manipulation:} Attempt to directly write to the Certificate Storage Register (CSR) bypassing the $\mu$-charging logic. Expected defense: CSR is write-protected, modifications trigger \texttt{ERR\_LOGIC} (\texttt{0xC43471A1}).
...
\begin{itemize}
    \item \textbf{CSR manipulation:} Trapped by hardware write-protection, \texttt{ERR\_LOGIC} raised.
    \item \textbf{Buffer overflow:} Caught by partition locality wall, execution aborted with \texttt{ERR\_LOCALITY}.
...
\textbf{Expected defense:} The verifier computes the CHSH value and checks $S \leq \frac{5657}{2000} \approx 2.8285$. If $S > 2.8285$, the verifier classifies the box as \textit{supra-quantum}, requiring a certificate and $\mu$-cost. Without a certificate, the verifier rejects with \texttt{ERR\_CHSH} (\texttt{0x0BADC45C}).
