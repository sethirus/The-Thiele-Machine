...
\node[field, fill=blue!10] (m) at (0, -1.5) {\texttt{vm\_mem} : list nat (65536)};
...
\begin{itemize}
    ...
    \item \textbf{vm\_mem}: Data memory of 65,536 words
...
\paragraph{What word masking does:} The \texttt{word64} function masks any number down to 64 bits---bitwise AND with \texttt{0xFFFFFFFFFFFFFFFF}. This exists for cross-layer fidelity: Coq's \texttt{nat} type is unbounded, but hardware registers are 64 bits. Without the mask, the Coq model and the hardware disagree on what happens at overflow.
...
\textbf{Why this matters:} Coq's \texttt{nat} is unbounded---\texttt{0xFFFFFFFFFFFFFFFF + 1} would be \texttt{0x10000000000000000} in Coq but \texttt{0x0000000000000000} in hardware (wraparound). By applying \texttt{word64} after every operation, the mathematical model matches register behavior exactly. When I prove something about the model, I'm proving it about the actual hardware, not some idealized version.
In the Coq kernel, write operations (\texttt{write\_reg} and \texttt{write\_mem}) mask values through \texttt{word64}, so every stored word is explicitly truncated rather than implicitly relying on the host language. This makes the arithmetic model match the RTL and avoids ambiguities where a high-level language might use unbounded integers.
...
\begin{itemize}
    ...
    \item \texttt{CALL}, \texttt{RET}: Subroutine call and return (\texttt{r31} = stack pointer)
...
Coq handles this proof by structural induction on the step relation: you do structural induction on the step relation, which means Coq generates one sub-goal per step constructor. Some instructions have multiple step rules for success and failure branches, so 46 opcodes expand into roughly 57 cases. Each one boils down to showing that \texttt{advance\_state} adds a non-negative nat. It's tedious but airtight.
...
A note of honesty: the linearity assumption (\texttt{is\_linear\_in\_z}) is doing most of the work here. If you already accept that measurement probabilities depend linearly on the Bloch vector's $z$-component, the Born rule is the unique solution satisfying the boundary conditions. The proof solves the system $a + b = 1$, $-a + b = 0$ to get $a = b = 1/2$, hence $P = (1+z)/2$. The theorem includes a \texttt{mu\_consistent} hypothesis, but this is unused in the actual proof---it reduces to the Bloch ball constraint already given. This is less ``deriving the Born rule from accounting'' and more ``the Born rule is the unique linear rule consistent with eigenstates.''
...
\begin{itemize}
    \item \path{coq/kernel/NoCloning.v}: 1,036 lines, 19 definitions/theorems
    \item \path{coq/kernel/Unitarity.v}: 657 lines, 25 definitions/theorems
    \item \path{coq/kernel/BornRule.v}: 341 lines, 21 definitions/theorems
    \item \path{coq/kernel/BornRuleLinearity.v}: Born rule as unique affine interpolation
    \item \path{coq/kernel/Purification.v}: 290 lines, 11 definitions/theorems
    \item \path{coq/kernel/TsirelsonGeneral.v}: 329 lines, 19 definitions/theorems
    \item \path{coq/kernel/AlgebraicCoherence.v}: Tsirelson from algebraic constraints
\end{itemize}
