...
\node[draw, fill=yellow!30, rounded corners, font=\normalsize, align=center, text width=3.5cm] at (0, -2.5) {
    ...
    \textit{Proven:} $\Delta\mu \ge |\phi|$\\[0.1cm]
    \textit{VM enforces:} $\log|\Omega| - \log|\Omega'| \le \Delta\mu$\\[0.1cm]
...
\end{tikzpicture}
\caption{The No Free Insight theorem: reducing the search space (gaining structural insight) requires paying $\mu$-cost. \textbf{Proven (StateSpaceCounting.v):} $\Delta\mu \ge |\phi|_{\text{bits}}$ for any formula $\phi$. \textbf{Enforced by VM:} The Python VM uses a conservative bound, charging $\mu = |\phi| + n$ (assuming one solution). This \emph{guarantees} $\Delta\mu \ge \log_2(|\Omega|) - \log_2(|\Omega'|)$ without computing the \#P-complete model count; may overcharge when multiple solutions exist.}
...
\node[draw, ultra thick, fill=green!30, rounded corners, minimum width=14.4cm, minimum height=2.6cm, align=center, text width=3.5cm] (master) at (0, -2) {
    ...
    Proven: $\Delta\mu \ge |\phi|$; VM enforces: $\log|\Omega| - \log|\Omega'| \le \Delta\mu$
};
