...
The transition rules define how state evolves. The active implementation exposes a 46-opcode ISA (39 original + 7 categorical morphism opcodes), defined in the formal step semantics.
...
\begin{itemize}
    ...
    \item \textbf{Observation \& Revelation} (3): Output, information disclosure, oracle queries (\texttt{EMIT}, \texttt{REVEAL}, \texttt{ORACLE\_HALTS})
...
\node[cat, fill=yellow!10] (meas) at (2.2, -1.4) {Compute\\ADD, SUB\\ORACLE\_HALTS};
...
\begin{itemize}
    \item \texttt{LASSERT}: Assert a formula, verified by certificate (LRAT proof or SAT model plus countermodel witness)
...
\begin{itemize}
    \item \texttt{ADD}, \texttt{SUB}: Integer arithmetic
...
\begin{itemize}
    ...
    \item \texttt{CALL}, \texttt{RET}: Subroutine call and return (push/pop return address)
...
\begin{itemize}
    ...
    \item \textbf{Trusted}: Certificate checkers (LRAT proof verifier, model validator, countermodel validator) in \path{coq/kernel/CertCheck.v}
...
\textbf{In practice}: Certificates are stored in \texttt{vm\_mem} by the program before executing \texttt{LASSERT}. For UNSAT this is an LRAT proof. For SAT it is a dual-witness package: one satisfying assignment and one falsifying assignment stored back-to-back. The instruction reads them via register-indexed base addresses (\texttt{freg}, \texttt{creg}). The kernel verifies the witnesses but does not search for solutions.
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item \textbf{kind field}: Selects which checker to invoke---\texttt{lassert\_sat} runs both \texttt{check\_model\_binary\_fn} and \texttt{check\_countermodel\_binary\_fn}; \texttt{lassert\_unsat} calls \texttt{check\_lrat}.
...
\begin{itemize}
    \item \textbf{check\_lrat}: Takes two strings (formula and LRAT proof), returns bool. Verified implementation of LRAT proof checking—proves that if it returns true, the formula is genuinely UNSAT.
...
\begin{itemize}
    ...
    \item Either an LRAT proof (for UNSAT, \texttt{kind = lassert\_unsat}) or two assignments stored back-to-back (for SAT, \texttt{kind = lassert\_sat}): one satisfying model and one falsifying countermodel
\end{itemize}
