...
\begin{enumerate}
    ...
    \item \textbf{Transition Rules $R$}: How the machine evolves---the active 47-opcode ISA
...
\paragraph{What word masking does:} The \texttt{word64} function masks any number down to 64 bits---bitwise AND with \texttt{0xFFFFFFFFFFFFFFFF}. I had to learn why this exists the hard way: Coq's \texttt{nat} type is unbounded, but hardware registers are 64 bits. Without the mask, the Coq model and the hardware disagree on what happens at overflow.
...
The transition rules define how state evolves. The active implementation exposes a 47-opcode ISA (40 original + 7 categorical morphism opcodes), defined in the formal step semantics.
