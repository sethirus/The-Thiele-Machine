...
% Main components
\node[box, fill=blue!20] (state) at (0,0) {State Space\\$S$};
\node[box, fill=green!20] (partition) at (3.5,0) {Partition Graph\\$\Pi$};
\node[box, fill=orange!20] (axioms) at (7,0) {Axiom Set\\$A$};
\node[box, fill=red!20] (rules) at (3.5,-2) {Transition Rules\\$R$};
\node[box, fill=purple!20] (logic) at (7,-2) {Logic Engine\\$L$};

% Central element
\node[box, fill=yellow!30, minimum width=3.5cm, minimum height=1.2cm] (mu) at (0,-2) {$\mu$-Ledger\\(Currency)};

% Arrows
\draw[arrow] (state) -- (partition) node[midway, above, font=\scriptsize] {decomposition};
\draw[arrow] (partition) -- (axioms) node[midway, above, font=\scriptsize] {constraints};
\draw[arrow] (rules) -- (state) node[midway, left, font=\scriptsize] {evolves};
\draw[arrow] (logic) -- (axioms) node[midway, right, font=\scriptsize] {verifies};
\draw[arrow] (rules) -- (mu) node[midway, above, font=\scriptsize] {charges};
\draw[dasharrow] (mu) -- (state) node[midway, left, font=\scriptsize] {bounds};

% Title
\node[above=0.5cm of partition, font=\bfseries] {The Thiele Machine: $T = (S, \Pi, A, R, L)$};

% Sections
\node[font=\scriptsize\itshape, below=0.1cm of state] {§3.2.1};
\node[font=\scriptsize\itshape, below=0.1cm of partition] {§3.2.2};
\node[font=\scriptsize\itshape, below=0.1cm of axioms] {§3.2.3};
\node[font=\scriptsize\itshape, below=0.1cm of rules] {§3.2.4};
\node[font=\scriptsize\itshape, below=0.1cm of logic] {§3.2.5};
\node[font=\scriptsize\itshape, below=0.1cm of mu] {§3.3};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
    field/.style={draw, minimum width=6cm, minimum height=0.7cm, font=\ttfamily\small},
    label/.style={font=\small, align=left}
]
% VMState box
\node[draw, thick, minimum width=7cm, minimum height=6cm, fill=gray!5] (vmstate) at (0,0) {};
\node[above=0.1cm of vmstate.north, font=\bfseries\ttfamily] {VMState};
...
% Annotations
\node[label, right=0.5cm of graph] {$\leftarrow$ \textit{Partition structure}};
\node[label, right=0.5cm of csrs] {$\leftarrow$ \textit{Control registers}};
\node[label, right=0.5cm of regs] {$\leftarrow$ \textit{Register file}};
\node[label, right=0.5cm of mem] {$\leftarrow$ \textit{Data memory}};
\node[label, right=0.5cm of pc] {$\leftarrow$ \textit{Program counter}};
\node[label, right=0.5cm of mu] {$\leftarrow$ \textbf{$\mu$-ledger (key!)}};
\node[label, right=0.5cm of err] {$\leftarrow$ \textit{Error flag}};
...
\centering
\begin{tikzpicture}[
    module/.style={draw, rounded corners, minimum width=2.5cm, minimum height=1.5cm, align=center},
    memory/.style={draw, minimum width=0.4cm, minimum height=0.4cm, font=\tiny}
]
...
% Module M1
\node[module, fill=blue!20] (M1) at (-2.5, 0) {Module $M_1$\\ID: 0};
\draw[blue, thick, ->] (M1.south) -- (-3.25, -2.7);
\draw[blue, thick, ->] (M1.south) -- (-2.75, -2.7);
\foreach \i in {0,1} {
    \node[memory, fill=blue!40] at (\i*0.5-4, -3) {\i};
}

% Module M2
\node[module, fill=green!20] (M2) at (1, 0) {Module $M_2$\\ID: 1};
\draw[green!60!black, thick, ->] (M2.south) -- (0.25, -2.7);
\draw[green!60!black, thick, ->] (M2.south) -- (0.75, -2.7);
\draw[green!60!black, thick, ->] (M2.south) -- (1.25, -2.7);
\foreach \i in {8,9,10} {
    \node[memory, fill=green!40] at (\i*0.5-4, -3) {\i};
}

% Module M3
\node[module, fill=orange!20] (M3) at (4.5, 0) {Module $M_3$\\ID: 2};
\draw[orange, thick, ->] (M3.south) -- (3.25, -2.7);
\foreach \i in {14} {
    \node[memory, fill=orange!40] at (\i*0.5-4, -3) {\i};
}

% Partition Graph box
\node[draw, dashed, thick, minimum width=12cm, minimum height=2.5cm, label={above:\textbf{PartitionGraph}}] at (1, 0) {};

% Next ID indicator
\node[draw, fill=purple!20, rounded corners] (nextid) at (4.5, 1.5) {\texttt{pg\_next\_id = 3}};

% Memory label
\node[below=0.3cm of m8, font=\small\itshape] {Memory addresses (0--15)};

% Axioms
\node[font=\scriptsize, below=0.1cm of M1] {$A = \{x > 0\}$};
\node[font=\scriptsize, below=0.1cm of M2] {$A = \{\}$};
\node[font=\scriptsize, below=0.1cm of M3] {$A = \{y \text{ prime}\}$};

% Key properties
\node[draw, fill=white, rounded corners, font=\scriptsize, align=left] at (-5.5, 0) {
    \textbf{Properties:}\\
    $\bullet$ ID monotonic\\
...
\centering
\begin{tikzpicture}[
    cat/.style={draw, rounded corners, minimum width=3.8cm, minimum height=2.2cm, align=center, font=\small},
    instr/.style={font=\scriptsize\ttfamily}
]
% Structural Operations
\node[cat, fill=blue!15] (struct) at (-4, 2) {
...
% Logical Operations
\node[cat, fill=green!15] (logic) at (0, 2) {
...
\node[cat, fill=orange!15] (cert) at (4, 2) {
...
\node[cat, fill=purple!15] (regmem) at (-4, -1) {
...
% Control Operations
\node[cat, fill=red!15] (ctrl) at (0, -1) {
...
\node[cat, fill=yellow!20] (meas) at (4, -1) {
...
% Central: mu-cost
\node[draw, thick, fill=yellow!40, circle, minimum size=1.5cm, font=\small\bfseries] (mu) at (0, 0.5) {$\mu$};

% Arrows showing cost
\draw[->, thick, gray] (struct.east) -- (mu);
\draw[->, thick, gray] (cert.west) -- (mu);
\draw[->, thick, gray] (logic.south) -- (mu);

% Title
\node[above=0.5cm of logic, font=\bfseries] {18-Instruction Set Architecture};

% Annotation
\node[below=0.3cm of regmem, font=\scriptsize\itshape] {Low $\mu$-cost};
\node[below=0.3cm of cert, font=\scriptsize\itshape] {High $\mu$-cost};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
    state/.style={draw, circle, minimum size=1.2cm, font=\small},
    arrow/.style={->, >=stealth, thick}
]
% States
\node[state, fill=blue!20] (s0) at (0,0) {$s_0$};
\node[state, fill=blue!25] (s1) at (3,0) {$s_1$};
\node[state, fill=blue!30] (s2) at (6,0) {$s_2$};
\node[state, fill=blue!35] (s3) at (9,0) {$s_3$};
\node[state, fill=blue!40] (sn) at (12,0) {$s_n$};

% Transitions
\draw[arrow] (s0) -- (s1) node[midway, above, font=\scriptsize] {$op_1$};
\draw[arrow] (s1) -- (s2) node[midway, above, font=\scriptsize] {$op_2$};
\draw[arrow] (s2) -- (s3) node[midway, above, font=\scriptsize] {$op_3$};
\draw[arrow, dashed] (s3) -- (sn);

% Mu values
\node[below=0.3cm of s0, font=\small] {$\mu_0$};
\node[below=0.3cm of s1, font=\small] {$\mu_1$};
\node[below=0.3cm of s2, font=\small] {$\mu_2$};
\node[below=0.3cm of s3, font=\small] {$\mu_3$};
\node[below=0.3cm of sn, font=\small] {$\mu_n$};

% Conservation law
\node[draw, fill=yellow!20, rounded corners, align=center, font=\small] at (6, -2) {
    \textbf{Conservation Law:} $\mu_n = \mu_0 + \sum_{i=1}^{n} \text{cost}(op_i)$
};

% Monotonicity
\draw[decorate, decoration={brace, amplitude=10pt, mirror}] 
    (0, -0.8) -- (12, -0.8) node[midway, below=0.4cm, font=\small\itshape] {
    $\mu_0 \le \mu_1 \le \mu_2 \le \cdots \le \mu_n$ (monotonic)
};

% Title
\node[above=0.8cm of s2, font=\bfseries] {$\mu$-Ledger: Monotonic Cost Accumulation};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
    module/.style={draw, rounded corners, minimum width=2cm, minimum height=1cm, align=center, font=\small},
    arrow/.style={->, >=stealth, thick}
]
% PNEW
\node[font=\bfseries] at (-5, 3.5) {PNEW};
\node[draw, dashed, fill=gray!10, minimum width=2cm, minimum height=1cm] (pnew_before) at (-5, 2) {region};
\draw[arrow] (-5, 1.2) -- (-5, 0.3);
\node[module, fill=blue!20] (pnew_after) at (-5, -0.5) {Module\\ID: $n$};
\node[font=\scriptsize\itshape] at (-5, -1.5) {Creates new module};

% PSPLIT
\node[font=\bfseries] at (0, 3.5) {PSPLIT};
\node[module, fill=green!30, minimum width=3cm] (psplit_before) at (0, 2) {Module $M$\\$\{0,1,2,3\}$};
\draw[arrow] (0, 1.2) -- (0, 0.3);
\node[module, fill=green!20, minimum width=1.3cm] (psplit_left) at (-1, -0.5) {$M_L$\\$\{0,1\}$};
\node[module, fill=green!20, minimum width=1.3cm] (psplit_right) at (1, -0.5) {$M_R$\\$\{2,3\}$};
\node[font=\scriptsize\itshape] at (0, -1.5) {Splits into disjoint parts};

% PMERGE
\node[font=\bfseries] at (5, 3.5) {PMERGE};
\node[module, fill=orange!20, minimum width=1.2cm] (pmerge_left) at (4, 2) {$M_1$};
\node[module, fill=orange!20, minimum width=1.2cm] (pmerge_right) at (6, 2) {$M_2$};
\draw[arrow] (5, 1.2) -- (5, 0.3);
\node[module, fill=orange!30, minimum width=2.5cm] (pmerge_after) at (5, -0.5) {$M_{12}$\\(merged)};
\node[font=\scriptsize\itshape] at (5, -1.5) {Combines disjoint modules};

% Cost annotations
\node[draw, fill=yellow!20, rounded corners, font=\scriptsize] at (-5, -2.3) {$\mu$-cost: low};
\node[draw, fill=yellow!20, rounded corners, font=\scriptsize] at (0, -2.3) {$\mu$-cost: medium};
\node[draw, fill=yellow!20, rounded corners, font=\scriptsize] at (5, -2.3) {$\mu$-cost: low};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
    box/.style={draw, rounded corners, minimum width=3cm, minimum height=2cm, align=center},
    arrow/.style={->, >=stealth, thick}
]
% Before: Large search space
\node[box, fill=red!20, minimum width=5cm, minimum height=3cm] (omega) at (-4, 0) {
    \textbf{Search Space $\Omega$}\\[0.3cm]
    \Large $2^n$ possibilities
};

% After: Reduced search space
\node[box, fill=green!20, minimum width=3cm, minimum height=2cm] (omega_prime) at (4, 0) {
    \textbf{Reduced $\Omega'$}\\[0.2cm]
    $2^{n-k}$ possibilities
};

% Arrow with cost
\draw[arrow, ultra thick, blue] (omega) -- (omega_prime) 
    node[midway, above, font=\small] {\textbf{Insight}}
    node[midway, below, font=\small, red] {Cost: $\Delta\mu \ge k$ bits};

% Conservation equation
\node[draw, fill=yellow!30, rounded corners, font=\small, align=center] at (0, -2.5) {
...
\node[draw, dashed, fill=blue!10, rounded corners, font=\scriptsize] at (-4, -2.5) {
    Predicate $P_{\text{weak}}$\\
    \textit{accepts more traces}
};
\node[draw, dashed, fill=green!10, rounded corners, font=\scriptsize] at (4, -2.5) {
    Predicate $P_{\text{strong}}$\\
    \textit{accepts fewer traces}
};
\draw[arrow, dashed] (-2, -2.5) -- (2, -2.5) node[midway, above, font=\tiny] {strengthening};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
    state/.style={draw, rounded corners, minimum width=3cm, minimum height=2.5cm, align=center},
    arrow/.style={->, >=stealth, thick}
]
% Original state
\node[state, fill=blue!15] (s) at (-3, 0) {
    \textbf{State $s$}\\[0.2cm]
    $\mu = \mu_0$\\
    $\Pi = \Pi_0$
};
...
% Shifted state
\node[state, fill=blue!15] (sk) at (3, 0) {
...
\draw[arrow, ultra thick, purple] (s) -- (sk) 
    node[midway, above, font=\small\bfseries] {Gauge Shift $+k$};

% Invariant annotation
\node[draw, fill=yellow!20, rounded corners, font=\small, align=center] at (0, -2) {
...
% Physical analogy
\node[draw, dashed, fill=green!10, rounded corners, font=\scriptsize, align=center] at (0, -3.5) {
...
\centering
\begin{tikzpicture}[
    concept/.style={draw, rounded corners, minimum width=2.5cm, minimum height=1.5cm, align=center, font=\small},
    result/.style={draw, thick, fill=yellow!20, rounded corners, minimum width=4cm, minimum height=1cm, align=center}
]
% Core concepts
\node[concept, fill=blue!20] (state) at (-4, 3) {State Space\\$S$};
\node[concept, fill=green!20] (partition) at (-1.5, 3) {Partition Graph\\$\Pi$};
\node[concept, fill=orange!20] (mu) at (1.5, 3) {$\mu$-Ledger\\Currency};
\node[concept, fill=red!20] (rules) at (4, 3) {Transition\\Rules $R$};

% Key theorems
\node[result] (mono) at (-2.5, 0.5) {\textbf{$\mu$-Monotonicity}\\$s'.\mu \ge s.\mu$};
\node[result] (nosig) at (2.5, 0.5) {\textbf{No-Signaling}\\Local ops $\not\to$ remote observables};

% Master result
\node[draw, ultra thick, fill=green!30, rounded corners, minimum width=8cm, minimum height=1.5cm, align=center] (master) at (0, -2) {
    \textbf{No Free Insight Theorem}\\
    $\log|\Omega| - \log|\Omega'| \le \Delta\mu$
};

% Arrows
\draw[->, thick] (state) -- (mono);
\draw[->, thick] (partition) -- (mono);
\draw[->, thick] (partition) -- (nosig);
\draw[->, thick] (rules) -- (nosig);
\draw[->, thick] (mu) -- (mono);
\draw[->, thick] (mono) -- (master);
\draw[->, thick] (nosig) -- (master);

% Chapter reference
\node[font=\scriptsize\itshape, below=0.3cm of master] {Foundation for Chapters 4 (Implementation), 5 (Verification), and 6 (Tsirelson)};
\end{tikzpicture}
