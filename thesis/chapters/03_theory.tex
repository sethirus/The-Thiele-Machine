...
The Logic Engine is an on-chip FSM that verifies logical consistency by reading formula and certificate data from \texttt{vm\_mem}. The program supplies formula and certificate by storing them in data memory and passing register indices to the instruction; the FSM checks them in-place without any external solver call.
...
\textbf{In practice}: Certificates (LRAT proofs or SAT models) are stored in \texttt{vm\_mem} by the program before executing \texttt{LASSERT}. The instruction reads them via register-indexed base addresses (\texttt{freg}, \texttt{creg}). The kernel verifies the certificate but does not search for solutions.
\end{tcolorbox}

\subsubsection{Certificate-Based Verification}

Rather than embedding an SMT solver, the Thiele Machine uses \textit{certificate-based verification}. Certificates live in \texttt{vm\_mem}; the instruction encodes register indices and a byte-length rather than an inline value:
\begin{lstlisting}
(* LASSERT instruction fields (VMStep.v): *)
(* freg : register holding formula base address in vm_mem  *)
(* creg : register holding certificate base address        *)
...
\begin{itemize}
    \item \textbf{freg / creg}: The program stores the formula string and certificate string anywhere in \texttt{vm\_mem} before executing \texttt{LASSERT}. The instruction holds only the register indices pointing to these locations---the actual bytes are never in the instruction word.
    \item \textbf{kind field}: Selects which checker to invoke---\texttt{lassert\_sat} calls \texttt{check\_model}; \texttt{lassert\_unsat} calls \texttt{check\_lrat}.
    \item \textbf{flen}: The byte length of the formula. This is the primary cost driver: $\Delta\mu = \mathtt{flen} \times 8 + S(\mathtt{cost})$, ensuring longer formulas cost proportionally more.
\end{itemize}
