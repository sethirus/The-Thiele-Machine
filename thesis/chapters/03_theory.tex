...
\centering
\begin{tikzpicture}[
    box/.style={draw, rounded corners=3pt, minimum width=5.5cm, minimum height=2.0cm, align=center, font=
ormalsize, inner sep=0.4cm},
    arrow/.style={->, >=Stealth, thick},
    dasharrow/.style={->, >=Stealth, line width=1.2pt, dashed},
    scale=1.05
, node distance=3cm]
% Main components
...
\node[box, fill=purple!20, align=center, text width=4.5cm, font=
ormalsize] (logic) at (8,-2.5) {Logic Engine\\$L$};

% Central element
\node[box, fill=yellow!30, minimum width=6.2cm, minimum height=2.2cm, align=center, text width=4.5cm, font=
ormalsize] (mu) at (0,-2) {$\mu$-Ledger\\(Currency)};

% Arrows
\draw[arrow] (state) -- (partition) node[pos=0.5, font=\small, above, yshift=6pt] {decomposition};
\draw[arrow] (partition) -- (axioms) node[pos=0.5, font=\small, above, yshift=6pt] {constraints};
\draw[arrow] (rules) -- (state) node[pos=0.5, font=\small, above, yshift=6pt] {evolves};
\draw[arrow] (logic) -- (axioms) node[pos=0.5, font=\small, above, yshift=6pt] {verifies};
\draw[arrow] (rules) -- (mu) node[pos=0.5, font=\small, above, yshift=6pt] {charges};
\draw[dasharrow] (mu) -- (state) node[pos=0.5, font=\small, above, yshift=6pt] {bounds};
...
% VMState box
\node[draw, line width=1.2pt, minimum width=12.6cm, minimum height=10.8cm, fill=gray!5] (vmstate) at (0,0) {};
...
% Highlight mu
\draw[line width=1.5pt, red!70!black] ($(mu.north west)+(-0.1,0.1)$) rectangle ($(mu.south east)+(0.1,-0.1)$);
\end{tikzpicture}
...
\begin{itemize}
    ...
    \item \textbf{vm\_mu (yellow, line width=1.2pt red border):} nat - \textbf{$\mu$-ledger (KEY!)}
...
\centering
\begin{tikzpicture}[
    module/.style={draw, rounded corners=3pt, minimum width=5.0cm, minimum height=2.4cm, align=center, font=
ormalsize},
...
\foreach \i in {0,...,15} {
    \node[memory, fill=gray!12] (m\i) at (\i*0.5-4, -3) {\i};
}

% Module M1
\node[module, fill=blue!12, align=center, text width=4.5cm, font=
ormalsize] (M1) at (-2.5, 0) {Module $M_1$\\ID: 0};
\draw[blue, line width=1.2pt, ->] (M1.south) -- (-3.25, -2.7);
\draw[blue, line width=1.2pt, ->] (M1.south) -- (-2.75, -2.7);
\foreach \i in {0,1} {
    \node[memory, fill=blue!40] at (\i*0.5-4, -3) {\i};
}

% Module M2
\node[module, fill=green!12, align=center, text width=4.5cm, font=
ormalsize] (M2) at (1, 0) {Module $M_2$\\ID: 1};
\draw[green!60!black, line width=1.2pt, ->] (M2.south) -- (0.25, -2.7);
\draw[green!60!black, line width=1.2pt, ->] (M2.south) -- (0.75, -2.7);
\draw[green!60!black, line width=1.2pt, ->] (M2.south) -- (1.25, -2.7);
\foreach \i in {8,9,10} {
    \node[memory, fill=green!40] at (\i*0.5-4, -3) {\i};
}

% Module M3
\node[module, fill=orange!20, align=center, text width=4.5cm, font=
ormalsize] (M3) at (4.5, 0) {Module $M_3$\\ID: 2};
\draw[orange, line width=1.2pt, ->] (M3.south) -- (3.25, -2.7);
\foreach \i in {14} {
    \node[memory, fill=orange!40] at (\i*0.5-4, -3) {\i};
}

% Partition Graph box
\node[draw, dashed, line width=1.2pt, minimum width=21.6cm, minimum height=4.6cm={above:\textbf{PartitionGraph}}] at (1, 0) {};

% Next ID indicator
\node[draw, fill=purple!20, rounded corners=3pt] (nextid) at (4.5, 1.5) {\texttt{pg\_next\_id = 3}};
...
% Key properties
\node[draw, fill=white, rounded corners=3pt, font=
ormalsize, align=left, align=center, text width=4.5cm] at (-5.5, 0) {
    \textbf{Properties:}\\
    $\bullet$ ID monotonic\\
...
\centering
\begin{tikzpicture}[
    cat/.style={draw, rounded corners=3pt, minimum width=6.2cm, minimum height=3.6cm, align=center, font=
ormalsize},
    instr/.style={font=
ormalsize\ttfamily},
    scale=1.05
...
\node[cat, fill=blue!15, align=center, text width=4.5cm] (struct) at (-4, 2) {
...
% Logical Operations
\node[cat, fill=green!15, align=center, text width=4.5cm] (logic) at (0, 2) {
...
\node[cat, fill=orange!15, align=center, text width=4.5cm] (cert) at (4, 2) {
...
\node[cat, fill=purple!15, align=center, text width=4.5cm] (regmem) at (-4, -1) {
...
% Control Operations
\node[cat, fill=red!15, align=center, text width=4.5cm] (ctrl) at (0, -1) {
...
\node[cat, fill=yellow!15, align=center, text width=4.5cm] (meas) at (4, -1) {
...
% Central: mu-cost
\node[draw, line width=1.2pt, fill=yellow!40, circle, minimum size=1.5cm, font=\normalsize\bfseries] (mu) at (0, 0.5) {$\mu$};

% Arrows showing cost
\draw[->, line width=1.2pt, gray] (struct.east) -- (mu);
\draw[->, line width=1.2pt, gray] (cert.west) -- (mu);
\draw[->, line width=1.2pt, gray] (logic.south) -- (mu);
...
% Transitions
\draw[arrow] (s0) -- (s1) node[pos=0.5, font=\small, above, yshift=6pt] {$op_1$};
\draw[arrow] (s1) -- (s2) node[pos=0.5, font=\small, above, yshift=6pt] {$op_2$};
\draw[arrow] (s2) -- (s3) node[pos=0.5, font=\small, above, yshift=6pt] {$op_3$};
...
% Conservation law
\node[draw, fill=yellow!15, rounded corners=3pt, align=center, font=
ormalsize, text width=4.5cm] at (6, -2) {
    \textbf{Conservation Law:} $\mu_n = \mu_0 + \sum_{i=1}^{n} \text{cost}(op_i)$
};

% Monotonicity
\draw[decorate, decoration={brace, amplitude=10pt, mirror}] (0, -0.8) -- (12, -0.8) node[pos=0.5, font=\small, above, yshift=6pt] {
...
\centering
\begin{tikzpicture}[
    module/.style={draw, rounded corners=3pt, minimum width=4.2cm, minimum height=2.0cm, align=center, font=
ormalsize},
    arrow/.style={->, >=Stealth, thick},
    scale=1.05
, node distance=3cm]
...
\node[draw, dashed, fill=gray!10, minimum width=4.5cm, minimum height=2.1cm] (pnew_before) at (-5, 2) {region};
...
\node[module, fill=green!30, minimum width=5.4cm, align=center, text width=4.5cm, font=
ormalsize] (psplit_before) at (0, 2) {Module $M$\\$\{0,1,2,3\}$};
\draw[arrow] (0, 1.2) -- (0, 0.3);
\node[module, fill=green!12, minimum width=2.4cm, align=center, text width=4.5cm, font=
...
\draw[arrow] (5, 1.2) -- (5, 0.3);
\node[module, fill=orange!30, minimum width=5.5cm, align=center, text width=4.5cm, font=
ormalsize] (pmerge_after) at (5, -0.5) {$M_{12}$\\(merged)};
\node[font=
ormalsize\itshape] at (5, -1.5) {Combines disjoint modules};

% Cost annotations
\node[draw, fill=yellow!15, rounded corners=3pt, font=
...
\centering
\begin{tikzpicture}[
    box/.style={draw, rounded corners=3pt, minimum width=5.0cm, minimum height=3.2cm, align=center, inner sep=0.4cm},
    arrow/.style={->, >=Stealth, thick},
    scale=1.05
, node distance=2.5cm]
% Before: Large search space
\node[box, fill=red!12, minimum width=8.2cm, minimum height=5.0cm, align=center, text width=4.5cm, font=
...
\node[box, fill=green!12, minimum width=5.0cm, minimum height=3.2cm, align=center, text width=4.5cm, font=
...
% Arrow with cost
\draw[arrow, line width=1.5pt, blue] (omega) -- (omega_prime) 
...
\node[draw, fill=yellow!30, rounded corners=3pt, font=
ormalsize, align=center, text width=4.5cm] at (0, -2.5) {
...
\node[draw, dashed, fill=blue!10, rounded corners=3pt, font=
ormalsize, align=center, text width=4.5cm] at (-4, -2.5) {
    Predicate $P_{\text{weak}}$ \\
    \textit{accepts more traces}
};
\node[draw, dashed, fill=green!10, rounded corners=3pt, font=
ormalsize, align=center, text width=4.5cm] at (4, -2.5) {
    Predicate $P_{\text{strong}}$ \\
    \textit{accepts fewer traces}
};
\draw[arrow, dashed] (-2, -2.5) -- (2, -2.5) node[pos=0.5, font=\small, above, yshift=6pt] {strengthening};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[scale=1.8, 
    state/.style={draw, rounded corners=3pt, minimum width=5.4cm, minimum height=4.6cm, align=center},
    arrow/.style={->, >=Stealth, thick}
, node distance=2.5cm]
% Original state
\node[state, fill=blue!15, align=center, text width=4.5cm, font=
ormalsize] (s) at (-3, 0) {
    \textbf{State $s$}\\[0.2cm]
    $\mu = \mu_0$\\
    $\Pi = \Pi_0$
...
\draw[arrow, line width=1.5pt, purple] (s) -- (sk) 
    node[pos=0.5, font=\small, above, yshift=6pt] {Gauge Shift $+k$};

% Invariant annotation
\node[draw, fill=yellow!15, rounded corners=3pt, font=
ormalsize, align=center, text width=4.5cm] at (0, -2) {
...
% Physical analogy
\node[draw, dashed, fill=green!10, rounded corners=3pt, font=
ormalsize, align=center, text width=4.5cm] at (0, -3.5) {
...
\centering
\begin{tikzpicture}[
    concept/.style={draw, rounded corners=3pt, minimum width=5.0cm, minimum height=2.4cm, align=center, font=
ormalsize},
    result/.style={draw, line width=1.2pt, fill=yellow!15, rounded corners=3pt, minimum width=6.2cm, minimum height=2.0cm, align=center, font=
ormalsize},
    scale=1.05
, node distance=3.0cm]
% Core concepts
\node[concept, fill=blue!12, align=center, text width=4.5cm] (state) at (-4, 3) {State Space\\$S$};
\node[concept, fill=green!12, align=center, text width=4.5cm] (partition) at (-1.5, 3) {Partition Graph\\$\Pi$};
\node[concept, fill=orange!20, align=center, text width=4.5cm] (mu) at (1.5, 3) {$\mu$-Ledger\\Currency};
\node[concept, fill=red!12, align=center, text width=4.5cm] (rules) at (4, 3) {Transition\\Rules $R$};

% Key theorems
\node[result, align=center, text width=4.5cm] (mono) at (-2.5, 0.5) {\textbf{$\mu$-Monotonicity}\\$s'.\mu \ge s.\mu$};
\node[result, align=center, text width=4.5cm] (nosig) at (2.5, 0.5) {\textbf{No-Signaling}\\Local ops $\not\to$ remote observables};

% Master result
\node[draw, line width=1.5pt, fill=green!30, rounded corners=3pt, minimum width=14.4cm, minimum height=2.6cm, align=center, text width=4.5cm] (master) at (0, -2) {
    \textbf{No Free Insight Theorem}\\
    $\log|\Omega| - \log|\Omega'| \le \Delta\mu$
};

% Arrows
\draw[->, line width=1.2pt] (state) -- (mono);
\draw[->, line width=1.2pt] (partition) -- (mono);
\draw[->, line width=1.2pt] (partition) -- (nosig);
\draw[->, line width=1.2pt] (rules) -- (nosig);
\draw[->, line width=1.2pt] (mu) -- (mono);
\draw[->, line width=1.2pt] (mono) -- (master);
\draw[->, line width=1.2pt] (nosig) -- (master);
