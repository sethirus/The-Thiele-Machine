...
\textbf{Key insight:} This chapter builds the formal foundation. The model's two key properties ($\mu$-monotonicity + locality) combine to prove No Free Insight. Note: the algebraic bound ($S \le 4$) applies to unconstrained computation where $\mu_{\text{inst}} = 0$; the classical bound ($S \le 2$) characterizes true zero-cost correlations; the Tsirelson bound ($2.8284...$) requires operational investment ($\mu_{\text{inst}} > 0$) while maintaining algebraic coherence ($\mu_{\text{corr}} = 0$).
