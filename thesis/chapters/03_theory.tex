...
\begin{enumerate}
    ...
    \item \textbf{Transition Rules $R$}: How the machine evolves---the 26-instruction ISA
...
% Record box
\node[draw, dashed, rounded corners=3pt, minimum width=3.6cm, minimum height=4.1cm] at (1.8, -1.65) {};
...
The transition rules define how state evolves. The Thiele Machine has 26 instructions, defined in the formal step semantics.
...
The 26 instructions break into groups:
\begin{itemize}
...
\end{tikzpicture}
\caption{The 26 instructions grouped by category. Structural, logical, and certification operations have high $\mu$-cost (they add knowledge). Data movement, control, and compute operations have low or zero cost.}
...
\textbf{Why it works:} Every instruction's cost is a \texttt{nat} (natural number, can't be negative), so the ledger can only go up or stay the same. The proof walks through each of the 32 step constructors (covering success and failure branches for all 26 instructions) and confirms that \texttt{advance\_state} adds \texttt{instruction\_cost instr} to the ledger, which is always $\geq 0$. This isn't ``probably true'' or ``true in tests''---it's checked for all possible executions.
...
I had to learn how Coq handles this kind of proof the hard way: you do structural induction on the step relation, which means Coq generates one sub-goal per step constructor. All 32 of them. Each one boils down to showing that \texttt{advance\_state} adds a non-negative nat. It's tedious but airtight.
...
\begin{itemize}
    \item \path{coq/kernel/NoCloning.v}: 940 lines, 23 definitions/theorems
    \item \path{coq/kernel/NoCloningFromMuMonotonicity.v}: 261 lines, 16 definitions/theorems (machine-native, \texttt{lia}-only)
    \item \path{coq/kernel/Unitarity.v}: 585 lines, 27 definitions/theorems
    \item \path{coq/kernel/BornRule.v}: 326 lines, 21 definitions/theorems
    \item \path{coq/kernel/BornRuleFromSymmetry.v}: 140 lines (refactored; non-circular, tensor consistency)
    \item \path{coq/kernel/Purification.v}: 286 lines, 11 definitions/theorems
    \item \path{coq/kernel/TsirelsonGeneral.v}: 320 lines, 28 definitions/theorems
    \item \path{coq/kernel/TsirelsonFromAlgebra.v}: 331 lines, 13 definitions/theorems (self-contained algebraic)
\end{itemize}
Total: 3,189 lines of machine-verified proofs across eight files showing quantum axioms follow from conservation-shaped hypotheses.
\end{tcolorbox}
