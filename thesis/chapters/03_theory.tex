...
\begin{tcolorbox}[thesisbox,title=Observables and State Projections]
...
\begin{tcolorbox}[thesisbox,colback=red!5!white,colframe=red!75!black,title=What is Trusted in Logic Engine L]
...
\begin{tcolorbox}[thesisbox,colback=yellow!10!white,colframe=orange!75!black,title=Encoding Dependence and Invariance]
...
\begin{itemize}
    ...
    \item \textbf{Algebraic Coherence (Definition):} A correlation matrix is \textit{algebraically coherent} when $\mu_{\text{corr}} = 0$, meaning the correlations satisfy the sum-of-squares constraint:
    \[
    A^2 + B^2 + C^2 + D^2 \leq 4
    \]
    where $A, B, C, D$ are the correlation coefficients. This constraint is proven in \texttt{TsirelsonGeneral.v} and is the necessary and sufficient condition for achieving the quantum bound.
...
\begin{tcolorbox}[thesisbox,colback=green!5!white,colframe=green!75!black,title=Coq-Verified Quantum Axioms]
All theorems in this section are machine-checked in Coq 8.18 with zero Admitted statements:
\begin{itemize}
    \item \path{coq/kernel/NoCloning.v}: 243 lines, 18 definitions/theorems
