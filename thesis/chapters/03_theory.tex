...
\begin{itemize}
    ...
    \item \textbf{\texttt{instr\_xor\_reg}}: Takes three register indices and a \texttt{mu\_delta}. XOR is reversible, so its structural cost is zero, but the field is still there---the type system forces that.
\end{itemize}

The 18 instructions break into groups:
\begin{itemize}
    \item \textbf{Partition Ops} (4): Structure creation and manipulation (\texttt{PNEW}, \texttt{PSPLIT}, \texttt{PMERGE}, \texttt{PDELETE})
    \item \textbf{Logic Ops} (2): Axiom assertion and certificate joining (\texttt{LASSERT}, \texttt{CERTJOIN})
    \item \textbf{Information Ops} (3): MDL accounting, discovery, revelation (\texttt{MDLACC}, \texttt{DISCOVER}, \texttt{REVEAL})
    \item \textbf{Data Movement} (4): Transfer, Python execution, CHSH trials (\texttt{DATAXFER}, \texttt{PYEXEC}, \texttt{CHSH\_TRIAL}, \texttt{ORACLE\_HALTS})
...
\textbf{Why it works:} Every instruction's cost is a \texttt{nat} (natural number, can't be negative), so the ledger can only go up or stay the same. The proof walks through each of the 23 step constructors and confirms that \texttt{advance\_state} adds \texttt{instruction\_cost instr} to the ledger, which is always $\geq 0$. This isn't ``probably true'' or ``true in tests''---it's checked for all possible executions.
...
I had to learn how Coq handles this kind of proof the hard way: you do structural induction on the step relation, which means Coq generates one sub-goal per step constructor. All 23 of them. Each one boils down to showing that \texttt{advance\_state} adds a non-negative nat. It's tedious but airtight.
...
\end{itemize}
Total: 3,988 lines of machine-verified proofs across eight files showing quantum axioms follow from conservation-shaped hypotheses.
\end{tcolorbox}
