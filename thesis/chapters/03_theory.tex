...
\begin{enumerate}
    ...
    \item \textbf{Transition Rules $R$}: How the machine evolves---the active 46-opcode ISA
    \item \textbf{Logic Engine $L$}: The on-chip FSM that reads formula and certificate bytes from \texttt{vm\_mem} and verifies them in-place
\end{enumerate}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!40!black,title=\textbf{A note on tuple structure}]
$\Pi$ and $A$ are not independent of $S$: in the Coq record, $\Pi$ is \texttt{S.vm\_graph} and $A$ is \texttt{module\_axioms} nested inside each \texttt{ModuleState} inside $\Pi$. The five components are therefore not orthogonal in the usual automata-theoretic sense. They are called out separately because each one generates a distinct class of proof obligations. $\Pi$ drives all partition-manipulation proofs; $A$ drives the logic-engine correctness proofs; $L$ is the certificate-checking oracle invoked exclusively by \texttt{LASSERT} in $R$. The decomposition is an analytical tool, not a claim that the five are disjoint sets.
\end{tcolorbox}
...
\subsubsection{The Formal Epistemological Gap}

The Turing model is a complete model of computable functions. It is provably silent on computational epistemology. This is not a philosophical claim---it is a machine-checked theorem.

\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=\textbf{Complete Independence Classification (Proven in Coq, Zero Admits)}]
Let $P_{\text{strict}} = (\texttt{vm\_mem}, \texttt{vm\_regs}, \texttt{vm\_pc})$ denote the strict classical projection---exactly what a Turing machine or RAM machine can observe. The following five independence results are formally proven:

\begin{enumerate}
    \item $\mu \perp P_{\text{strict}}$: no function of (mem, regs, pc) recovers $\mu$ \hfill \texttt{turing\_ram\_mu\_necessity}
    \item $\texttt{certified} \perp P_{\text{strict}}$: no function of (mem, regs, pc) recovers certification \hfill \texttt{turing\_ram\_cert\_necessity}
    \item $\texttt{certified} \perp (P_{\text{strict}}, \mu)$: knowing $\mu$ does not determine certification \hfill \texttt{cost\_model\_cert\_necessity}
    \item $\mu \perp (P_{\text{strict}}, \texttt{certified})$: knowing certification does not determine $\mu$ \hfill \texttt{cert\_model\_mu\_necessity}
    \item $\texttt{vm\_graph} \perp (P_{\text{strict}}, \mu, \texttt{certified})$: the categorical layer is independent of the full $\mu$-ledger shadow \hfill \texttt{graph\_not\_recoverable\_from\_P\_full}
\end{enumerate}

\textbf{Corollary} (\texttt{thiele\_state\_three\_component\_independence}): The Thiele Machine has exactly three independent non-classical state components---$\mu$, \texttt{vm\_certified}, and \texttt{vm\_graph}---each provably irrecoverable from any combination of the others together with $P_{\text{strict}}$.

\textbf{Minimality} (\texttt{P\_full\_is\_minimal\_complete\_extension}): The projection $P_{\text{full}} = (P_{\text{strict}}, \mu, \texttt{certified})$ is the minimal extension of classical state that determines both $\mu$ and certification. Dropping either field destroys the corresponding completeness. Adding \texttt{vm\_graph} does not affect $\mu$ or certification completeness---graph structure is a third independent axis.
\end{tcolorbox}

\textbf{The separation is universal, not an artifact of crafted witnesses.} \texttt{mu\_ledger\_necessity\_universal} (\path{coq/NecessityOfMuLedger.v}) proves that from \emph{any} VMState---not only from a specially constructed starting point---the instructions \texttt{CERTIFY 0} and \texttt{PNEW [] 0} produce identical strict classical shadows but strictly different $\mu$ and certification. \texttt{po1\_trace\_necessity} proves that prepending any common prefix program of any length does not close the gap. No amount of classical computation makes the ledger visible to a classical observer.

\textbf{Interpretation.} A Turing machine running inside the Thiele Machine computes the same functions it always could. What it cannot do---provably, not by design limitation---is issue or observe an unforgeable receipt for what those computations cost in structural knowledge. The $\mu$-ledger lives in a part of state space that is structurally opaque to any classical observer. This is the formal content of the claim that the Turing model is silent on computational epistemology.
...
% Record box
\node[draw, dashed, rounded corners=3pt, minimum width=3.6cm, minimum height=6.1cm] at (1.8, -2.65) {};
...
\node[field, fill=blue!10] (m) at (0, -1.5) {\texttt{vm\_mem} : list nat (128)};
...
\node[field, fill=yellow!15] (mut) at (0, -3.0) {\texttt{vm\_mu\_tensor} : list nat (4$\times$4)};
\node[field, fill=red!10] (e) at (0, -3.5) {\texttt{vm\_err} : bool};
\node[field, fill=purple!10] (la) at (0, -4.0) {\texttt{vm\_logic\_acc} : nat};
\node[field, fill=purple!10] (ms) at (0, -4.5) {\texttt{vm\_mstatus} : nat};
\node[field, fill=cyan!10] (wt) at (0, -5.0) {\texttt{vm\_witness} : WitnessCounts};
\node[field, fill=cyan!10] (ct) at (0, -5.5) {\texttt{vm\_certified} : bool};
...
Record VMState := {
  ...
  vm_pc : nat;
  vm_mu : nat;
  vm_mu_tensor : list nat;    (* Flattened 4x4 mu-tensor, row-major, 16 entries *)
  vm_err : bool;
...
\begin{itemize}
    ...
    \item \textbf{vm\_regs}: A register file of 16 general-purpose registers (canonical hardware bound)
    \item \textbf{vm\_mem}: Data memory of $128$ words (the \texttt{MEM\_SIZE} parameter; the kernel proofs are parametric in this constant, so the same machinery scales up linearly to any target)
    \item \textbf{vm\_pc}: The program counter
    \item \textbf{vm\_mu}: The $\mu$-ledger accumulator
    \item \textbf{vm\_mu\_tensor}: A flattened $4 \times 4$ cost tensor (16 entries, row-major). The \texttt{REVEAL} instruction charges specific tensor entries, and the Bianchi invariant ($\sum\text{tensor}[i] \le \mu$) is enforced every cycle in hardware. This connects $\mu$-accounting to metric-space geometry.
    \item \textbf{vm\_err}: Error flag (latching)
    \item \textbf{vm\_logic\_acc}: Logic engine accumulator. High-value operations (REVEAL, PDISCOVER, CHSH\_TRIAL) require this to hold the gate key \texttt{0xCAFEEACE}. Mirrors the RTL \texttt{logic\_acc} register.
    \item \textbf{vm\_mstatus}: Operational mode flag. Certain structural operations check this field. The $\mu$-ledger charges unconditionally regardless of this flag---the necessity proofs (\path{coq/NecessityOfMuLedger.v}) run from states with \texttt{vm\_mstatus = 0} and \texttt{CERTIFY} still charges $\mu$ by the definition of \texttt{vm\_apply}. The ledger is not a mode-dependent feature; it is part of the machine's formal semantics at all times.
    \item \textbf{vm\_witness}: Eight CHSH trial counters (same/different $\times$ four setting pairs). Updated by \texttt{CHSH\_TRIAL} and read by the certification path.
    \item \textbf{vm\_certified}: State-based certification flag, set by the \texttt{CERTIFY} instruction when the witness counters satisfy the CHSH threshold. Once set, it cannot be cleared---certification is irreversible.
\end{itemize}
...
\paragraph{What word masking does:} The \texttt{word64} function masks any number down to 64 bits---bitwise AND with \texttt{0xFFFFFFFFFFFFFFFF}. This gives the kernel a well-defined finite-width semantics: Coq's \texttt{nat} type is unbounded, so without an explicit mask, kernel arithmetic would have no overflow point. The Kami hardware specification narrows further to 32 bits (\texttt{WordSz := 32} in \path{coq/kami_hw/ThieleTypes.v}); cross-layer comparison reduces the kernel's 64-bit values modulo $2^{32}$ when checking against the RTL.

\textbf{Step by step:}
\begin{enumerate}
    \item \textbf{\texttt{N.ones 64}}: Creates the bitmask \texttt{0xFFFFFFFFFFFFFFFF}---64 consecutive 1-bits. The \texttt{N} type is Coq's binary natural number representation, which is what bit operations need.
    \item \textbf{\texttt{N.of\_nat x}}: Converts from Coq's mathematical \texttt{nat} (which proofs use) to binary \texttt{N} (which bit operations need). Two representations of the same number, different purposes.
    \item \textbf{\texttt{N.land}}: Bitwise AND. ANDing any number with \texttt{0xFFFFFFFFFFFFFFFF} keeps only the lower 64 bits. Example: \texttt{0x1FFFFFFFFFFFFFFFF AND 0xFFFFFFFFFFFFFFFF = 0xFFFFFFFFFFFFFFFF}.
    \item \textbf{\texttt{N.to\_nat}}: Converts back to \texttt{nat} for the rest of the formal model.
\end{enumerate}

\textbf{Why this matters:} Coq's \texttt{nat} is unbounded---\texttt{0xFFFFFFFFFFFFFFFF + 1} would be \texttt{0x10000000000000000} in Coq, but the kernel needs a finite overflow point so its arithmetic matches what executable backends produce. By applying \texttt{word64} after every register write, the kernel pins itself to 64-bit modular semantics. The RTL further narrows this to 32-bit modular arithmetic; the bisimulation in \path{coq/kernel/HardwareBisimulation.v} establishes correspondence between the kernel's 64-bit values and the RTL's 32-bit registers within the tested range.
In the Coq kernel, write operations (\texttt{write\_reg} and \texttt{write\_mem}) mask values through \texttt{word64}, so every stored word is explicitly truncated rather than implicitly relying on the host language. This pins down the kernel's overflow behavior and avoids ambiguities where a high-level language might use unbounded integers.
...
\begin{lstlisting}
Record CouplingData := {
  coupling_pairs : list (nat * nat);  (* (source_cell, target_cell) pairs *)
...
Record MorphismState := {
  ...
  morph_cert_cost  : nat  (* mu paid when MORPH_ASSERT certified this; 0 = uncertified *)
}.
...
\begin{itemize}
    \item \textbf{pg\_next\_id}: Acts as a monotonic counter so module IDs stay unique. Starting from 0, each new module increments this value. This prevents ID collisions and provides a total ordering over module creation time.
...
\begin{itemize}
    ...
    \item \textbf{module\_axioms}: Logical constraints about the data in this region. For example, ``all values are positive'' or ``this region stores a sorted array.'' Constraints are verified on-chip by the logic engine FSM against program-provided certificates in \texttt{vm\_mem}.
    \item \textbf{module\_mu\_tensor}: A per-module $4 \times 4$ metric tensor (16 entries, row-major). Defaults to all zeros (flat space). The \texttt{TENSOR\_SET} instruction writes entries; \texttt{TENSOR\_GET} reads them. This connects partition-level geometry to the global $\mu$-tensor in \texttt{VMState}.
\end{itemize}

\textbf{Design Rationale:} Why lists instead of sets or arrays? Because Coq's standard library has extensive list-handling lemmas, making verification easier. The O(n) lookup cost is acceptable---the number of modules is typically small ($<$100), and this is a \emph{specification}, not an optimized implementation.
...
The partition graph must satisfy a three-part well-formedness invariant:
\begin{lstlisting}
...
\paragraph{Understanding Well-Formedness:} Three conditions must hold simultaneously.

\textbf{The Three Conditions:}
\begin{itemize}
    \item \textbf{all\_ids\_below}: Every \texttt{ModuleID} in \texttt{pg\_modules} is strictly less than \texttt{pg\_next\_id}. This keeps \texttt{pg\_next\_id} genuinely fresh—creating a new module can safely use it.
    \item \textbf{all\_morph\_ids\_below}: Every \texttt{MorphismID} in \texttt{pg\_morphisms} is strictly less than \texttt{pg\_next\_morph\_id}. Same freshness guarantee for the morphism ID counter.
    \item \textbf{all\_morph\_endpoints\_valid}: Every morphism's \texttt{morph\_source} and \texttt{morph\_target} IDs refer to modules that actually exist in \texttt{pg\_modules}. A dangling-endpoint morphism would make the categorical structure unsound.
    \item \textbf{g.(field)}: Coq syntax for accessing record fields.
\end{itemize}

\textbf{Why Three Conditions?} Adding morphisms (the MORPH family) doubled the surface area of the invariant. The module-ID condition was not enough once arrows between modules entered the picture—you also need morphism IDs to be fresh and arrows to point at real endpoints. Without the endpoint condition, \texttt{MORPH\_ASSERT} could certify a morphism whose source or target had been deleted.

This invariant is proven to be preserved by all graph operations, including:
\begin{itemize}
    \item \texttt{graph\_add\_module\_preserves\_wf} — new module, increments \texttt{pg\_next\_id}
    \item \texttt{graph\_remove\_preserves\_wf} — cascades morphism deletion first
    \item \texttt{graph\_add\_morphism\_preserves\_wf} — new arrow, increments \texttt{pg\_next\_morph\_id}
    \item \texttt{graph\_delete\_morphism\_preserves\_wf} — single arrow removal
    \item \texttt{graph\_cascade\_delete\_morphisms\_preserves\_wf} — bulk deletion when a module is removed
    \item \texttt{graph\_compose\_morphisms\_preserves\_wf}, \texttt{graph\_tensor\_morphisms\_preserves\_wf}
    \item \texttt{wf\_graph\_lookup\_beyond\_next\_id}
\end{itemize}
The well-formedness invariant is deliberately restricted to ID discipline and endpoint validity. It does \emph{not} require disjointness or coverage; those properties are enforced locally by the specific graph operations that need them. Keeping the invariant to these three conditions means every proof about step semantics has a small, fixed obligation to discharge.
...
\begin{enumerate}
    \item \textbf{Equality Checking:} Normalized regions can be compared directly without worrying about further transformations.
    \item \textbf{Proof Simplification:} When reasoning about operations, \texttt{normalize(normalize(r))} simplifies to \texttt{normalize(r)}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Preserve the metric tensor: \texttt{module\_mu\_tensor := m.(module\_mu\_tensor)}
...
The transition rules define how state evolves. The active implementation exposes a 46-opcode ISA, defined in the formal step semantics.
...
\paragraph{Why the instruction type matters:} Coq forces you to list every possible instruction up front---there's no way to sneak in a new one later without updating every proof that pattern-matches on instructions. That was painful during development (every time I added an instruction, dozens of proofs broke), but it means the proofs cover \emph{every} instruction, proven.

The key design choice: every instruction carries a \texttt{mu\_delta} field. The cost isn't bolted on after the fact---it's baked into the instruction itself, so you literally cannot define an instruction without saying what it costs. That was intentional. It means every proof about the ledger gets the cost for free, just by pattern matching.

\textbf{Reading the constructor parameters:} Each line in the listing above is a constructor with its data. For example:
\begin{itemize}
    \item \textbf{\texttt{instr\_psplit}}: Takes a \texttt{ModuleID} (which module to split), two \texttt{list nat} (the two disjoint sub-regions), and a \texttt{mu\_delta} cost. Splitting a module reveals internal structure, and the cost reflects how much structural information is being exposed.
    \item \textbf{\texttt{instr\_pnew}}: Takes a \texttt{list nat} (the region to allocate) and a \texttt{mu\_delta} cost. Creating a new module is asserting new structure---that costs something.
    \item \textbf{\texttt{instr\_xor\_swap}}: Takes two register indices and a \texttt{mu\_delta}. XOR is reversible, so its structural cost is zero, but the field is still there---the type system forces that.
\end{itemize}

The active 46 opcodes break into groups:
\begin{itemize}
    \item \textbf{Partition Ops} (4): Structure creation, splitting, merging, and discovery (\texttt{PNEW}, \texttt{PSPLIT}, \texttt{PMERGE}, \texttt{PDISCOVER})
    \item \textbf{Logic Ops} (3): Axiom assertion, certificate joining, and MDL accounting (\texttt{LASSERT}, \texttt{LJOIN}, \texttt{MDLACC})
    \item \textbf{Data Transfer} (2): Register transfer and immediate loads (\texttt{XFER}, \texttt{LOAD\_IMM})
    \item \textbf{Certification} (1): Bell test trials (\texttt{CHSH\_TRIAL})
    \item \textbf{XOR Ops} (4): Reversible computation primitives (\texttt{XOR\_LOAD}, \texttt{XOR\_ADD}, \texttt{XOR\_SWAP}, \texttt{XOR\_RANK})
    \item \textbf{Observation \& Revelation} (2): Output and information disclosure (\texttt{EMIT}, \texttt{REVEAL})
    \item \textbf{Memory \& ALU} (4): Load, store, addition, subtraction (\texttt{LOAD}, \texttt{STORE}, \texttt{ADD}, \texttt{SUB})
    \item \textbf{Control Flow} (5): Jumps, branches, subroutines, halt (\texttt{JUMP}, \texttt{JNEZ}, \texttt{CALL}, \texttt{RET}, \texttt{HALT})
    \item \textbf{I/O and Heap} (5): State snapshots, port access, and heap memory (\texttt{CHECKPOINT}, \texttt{READ\_PORT}, \texttt{WRITE\_PORT}, \texttt{HEAP\_LOAD}, \texttt{HEAP\_STORE})
    \item \textbf{Certification} (1): Proof-carrying execution attestation (\texttt{CERTIFY})
    \item \textbf{ALU Extensions} (6): Bitwise and arithmetic (\texttt{AND}, \texttt{OR}, \texttt{SHL}, \texttt{SHR}, \texttt{MUL}, \texttt{LUI})
    \item \textbf{Per-Module Tensor} (2): Module tensor read/write (\texttt{TENSOR\_SET}, \texttt{TENSOR\_GET})
    \item \textbf{Categorical Morphisms} (7): Typed morphism creation, composition, identity, deletion, assertion, tensor product, and inspection (\texttt{MORPH}, \texttt{COMPOSE}, \texttt{MORPH\_ID}, \texttt{MORPH\_DELETE}, \texttt{MORPH\_ASSERT}, \texttt{MORPH\_TENSOR}, \texttt{MORPH\_GET})
\end{itemize}
...
\node[cat, fill=green!15] (logic) at (0, 1.8) {Logical\\LASSERT, LJOIN\\MDLACC};
\node[cat, fill=orange!15] (cert) at (2.2, 1.4) {Certification\\REVEAL, EMIT\\CHSH\_TRIAL};

% Low-cost categories (bottom)
\node[cat, fill=violet!10] (reg) at (-2.2, -1.4) {Register/Memory\\XFER, LOAD\_IMM\\LOAD, STORE, XOR\_*};
\node[cat, fill=red!10] (ctrl) at (0, -1.8) {Control\\JUMP, JNEZ, CALL\\RET, HALT};
\node[cat, fill=yellow!10] (meas) at (2.2, -1.4) {Compute\\ADD, SUB\\MUL, AND, OR};
...
\end{tikzpicture}
\caption{The active opcodes grouped by category. Structural, logical, and certification operations have high $\mu$-cost (they add knowledge). Data movement, control, and compute operations have low or zero cost.}
...
\begin{itemize}
    \item \texttt{LASSERT}: Assert a formula with a dual-witness certificate (one satisfying assignment + one falsifying assignment). SAT path (\texttt{kind=true}) checks both witnesses. UNSAT path (\texttt{kind=false}) always fails at the kernel level---this gap is documented in \path{coq/kernel/VMStep.v}.
...
\textbf{Data Transfer and Register/Memory Operations:}
\begin{itemize}
    \item \texttt{XFER}: Transfer between registers
    \item \texttt{LOAD\_IMM}: Load an 8-bit immediate value into a register
    \item \texttt{LOAD}, \texttt{STORE}: Data memory access
    \item \texttt{XOR\_LOAD}, \texttt{XOR\_ADD}, \texttt{XOR\_SWAP}, \texttt{XOR\_RANK}: Bitwise/matrix operations
\end{itemize}

\textbf{Compute Operations:}
\begin{itemize}
    \item \texttt{ADD}, \texttt{SUB}: 64-bit modular arithmetic
    \item \texttt{MUL}, \texttt{AND}, \texttt{OR}, \texttt{SHL}, \texttt{SHR}: Extended ALU
    \item \texttt{LUI}: Load upper immediate ($r_d \leftarrow v \ll 8$)
\end{itemize}

\textbf{Control Flow Operations:}
\begin{itemize}
    \item \texttt{JUMP}: Unconditional jump
    \item \texttt{JNEZ}: Jump if register is not equal to zero
    \item \texttt{CALL}, \texttt{RET}: Subroutine call and return (\texttt{r15} = stack pointer)
    \item \texttt{HALT}: Advances PC and charges $\delta$. Execution stops at the runner level when PC exceeds the program length; a HALT mid-program does not stop the kernel step relation.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Initial state} (\texttt{VMState}): Where the machine starts
    \item \textbf{Instruction} (\texttt{vm\_instruction}): What operation to perform
    \item \textbf{Final state} (\texttt{VMState}): Where the machine ends up
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Proof-Friendliness}: Inductive relations are easier to reason about in Coq—you can induct on derivation trees.
\end{itemize}
...
\paragraph{Understanding the step\_pnew rule:} The kernel's step relation uses hardware-aligned semantics rather than the more abstract \texttt{graph\_pnew} (which still exists in \path{coq/kernel/VMState.v} as the conceptual specification). Concretely, \texttt{step\_pnew} takes the size $sz$ of the normalized input region, allocates a fresh module whose region is \texttt{List.seq 0 sz} (sequential addresses 0..$sz-1$), and lets the new module ID wrap modulo 64 to stay within the hardware's \texttt{NUM\_MODULES} bound. The original abstract semantics---``return the existing module if the same region was previously allocated''---is therefore implemented by \texttt{graph\_pnew} in \path{coq/kernel/VMState.v} but bypassed by the live step relation in favour of the form the RTL can actually realize. Cross-layer parity is what motivated this rewrite.

\subsection{Logic Engine $L$}

The Logic Engine is an on-chip FSM that verifies logical consistency by reading formula and witness data from \texttt{vm\_mem}. The program supplies formula and witnesses by storing them in data memory and passing register indices to the instruction; the FSM checks them in-place without any external solver call.
...
\begin{itemize}
    ...
    \item \textbf{Trusted}: Certificate checkers (SAT model validator, countermodel validator) in \path{coq/kernel/CertCheck.v}
    \item \textbf{Soundness guarantee}: A false assertion cannot be accepted by the kernel, only fail to be proven
    \item \textbf{Completeness}: Not proven---the solver may fail to find proofs that exist
    \item \textbf{TCB addition}: Hash functions (SHA-256), certificate parsers, and the Coq extraction correctness
    \item \textbf{UNSAT path status}: \texttt{kind=false} (UNSAT) always fails at the kernel level---\texttt{lassert\_check\_ok} returns \texttt{false} for this path. This is a documented gap in \path{coq/kernel/VMStep.v}. \texttt{check\_lrat} exists in \texttt{CertCheck.v} but is \emph{not} invoked by the step semantics.
\end{itemize}

\textbf{In practice}: Certificates are stored in \texttt{vm\_mem} by the program before executing \texttt{LASSERT}. For SAT (\texttt{kind=true}) this is a dual-witness package: one satisfying assignment and one falsifying assignment stored back-to-back. The instruction reads them via register-indexed base addresses (\texttt{freg}, \texttt{creg}). For UNSAT (\texttt{kind=false}) the kernel step always fails---there is no LRAT-checking path in the current step semantics, only in \texttt{CertCheck.v} as a library. The kernel verifies the certificate for SAT but does not search for solutions.
\end{tcolorbox}

\subsubsection{Certificate-Based Verification}

Rather than embedding an SMT solver, the Thiele Machine uses \textit{certificate-based verification}. Certificates live in \texttt{vm\_mem}; the instruction encodes register indices and an encoded formula-unit count rather than an inline value:
\begin{lstlisting}
...
(* flen : encoded formula-unit count; Delta_mu = flen*8 + S(cost) *)
(* cost : explicit cost floor                              *)

Definition check_lrat : string -> string -> bool := CertCheck.check_lrat.
Definition check_model_binary_fn : list nat -> (nat -> nat) -> bool := CertCheck.check_model_binary_fn.
Definition check_countermodel_binary_fn : list nat -> (nat -> nat) -> bool := CertCheck.check_countermodel_binary_fn.
\end{lstlisting}

\paragraph{Understanding Certificate-Based Verification:}
\textbf{Memory-Resident Certificates:}
\begin{itemize}
    \item \textbf{freg / creg}: The program stores the encoded formula and witness block anywhere in \texttt{vm\_mem} before executing \texttt{LASSERT}. The instruction holds only the register indices pointing to these locations---the actual bit payload is never in the instruction word.
    \item \textbf{kind field}: \texttt{kind=true} (SAT) runs both \texttt{check\_model\_binary\_fn} and \texttt{check\_countermodel\_binary\_fn}. \texttt{kind=false} (UNSAT) always returns false---the UNSAT verification path is not implemented in the current kernel step function.
    \item \textbf{flen}: The encoded formula-unit count. This is the primary cost driver: $\Delta\mu = \mathtt{flen} \times 8 + S(\mathtt{cost})$, so longer formulas cost proportionally more in bits.
\end{itemize}

\textbf{The Checker Functions:}
\begin{itemize}
    \item \textbf{check\_lrat}: Takes two strings (formula and LRAT proof), returns bool. Verified LRAT proof checker---proves that if it returns true, the formula is genuinely UNSAT. This function exists in \texttt{CertCheck.v} and is imported in \texttt{VMStep.v} but is \textbf{not invoked} by the current \texttt{lassert\_check\_ok} step function. UNSAT checking via LRAT is a documented future capability, not a live code path.
    \item \textbf{check\_model\_binary\_fn}: Takes binary formula words plus a memory-backed assignment function and returns bool. It confirms the satisfying witness.
    \item \textbf{check\_countermodel\_binary\_fn}: Takes the same formula plus a second assignment and returns bool iff at least one clause is falsified. This is the non-triviality witness that blocks tautology inflation.
    \item \textbf{:= CertCheck.\ldots}: These are definition bindings—the functions are implemented in the CertCheck module.
\end{itemize}
...
\textbf{Certificate Size and $\mu$-Cost:} The encoded formula-unit count ($\mathtt{flen}$) directly sets the formula-component of the $\mu$-cost: $\Delta\mu = \mathtt{flen} \times 8 + S(\mathtt{cost})$. A structurally richer formula costs more, encoding the information-theoretic principle that each bit of constraint has exactly one bit of cost.

An \texttt{LASSERT} instruction reads from \texttt{vm\_mem}:
\begin{itemize}
    \item An encoded formula (at \texttt{R[freg]}, \texttt{flen} units, eight concrete bits per unit)
    \item For SAT (\texttt{kind = true}): two assignments stored back-to-back---one satisfying model and one falsifying countermodel. For UNSAT (\texttt{kind = false}): the current kernel step always fails; no certificate content is read.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Simplicity}: A single counter keeps the invariant explicit: one ordered value that every step updates by addition.
    \item \textbf{Tamper detection}: Any attempt to change $\mu$ outside \texttt{instruction\_cost} and \texttt{apply\_cost} shows up as a proof obligation failure in the step semantics.
    \item \textbf{Comparison}: One scalar value makes cross-layer checks deterministic because each trace has a single ledger value at every step.
...
\begin{itemize}
    ...
    \item \textbf{Underscore (\_)}: Means "ignore this parameter." The only thing that matters here is extracting the \texttt{cost} field.
...
\subsubsection{Uniqueness of $\mu$: The Initiality Theorem}

The $\mu$-ledger is not merely \emph{a} valid cost measure---it is \emph{the} cost measure. The proof has two parts with different statuses.

\textbf{Part 1 (ISA declares costs; information theory bounds them below):} Instruction costs are declared in \path{coq/kernel/VMStep.v} as part of the ISA. \path{coq/kernel/MuCostDerivation.v} proves unconditionally that these costs satisfy information-theoretic lower bounds: any implementation paying both the Shannon entropy cost ($\ge \log_2(\Omega/\Omega')$ for state-space reduction) and the description complexity cost ($\ge |\phi|_{\text{bits}}$ for the certified formula) must pay at least \texttt{lassert\_total\_cost} (\texttt{cost\_necessity}). Reversible operations (\texttt{PNEW}, \texttt{PSPLIT}, \texttt{PMERGE}) are proven to require zero cost (\texttt{partition\_ops\_zero\_cost}). These are structural results, zero Admitted. \emph{What is not claimed here:} that costs are forced to equal (not merely exceed) these minima from Coq semantics alone. The conversion of $\mu$-costs to physical energy requires the Landauer-Unruh calibration hypothesis, which is a \textbf{(C)}-tier conditional and explicitly tagged as such throughout the development.

\textbf{Part 2 (Costs $\to$ Unique $\mu$, unconditional):} Given whatever costs the ISA declares, \path{coq/kernel/MuInitiality.v} proves:

\begin{theorem}[$\mu$-Initiality, \texttt{mu\_is\_initial\_monotone} --- unconditional, zero Admitted]
For any function $M : \texttt{VMState} \to \mathbb{N}$, if $M$ is instruction-consistent (advances by \texttt{instruction\_cost} at each step) and $M(\texttt{init\_state}) = 0$, then $M(s) = s.\texttt{vm\_mu}$ for every reachable state $s$.
\end{theorem}

This is unconditional. It does not depend on where the instruction costs came from. Any two instruction-consistent monotones starting at zero must agree everywhere on reachable states (\texttt{consistent\_monotones\_agree}). There is no alternative accounting. The bedrock claim is therefore: \emph{given the ISA cost declarations, $\mu$ is the unique cost functional. There is no design freedom in $\mu$ once the ISA is fixed.}
...
\paragraph{What this theorem says:} For \emph{any} state, \emph{any} instruction, and \emph{any} resulting state: if the step is valid, the ledger didn't go down. That's it. No exceptions, no edge cases. Coq checked every branch of the step relation and confirmed the arithmetic.

\textbf{Why it works:} Every instruction's cost is a \texttt{nat} (natural number, can't be negative), so the ledger can only go up or stay the same. The proof walks through the step constructors covering success and failure branches across the active opcode set and confirms that \texttt{advance\_state} adds \texttt{instruction\_cost instr} to the ledger, which is always $\geq 0$. This isn't ``probably true'' or ``true in tests''---it's checked for all possible executions.

\textbf{What this proves:}
\begin{enumerate}
    \item \textbf{No Negative Costs}: Instructions can't push the ledger backwards
    \item \textbf{No Accounting Bugs}: Even with complex state updates involving partition splits, merges, and data transfers, the ledger never decreases
    \item \textbf{Temporal Ordering}: If state $s_2$ was reached from $s_1$, then $\mu_2 \geq \mu_1$---the ledger gives you a clock for free
    \item \textbf{No Rewinds}: Can't ``undo'' structural knowledge by stepping backward
\end{enumerate}

Coq handles this proof by structural induction on the step relation: one sub-goal per step constructor. Some instructions have multiple step rules for success and failure branches, so 46 opcodes expand into 58 step cases (verified by \texttt{grep -c "\^| step\_" VMStep.v}). Each case boils down to showing that the \texttt{advance\_state} family adds a non-negative \texttt{nat} via \texttt{apply\_cost}. It is tedious but airtight.
...
\begin{enumerate}
    ...
    \item \textbf{Monotonicity Bound}: The machine cannot decrease $\mu$ (by design of unsigned costs)
\end{enumerate}
...
\textbf{Step-by-Step Execution:}
\begin{enumerate}
    \item \textbf{Normalization}: \texttt{normalize\_region region} removes duplicates via \texttt{nodup}, preserving first-occurrence order. So \texttt{[1;2;2;3]} becomes \texttt{[1;2;3]}, but \texttt{[3;1;2]} stays \texttt{[3;1;2]}---the Coq model does not sort. (The Python VM uses \texttt{sorted(set(region))} for a stricter canonical form; the Coq model uses \texttt{nodup} for provability.)
...
\texttt{PSPLIT} replaces a module with two sub-modules. Two layers exist:
\begin{itemize}
    \item \textbf{Abstract specification} (\texttt{graph\_psplit} in \path{coq/kernel/VMState.v}): the chapter's listing above. Requires \texttt{partition\_valid original left right} (left and right partition the original region: subset, disjoint, union).
    \item \textbf{Live step relation} (\texttt{step\_psplit} via \texttt{graph\_hw\_psplit} in \path{coq/kernel/VMStep.v}): the abstract \texttt{left}/\texttt{right} parameters are accepted but \emph{ignored}. The split is always equal halves --- $\texttt{left\_sz} = \texttt{orig\_sz}/2$, $\texttt{right\_sz} = \texttt{orig\_sz} - \texttt{left\_sz}$. This is what the RTL actually does, and the cross-layer parity proofs establish refinement against this hardware-aligned form, not the abstract one.
\end{itemize}

\textit{Intuition:} \texttt{PSPLIT} takes a module and slices it in two. The abstract specification documents the conceptual partition contract; the kernel step relation implements the hardware behaviour and lets the program-supplied split parameters be discarded.
...
\begin{itemize}
    ...
    \item \textbf{Use Case}: When checking locality properties, only region changes matter
\end{itemize}
...
\end{itemize}
These physical laws become \texttt{bool}-valued functions I can prove theorems about.
...
\textbf{Set-Theoretic Interpretation:} Thinking of predicates as sets of traces they accept:
\begin{itemize}
...
\begin{enumerate}
    ...
    \]
    Since $|\Omega'| \ge 1$ for satisfiable formulas, this \emph{proves} $\Delta\mu \ge \log_2(|\Omega|) - \log_2(|\Omega'|)$ (may overcharge when multiple solutions exist).
\end{enumerate}
\end{theorem}
...
Theorem nonlocal_correlation_requires_revelation :
  forall (trace : Trace) (s_init s_final : VMState) (fuel : nat),
    ...
    (exists n fa ca k fl mu,
       nth_error trace n = Some (instr_lassert fa ca k fl mu)) \/
    (exists n mid p c mu,
       nth_error trace n = Some (instr_morph_assert mid p c mu)).
\end{lstlisting}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item \texttt{(exists ... instr\_morph\_assert ...)}: Contains MORPH\_ASSERT (categorical-morphism certification)
    \end{enumerate}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{No Hidden Variables Loophole}: The theorem closes the loophole where someone might claim "the structure was always there, I just measured it"
...
\begin{itemize}
    ...
    \item \textbf{MORPH\_ASSERT}: Certify a categorical morphism between modules (adds structural arrow with certificate cost)
\end{itemize}

\textbf{Falsification Criterion:} If someone claims "I achieved supra-quantum correlations without paying computational cost," inspect their trace. This theorem proves you'll find at least one high-cost instruction. If not, the claim is provably false.
...
A gauge transformation shifts the $\mu$-ledger by a constant while preserving every other field:
\begin{lstlisting}
...
\textbf{Practical Implication:} When verifying 3-way isomorphism (Coq, Python, Verilog), you only need to check that $\mu$ \emph{changes} match, not absolute values. If implementation A starts at $\mu=0$ and B starts at $\mu=1000$, that's fine—just verify increments are identical.
...
Here's the thing nobody told you about quantum mechanics: it's bookkeeping. Every quantum axiom---no-cloning, unitarity, the Born rule, purification---these all follow from the same conservation constraints I've been building. You set $\mu = 0$ and suddenly you can't clone, can't be non-unitary, can't have any probability rule other than Born's. The mathematics demands it.

A word of honesty, though: the physics enters through the hypotheses. Each proof below assumes a conservation constraint motivated by $\mu$-accounting, then derives the corresponding quantum axiom as an arithmetic consequence. The proofs show these axioms are \textit{connected}---they all follow from conservation-shaped assumptions---but the assumptions themselves encode the physical content. That is still valuable. It means these axioms are not independent postulates you have to swallow whole. They are structurally linked through a common accounting principle.
...
\textbf{How does this work?} The proof models a cloning operation as four real numbers: input information $I$, two output copies (both $= I$ for perfect cloning), and $\mu$-cost. Conservation requires $\text{out}_1 + \text{out}_2 \leq I + \mu$. With perfect cloning, that's $2I \leq I + \mu$, so $\mu \geq I > 0$. This is real arithmetic---the proof closes with \texttt{lra}. The conservation hypothesis is doing all the work. But that's the point: if you accept the $\mu$-accounting framework, no-cloning is not a separate postulate you need to assume. It drops out.
...
\begin{itemize}
    \item \textbf{Perfect Cloning is Impossible at Zero Cost:} If an operation is nontrivial, respects the conservation constraint, and achieves perfect cloning, then it cannot be zero-cost. The proof is the arithmetic fact $2I \leq I + 0$ contradicts $I > 0$.
...
Quantum time evolution is unitary. The accounting version: non-unitary evolution loses information (the Bloch vector shrinks), and the conservation hypothesis says information loss is bounded by $\mu$-cost.

\begin{theorem}[Unitarity from Conservation]
If the conservation constraint holds (\texttt{respects\_info\_conservation}: information loss $\leq$ $\mu$-cost for all states), and some evolution actually loses information (\texttt{info\_loss > 0}), then $\mu$-cost must be positive.
\end{theorem}

Proven in \path{coq/kernel/Unitarity.v}---the proof is one step of real arithmetic: from $\text{info\_loss} > 0$ and $\text{info\_loss} \leq \mu$, conclude $\mu > 0$. Like the no-cloning result, the physics is in the hypothesis (\texttt{respects\_info\_conservation}), and the conclusion follows by inequality chaining.
\begin{lstlisting}
...
This is the big one. The Born rule---probability equals amplitude squared---is universally taught as a postulate. The accounting version: the Born rule is the \textit{unique} linear probability rule consistent with eigenstate boundary conditions.
...
\begin{enumerate}
    ...
    \item \textbf{Eigenstate conditions:} $P(0,0,1,0) = 1$ and $P(0,0,-1,0) = 0$
    \item \textbf{Linearity in Bloch $z$-component:} $P(x,y,z,0) = az + b$ for some constants $a, b$
\end{enumerate}
\end{theorem}

A note of honesty: the linearity assumption (\texttt{is\_linear\_in\_z}) is doing most of the work here. If you already accept that measurement probabilities depend linearly on the Bloch vector's $z$-component, the Born rule is the unique solution satisfying the boundary conditions. The proof solves the system $a + b = 1$, $-a + b = 0$ to get $a = b = 1/2$, hence $P = (1+z)/2$. The full statement \texttt{valid\_linear\_rule\_is\_born\_with\_cost\_side\_condition} also takes a \texttt{measurement\_cost\_nonnegative} hypothesis, but it is unused in the actual proof---it reduces to the Bloch ball constraint already given. This is less ``deriving the Born rule from accounting'' and more ``the Born rule is the unique linear rule consistent with eigenstates.''

Proven in \path{coq/kernel/BornRule.v} as \texttt{valid\_linear\_rule\_is\_born\_with\_cost\_side\_condition}:
\begin{lstlisting}
...
\end{lstlisting}
The companion lemma \texttt{linear\_implies\_born} drops the cost-side hypothesis entirely; the file's own comment notes that \texttt{measurement\_cost\_nonnegative} is kept for compatibility with the surrounding development but is unused inside the proof.

\paragraph{Why Not Some Other Rule?}
Given the linearity assumption, other rules fail the boundary conditions:
\begin{itemize}
    \item Any probability rule linear in $z$ satisfying $P(z=1) = 1$ and $P(z=-1) = 0$ must be $(1+z)/2$
    \item Non-linear rules would require additional structure beyond the hypothesis
\end{itemize}
...
The Tsirelson bound $S \le 2\sqrt{2}$ limits quantum correlations. The proof in \path{coq/kernel/TsirelsonGeneral.v} is genuine algebra---probably the most substantive result in this section:
\begin{lstlisting}
...
The physics enters through the hypotheses. The row constraints $e_{00}^2 + e_{01}^2 \leq 1$ come from the NPA-1 moment matrix---a quantum-mechanical object (the positive semidefiniteness of the matrix of correlators from projective measurements on shared quantum states). The algebra is genuine:
\begin{itemize}
    \item \textbf{Row Constraints:} Each pair of correlators lies within the unit disk. This is the NPA-1 condition, which encodes the quantum content.
    \item \textbf{Algebraic Bound:} Given those constraints, the proof uses a sum-of-squares decomposition to show $S^2 \leq 8$. This part is real, machine-checked algebra.
...
\end{itemize}
The $\mu$-accounting framing says: setting $\mu_{\text{corr}} = 0$ (zero correlation cost) is exactly the condition of algebraic coherence, which forces the row constraints. Whether this identification is physically meaningful is a claim about the framework, not something the algebra proves.

\subsection{Why This Matters}

So what have I actually done here? Let me be honest about it.

Each proof takes a conservation constraint---motivated by $\mu$-accounting---and shows that a quantum axiom follows as arithmetic. The physics is in the hypotheses. I have not derived quantum mechanics from nothing. What I have shown is that these axioms are \textit{connected}: they all follow from conservation-shaped assumptions, which means they are not logically independent postulates you have to accept one by one.

\begin{itemize}
    \item \textbf{If you accept conservation constraints:} No-cloning, unitarity, and the Born rule drop out mechanically
    \item \textbf{If you don't:} These proofs don't help---the physics is in the assumptions
\end{itemize}

The Tsirelson bound is the strongest result: genuine algebra from row-norm constraints, with a real sum-of-squares identity. The others are lighter---arithmetic consequences of their hypotheses. That is still useful. It tells you the $\mu$-accounting framework is consistent with quantum mechanics, and it shows where the quantum content actually lives (in the conservation constraints, not in separate postulates).

\begin{tcolorbox}[colback=green!5!white,colframe=green!75!black,title=\textbf{Coq-Verified Quantum Axioms}]
All theorems in this section are machine-checked in Coq with zero \texttt{Admitted} statements. Line counts (\texttt{wc -l}) as of this writing:
\begin{itemize}
    \item \path{coq/kernel/NoCloning.v}: 969 lines
    \item \path{coq/kernel/Unitarity.v}: 585 lines
    \item \path{coq/kernel/BornRule.v}: 313 lines
    \item \path{coq/kernel/BornRuleLinearity.v}: 871 lines (Born rule as unique affine interpolation)
    \item \path{coq/kernel/Purification.v}: 251 lines
    \item \path{coq/kernel/TsirelsonGeneral.v}: 313 lines
    \item \path{coq/kernel/AlgebraicCoherence.v}: 775 lines (Tsirelson from algebraic constraints)
\end{itemize}
Total: 4,077 lines of machine-verified proof across seven files showing quantum axioms follow from conservation-shaped hypotheses.
\end{tcolorbox}
...
\draw[->] (nfi) -- (quant);
\end{tikzpicture}
\caption{Chapter 3 structure: the formal 5-tuple yields two key properties ($\mu$-monotonicity and no-signaling), which combine to prove No Free Insight. Conservation-shaped hypotheses then yield quantum axiom conditionals.}
...
\begin{enumerate}
    ...
    \item \textbf{Quantum Axioms from $\mu$-Accounting}: The fundamental axioms of quantum mechanics---no-cloning, unitarity, the Born rule, purification, and the Tsirelson bound---follow from conservation-shaped hypotheses motivated by $\mu$-accounting. The physics enters through the assumptions; the proofs verify logical entailment:
    \begin{itemize}
        \item \textbf{No-Cloning}: Conservation hypothesis ($\text{out}_1 + \text{out}_2 \leq I + \mu$) makes zero-cost cloning contradictory
        \item \textbf{Unitarity}: Info-loss bounded by $\mu$-cost; positive info-loss forces positive $\mu$
        \item \textbf{Born Rule}: Unique linear probability rule satisfying eigenstate boundary conditions
        \item \textbf{Purification}: Mixed states have purifications (Bloch sphere geometry)
        \item \textbf{Tsirelson Bound}: $S \le 2\sqrt{2}$ from row-norm constraints via sum-of-squares algebra
    \end{itemize}
\end{enumerate}

These formal foundations enable the implementation (Chapter 4), verification (Chapter 5), and evaluation (Chapter 6). The quantum axiom proofs (4,077 lines of Coq across seven files with zero \texttt{Admitted} statements) show that quantum mechanics is consistent with the $\mu$-accounting framework: each axiom follows from a conservation-shaped hypothesis. The physics is in the assumptions, but the structural connection between axioms---all flowing from conservation---is genuine and machine-verified. Key point: under \emph{Total $\mu$-Accounting}, setting $\mu_{\text{total}} = 0$ requires all components ($\mu_{\text{inst}}$ and $\mu_{\text{corr}}$) to be zero, where $\mu_{\text{corr}} = 0$ is exactly the condition of \emph{Algebraic Coherence} required to recover the Tsirelson bound $S \le 2.8284...$. Without enforcing $\mu_{\text{corr}} = 0$, the system is only bounded by the algebraic limit $S \le 4$.
