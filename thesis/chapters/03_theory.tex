...
\begin{enumerate}
    ...
    \item \textbf{Transition Rules $R$}: How the machine evolves---the 31-instruction ISA
...
% Record box
\node[draw, dashed, rounded corners=3pt, minimum width=3.6cm, minimum height=5.1cm] at (1.8, -2.15) {};
...
\node[field, fill=blue!10] (m) at (0, -1.5) {\texttt{vm\_mem} : list nat (4096)};
...
\begin{itemize}
    ...
    \item \textbf{vm\_mem}: Data memory of 4,096 words
...
\paragraph{What word masking does:} The \texttt{word32} function masks any number down to 32 bits---bitwise AND with \texttt{0xFFFFFFFF}. I had to learn why this exists the hard way: Coq's \texttt{nat} type is unbounded, but hardware registers are 32 bits. Without the mask, the Coq model and the hardware disagree on what happens at overflow.

\textbf{Step by step:}
\begin{enumerate}
    \item \textbf{\texttt{N.ones 32}}: Creates the bitmask \texttt{0xFFFFFFFF}---32 consecutive 1-bits. The \texttt{N} type is Coq's binary natural number representation, which is what bit operations need.
    ...
    \item \textbf{\texttt{N.land}}: Bitwise AND. ANDing any number with \texttt{0xFFFFFFFF} keeps only the lower 32 bits. Example: \texttt{0x1FFFFFFFF AND 0xFFFFFFFF = 0xFFFFFFFF}.
    \item \textbf{\texttt{N.to\_nat}}: Converts back to \texttt{nat} for the rest of the formal model.
\end{enumerate}

\textbf{Why this matters:} Coq's \texttt{nat} is unbounded---\texttt{0xFFFFFFFF + 1} would be \texttt{0x100000000} in Coq but \texttt{0x00000000} in hardware (wraparound). By applying \texttt{word32} after every operation, the mathematical model matches register behavior exactly. When I prove something about the model, I'm proving it about the actual hardware, not some idealized version.
...
The transition rules define how state evolves. The Thiele Machine has 31 instructions, defined in the formal step semantics.
...
The 31 instructions break into groups:
\begin{itemize}
...
\end{tikzpicture}
\caption{The 31 instructions grouped by category. Structural, logical, and certification operations have high $\mu$-cost (they add knowledge). Data movement, control, and compute operations have low or zero cost.}
...
\textbf{Why it works:} Every instruction's cost is a \texttt{nat} (natural number, can't be negative), so the ledger can only go up or stay the same. The proof walks through each of the 39 step constructors (covering success and failure branches for all 31 instructions) and confirms that \texttt{advance\_state} adds \texttt{instruction\_cost instr} to the ledger, which is always $\geq 0$. This isn't ``probably true'' or ``true in tests''---it's checked for all possible executions.
...
I had to learn how Coq handles this kind of proof the hard way: you do structural induction on the step relation, which means Coq generates one sub-goal per step constructor. All 39 of them. Each one boils down to showing that \texttt{advance\_state} adds a non-negative nat. It's tedious but airtight.
...
\begin{itemize}
    \item \path{coq/kernel/NoCloning.v}: 947 lines, 19 definitions/theorems
    \item \path{coq/kernel/NoCloningFromMuMonotonicity.v}: 281 lines, 14 definitions/theorems (machine-native, \texttt{lia}-only)
    \item \path{coq/kernel/Unitarity.v}: 592 lines, 25 definitions/theorems
    \item \path{coq/kernel/BornRule.v}: 342 lines, 21 definitions/theorems
    \item \path{coq/kernel/BornRuleFromSymmetry.v}: 144 lines (refactored; non-circular, tensor consistency)
    \item \path{coq/kernel/Purification.v}: 290 lines, 11 definitions/theorems
    \item \path{coq/kernel/TsirelsonGeneral.v}: 324 lines, 19 definitions/theorems
    \item \path{coq/kernel/TsirelsonFromAlgebra.v}: 339 lines, 14 definitions/theorems (self-contained algebraic)
\end{itemize}
Total: 3,259 lines of machine-verified proofs across eight files showing quantum axioms follow from conservation-shaped hypotheses.
\end{tcolorbox}
