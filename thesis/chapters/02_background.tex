...
These three bounds are among the 6 hard mathematical facts stated explicitly in \path{coq/kernel/AssumptionBundle.v}. I do not prove them in Coq---that would require massive real analysis libraries. Instead, I state them as verifiable assumptions and make every dependency transparent. Run \texttt{Print Assumptions} on any theorem and you see exactly which hard facts it uses.
...
For the Thiele Machine, this matters because the claims are not hand-waved. The 272 Coq files containing 2,154 theorems and lemmas were all checked by the compiler. Every property---$\mu$-monotonicity, No Free Insight, observational no-signaling---is established by a proof that the machine accepted, not by an argument I wrote in prose.
...
\begin{enumerate}
    ...
    \item \textbf{Documented assumptions}: External mathematical results (e.g., Tsirelson's theorem, Fine's theorem, Bell's inequality) are stated explicitly as a \texttt{HardMathFacts} record in \path{coq/kernel/AssumptionBundle.v}---6 assumptions total, each with justification. These are not Coq \texttt{Axiom} declarations; they are bundled as a record type and threaded through proofs as explicit parameters, making every dependency transparent.
\end{enumerate}
