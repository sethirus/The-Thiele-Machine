...
Turing's 1936 machine \cite{turing1936computable} is elegant. It's also the source of everything wrong with how we think about computation. Here's the formal definition---a 7-tuple:
\[
...
The $\mu$-ledger is the computational analog of thermodynamic entropy. Both are monotonically increasing. Both track irreversible commitments. The difference: thermodynamic entropy tracks energy dissipation; the $\mu$-ledger tracks structural information cost.

Landauer's principle says erasing one bit costs $k_B T \ln 2$ joules. The Thiele Machine says asserting one bit of structural constraint costs at least one $\mu$-bit. The parallel is not an analogy---it is a structural isomorphism between information processing and thermodynamics, and the Coq proofs in \path{coq/kernel/MuLedgerConservation.v} establish the monotonicity formally.

Every \texttt{LASSERT} (asserting a constraint), every \texttt{PSPLIT} (partitioning state), every \texttt{REVEAL} (disclosing information) increases $\mu$. Just as every bit erasure increases thermodynamic entropy. The second law of thermodynamics has a computational counterpart: the $\mu$-ledger never decreases.
...
Three bounds matter:
\begin{itemize}
    \item \textbf{Classical bound} ($S \le 2$): If Alice and Bob share only classical correlations---local hidden variables---then $|S| \le 2$. This is Bell's inequality, provable by enumerating all 16 deterministic strategies. Stated as \texttt{local\_S\_2} in \path{coq/kernel/AssumptionBundle.v}.
    \item \textbf{Quantum bound} ($S \le 2\sqrt{2} \approx 2.828$): If they share quantum entanglement, the maximum is the Tsirelson bound. This requires Tsirelson's 1980 theorem \cite{tsirelson1980quantum}, stated as \texttt{tsir\_from\_coh} in the same file.
    \item \textbf{Algebraic bound} ($S \le 4$): The absolute maximum from triangle inequality alone, achievable only by hypothetical ``PR-box'' correlations that violate quantum mechanics. Stated as \texttt{valid\_S\_4}.
\end{itemize}
...
\path{coq/kernel/HardMathFactsProven.v} constructs a concrete instance of this record with zero axioms and zero admits. All six are machine-checked proofs from first principles: \texttt{norm\_E\_bound} from probability axioms, \texttt{valid\_S\_4} from the triangle inequality, \texttt{local\_S\_2} from exhaustive 16-case enumeration, \texttt{pr\_no\_ext} for PR-box monogamy, \texttt{symm\_coh\_bound} for symmetric coherence, and \texttt{tsir\_from\_coh} for Tsirelson's bound via algebraic coherence. The bundle is a theorem, not an assumption. Run \texttt{Print Assumptions} on any downstream result and Coq reports only standard library axioms.
...
For the Thiele Machine, this matters because the claims are not hand-waved. The full Coq corpus (about 80,000 lines and over 2,300 theorem-like declarations) is checked by the compiler. Every property---$\mu$-monotonicity, No Free Insight, observational no-signaling, oracle impossibility---is established by a proof that the machine accepted, not by an argument I wrote in prose.
...
\item \textbf{Quantum Correlations} (\S2.5): Bell's theorem and the CHSH inequality reveal that quantum mechanics permits correlations up to $2\sqrt{2}$ but no higher. The Thiele Machine connects this bound to $\mu$-accounting---the 6 hard mathematical assumptions in \path{coq/kernel/AssumptionBundle.v} make the dependency chain explicit.
\end{enumerate}
...
\begin{itemize}
    ...
    \item All proofs satisfy the Inquisitor Standard---no admits, documented assumptions only
\end{itemize}
