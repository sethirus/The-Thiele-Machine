...
Before I dive into the Thiele Machine, you need to understand \textit{what problem it solves}. This chapter covers the foundations required for the formal model:
\begin{itemize}
...
This is reflected directly in the Coq kernel and the OCaml runner extracted from it: every instruction declares an explicit \texttt{mu\_delta}, and structural operations (\texttt{PNEW}, \texttt{PSPLIT}, \texttt{LASSERT}) carry their own per-instruction charges. The Python wrapper consumes the runner's JSON output rather than re-implementing the cost rule.
...
The erasure of one bit of information in a computing device dissipates at least $k_B T \ln 2$ joules of heat into the environment.
\end{theorem}
...
\textbf{Important caveat:} This is \textit{not} a derivation of $h$. Rather, defining $\tau_\mu := h / (4 E_{\text{landauer}})$ makes the relationship a tautology. The relationship establishes algebraic consistency but provides no predictive power without an independent derivation of $\tau_\mu$.
...
Three bounds matter:
\begin{itemize}
    \item \textbf{Classical bound} ($S \le 2$): If Alice and Bob share only classical correlations---local hidden variables---then $|S| \le 2$. This is Bell's inequality, provable by enumerating all 16 deterministic strategies. The upper bound is formalized via Fine's theorem in \path{coq/kernel/MinorConstraints.v}; the achievability of $S = 2$ at $\mu = 0$ is proven constructively in \path{coq/kernel/ClassicalBound.v}.
    \item \textbf{Quantum bound} ($S \le 2\sqrt{2} \approx 2.828$): If they share quantum entanglement, the maximum is the Tsirelson bound \cite{tsirelson1980quantum}. The kernel proves this through three complementary results. \path{coq/kernel/AlgebraicCoherence.v} proves a rational over-approximation $4e \le 5657/2000$ for the symmetric configuration ($E_{00}=E_{01}=E_{10}=e$, $E_{11}=-e$) using $3\times 3$ NPA moment-matrix minor constraints (\texttt{symmetric\_tsirelson\_bound}). \path{coq/kernel/TsirelsonGeneral.v} proves the exact real-arithmetic bound $|S| \le 2\sqrt{2}$ from row-norm hypotheses ($e_{00}^2 + e_{01}^2 \le 1$, $e_{10}^2 + e_{11}^2 \le 1$) via Cauchy--Schwarz (\texttt{tsirelson\_bound\_abs}). \path{coq/kernel/QuantumPartitionPSD.v} closes the chain: the biconditional \texttt{column\_contractive\_iff\_quantum\_realizable} shows that Thiele honesty (column-contractivity) coincides exactly with zero-marginal NPA realizability, discharging the row-norm hypotheses on the kernel side. The general algebraic case (without row-norm hypotheses) is proven only as $|S| \le 4$ (\texttt{chsh\_bound\_4}); recovering the tight bound there requires a sum-of-squares hypothesis that the file documents explicitly.
    \item \textbf{Algebraic bound} ($S \le 4$): The absolute maximum from triangle inequality alone, achievable only by hypothetical ``PR-box'' correlations that violate quantum mechanics. Formalized as \texttt{valid\_box\_S\_le\_4} in \path{coq/kernel/BoxCHSH.v} and \texttt{chsh\_bound\_4} in \path{coq/kernel/AlgebraicCoherence.v}.
\end{itemize}
...
These three bounds are proven from first principles in the active kernel files \path{coq/kernel/ClassicalBound.v}, \path{coq/kernel/BoxCHSH.v}, \path{coq/kernel/AlgebraicCoherence.v}, \path{coq/kernel/MinorConstraints.v}, \path{coq/kernel/TsirelsonGeneral.v}, and \path{coq/kernel/QuantumPartitionPSD.v}, with zero global \texttt{Axiom} or \texttt{Parameter} declarations and zero \texttt{Admitted} proofs. Running \texttt{Print Assumptions} on the downstream theorems reports only Coq's standard-library axioms (the Reals development pulls in \texttt{ClassicalDedekindReals.sig\_forall\_dec} and \texttt{FunctionalExtensionality.functional\_extensionality\_dep}); \texttt{ClassicalBound.classical\_bound\_achieved}, \texttt{AlgebraicCoherence.symmetric\_tsirelson\_bound}, and \texttt{BoxCHSH.box\_chsh\_bound\_algebraic} are reported as ``Closed under the global context.''
...
The trusted computing base is small---Coq's kernel is roughly 30,000 lines of OCaml. If you trust that kernel (and thousands of mathematicians do, daily), then you trust every proof it accepts. The alternative is trusting my prose. I know which one I would pick.

For the Thiele Machine, this matters because the claims are not hand-waved. The active Coq proof tree is checked by the compiler. Every property---$\mu$-monotonicity, $\mu$-initiality, No Free Insight, observational no-signaling---is established by a proof that the machine accepted, not by an argument I wrote in prose.
...
\begin{enumerate}
    ...
    \item \textbf{Documented assumptions}: External mathematical results (e.g., Tsirelson's theorem, Fine's theorem, Bell's inequality) are proven from first principles in the kernel rather than postulated. The active audited tree (\path{coq/kernel/}, \path{coq/kami_hw/}) contains zero global \texttt{Axiom} or \texttt{Parameter} declarations. The two named gaps that remain---the bridge axiom $A_{QM}$ in \path{coq/PhysicsConditionalClosure.v} and \texttt{bsc\_kami\_compilation\_trusted} in \path{coq/kernel/VerilogRTLCorrespondence.v}---are kept as Section Variables, so they appear as explicit hypotheses on any theorem that consumes them rather than as silent global postulates.
\end{enumerate}

This is enforced by \path{scripts/inquisitor.py}---a static analyzer that scans the active Coq tree in CI. It checks for \texttt{Admitted}, \texttt{admit}, project-local axioms, circular definitions, tautological proofs, smuggled constraints, mu-cost consistency, CHSH bounds, axiom dependencies, and more. If a theorem says ``Proven,'' it compiles to \texttt{Qed}.
...
\item \textbf{Quantum Correlations} (\S2.5): Bell's theorem and the CHSH inequality reveal that quantum mechanics permits correlations up to $2\sqrt{2}$ but no higher. The Thiele Machine connects this bound to $\mu$-accounting---these bounds are mechanically proven from first principles across \path{coq/kernel/ClassicalBound.v}, \path{coq/kernel/BoxCHSH.v}, \path{coq/kernel/AlgebraicCoherence.v}, \path{coq/kernel/MinorConstraints.v}, \path{coq/kernel/TsirelsonGeneral.v}, and \path{coq/kernel/QuantumPartitionPSD.v}, with no project-local global axioms.
\end{enumerate}
...
\begin{itemize}
    ...
    \item All proofs satisfy the Inquisitor Standard---zero admits, zero global \texttt{Axiom}/\texttt{Parameter} declarations in the active audited tree, named Section-Variable trust boundaries ($A_{QM}$, BSC compilation), and hard mathematical dependencies made explicit
\end{itemize}
