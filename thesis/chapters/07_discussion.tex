...
\begin{enumerate}
    \item \texttt{halting\_undecidable}: No total machine decides halting.
    \item \texttt{oracle\_halts\_costs\_mu}: Any partial oracle that resolves halting instances requires $\mu$-cost proportional to the input.
    \item \texttt{hypercomputation\_bounded}: No finite-$\mu$ trace can compute a function that a Turing Machine cannot.
\end{enumerate}
