...
\paragraph{Executed bridge runs.} The harness in Chapter 6 generated a JSON artifact for the four workloads (singleton pools of 2/4/16/64 elements; default: `results/thermo_experiment.json`, gitignored; override with `--out`). Python reports $\mu=\{2,3,5,7\}$; the extracted runner and RTL now report the same $\mu_{\text{raw}}$ because the μ-delta is explicitly encoded in the Coq trace and RTL instruction word, and the Python VM consumes that same μ-delta (disabling implicit MDLACC) for these workloads. With this encoding in place, `EVIDENCE\_STRICT` succeeds without normalization. The ledger still enforces $\mu \ge \log_2(|\Omega|/|\Omega'|)$ for each run; the $\mu/\log_2$ ratios (2.0, 1.5, 1.25, 1.167) quantify the slack now surfaced to reviewers.
