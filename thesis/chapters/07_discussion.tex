...
The model also produces six quantitative predictions (QD1--QD6) where it diverges from standard quantum mechanics. These are computed by the archived experiment script \path{archive/experiments\_unused/experiments/qm\_divergent\_predictions.py} and include projections for measurement-time ratios, modified Tsirelson bounds under finite $\mu$-budgets, and temperature-dependent CHSH suppression. Each prediction names a concrete experimental observable, a numerical value, and the measurement that would falsify it. These predictions are \textbf{speculative}---they follow from the $\mu$-accounting framework's axioms but have not been experimentally tested. If future experiments confirm or refute any of these values, the theory's empirical standing changes accordingly. The point is that the model commits to numbers rather than hedging.
...
receipt = {
    ...
    "mu_cost": cost,
    "signature": HMAC(canonical_json(payload))
}
\end{lstlisting}
...
\begin{itemize}
    ...
    \item \textbf{"signature": HMAC(...)} — Keyed hash authenticating the receipt. (The Ed25519 digital-signature module is available but archived; an HMAC or similar mechanism would serve the same tamper-detection role.)
\end{itemize}

\textbf{Why is this tamper-evident?} The chain is verified by checking:
\begin{enumerate}
    \item \textbf{Signature validity:} Every receipt is authenticated by the kernel key.
...
\end{enumerate}
Breaking the chain requires forging a keyed hash or finding a SHA-256 collision.


The receipt chain design is specified in the formal kernel and enforced in the RTL implementation (\path{thielecpu/hardware/rtl/thiele_cpu_kami.v}). The chain is therefore an engineered artifact with concrete hash formats, not an abstract promise.
...
DATA_MEMORY = 4096     (* MemSize in ThieleTypes.v *)
\end{lstlisting}
...
\begin{itemize}
    ...
    \item \textbf{DATA\_MEMORY = 4096} — Total data memory words. Partition regions are bounded by data memory depth, so the maximum practical region size is 4096 elements. Regions are stored as (base, size) pairs in the 64-slot partition table.
\end{itemize}

\textbf{Why these limits?} Hardware constraints:
\begin{itemize}
    \item \textbf{FPGA resources:} The current synthesized design uses ${\sim}2{,}800$ LUTs. Scaling \texttt{NUM\_MODULES} beyond 64 would increase the partition table storage and disjointness-checking logic proportionally.
...
\textbf{Real-world adequacy:} For many experiments (CHSH, Grover, Shor), 64 modules with 4096-word data memory are sufficient. For example:
\begin{itemize}
