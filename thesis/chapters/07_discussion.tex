...
\begin{tikzpicture}[scale=1.8, 
    node distance=3.0cm and 2cm,
    box/.style={rectangle, draw=black, line width=0.8pt, rounded corners=3pt, minimum width=5.5cm, minimum height=2.1cm, align=center, fill=blue!10, inner sep=0.4cm},
    topic/.style={rectangle, draw, rounded corners=3pt, minimum width=4.5cm, minimum height=2.0cm, align=center, fill=green!10},
    arrow/.style={->, line width=1.2pt, >=Stealth}
]
% Central question
\node[box, fill=red!12, minimum width=7.2cm, align=center, text width=4.5cm, font=
ormalsize] (meaning) {From Proofs\\to Meaning};

% Four topic areas
\node[topic, above left=2.0cm and 2cm of meaning, align=center, text width=4.5cm] (physics) {Physics\\Connections\\(§7.2--7.3)};
\node[topic, above right=2.0cm and 2cm of meaning, align=center, text width=4.5cm] (complexity) {Complexity\\Theory\\(§7.4)};
\node[topic, below left=2.0cm and 2cm of meaning, align=center, text width=4.5cm] (ai) {AI \& Trust\\(§7.5--7.6)};
\node[topic, below right=2.0cm and 2cm of meaning, align=center, text width=4.5cm] (future) {Limitations\\Future Work\\(§7.7--7.8)};
...
% Key concepts
\node[right=0.5cm of physics,  font=\small, xshift=10pt] {Landauer, Noether, Bell};
\node[right=0.5cm of complexity,  font=\small, xshift=10pt] {Time Tax, $\text{P}_\mu$, $\text{NP}_\mu$};
\node[left=0.5cm of ai,  font=\small, xshift=-10pt] {Hallucinations, Receipts};
\node[left=0.5cm of future,  font=\small, xshift=-10pt] {Quantum, Distributed};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.8, 
    ...
    phys/.style={rectangle, draw, rounded corners=3pt, minimum width=5.5cm, minimum height=2.0cm, align=center, fill=blue!12},
    comp/.style={rectangle, draw, rounded corners=3pt, minimum width=5.5cm, minimum height=2.0cm, align=center, fill=green!12},
    arrow/.style={<->, line width=1.2pt, >=Stealth, dashed}
]
% Physics column
...
\node[right=1.0cm of mugauge,  text width=3cm, align=center, font=\small, xshift=10pt] {Not metaphor:\\formal isomorphism};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.8, 
    ...
    layer/.style={rectangle, draw, rounded corners=3pt, minimum width=5.4cm, minimum height=2.1cm, align=center},
    arrow/.style={->, line width=1.2pt, >=Stealth}
]
% Three layers
\node[layer, fill=blue!12, align=center, text width=4.5cm] (abstract) {Abstract\\$\mu$-ledger charges $n$ bits};
\node[layer, fill=yellow!15, below=2.0cm of abstract, align=center, text width=4.5cm] (bridge) {Bridge Postulate\\$Q_{\min} = k_B T \ln 2 \cdot \mu$};
\node[layer, fill=green!12, below=2.0cm of bridge, align=center, text width=4.5cm] (physical) {Physical\\$Q \ge k_B T \ln 2 \cdot n$};

% Arrows
\draw[arrow] (abstract) -- node[right,  above, yshift=6pt, pos=0.5, font=\small] {Proven in Coq} (bridge);
\draw[arrow] (bridge) -- node[right,  above, yshift=6pt, pos=0.5, font=\small] {Empirical claim} (physical);

% Annotations
\node[right=2.9cm of abstract,  text width=3cm, align=center, font=\small, xshift=10pt] {Kernel theorem:\\$\mu \ge \log_2(|\Omega|/|\Omega'|)$};
\node[right=2.9cm of physical,  text width=3cm, align=center, font=\small, xshift=10pt] {Falsifiable prediction:\\energy scales with $\mu$};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.8, 
    node distance=3.0cm and 2cm,
    box/.style={rectangle, draw=black, line width=0.8pt, rounded corners=3pt, minimum width=5.4cm, minimum height=2.6cm, align=center, inner sep=0.4cm},
    arrow/.style={<->, line width=1.2pt, >=Stealth}
]
% Two strategies
\node[box, fill=red!12, align=center, text width=4.5cm, font=
ormalsize] (blind) {Blind Search\\$T = O(2^n)$\\$\mu = O(1)$};
\node[box, fill=green!12, right=5.9cm of blind, align=center, text width=4.5cm, font=
ormalsize] (sighted) {Sighted Search\\$T = O(k \cdot 2^{n/k})$\\$\mu = O(k)$};
\draw[arrow] (blind) -- (sighted) node[midway, above, font=\small] {Trade-off};

% Total cost annotation
\node[draw, rounded corners=3pt, fill=yellow!15, minimum width=9.0cm, font=\small, yshift=-6pt] at (2.9,-2) {$\text{Total Cost} = T(x) + \mu(x)$};

% Annotations
\node[below=0.4cm of blind, red, font=\small] {High time, low structure};
\node[below=0.4cm of sighted, green!50!black, font=\small] {Low time, high structure};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.8, 
    ...
    box/.style={rectangle, draw=black, line width=0.8pt, rounded corners=3pt, minimum width=5.5cm, minimum height=2.1cm, align=center, inner sep=0.4cm},
    arrow/.style={->, line width=1.2pt, >=Stealth}
]
% LLM path (top)
\node[box, fill=red!12, align=center, text width=4.5cm, font=
ormalsize] (llm) {LLM\\Generates};
\node[box, right=3.9cm of llm, fill=red!10, align=center, text width=4.5cm, font=
ormalsize] (output1) {Output\\(hallucination risk)};

% Thiele path (bottom)
\node[box, fill=blue!12, below=2.9cm of llm, align=center, text width=4.5cm, font=
ormalsize] (predict) {Model\\Predicts};
\node[box, fill=yellow!15, right=2.9cm of predict, align=center, text width=4.5cm, font=
ormalsize] (certify) {VM\\Certifies};
\node[box, fill=green!12, right=2.9cm of certify, align=center, text width=4.5cm, font=
ormalsize] (output2) {Output\\(verified)};
\node[box, fill=red!10, below=1.6cm of certify, align=center, text width=4.5cm, font=
ormalsize] (penalty) {$\mu$-penalty\\if false};

% Arrows
\draw[arrow] (predict) -- node[above, font=\small] {hypothesis} (certify);
\draw[arrow] (certify) -- node[above, font=\small] {receipt} (output2);
\draw[arrow] (certify) -- node[right, font=\small] {if false} (penalty);

% Labels
\node[left=0.5cm of llm,  above, font=\small, xshift=-10pt] {Classic AI:};
\node[left=0.5cm of predict,  above, font=\small, xshift=-10pt] {Thiele AI:};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[scale=1.8, 
    node distance=3.0cm and 1cm,
    receipt/.style={rectangle, draw, rounded corners=3pt, minimum width=4.5cm, minimum height=2.6cm, align=center, fill=blue!10},
    arrow/.style={->, line width=1.2pt, >=Stealth}
]
% Receipt chain
\node[receipt, align=center, text width=4.5cm] (r1) {Receipt 1\\$H_0$};
\node[receipt, right=2.9cm of r1, align=center, text width=4.5cm] (r2) {Receipt 2\\$H_1$};
\node[receipt, right=2.9cm of r2, align=center, text width=4.5cm] (r3) {Receipt 3\\$H_2$};
\node[right=1.0cm of r3] (dots) {$\cdots$};
\node[receipt, right=1.0cm of dots, align=center, text width=4.5cm] (rn) {Receipt $n$\\$H_{n-1}$};
...
% Receipt structure
\node[below=2.9cm of $(r2)!0.5!(r3)$, draw, rounded corners=3pt, fill=yellow!15, minimum width=10.8cm, minimum height=3.6cm, align=left, font=\ttfamily\scriptsize, align=center, text width=4.5cm] (struct) {
receipt = \{\\
...
% Annotations
\node[above=0.5cm of r2,  font=\small, yshift=6pt] {Tamper-evident chain};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.8, 
    node distance=3.0cm and 1.5cm,
    current/.style={rectangle, draw, rounded corners=3pt, minimum width=5.4cm, minimum height=2.1cm, align=center, fill=green!12},
    future/.style={rectangle, draw, rounded corners=3pt, minimum width=5.4cm, minimum height=2.1cm, align=center, fill=blue!10},
    arrow/.style={->, line width=1.2pt, >=Stealth, dashed}
]
% Current state
\node[current, align=center, text width=4.5cm] (now) {Current Thiele Machine\\206 proofs, 3 layers};

% Future directions
\node[future, above right=2.0cm and 2cm of now, align=center, text width=4.5cm] (quantum) {Quantum\\Integration};
\node[future, right=3.9cm of now, align=center, text width=4.5cm] (distributed) {Distributed\\Execution};
\node[future, below right=2.0cm and 2cm of now, align=center, text width=4.5cm] (language) {Programming\\Language};
...
% Annotations
\node[right=0.5cm of quantum,  text width=3cm, font=\small, xshift=10pt] {Entanglement as partition structure};
\node[right=0.5cm of distributed,  text width=3cm, font=\small, xshift=10pt] {Modules $\rightarrow$ network nodes};
\node[right=0.5cm of language,  text width=3cm, font=\small, xshift=10pt] {First-class partitions, $\mu$-tracking};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.8, 
    node distance=3.0cm and 1.5cm,
    box/.style={rectangle, draw=black, line width=0.8pt, rounded corners=3pt, minimum width=5.5cm, minimum height=2.1cm, align=center, fill=blue!10, inner sep=0.4cm},
    result/.style={rectangle, draw, rounded corners=3pt, minimum width=6.2cm, minimum height=2.2cm, align=center, fill=green!12},
    arrow/.style={->, line width=1.2pt, >=Stealth}
]
% Four areas
\node[box, align=center, text width=4.5cm, font=
ormalsize] (physics) {Physics\\Connections};
\node[box, right=2.0cm of physics, align=center, text width=4.5cm, font=
ormalsize] (complexity) {Complexity\\Theory};
\node[box, right=2.0cm of complexity, align=center, text width=4.5cm, font=
ormalsize] (ai) {AI \&\\Trust};
\node[box, right=2.0cm of ai, align=center, text width=4.5cm, font=
ormalsize] (future) {Future\\Work};

% Central result
\node[result, below=2.9cm of $(complexity)!0.5!(ai)$, align=center, text width=4.5cm] (result) {Structure as\\Conserved Resource};
...
% Key insight
\node[below=1.0cm of result, draw, rounded corners=3pt, fill=yellow!15, minimum width=14.4cm,  font=\small, yshift=-6pt] {$\mu$-accounting unifies computation, physics, and verification};
\end{tikzpicture}
