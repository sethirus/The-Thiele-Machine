...
The Einstein side of the physics bridge now has a sharper status than earlier drafts of this thesis assumed. In the current repository, the uniform diagonal boundary-of-4-simplex regime is proved directly in Coq through the full tensor pipeline. The stronger statement that arbitrary non-uniform diagonal boundary assignments force off-diagonal Ricci to vanish is not merely unproved; it is formally refuted for the current discrete derivative operator. That means the remaining question is no longer ``can I push harder and prove it anyway?'' but ``is the present operator the right physical discretization?''


\section{What the results actually imply}

Three kinds of claims live in this chapter and I'm going to be explicit about which is which. The first kind is proven: the $\mu$-hierarchy, No Free Insight, the Thiele-honesty biconditional with NPA realizability. These are theorems. The second kind is conditional: the Landauer thermodynamic bridge connects the ledger to physical energy, but only if you accept the bridge postulate; the postulate is stated explicitly and it is empirically falsifiable. The third kind is proposed but not proven: the structure-aware complexity classes ($\text{P}_\mu$, $\text{NP}_\mu$) are vocabulary for reasoning about the time/structure trade-off, not settled results in complexity theory. I am drawing implications of all three types in this chapter. I will not conflate them.
...
\textbf{Example:} If a trace performs 100 bit erasures, the ledger must grow by at least 100 $\mu$-bits. If the ledger only grows by 50, the proof proves this trace is invalid (it would have been rejected during execution).
...
\textbf{Example:} Suppose state $s$ has modules $\{A, B, C\}$ and you execute \texttt{PSPLIT A \{0,1\} \{2,3\}}. The theorem proves:
\begin{itemize}
...
\textbf{Why does this matter?} This theorem proves that:
\begin{enumerate}
...
\end{itemize}
The theorem proves both VMs have identical partition structures at the end. The absolute $\mu$ differs by 1000, but this is a gauge artifact---the \textit{structural work} ($\Delta\mu = 100$) is the same.
...
\paragraph{Translation dictionary.} Let $|\Omega|$ be the admissible microstate count of an $n$-bit device ($|\Omega| = 2^n$ at fixed resolution). A revelation step $\Omega \to \Omega'$ (e.g., \texttt{PNEW}, \texttt{PSPLIT}, \texttt{MDLACC}, \texttt{REVEAL}) shrinks the space by $|\Omega|/|\Omega'|$. The Coq kernel proves $\mu \ge |\phi|_{\text{bits}}$ (description length). The ISA charging rule for \texttt{LASSERT}, $\Delta\mu = \texttt{flen} \times 8 + S(\texttt{cost})$, dominates the conservative reading $|\Omega|=2^n$, $|\Omega'|=1$; the rule may overcharge when multiple solutions exist, avoiding \#P-complete model counting. The Python wrapper consumes that ledger from the OCaml runner without re-deriving it. The system adopts the bridge postulate that charging $\mu$ bits lower-bounds dissipated heat/work: $Q_{\min} = k_B T \ln 2 \cdot \mu$, with an explicit inefficiency factor $\epsilon \ge 1$ for real devices. This postulate is external to the kernel and is presented as an empirical claim.
...
\paragraph{Executed bridge runs.} The evaluation in Chapter 6 reports the four workloads (singleton pools of 2/4/16/64 elements). Python reports $\mu=\{2,3,5,7\}$; the extracted runner and RTL report the same $\mu_{\text{raw}}$ because the μ-delta is explicitly encoded in the trace and instruction word, and the reference VM consumes that same μ-delta (disabling implicit MDLACC) for these workloads. With this encoding in place, the strict-evidence test surface succeeds without renormalization. The ledger still enforces $\mu \ge \log_2(|\Omega|/|\Omega'|)$ for each run; the $\mu/\log_2$ ratios (2.0, 1.5, 1.25, 1.167) quantify the slack now surfaced to reviewers.
...
A complexity-theoretic framework is vacuous if it cannot express ordinary computation. The Coq formalization proves that the Thiele Machine is at least Turing-complete via a two-step embedding chain: every Turing Machine reduces to a Minsky Machine, and every Minsky Machine reduces to a Thiele Machine trace. The composed result is \texttt{thiele\_turing\_complete\_via\_minsky\_tc} (and its ISA variant \texttt{thiele\_isa\_turing\_complete\_via\_minsky\_tc}) in \texttt{coq/ThieleMachineComplete.v}.
...
The synthesized RTL exposes the following bounded resource budget:
\begin{lstlisting}
REG_COUNT   = 16  (* RegCount   in ThieleTypes.v *)
NUM_MODULES = 64  (* PTableSz   in ThieleTypes.v *)
DATA_MEMORY = 128 (* MemSize    in ThieleTypes.v *)
DESC_TABLES = 16  (* MorphTableSz, CouplingDescSz,
                     FormulaDescSz, CertDescSz, DescMetaSz,
                     CouplingPairSz: each capped at 16 slots. *)
\end{lstlisting}

\paragraph{Parameter meanings:}
\begin{itemize}
    \item \textbf{REG\_COUNT = 16} — Per \path{coq/kami_hw/ThieleTypes.v}, \texttt{RegCount := 16} general-purpose registers indexed by a $4$-bit field (\texttt{RegIdxSz := 4}). The hardware-side \texttt{regs} register is therefore $16 \times 32 = 512$ bits wide; register indices above $15$ wrap via \texttt{r mod RegCount} in the live step relation.

    \item \textbf{NUM\_MODULES = 64} — \texttt{PTableSz := 64} partition slots indexed by a $6$-bit field (\texttt{PTableIdxSz := 6}). Each slot is one \texttt{WordSz}-bit word (\texttt{WordSz := 32}) inside the on-chip \texttt{ptTable} register; module IDs above $63$ wrap via \texttt{m mod 64} in the live step relation.

    \item \textbf{DATA\_MEMORY = 128} — Total data memory words in the synthesized RTL. The kernel's \texttt{MEM\_SIZE} is parametric, so kernel proofs hold at any depth; the synthesized RTL fixes it to keep the read-port mux logic small. Partition regions are bounded by data memory depth.

    \item \textbf{DESC\_TABLES = 16} — Each auxiliary descriptor table (morphism, coupling, formula, certificate, descriptor-metadata, coupling-pair) is bounded to $16$ slots in the synthesized RTL. The kernel ISA is unchanged but synthesized programs trap on the $17$th allocation in any one table.
\end{itemize}

\paragraph{FPGA resources.}
On the synthesized RTL, \texttt{yosys synth\_xilinx -family xc7 -abc9 -nodsp} reports approximately $13{,}648$ logic cells (sum of LUT1--LUT6 and INV) plus $173$ \texttt{CARRY4}, $3{,}580$ \texttt{FDRE} flip-flops, $64$ \texttt{RAM64M} distributed-RAM primitives, $2$ \texttt{RAMB36E1} block RAMs, $1$ \texttt{BUFG}, and the I/O buffers for the five wrapper pins, totalling $18{,}546$ cells on the wrapper top \texttt{thiele\_cpu\_top}. This corresponds to roughly $66\%$ of the xc7a35t's $\sim 20{,}800$ LUT budget. The locality-checking logic guards every partition-table access through the active module slot.

\paragraph{Comparison to software.}
The Python reference VM has no hard limits---it uses dynamic data structures (\texttt{dict}, \texttt{set}) that grow as needed. The hardware must pre-allocate resources, leading to fixed capacity.

\paragraph{Scaling.}
The Coq kernel proofs are parametric over \texttt{REG\_COUNT}, \texttt{MEM\_SIZE}, and \texttt{PTableSz}; rebuilding with larger values produces a faithfully larger machine, the same OCaml extraction shape, and a re-extracted RTL whose silicon footprint scales linearly. Industrial workloads (large SAT instances, deep Shor factorizations) live above the current hardware budget but inside the kernel's mathematical envelope.
...
The \texttt{LASSERT} instruction uses an on-chip FSM that reads formula and witness data directly from \texttt{vm\_mem}:
\begin{lstlisting}
...
\textbf{What is this instruction?} \texttt{LASSERT} adds a logical constraint to a partition module. The formula and witness block are stored in the VM's data memory (\texttt{vm\_mem}); the instruction holds only register indices that point to them. This keeps the instruction word compact while allowing arbitrarily large formulas.

\textbf{Parameter breakdown:}
\begin{itemize}
    \item \textbf{formula\_addr\_reg : nat} — Register holding the base address of the encoded formula in \texttt{vm\_mem}.
    \item \textbf{cert\_addr\_reg : nat} — Register holding the base address of the witness block in \texttt{vm\_mem}.
    \item \textbf{cert\_kind : bool} — \texttt{true} = SAT path: checks satisfying model plus falsifying countermodel. \texttt{false} = UNSAT path: always fails in the current kernel (LRAT checking exists in \texttt{CertCheck.v} but is not invoked by the step semantics).
    \item \textbf{formula\_len : nat} — Encoded formula-unit count in memory. Directly sets the formula-component of the $\mu$-cost.
    \item \textbf{mu\_delta : nat} — Explicit cost floor. The total $\mu$-cost charged is $\mathtt{formula\_len} \times 8 + S(\mathtt{mu\_delta})$, so cost is always at least 1.
\end{itemize}
...
\]
where $\mathtt{formula\_len}$ is the formula's encoded-unit count, with eight concrete bits charged per unit. Longer formulas cost proportionally more, encoding the information-theoretic principle that structurally richer claims are more expensive. $S(\mathtt{mu\_delta})$ proves the cost is at least 1, even for the empty formula.

\textbf{Example workflow:}
\begin{enumerate}
    \item Store the encoded formula for \texttt{"(and (or x0 x1) (or (not x0) x2))"} at address 100 in \texttt{vm\_mem}; store a satisfying model and a falsifying countermodel back-to-back at address 200.
    ...
    \item The on-chip FSM reads the formula and both witnesses from memory, verifies that one satisfies and the other falsifies, and updates the partition's axiom set.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item A Thiele-honesty $\iff$ NPA-realizability biconditional (zero axioms, proven as \texttt{column\_contractive\_iff\_quantum\_realizable} in \path{coq/kernel/QuantumPartitionPSD.v}), making the quantum connection formally provable rather than merely analogical
\end{enumerate}
