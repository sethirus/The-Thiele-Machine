...
The model commits to concrete, testable predictions about its own behavior: every VM operation scales linearly (or better) in $\mu$-cost, and the cost axiom re-derives known physics bounds (CHSH $\le 2$, Tsirelson $\le 2\sqrt{2}$, Landauer erasure). These are machine-checked and experimentally validated in Chapter~11. Whether the cost axiom produces genuinely \emph{new} physical predictions---predictions that diverge from standard quantum mechanics---remains an open question. The theory currently re-derives known results from new axioms; it does not yet predict something that quantum mechanics alone does not. If it eventually does, the predictions will be stated with the same falsification discipline applied throughout this work.
...
\begin{enumerate}
    ...
    \item Concrete falsifiable $\mu$-cost scaling bounds and physics-bridge re-derivations (CHSH, Tsirelson, Landauer), with the open question of whether genuinely new physical predictions will emerge
\end{enumerate}
