...
% Container
\node[draw, thick, rounded corners=2pt, minimum width=5.2cm, minimum height=4.4cm, label={[font=\footnotesize\bfseries]above:VMState Record}] (container) at (2.6,1.8) {};

% Fields
\node[field, fill=blue!12] (graph) at (0.3,3.6) {vm\_graph : PartitionGraph};
\node[field, fill=blue!12] (csrs) at (0.3,3.05) {vm\_csrs : CSRState};
\node[field, fill=green!12] (regs) at (0.3,2.5) {vm\_regs : list nat};
\node[field, fill=green!12] (mem) at (0.3,1.95) {vm\_mem : list nat};
\node[field, fill=purple!12] (pc) at (0.3,1.4) {vm\_pc : nat};
\node[field, fill=red!15, line width=1.2pt, draw=red!70] (mu) at (0.3,0.85) {vm\_mu : nat};
\node[field, fill=gray!12] (err) at (0.3,0.3) {vm\_err : bool};

% Annotations
\node[font=\scriptsize, anchor=west] at (4.3,2.5) {32 regs};
\node[font=\scriptsize, anchor=west] at (4.3,1.95) {256 words};
\node[font=\scriptsize, anchor=west, red!70] at (4.3,0.85) {$\mu$-ledger};

% Data brace
\draw[decorate, decoration={brace, amplitude=4pt}] (4.2,2.75) -- (4.2,1.7) node[midway, right=5pt, font=\scriptsize] {Data};
\end{tikzpicture}
\caption{VMState record: seven fields, with \texttt{vm\_mu} (the $\mu$-ledger) as the central cost accumulator.}
\end{figure}
...
The step relation is an inductive predicate with 23 constructors covering the 18 opcodes. Several opcodes have both success and failure constructors (e.g., \texttt{step\_psplit} and \texttt{step\_psplit\_failure}), so the constructor count exceeds the opcode count. Each constructor states exact preconditions and the resulting next state:
\begin{lstlisting}
...
\textbf{23 Constructors} covering the 18 opcodes (some have success/failure variants):
\begin{itemize}
