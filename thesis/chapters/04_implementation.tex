...
The tests compare \textit{state projections} rather than every internal variable. The projections are suite-specific: the compute gate in \path{tests/test_rtl_compute_isomorphism.py} compares registers and memory, while the partition gate in \path{tests/test_partition_isomorphism_minimal.py} compares canonicalized module regions from the partition graph. The comprehensive bridge verification happens in \path{tests/proof_three_layer_isomorphism.py}, which confirms the full contract. The extracted runner emits a full JSON snapshot (pc, $\mu$, err, regs, mem, CSRs, graph), but the RTL testbench exposes only the fields required by each gate.
...
\begin{itemize}
    ...
    \item \textbf{Step semantics}: the 26-instruction ISA and the inductive transition rules.
...
% Container
\node[draw, thick, rounded corners=2pt, minimum width=5.2cm, minimum height=5.0cm, label={[font=\footnotesize\bfseries]above:VMState Record}] (container) at (2.6,1.8) {};
...
\node[font=\scriptsize, anchor=west] at (4.3,2.25) {256 words};
...
\end{tikzpicture}
\caption{VMState record: eight fields, with \texttt{vm\_mu} (the $\mu$-ledger) and \texttt{vm\_mu\_tensor} (the 4$\times$4 cost tensor) as cost accumulators.}
\end{figure}

\subsection{The VMState Record}

The state is defined as a record with eight components:
\begin{lstlisting}
Record VMState := {
...
\begin{quote}
\textit{Author's Note (Devon): Look, I know what you're thinking. ``Eight fields? That's it?'' Yeah. That's it. Every computation this machine does boils down to shuffling values between these eight buckets. It's like a car---looks complicated under the hood, but at the end of the day it's just ``make explosions, turn wheels.'' Here it's ``move bits, track cost.''}
\end{quote}
...
The step relation is an inductive predicate with 32 constructors covering the 26 opcodes. Several opcodes have both success and failure constructors (e.g., \texttt{step\_psplit} and \texttt{step\_psplit\_failure}), so the constructor count exceeds the opcode count. Each constructor states exact preconditions and the resulting next state:
\begin{lstlisting}
...
\textbf{32 Constructors} covering the 26 opcodes (some have success/failure variants):
\begin{itemize}
...
Definition advance_state (s : VMState) (instr : vm_instruction)
  ...
  {| vm_graph := graph';
     ...
     vm_mu_tensor := apply_tensor_cost s instr;
     vm_err := err' |}.
\end{lstlisting}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Output: \path{build/thiele\_core.ml} ($\sim$3,500 lines of OCaml)
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Peano trap}: Coq's \texttt{nat} extracts to unary arithmetic by default. Without the \texttt{Extract Constant Nat.*} overrides and the post-extraction patching script (\path{scripts/patch\_extracted\_nat.py}), expressions like \texttt{Nat.pow 2 31} would trigger $\sim$2 billion recursive calls and crash with a stack overflow
...
\begin{enumerate}
    ...
    \item \textbf{Patch Nat module}: \texttt{python3 scripts/patch\_extracted\_nat.py build/thiele\_core.ml} --- replaces the Peano-recursive \texttt{module Nat} body in the extracted OCaml with native \texttt{int} implementations (\texttt{add = (+)}, \texttt{mul = ( * )}, \texttt{pow} via loop)
...
\end{enumerate}
The patching step exists because Coq's \texttt{Extract Constant} directives don't affect functions defined \emph{inside} the generated \texttt{module Nat = struct ... end} block---a known limitation of the extraction mechanism. The script is idempotent and regex-based, so it works across Coq versions without manual intervention.
...
\begin{itemize}
    ...
    \item \textbf{ISA decoding}: a compact representation of the 26 opcodes.
...
The \texttt{PYEXEC} instruction executes user-supplied code. With sandboxing enabled: restricted to safe builtins and an AST allowlist. With sandboxing disabled: trusted host callback. Either way, side effects are observable in the trace, and structural information revealed is charged in $\mu$.
...
The Inquisitor (\path{scripts/inquisitor.py}, 3,000+ lines) is a heuristic static analysis tool that scans the Coq source tree for ``proof smells''---patterns that indicate incomplete, vacuous, or dishonest proofs. It goes far beyond simple pattern matching.
...
\begin{lstlisting}
$ yosys -p "read_verilog thiele_cpu_unified.v; synth_xilinx -top thiele_cpu"
\end{lstlisting}
...
vvp thiele_cpu_tb
    ...
    \begin{verbatim}
yosys -p "read_verilog thiele_cpu_unified.v; synth_xilinx -top thiele_cpu; write_json netlist.json"
    \end{verbatim}
    \begin{itemize}
