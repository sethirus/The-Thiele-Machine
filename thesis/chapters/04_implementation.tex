...
% Three layers
\node[layer, fill=blue!15, draw=blue!60] (coq) at (0,2.4) {\textbf{Layer 1: Coq (Formal)}\\2,154 verified theorems};
...
\node[font=\tiny, text=green!50!black] at (2.2,-0.55) {272 compile};
...
The Inquisitor (\path{scripts/inquisitor.py}, $\sim$1,500 lines) is a heuristic static analysis tool that scans the Coq source tree for ``proof smells''---patterns that indicate incomplete, vacuous, or dishonest proofs. It goes far beyond simple pattern matching.
...
% Three layers
\node[layer, fill=blue!15] (coq) at (-2.4,1.2) {\textbf{Coq}\\2,154 theorems};
