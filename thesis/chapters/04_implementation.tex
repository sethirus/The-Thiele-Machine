...
\end{tikzpicture}
\caption{3-Layer Isomorphism: three independent implementations bound by a single invariant.}
\end{figure}
...
That's why the system was built three times. Not out of masochism (though I question that sometimes), but because I needed to \textit{know}. I needed to see the same answer come out of three completely different ``workshops''---one that speaks pure math (Coq), one that speaks Python like a normal person, and one that speaks to actual hardware in Verilog.
...
\textbf{Answer}: Build three independent implementations and verify they produce \textit{identical results} for all inputs. This makes the thesis rebuildable: every layer can be re-implemented from definitions here, and any mismatch is detectable.
...
\begin{tcolorbox}[thesisbox,colback=blue!5!white,colframe=blue!75!black,title=3-Layer Isomorphism Contract]
...
\begin{itemize}
    \item \textbf{Compute gate}: registers[0:31], memory[0:4095]
...
\begin{itemize}
    ...
    \item Section 4.4: Python VM (state class, partition operations, receipt generation)
    \item Section 4.5: Verilog RTL (CPU module, $\mu$-ALU, logic engine interface)
...
The binding constraint: for any instruction sequence $\tau$, the state projections must be identical across all three layers. The projections are suite-specific (registers/memory for compute traces; module regions for partition traces), while the extracted runner provides a superset of observables that can be compared when a gate requires it.
...
\begin{itemize}
    ...
    \item \textbf{Step semantics}: the 31-instruction ISA and the inductive transition rules.
...
% Container
\node[draw, thick, rounded corners=2pt, minimum width=5.2cm, minimum height=5.9cm, label={[font=\footnotesize\bfseries]above:VMState Record}] (container) at (2.6,1.35) {};
...
\node[font=\scriptsize, anchor=west] at (4.3,2.25) {4096 words};
...
\end{tikzpicture}
\caption{VMState record: ten fields, with \texttt{vm\_mu} (the $\mu$-ledger) and \texttt{vm\_mu\_tensor} (the 4$\times$4 cost tensor) as cost accumulators.}
\end{figure}

\subsection{The VMState Record}

The state is defined as a record with ten components:
\begin{lstlisting}
Record VMState := {
...
\begin{quote}
\textit{Author's Note (Devon): Look, I know what you're thinking. ``Ten fields? That's it?'' Yeah. That's it. Every computation this machine does boils down to shuffling values between these ten buckets. It's like a car---looks complicated under the hood, but at the end of the day it's just ``make explosions, turn wheels.'' Here it's ``move bits, track cost.''}
\end{quote}
...
\begin{itemize}
    ...
    \item \textbf{vm\_mem}: 4,096 words of data memory
...
The step relation is an inductive predicate with 39 constructors covering the 31 opcodes. Several opcodes have both success and failure constructors (e.g., \texttt{step\_psplit} and \texttt{step\_psplit\_failure}), so the constructor count exceeds the opcode count. Each constructor states exact preconditions and the resulting next state:
\begin{lstlisting}
...
\textbf{39 Constructors} covering the 31 opcodes (some have success/failure variants):
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{vm\_mu\_tensor := apply\_tensor\_cost s instr}: Update the 4$\times$4 inter-category cost tensor
...
The formal definitions are extracted to a functional evaluator to create a reference semantics. The extraction entrypoint \path{coq/Extraction.v} imports 13 kernel modules and extracts over 100 symbols across every subsystem---core VM semantics, receipt integrity, CHSH pipeline, semantic and operational $\mu$-cost, ledger conservation, falsifiable predictions, three-layer isomorphism, hardware bisimulation, and Tsirelson bound computation:
\begin{lstlisting}
...
From Kernel Require Import VMState VMStep SimulationProof
  ...
  MuCostModel MuLedgerConservation FalsifiablePrediction
...
Extraction "../build/thiele_core.ml"
  ...
  (* CHSH pipeline *)
  CHSHExtraction.chsh_from_vm_trace CHSH.chsh_local_z ...
  (* Three-layer isomorphism *)
  ThreeLayerIsomorphism.project_vmstate ...
  (* ~100 symbols total across 13 subsystems *)
\end{lstlisting}
...
\begin{enumerate}
    ...
    \item \textbf{Extract Constant Nat.*}: Override Peano-recursive arithmetic with native \texttt{int} operations (\texttt{(+)}, \texttt{( * )}, etc.) to prevent stack overflows from unary recursion at runtime
    \item \textbf{From Kernel Require Import ...}: Pull in 13 kernel modules covering the full formal development
    \item \textbf{Extraction "../build/thiele\_core.ml" names}: Extract 100+ named definitions to a single OCaml file
    \begin{itemize}
        \item \texttt{vm\_apply}, \texttt{run\_vm}: The executable step and multi-step evaluators
        \item \texttt{receipt\_mu\_consistent\_b}: Receipt integrity checker
        \item \texttt{project\_vmstate}: Three-layer isomorphism projection
        \item Output: \path{build/thiele\_core.ml} ($\sim$4,500 lines of OCaml)
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Peano trap}: Coq's \texttt{nat} extracts to unary arithmetic by default. Without the \texttt{Extract Constant Nat.*} overrides in \path{coq/Extraction.v}, expressions like \texttt{Nat.pow 2 31} would trigger $\sim$2 billion recursive calls and crash with a stack overflow
...
\begin{enumerate}
    ...
    \item \textbf{Native overrides}: The \texttt{Extract Constant} directives in \path{coq/Extraction.v} replace Peano-recursive \texttt{Nat.*} functions with native \texttt{int} operations (\texttt{add = (+)}, \texttt{mul = ( * )}, \texttt{pow} via loop), preventing stack overflows from unary recursion at runtime
    \item \textbf{Compile runner}: \texttt{ocamlfind ocamlopt build/thiele\_core.ml tools/extracted\_vm\_runner.ml -o build/extracted\_vm\_runner}
...
\begin{itemize}
    ...
    \item \textbf{ISA decoding}: a compact representation of the 31 opcodes.
...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
The key fact: the VM owns a \texttt{State} object (mirroring the Coq record) and also keeps a minimal register file and scratch memory used by the XOR opcodes that map directly to RTL. This separation is intentional---\texttt{State} captures partition and $\mu$-ledger semantics, while the auxiliary arrays let the VM exercise hardware-style instructions without introducing a second notion of state.
...
\end{array}
\]
The additional fields (\texttt{mu\_ledger}, \texttt{partition\_masks}, \texttt{program}) bridge to the other layers. \texttt{mu\_ledger} makes $\mu$-accounting explicit. \texttt{partition\_masks} provides hardware-aligned region encoding. \texttt{program} aligns with \texttt{CoreSemantics.State.program}---the kernels \texttt{VMState} does not carry a program field, but the executable state does.
...
\end{itemize}
The first branch of \texttt{pnew} demonstrates idempotent discovery: creating a module for a region that already exists returns the existing ID. Module IDs stay stable across layers, and $\mu$-cost is never paid twice.
...
\centering
\begin{tikzpicture}[
    mod/.style={draw, rounded corners=2pt, minimum width=2.2cm, minimum height=0.7cm, font=\scriptsize, align=center, thick},
    ext/.style={draw, dashed, rounded corners=2pt, minimum width=1.8cm, minimum height=0.6cm, font=\scriptsize, align=center},
]
% Top-level CPU
\node[mod, fill=blue!15, draw=blue!60] (cpu) at (0,2) {\textbf{thiele\_cpu}\\main core};

% Sub-modules
\node[mod, fill=orange!15, draw=orange!70] (alu) at (-2.4,0.4) {$\mu$-ALU\\Q16.16};
\node[mod, fill=green!15, draw=green!50!black] (part) at (0,0.4) {partition\\core};
\node[mod, fill=purple!15, draw=purple!60] (logic) at (2.4,0.4) {logic\\interface};

% External
\node[ext, fill=gray!8] (z3) at (2.4,-0.8) {Z3 SMT\\(external)};

% Connections
\draw[->, thick] (cpu) -- (alu) node[midway, left, font=\tiny] {op};
\draw[->, thick] (cpu) -- (part);
\draw[->, thick] (cpu) -- (logic) node[midway, right, font=\tiny] {cert};
\draw[->, thick, dashed] (logic) -- (z3);

% Signal labels
\draw[->, orange!70, thick] (alu) -- +(0,-0.7) node[below, font=\tiny] {$\mu$-result};
\end{tikzpicture}
\caption{Hardware module hierarchy within \texttt{thiele\_cpu\_unified.v}.}
\end{figure}

\subsection{Module Hierarchy}

The hardware mirrors the formal model: the core executes the ISA, the accounting unit enforces $\mu$-monotonicity, and the logic interface brokers certificate checks. This makes the physical design a direct embodiment of the formal step relation.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \texttt{mem\_rdata}: Memory read data---what memory returns when reading from an address.
    \end{itemize}
...
\centering
\begin{tikzpicture}[
    state/.style={draw, rounded corners=2pt, minimum width=1cm, minimum height=0.45cm, font=\tiny\bfseries, align=center, thick},
    >=Stealth,
]
% Main pipeline
\node[state, fill=blue!15] (fetch) at (0,1.5) {FETCH};
\node[state, fill=blue!15] (decode) at (1.6,1.5) {DECODE};
\node[state, fill=blue!15] (exec) at (3.2,1.5) {EXECUTE};
\node[state, fill=blue!15] (mem) at (4.8,1.5) {MEMORY};
\node[state, fill=blue!15] (comp) at (6.4,1.5) {COMPLETE};
...
% Branch states
\node[state, fill=gray!20] (alu) at (1.6,0) {ALU\\WAIT};
\node[state, fill=yellow!25] (logic) at (3.6,0) {LOGIC};
\node[state, fill=cyan!20] (py) at (5.6,0) {PYTHON};

% Branch arrows
\draw[->, thick, dashed] (exec) -- (alu);
\draw[->, thick, dashed] (exec) -- (logic);
\draw[->, thick, dashed] (exec) -- (py);
\draw[->, thick] (alu) -- (exec);
\draw[->, thick] (logic) -- (comp);
\draw[->, thick] (py) -- (comp);

% Return arrow
\draw[->, thick] (comp.north) -- +(0,0.4) -| (fetch.north);
\end{tikzpicture}
\caption{12-state FSM: classic 5-stage RISC pipeline extended with branch states for oracles and multi-cycle operations.}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{[3:0]}: 4-bit wide value (can represent $2^4 = 16$ states). The design uses 12 of the 16 possible encodings.
...
\begin{itemize}
    ...
    \item \textbf{Alternative (One-Hot)}: Could use 12 bits, one per state, only one bit set at a time. Faster transitions but uses more flip-flops. Binary encoding was chosen for compactness.
\end{itemize}
...
\textbf{Why 12 States?} Classic RISC processors (e.g., MIPS) use 5 stages (Fetch, Decode, Execute, Memory, Writeback). The Thiele CPU has additional states because:
\begin{itemize}
...
\centering
\begin{tikzpicture}[
    block/.style={draw, rounded corners=2pt, minimum height=0.5cm, font=\scriptsize, align=center, thick},
    port/.style={font=\tiny, anchor=east},
    >=Stealth,
]
% Main ALU block
\node[block, fill=orange!15, draw=orange!70, minimum width=2.4cm, minimum height=1.8cm] (alu) at (0,0) {\textbf{$\mu$-ALU}\\Q16.16};

% LOG2 LUT
\node[block, fill=cyan!15, minimum width=1.6cm] (lut) at (0,1.6) {LOG2 LUT\\{\tiny 256 entries}};
\draw[<->, thick] (alu.north) -- (lut.south);

% Inputs (left)
\draw[<-, thick] (alu.west) ++(0,0.4) -- +(-1.2,0) node[left, font=\tiny] {operand\_a [31:0]};
\draw[<-, thick] (alu.west) ++(0,0) -- +(-1.2,0) node[left, font=\tiny] {operand\_b [31:0]};
\draw[<-, thick] (alu.west) ++(0,-0.4) -- +(-1.2,0) node[left, font=\tiny] {op [2:0], valid};

% Outputs (right)
\draw[->, thick] (alu.east) ++(0,0.3) -- +(1.2,0) node[right, font=\tiny] {result [31:0]};
\draw[->, thick] (alu.east) ++(0,-0.1) -- +(1.2,0) node[right, font=\tiny] {ready};
\draw[->, thick] (alu.east) ++(0,-0.5) -- +(1.2,0) node[right, font=\tiny] {overflow};

% Operations box
\node[block, fill=yellow!15, minimum width=4.2cm, font=\tiny] (ops) at (0,-1.5) {0:ADD~~1:SUB~~2:MUL~~3:DIV~~4:LOG2~~5:INFO\_GAIN};

% Q16.16 annotation
\node[font=\tiny, anchor=west, text=orange!70] at (1.5,1.6) {$1.0 = \mathtt{0x00010000}$};
\end{tikzpicture}
\caption{$\mu$-ALU: Q16.16 fixed-point arithmetic unit with 256-entry LOG2 lookup table.}
\end{figure}

\subsection{The $\mu$-ALU}

\begin{quote}
\textit{Author's Note (Devon): This is where the magic happens, folks. The $\mu$-ALU is like the odometer on your car---except instead of miles, it tracks information. Every time you ``look'' at something, every time you reveal structure, every time you make a decision---the odometer ticks up. And just like your car's odometer, it only goes one direction. No rolling back. That's the whole game.}
\end{quote}

The $\mu$-ALU (defined as a module inside \texttt{thiele\_cpu\_unified.v}) implements Q16.16 fixed-point arithmetic:
\begin{lstlisting}
module mu_alu (
    input wire clk,
    input wire rst_n,
...
\textbf{Isomorphism Requirement:} The exact same 256 values exist in all three layers:
\begin{itemize}
...
The logic engine interface is integrated directly into the \texttt{thiele\_cpu} module within \texttt{thiele\_cpu\_unified.v}. The CPU exposes \texttt{logic\_req} and \texttt{logic\_ack} signals for external solver handshaking:
\begin{lstlisting}
module lei (
    input wire clk,
    input wire rst_n,
...
\draw[->, thick] (cmp) -- (pass);
\end{tikzpicture}
\caption{Isomorphism gate: same trace executed on all three layers, canonicalized states compared.}
\end{figure}
...
The Inquisitor (\path{scripts/inquisitor.py}, 6,700+ lines) is a heuristic static analysis tool that scans the Coq source tree for ``proof smells''---patterns that indicate incomplete, vacuous, or dishonest proofs. It goes far beyond simple pattern matching.
...
\textbf{Allowlist.} A JSON allowlist (\path{scripts/inquisitor\_allowlist.json}) documents the mathematically-justified exceptions: currently 5 entries across \texttt{MinimalE.v}, \texttt{Composition.v}, and \texttt{AssumptionBundle.v}, each with a written justification for why the finding is acceptable.

The CI gate (\path{scripts/check\_inquisitor\_clean.sh}) enforces a single rule: \textbf{zero HIGH findings}. If a single HIGH finding exists anywhere in the Coq tree, CI fails. No exceptions, no overrides.
...
\begin{lstlisting}
$ yosys -p "read_verilog thiele_cpu_kami.v; synth_ecp5 -top thiele_cpu"
\end{lstlisting}
...
vvp thiele_cpu_tb
    ...
    \begin{verbatim}
yosys -p "read_verilog thiele_cpu_kami.v; synth_ecp5 -top thiele_cpu; write_json netlist.json"
    \end{verbatim}
    \begin{itemize}
...
\end{tikzpicture}
\caption{Chapter 4 summary: three independent implementations bound by the isomorphism invariant.}
\end{figure}
...
The binding across layers is not aspirational---it's enforced through automated isomorphism gates. The Inquisitor ensures no admits, documented axioms only, and no semantic divergences ever hit the main branch.
