...
In the codebase, these ingredients are implemented by receipt parsing and signature checks, trace replays in the domain-specific checkers, and explicit $\mu$-cost rules inside the C-modules themselves. The verifier modules were originally prototyped in Python (now archived under \path{archive/verifier_unused/}); the formal protocol semantics are defined in the Coq kernel.
...
\end{itemize}
Each module corresponds to a concrete verifier implementation (now archived under \path{archive/verifier_unused/}; for example, \texttt{c\_randomness.py}, \texttt{c\_tomography.py}, \texttt{c\_entropy2.py}, and \texttt{c\_causal.py}). This makes the certification rules auditable and runnable, not just conceptual.
...
\begin{itemize}
    ...
    \item \textbf{"files": [...]} — The \textbf{manifest} of artifacts. Each entry contains:
    \begin{itemize}
    ...
    \item \textbf{"public\_key": "..."} — The public key needed to verify the signature, embedded for self-contained verification (though it must be trusted via a separate channel or manifest).
...
\begin{enumerate}
    \item Recomputes SHA-256 hashes of all referenced files.
    \item Checks that recomputed hashes match those in the \texttt{files} list.
    \item Recomputes the \texttt{global\_digest} from the file list.
    \item Verifies the EdDSA signature using the provided public key.
    \item Parses \texttt{claim.json} (if present) to extract the scientific claim.
\end{enumerate}
...
The reference implementation for TRS-1.0 verification lives in \path{archive/verifier_unused/receipt_protocol.py} and the protocol semantics are specified in the Coq kernel. This ensures that the protocol described here is backed by a concrete parser and validator.
...
\textbf{Example disclosure:} "The randomness source is a quantum vacuum fluctuation detector with 0.95 bits/photon..." This disclosure is stored in \texttt{nonlocality.json} and \textit{costs} $\mu$ because it reveals structural facts.

\textbf{Without disclosure:} If you claim $H_{\min} = 0.95$ but provide no disclosure (or insufficient \texttt{disclosure\_bits} in \texttt{nonlocality.json}), the verifier \textbf{rejects} the claim. Why? Because you could be lying---using a PRNG and claiming it's quantum randomness. No Free Insight forbids this.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{r\_op r} — Extracts the opcode from receipt $r$ (e.g., \texttt{RAND\_TRIAL\_OP = 1001}).
...
\begin{itemize}
    \item The partition must be \textbf{receipted} (included in the TRS manifest as \texttt{coarse\_graining.json}).
...
\textbf{Disclosure requirement:} The verifier checks that \texttt{coarse\_graining.json} appears in the receipt and charges:
\[
...
\subsection{The Markov Equivalence Problem}
...
\begin{itemize}
    ...
    \item \textbf{Assumptions:} Explicit causal assumptions (e.g., ``We assume $A$ and $C$ do not share hidden confounders''). These assumptions are \textit{structural information} that must be disclosed in \texttt{assumptions.json}.
\end{itemize}
...
C-CAUSAL verifier:
\begin{enumerate}
    \item Loads \texttt{causal.receipt.json} (which must list \texttt{samples.csv} and \texttt{assumptions.json}).
    \item Checks \texttt{assumptions.json} for interventions or assumptions. Not found.
...
\begin{enumerate}
    ...
    \item Add assumptions (e.g., ``We assume temporal ordering: $A$ precedes $B$ precedes $C$'') $\to$ disclose in \texttt{assumptions.json}, costs $\mu = 8192$ bits.
\end{enumerate}
