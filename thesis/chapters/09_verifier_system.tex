...
\begin{enumerate}
    \item \textbf{PNEW} creates a module with region $[0, 256]$, charging $\mu = 1$.

    \item \textbf{LASSERT} attempts to add a CNF formula to the module without supplying a SAT certificate. The kernel requires a valid satisfying assignment or LRAT proof to accept the assertion.

    \item \textbf{assert result.vm\_err is True} --- The VM rejects the uncertified assertion. The $\mu$-cost was still charged (assert $\mu \ge 2$), but the structural claim is not accepted into the partition graph.
\end{enumerate}
