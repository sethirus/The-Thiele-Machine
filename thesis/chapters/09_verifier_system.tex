...
\begin{quote}
\textit{Author's Note (Devon): This is the chapter where the question ``how do I know you're not lying?'' gets a real answer. Not ``trust me.'' Not ``I ran the tests.'' A receipt. Every claim the machine makes comes with a cryptographic paper trail that anyone can replay from scratch. You do not need to trust me. You need to trust hash functions and the Coq kernel, and both of those are open source. That is a different kind of claim than ``I promise.''}
\end{quote}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{"sha256"} — The SHA-256 hash of the file content. Used as the integrity check value.
    \end{itemize}
...
The active implementation lives in \path{scripts/create_receipt.py}, \path{scripts/create_execution_receipt.py}, \path{tools/verify_trs10.py}, and \path{thielecpu/receipt.py}.
...
\textbf{What is this test?} This is the \textbf{flagship falsifier} for certificate enforcement: an \texttt{LASSERT} instruction issued \textbf{without providing the required SAT dual witness package} is detected and rejected by the Coq-extracted OCaml runner. The VM sets \texttt{vm\_err = True} because certification now requires both a satisfying assignment and a falsifying assignment.

\textbf{Test structure:}
\begin{enumerate}
    \item \textbf{LASSERT} attempts to assert a CNF formula without supplying the SAT model-plus-countermodel package. The formula is referenced via register-indexed \texttt{vm\_mem} addressing (on-chip model); no external solver is invoked.
