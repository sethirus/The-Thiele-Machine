...
In the codebase, these ingredients are implemented by receipt parsing and signature checks (\path{verifier/receipt_protocol.py}), trace replays in the domain-specific checkers (for example \path{verifier/c_randomness.py}), and explicit $\mu$-cost rules inside the C-modules themselves.
...
The reference implementation for TRS-1.0 verification lives in \path{verifier/receipt_protocol.py} and the conformance tests in \path{tests/trs_conformance/test_trs10.py}. This ensures that the protocol described here is backed by a concrete parser and validator.
