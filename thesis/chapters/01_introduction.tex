...
\node[box, fill=gray!20] (turing) at (-4, 2) {Turing\\Machine};
\node[box, fill=gray!20] (ram) at (-4, 0) {RAM\\Model};
\node[box, fill=gray!20] (search) at (-4, -2) {Blind\\Search};

% Right side: Thiele (Sighted)
\node[box, fill=green!20] (thiele) at (4, 0) {Thiele\\Machine};

% Center: The transformation
\node[box, fill=yellow!30, minimum width=3cm] (mu) at (0, 0) {$\mu$-bit\\Accounting};

% Arrows
\draw[arrow] (turing) -- (mu) node[midway, above, label] {structure cost};
\draw[arrow] (ram) -- (mu) node[midway, above, label] {};
\draw[arrow] (search) -- (mu) node[midway, below, label] {insight tax};
\draw[arrow] (mu) -- (thiele) node[midway, above, label] {explicit structure};

% Labels
\node[below=0.3cm of turing, label] {$O(2^n)$ worst case};
\node[below=0.3cm of thiele, label] {$O(k \cdot 2^{n/k})$ with structure};

% Title
\node[above=0.8cm of mu, font=\bfseries] {The Paradigm Shift};

% Annotations
\draw[dashed, gray] (-6, -3.5) -- (-6, 3.5) -- (-2, 3.5) -- (-2, -3.5) -- cycle;
\node[above] at (-4, 3.5) {\textbf{Classical Models}};

\draw[dashed, gray] (2, -3.5) -- (2, 3.5) -- (6, 3.5) -- (6, -3.5) -- cycle;
\node[above] at (4, 3.5) {\textbf{Thiele Machine}};
...
% Axes
\draw[->, thick] (0,0) -- (10,0) node[right] {Problem Size $n$};
\draw[->, thick] (0,0) -- (0,6) node[above] {Time};

% Exponential curve (blind search)
\draw[thick, red, domain=0:4.5, samples=100] 
    plot (\x*2, {0.1*exp(\x)}) node[right] {$O(2^n)$ \textit{Blind}};

% Linear/polynomial curve (structured search)  
\draw[thick, green!60!black, domain=0:9, samples=100]
    plot (\x, {0.3*\x + 0.1}) node[right] {$O(k \cdot 2^{n/k})$ \textit{Sighted}};

% Annotation
\draw[<->, dashed, blue, thick] (6, 0.9) -- (6, 4.5);
\node[blue, right] at (6.2, 2.7) {\textbf{Time Tax}};

% Grid lines
\draw[gray, dashed, very thin] (0,1) -- (9,1);
\draw[gray, dashed, very thin] (0,2) -- (9,2);
\draw[gray, dashed, very thin] (0,3) -- (9,3);
\draw[gray, dashed, very thin] (0,4) -- (9,4);
\draw[gray, dashed, very thin] (0,5) -- (9,5);
...
\begin{tikzpicture}[
    scale=0.85,
    transform shape,
    component/.style={rectangle, draw, rounded corners, minimum width=2.2cm, minimum height=1cm, align=center, thick},
    arrow/.style={->, thick, >=stealth},
    label/.style={font=\small}
]

% State space S
\node[component, fill=blue!15] (S) at (-4, 2) {$S$\\State Space};

% Partition space Π  
\node[component, fill=green!15] (Pi) at (-4, 0) {$\Pi$\\Partitions};

% Axiom set A
\node[component, fill=yellow!15] (A) at (0, 2) {$A$\\Axioms};

% Transition rules R
\node[component, fill=orange!15] (R) at (4, 2) {$R$\\Transitions};

% Logic Engine L
\node[component, fill=purple!15] (L) at (4, 0) {$L$\\Logic Engine};

% μ-Ledger (central)
\node[component, fill=red!20, minimum width=2.5cm, minimum height=1.2cm] (mu) at (0, 0) {$\mu$-Ledger\\(monotonic)};

% Arrows showing relationships
\draw[arrow] (S) -- (Pi) node[midway, left, label] {decompose};
\draw[arrow] (Pi) -- (mu) node[midway, above, label] {track};
\draw[arrow] (A) -- (mu) node[midway, right, label] {charge};
\draw[arrow] (R) -- (mu) node[midway, above, label] {update};
\draw[arrow] (L) -- (A) node[midway, right, label] {verify};
\draw[arrow, dashed] (S) -- (A) node[midway, above, label] {constrain};
\draw[arrow, dashed] (R) -- (S) node[midway, above, label] {transform};

% Bounding box
\draw[thick, dashed, gray, rounded corners] (-5.5, -1.2) rectangle (5.5, 3.2);
\node[above] at (0, 3.2) {\textbf{Thiele Machine} $T = (S, \Pi, A, R, L)$};

% Key insight annotation
\node[below=0.3cm of mu, label, align=center] {
    \textit{Every structural operation}\\
    \textit{increments $\mu$}
};
...
% Arrow with μ cost
\draw[->, ultra thick, red!70!black] (3, 0) -- (5, 0);
\node[above, font=\bfseries] at (4, 0.3) {$\Delta\mu$ bits};
\node[below, font=\small\itshape] at (4, -0.3) {(structural cost)};

% Right: Smaller search space Ω'
\draw[thick, fill=green!20] (7.5, 0) circle (1.2cm);
...
% The key equation
\node[draw, thick, fill=yellow!20, rounded corners, align=center] at (4, -5.5) {
...
\begin{tikzpicture}[
    scale=0.9,
    transform shape,
    layer/.style={rectangle, draw, rounded corners, minimum width=4cm, minimum height=1.5cm, align=center, thick},
    arrow/.style={<->, thick, >=stealth, blue},
    label/.style={font=\small\itshape}
]

% Layer 1: Coq
\node[layer, fill=purple!20] (coq) at (0, 4) {
    \textbf{Layer 1: Coq}\\
...
% Layer 2: Python
\node[layer, fill=blue!20] (python) at (0, 2) {
...
% Layer 3: Hardware
\node[layer, fill=green!20] (rtl) at (0, 0) {
...
\draw[arrow] (coq.south) -- (python.north) 
    node[midway, right, label] {Bisimulation};
\draw[arrow] (python.south) -- (rtl.north)
    node[midway, right, label] {Bisimulation};

% Properties on the left
\node[left=2cm of coq, align=right] (p1) {
    \textbf{Properties:}\\
    $\mu$-monotonicity\\
    No Free Insight\\
    CHSH bounds
...
\node[left=2cm of python, align=right] (p2) {
    \textbf{Properties:}\\
    Same $\mu$-ledger\\
...
\node[left=2cm of rtl, align=right] (p3) {
    \textbf{Properties:}\\
    Same gate-level $\mu$\\
...
% Title
\node[above=0.5cm of coq, font=\bfseries\large] {3-Layer Isomorphism Architecture};

% Right side: verification flow
\node[right=2cm of coq, align=left] (v1) {
    \textbf{Verification:}\\
    Coq type-checker\\
    Zero admits
};

\node[right=2cm of python, align=left] (v2) {
    \textbf{Verification:}\\
    pytest suite\\
    1,115 tests
};

\node[right=2cm of rtl, align=left] (v3) {
    \textbf{Verification:}\\
    Icarus Verilog\\
    Yosys synthesis
};

\draw[->, dashed, gray] (coq.east) -- (v1.west);
\draw[->, dashed, gray] (python.east) -- (v2.west);
\draw[->, dashed, gray] (rtl.east) -- (v3.west);

\end{tikzpicture}
\caption{The 3-layer isomorphism architecture. Each layer implements identical semantics with the same $\mu$-ledger accounting. Bisimulation proofs ensure that properties proven in Coq transfer to Python and hardware. The right side shows verification tooling at each level.}
