...
\begin{enumerate}
    \item \textbf{Formally proven}: Machine-checked proofs in the Coq proof assistant (over 2,300 theorem-like statements across the active proof corpus, totaling {$\sim$}80,000 lines)
    \item \textbf{Implemented}: Working code in Python (20,786 lines) and synthesizable Verilog RTL (unified design, 4,346 hand-written lines across 14 files; 265,695 total including synthesis output)
    \item \textbf{Tested}: 828 automated tests across 82 test files verify that theory and implementation match
...
The three proven principles are: (i) $\mu$-monotonicity (\path{coq/kernel/MuLedgerConservation.v}), (ii) revelation requirements for strengthening (\path{coq/kernel/NoFreeInsight.v}), and (iii) observational locality (\path{coq/kernel/ObserverDerivation.v}). These ensure that insight is never free---it must be paid for in $\mu$-cost.
...
Implementation: \path{coq/kernel/VMState.v} and \path{coq/kernel/VMStep.v} (kernel), \path{coq/kernel/KernelPhysics.v} and \path{coq/kernel/KernelNoether.v} (physics), \path{coq/kernel/RevelationRequirement.v} (CHSH).

\textbf{The Inquisitor Standard:} The project enforces a zero-tolerance policy for incomplete proofs. No \texttt{Admitted}. No \texttt{admit} tactics. Zero custom axioms---all proofs depend only on Coq's standard library. The \path{scripts/inquisitor.py} tool scans every Coq file and blocks commits that contain \texttt{Admitted} or \texttt{admit}. If a theorem says ``Proven,'' it's actually proven.
...
Implementation: \path{thielecpu/state.py} (state), \path{thielecpu/vm.py} (engine), \path{thielecpu/crypto.py} (signing).
...
The RTL runs on Icarus Verilog simulation with Yosys-compatible synthesis defines (\texttt{YOSYS\_LITE}). Vivado synthesis scripts target Xilinx FPGAs when the toolchain is available.
...
\begin{enumerate}
    ...
    \item \textbf{Oracle Impossibility and Turing Subsumption:} Mechanized proof that no finite oracle resolves all $\mu$-bounded halting questions (\path{coq/kernel/OracleImpossibility.v}), plus a direct Minsky-machine embedding proving Thiele subsumes Turing (\path{coq/modular_proofs/TM_to_Minsky.v}).
    ...
    \item \textbf{Empirical Artifacts:} Reproducible demos including certified randomness, polynomial-time structured Tseitin solutions, Verilog co-simulation test suite, and cross-layer comprehensive verification.
\end{enumerate}
...
\textbf{Appendix: Complete Theorem Index} provides a comprehensive catalog of all theorem-containing files with their key results.
