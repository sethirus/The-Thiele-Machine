...
\begin{tikzpicture}[
    scale=1.0,
    node distance=3.0cm,
    box/.style={rectangle, draw=black, line width=0.8pt=black, line width=0.8pt, rounded corners=3pt, 
                minimum width=5.5cm, minimum height=2.2cm, align=center, 
                inner sep=0.4cm, inner sep=0.4cm},
    arrow/.style={->, line width=1.2pt, >=Stealth},
    label/.style={font=\small, text=black!70}
]

% Left side: Classical (Blind) - vertically aligned
\node[box, fill=gray!15] (turing) at (-6, 3) {
    \textbf{Turing Machine}
};
\node[box, fill=gray!15] (ram) at (-6, 0) {
    \textbf{RAM Model}
};
\node[box, fill=gray!15] (search) at (-6, -3) {
    \textbf{Blind Search}
};

% Center: The transformation
\node[box, fill=yellow!15, minimum width=6.0cm] (mu) at (0, 0) {
    \textbf{$\mu$-bit Accounting}\\[0.2cm]
    \small Information Cost Tracking
};

% Right side: Thiele (Sighted)
\node[box, fill=green!15, minimum height=3.0cm] (thiele) at (6, 0) {
    \textbf{Thiele Machine}\\[0.2cm]
    \small Structure-Aware\\Computing
};

% Arrows with labels
\draw[arrow] (turing.east) -- (mu.west |- turing.east) 
    node[midway, above, label] {structure cost};
\draw[arrow] (ram.east) -- (mu.west);
\draw[arrow] (search.east) -- (mu.west |- search.east) 
    node[midway, below, label] {insight tax};
\draw[arrow] (mu.east) -- (thiele.west) 
    node[midway, above, label] {explicit structure};

% Complexity annotations
\node[below=0.8cm of turing, font=\small\itshape, text=black!60] 
    {$O(2^n)$ worst case};
\node[below=0.8cm of thiele, font=\small\itshape, text=black!60] 
    {$O(k \cdot 2^{n/k})$ with structure};

% Title
\node[above=1.2cm of mu, font=\Large\bfseries] {The Paradigm Shift};

% Group annotations with proper boundaries
\draw[dashed, line width=0.6pt, gray!60, rounded corners=5pt] 
    (-8.5, -4.5) rectangle (-3.5, 4.5);
\node[above=0.1cm] at (-6, 4.5) {\textbf{Classical Models}};

\draw[dashed, line width=0.6pt, gray!60, rounded corners=5pt] 
    (3.5, -4.5) rectangle (8.5, 4.5);
\node[above=0.1cm] at (6, 4.5) {\textbf{Thiele Machine}};
...
\begin{tikzpicture}[
    scale=1.1,
    axis/.style={->, line width=1.2pt},
    curve/.style={line width=1.5pt, smooth},
    grid/.style={gray!40, dashed, line width=0.3pt}
]
...
% Axes with labels
\draw[axis] (-0.3,0) -- (10.5,0) node[right, font=\small\bfseries] {Problem Size $n$};
\draw[axis] (0,-0.3) -- (0,6.2) node[above, font=\small\bfseries] {Time (log scale)};

% Exponential curve (blind search) - RED
\draw[curve, red!70!black, domain=0:4.5, samples=100] 
    plot (\x*2, {0.1*exp(\x)});
\node[red!70!black, font=\small\bfseries, right] at (9.2, 5.2) {$O(2^n)$ Blind};

% Linear curve (structured search) - GREEN
\draw[curve, green!50!black, domain=0:9.5, samples=100]
    plot (\x, {0.3*\x + 0.1});
\node[green!50!black, font=\small\bfseries, right] at (9.2, 3.1) {$O(k \cdot 2^{n/k})$ Sighted};

% Time Tax annotation - BLUE
\draw[<->, blue!70, line width=1.5pt] (6.1, 0.9) -- (6.1, 4.5);
\node[blue!70, font=\small\bfseries, fill=white, inner sep=2pt, rounded corners=3pt] 
    at (7.5, 2.7) {Time Tax};
...
\node[left, font=\footnotesize] at (-0.2, 1) {$10^2$};
\node[left, font=\footnotesize] at (-0.2, 2) {$10^4$};
\node[left, font=\footnotesize] at (-0.2, 3) {$10^6$};
\node[left, font=\footnotesize] at (-0.2, 4) {$10^8$};
\node[left, font=\footnotesize] at (-0.2, 5) {$10^{10}$};
...
\begin{tikzpicture}[
    scale=1.05,
    node distance=3.0cm,
    component/.style={rectangle, draw, rounded corners=3pt, minimum width=4.5cm, minimum height=2.0cm, align=center, line width=1.2pt, font=
ormalsize},
    arrow/.style={->, line width=1.2pt, >=Stealth},
...
% State space S
\node[component, fill=blue!15, align=center, text width=4.5cm, font=
ormalsize] (S) at (-4, 2) {$S$\\State Space};

% Partition space Π  
\node[component, fill=green!15, align=center, text width=4.5cm, font=
ormalsize] (Pi) at (-4, 0) {$\Pi$\\Partitions};

% Axiom set A
\node[component, fill=yellow!15, align=center, text width=4.5cm, font=
ormalsize] (A) at (0, 2) {$A$\\Axioms};

% Transition rules R
\node[component, fill=orange!15, align=center, text width=4.5cm, font=
ormalsize] (R) at (4, 2) {$R$\\Transitions};

% Logic Engine L
\node[component, fill=purple!15, align=center, text width=4.5cm, font=
ormalsize] (L) at (4, 0) {$L$\\Logic Engine};

% μ-Ledger (central)
\node[component, fill=red!12, minimum width=5.5cm, minimum height=2.2cm, align=center, text width=4.5cm, font=
ormalsize] (mu) at (0, 0) {$\mu$-Ledger\\(monotonic)};

% Arrows showing relationships
\draw[arrow] (S) -- (Pi) node[pos=0.5, left, xshift=-3pt, font=\small] {decompose};
\draw[arrow] (Pi) -- (mu) node[pos=0.5, above, yshift=3pt, font=\small] {track};
\draw[arrow] (A) -- (mu) node[pos=0.5, right, xshift=3pt, font=\small] {charge};
\draw[arrow] (R) -- (mu) node[pos=0.5, above, yshift=3pt, font=\small] {update};
\draw[arrow] (L) -- (A) node[pos=0.5, right, xshift=3pt, font=\small] {verify};
\draw[arrow, dashed] (S) -- (A) node[pos=0.5, above, yshift=3pt, font=\small] {constrain};
\draw[arrow, dashed] (R) -- (S) node[pos=0.5, above, yshift=3pt, font=\small] {transform};

% Bounding box
\draw[line width=1.2pt, dashed, gray, rounded corners=3pt] (-5.5, -1.2) rectangle (5.5, 3.2);
...
\draw[decorate, decoration={brace, amplitude=10pt, mirror}] (-2.5, -4) -- (2.5, -4);
\node[below] at (0, -4.5) {$\log_2(\Omega)$ bits of uncertainty};

\draw[decorate, decoration={brace, amplitude=8pt, mirror}] (6.3, -4) -- (8.7, -4);
\node[below] at (7.5, -4.5) {$\log_2(\Omega')$ bits};

% The key equation
\node[draw, line width=1.2pt, fill=yellow!15, rounded corners=3pt, align=center, text width=4.5cm] at (4, -5.5) {
...
\begin{tikzpicture}[
    scale=1.0,
    node distance=3.0cm,
    layer/.style={rectangle, draw, rounded corners=3pt, minimum width=7.2cm, minimum height=2.6cm, align=center, thick},
    arrow/.style={<->, line width=1.2pt, >=Stealth, blue},
    every node/.style={font=
ormalsize}
]

% Layer 1: Coq
\node[layer, fill=purple!20, align=center, text width=4.5cm] (coq) at (0, 4) {
    \textbf{Layer 1: Coq}\\
...
% Layer 2: Python
\node[layer, fill=blue!12, align=center, text width=4.5cm] (python) at (0, 2) {
...
% Layer 3: Hardware
\node[layer, fill=green!12, align=center, text width=4.5cm] (rtl) at (0, 0) {
...
\draw[arrow] (coq.south) -- (python.north) 
    node[pos=0.5, right, xshift=10pt, font=\small] {Bisimulation};
\draw[arrow] (python.south) -- (rtl.north)
    node[pos=0.5, right, xshift=10pt, font=\small] {Bisimulation};

% Properties on the left
\node[left=3.9cm of coq, align=right, align=center, text width=4.5cm] (p1) {
    \textbf{Properties:}\\
    $\mu$-monotonicity\\
    No Free Insight\\
    CHSH bounds
...
\node[left=3.9cm of python, align=right, align=center, text width=4.5cm] (p2) {
    \textbf{Properties:}\\
    Same $\mu$-ledger\\
...
\node[left=3.9cm of rtl, align=right, align=center, text width=4.5cm] (p3) {
    \textbf{Properties:}\\
    Same gate-level $\mu$\\
...
\draw[->, dashed, gray] (p1.east) -- (coq.west);
\draw[->, dashed, gray] (p2.east) -- (python.west);
\draw[->, dashed, gray] (p3.east) -- (rtl.west);

% Bottom annotation
\node[below=2.9cm of rtl, align=center,  text width=4.5cm, font=\small, yshift=-6pt] {
