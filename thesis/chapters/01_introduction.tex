...
This document presents the Thiele Machine, a formal model of computation with explicit structural cost accounting. The implementation includes machine-verified proofs in Coq, a working virtual machine, and synthesizable hardware---all implementing the same computational model, all provably isomorphic.

\subsection{Scope and Claims Boundary}

This thesis makes claims at three levels:
...
The \textit{Thiele Machine} is a model of computation where \textbf{structural information costs something}.
...
\begin{itemize}
    \item \textbf{Problem}: Computers can be slow on some problems (years to solve) and fast on others (milliseconds). Why?
    ...
    \item \textbf{The Contribution}: This thesis presents a computer model where structural knowledge is explicit, measurable, and costly. This formalizes \textit{why} some problems are hard and how that hardness can be characterized.
\end{itemize}

\subsection{What Makes This Work Different}

Every major claim is:
\begin{enumerate}
    \item \textbf{Formally proven}: Machine-checked proofs in the Coq proof assistant (1,636 theorems and lemmas across 260 files, totaling 55,098 lines)
    \item \textbf{Implemented}: Working code in Python (19,516 lines) and Verilog hardware description (43 files)
    \item \textbf{Tested}: Automated tests verify that theory and implementation match
    \item \textbf{Falsifiable}: The thesis specifies what would disprove each claim
\end{enumerate}

Every claim has a concrete falsification condition. If you find a counterexample, the Coq proof won't compile. The Python VM emits signed receipts. The RTL testbench produces JSON snapshots. All three are compared automatically. The claims are bound to executable evidence.
...
If you want to see structure, what do you pay? That's what $\mu$-bits measure. The model charges explicitly for operations that add or refine structure. The proven result: you can't strengthen predicates for free. $\mu > 0$, always. The Coq proofs verify this. 
...
\textbf{The Inquisitor Standard:} The project enforces a zero-tolerance policy. No \texttt{Admitted}. No \texttt{admit} tactics. No \texttt{Axiom} declarations. The \path{scripts/inquisitor.py} tool scans every Coq file and blocks commits that violate this. If a theorem says ``Proven,'' it's actually proven.
...
Every claim is falsifiable and open to scrutiny.
...
\begin{itemize}
    ...
    \item \textbf{Chapter 10: Extended Proof Architecture} covers the full 260-file Coq development (1,636 theorems, 55,098 lines) including the ThieleMachine proofs, Theory of Everything results, and impossibility theorems.
