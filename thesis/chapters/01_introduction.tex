...
\begin{enumerate}
    \item \textbf{Formally proven}: Machine-checked proofs in the Coq proof assistant (2,012 theorems and lemmas across 268 files, totaling 57,744 lines)
    \item \textbf{Implemented}: Working code in Python (19,174 lines) and Verilog hardware description (46 files)
...
\textbf{The Inquisitor Standard:} The project enforces a zero-tolerance policy for incomplete proofs. No \texttt{Admitted}. No \texttt{admit} tactics. External axioms (5 total, primarily structural parameters and one logic axiom) are documented and identified. The \path{scripts/inquisitor.py} tool scans every Coq file and blocks commits that contain \texttt{Admitted} or \texttt{admit}. If a theorem says ``Proven,'' it's actually proven.
