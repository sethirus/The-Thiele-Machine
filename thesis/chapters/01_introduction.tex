...
\textbf{Role in thesis:} This diagram motivates the central question: \textit{What is the cost of sight?} If we can pay some resource ($\mu$) to gain structural knowledge, how much must we pay for a given speedup? The thesis answers: Coq proves $\Delta\mu \ge |\phi|_{\text{bits}}$ (description length), and the Python VM enforces $\Delta\mu \ge \log_2(\Omega) - \log_2(\Omega')$ by computing actual state space reduction.
...
\begin{quote}
    \textit{Any reduction in search space must be paid for by proportional investment of structural information ($\mu$-bits). Computational time can be traded for $\mu$-cost, but there is no free insight: Coq proves $\Delta\mu \ge |\phi|_{\text{bits}}$, and the VM enforces $\log|\Omega| - \log|\Omega'| \le \Delta\mu$ by construction.}
\end{quote}
...
\end{tikzpicture}
\caption{The No Free Insight theorem visualized. Reducing the search space from $\Omega$ to $\Omega'$ requires paying $\mu$-cost. \textbf{Proven in Coq:} $\Delta\mu \ge |\phi|_{\text{bits}}$ for any formula $\phi$ (StateSpaceCounting.v). \textbf{Enforced by VM:} The Python VM uses a conservative bound, charging $\mu = |\phi| + n$ (assumes single solution). This \emph{guarantees} $\Delta\mu \ge \log_2(|\Omega|) - \log_2(|\Omega'|)$ without \#P-complete model counting; may overcharge when multiple solutions exist.}
...
\begin{itemize}
    ...
    \item \textbf{Yellow box (bottom):} The conservation law equation: $\Delta\mu \ge \log_2(\Omega) - \log_2(\Omega')$. This is enforced by the VM computing semantic state space reduction.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Layer 1: Coq (purple, top):} Machine-checked proofs---206 files, 1400+ theorems. This is the \textit{mathematical ground truth}. Properties proven: $\mu$-monotonicity (ledger never decreases), No Free Insight (strengthening requires $\mu > 0$), CHSH bounds (quantum correlations require revelation events).
...
\begin{enumerate}
    ...
    \item \textbf{The No Free Insight Theorem:} A mechanically verified proof that predicate strengthening requires structure-adding operations charging $\mu \ge |\phi|_{\text{bits}}$ for any formula $\phi$, establishing a quantitative conservation law for computational insight. The Python VM \emph{guarantees} $\Delta\mu \ge \log_2(|\Omega|) - \log_2(|\Omega'|)$ using a conservative bound ($\text{before} = 2^{n}$, $\text{after} = 1$).
