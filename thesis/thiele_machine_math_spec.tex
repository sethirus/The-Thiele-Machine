...
\begin{itemize}
    \item $\texttt{LASSERT}(\mathit{freg}, \mathit{creg}, \mathit{kind}, \mathit{flen}, \mathit{cost})$: Assert a formula read from \texttt{vm\_mem}[\texttt{R[freg]}] (length $\mathit{flen}$ bytes) against a certificate at \texttt{vm\_mem}[\texttt{R[creg]}]; $\Delta\mu = \mathit{flen} \times 8 + S(\mathit{cost}) \geq 1$.
...
\subsection{Categorical Morphism Operations (Phase 7)}
...
The active implementation exposes a 47-opcode ISA (40 original + 7 categorical morphism opcodes added in Phase 7). This condensed mathematical specification presents the core instruction families relevant to the kernel semantics and cost model. Every instruction carries an explicit $\Delta\mu$ parameter; the function $\texttt{instruction\_cost}$ extracts it.
...
\textbf{LASSERT}: Read formula from \texttt{vm\_mem}[\texttt{R[formula\_addr\_reg]}] (length \texttt{formula\_len} bytes) and certificate from \texttt{vm\_mem}[\texttt{R[cert\_addr\_reg]}]. Verify using \texttt{cert\_kind} (false = LRAT/UNSAT, true = SAT model). If valid, append the formula to $A_m$; if invalid, set $\mathit{err} \gets \mathrm{true}$. Cost: $\mathtt{formula\_len} \times 8 + S(\mathtt{mu\_delta})$.
...
\begin{theorem}[No Free Lunch {\normalfont (NoFreeLunch.v --- archived)}]
...
\begin{theorem}[Abstract Simulation {\normalfont (Embedding\_TM.v --- archived, modular\_proofs/TM\_to\_Minsky.v --- archived)}]
...
\begin{theorem}[Semantic Strictness {\normalfont (ThieleFoundations.v --- archived)}]
...
\begin{theorem}[Diagonal Argument {\normalfont (kernel/OracleImpossibility.v --- archived)}]
...
\section{Confluence}
\begin{theorem}[Church--Rosser {\normalfont (Confluence.v --- archived)}]
...
\begin{theorem}[Module Independence {\normalfont (ThieleUnificationTensor.v --- archived)}]
...
\begin{theorem}[Amortization {\normalfont (AmortizedAnalysis.v --- archived)}]
...
\begin{theorem}[Noether's Theorem {\normalfont (KernelNoether.v, RepresentationTheorem.v --- archived)}]
\leavevmode
\begin{enumerate}
...
\begin{theorem}[1D Is Insufficient {\normalfont (TwoDimensionalNecessity.v --- archived)}]
...
\begin{theorem}[Rotation Group {\normalfont (ComplexNecessity.v --- archived)}]
...
\begin{theorem}[Born Rule from Tensor Consistency {\normalfont (BornRuleFromSymmetry.v --- archived/superseded; active derivation in \texttt{kernel/BornRuleLinearity.v})}]
...
\begin{theorem}[No-Cloning from $\mu$-Monotonicity {\normalfont (NoCloningFromMuMonotonicity.v --- archived/superseded; active proof in \texttt{kernel/NoCloning.v})}]
...
\begin{theorem}[REVEAL Is Irreversible {\normalfont (ObservationIrreversibility.v --- archived)}]
...
\begin{remark}[Epistemological Status: \textbf{(C)} Conditional Derivation---Circularity Resolved]
The original Coq proof (\texttt{TsirelsonDerivation.v} --- archived/superseded) encoded $2\sqrt{2}$ in the definition of $\mu_{\mathrm{corr}}$, making its derivation status \textbf{(R)}.
...
\begin{theorem}[Emergent Schr\"odinger {\normalfont (EmergentSchrodinger.v --- archived)}]
...
\begin{theorem}[Planck Consistency Relation {\normalfont (PlanckDerivation.v --- archived)} --- \textbf{(R)}]
...
\begin{enumerate}
    ...
    \item The resulting geometry satisfies Einstein's field equations
    \item This emergence is necessary, not contingent on physical assumptions
\end{enumerate}

The result does \emph{not} claim that physical spacetime \emph{is} implemented by a Turing machine---that would be a \textbf{(C)} conditional claim requiring empirical validation. Rather, it demonstrates that \emph{if} a computational substrate exists with $\mu$-cost tracking, \emph{then} Einstein's equations necessarily emerge.
\end{remark}

\begin{corollary}[Spacetime is Emergent]
Spacetime geometry is not fundamental physics but an emergent property of information processing.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:einstein_main} constructs spacetime (metric, curvature, field equations) from computational primitives (VM states, operations, $\mu$-costs) with no physical axioms. Therefore, spacetime is a derived concept, not a foundational one.
\end{proof}
...
\begin{theorem}[Dissipative Model {\normalfont (DissipativeEmbedding.v --- archived)}]
...
\begin{theorem}[Reversible Physics {\normalfont (PhysicsEmbedding.v --- archived, WaveEmbedding.v --- archived)}]
...
\begin{theorem}[Logic Is Physics {\normalfont (LogicToPhysics.v --- archived)}]
...
\section{The Morphism Layer (Phase 7 Extension)}

Phase 7 of the implementation adds a concrete category directly to the machine state. This is not an abstract analogy — it is an executable layer of 7 opcodes (0x27--0x2D) with formally verified laws.
...
\begin{theorem}[Optimal Quartet {\normalfont (ArchTheorem.v --- archived)}]
...
Einstein Equations (General) & \texttt{kernel/EinsteinEquations4D.v} \\
