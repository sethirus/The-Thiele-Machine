...
The complete state is a 10-tuple:
\[ S = (G, C, R, \mathit{Mem}, \mathit{PC}, \mu, T_\mu, \mathit{err}, \ell, m) \]
where $G \in \Gspace$ is the partition graph, $C \in \N^4$ comprises the CSRs (certificate address, status, error code, heap base), $R \in \N^{32}$ is the register file, $\mathit{Mem} \in \N^{4096}$ is main memory, $\mathit{PC} \in \N$ is the program counter, $\mu \in \N$ is the $\mu$-ledger, $T_\mu \in \N^{16}$ is the flattened $4\times 4$ $\mu$-tensor (row-major), $\mathit{err} \in \B$ is the global error flag (latching), $\ell \in \N$ is the logic-engine accumulator, and $m \in \N$ is the machine status register ($0$~=~Turing mode, $1$~=~Thiele mode).
\end{definition}
...
The full ISA comprises 31 instructions (5~structural + 4~logical + 5~reversible ALU + 9~standard ALU/control + 8~auxiliary). Every instruction carries an explicit $\Delta\mu$ parameter; the function $\texttt{instruction\_cost}$ extracts it.
...
The previous sections treated spacetime as a 1D projection via the $\mu$-metric. We now extend to full 4D geometry with discrete differential structure, proving that Einstein's field equations emerge from computational dynamics.
