...
The document covers: the full state space and operational semantics; No Free Insight; $\mu$-ledger necessity and minimality; computational universality and Turing strictness; gauge symmetry and Noether conservation; conditional connections to quantum mechanics (Born rule, Tsirelson bound, no-cloning, unitarity, complex amplitudes, Schr\"odinger equation) under named physical axioms; spacetime emergence and the Landauer bridge; the Thiele Manifold and self-reference tower; and falsifiable predictions with concrete failure modes.
\end{abstract}
...
\section{Four-Dimensional Spacetime Geometry}

The previous sections treated spacetime as a 1D projection via the $\mu$-metric. The next sections extend to full 4D geometry with discrete differential structure, building discrete differential geometry from computational primitives.
...
\begin{theorem}[Einstein Equations in Vacuum {\normalfont (EinsteinEquations4D.v)} --- \textbf{(S)}]
\label{thm:einstein_main}
For any VM state $s$, 4D simplicial complex $\mathcal{K}$, spacetime indices $\mu, \nu$, and vertex $v$:
\begin{equation}
\boxed{\forall w,\; \text{mass}(w) = 0 \quad \implies \quad G_{\mu\nu}(s,\mathcal{K},v) = 8\pi G \, T_{\mu\nu}(s,\mathcal{K},v)}
\end{equation}
\textbf{Note:} This is the vacuum case where both sides equal zero ($0 = 0$). The gravitational constant $G := 1/(8\pi)$ is a \textbf{unit choice} (a definition), not a derived quantity. In vacuum, all masses vanish, so $T_{\mu\nu} = 0$ and uniform-mass (zero) yields $G_{\mu\nu} = 0$; the equation reduces to $0 = 8\pi \cdot \frac{1}{8\pi} \cdot 0 = 0$. The non-vacuum case is \textbf{not proven} and would require discrete Bianchi identities not present in the formalization.
\end{theorem}
...
This is a \textbf{(S) Structural} result: it proves a mathematical relationship between computational primitives (VM states, $\mu$-costs) and geometric quantities (curvature tensors) with no physical axioms. The theorem establishes that:
\begin{enumerate}
    \item Spacetime geometry (metric, curvature) can be constructed from pure computation
    \item The resulting geometry satisfies the vacuum Einstein equation in the formally verified zero-mass regime
    \item Beyond that vacuum regime, the repository contains a conditional full-tensor bridge with named premises, a proved uniform diagonal boundary witness in the active operator, and a separately proved non-uniform boundary witness in a concrete affine metric-scaled redesign branch rather than one universal unconditional theorem
\end{enumerate}

The result does \emph{not} claim that physical spacetime \emph{is} implemented by a Turing machine---that would be a \textbf{(C)} conditional claim requiring empirical validation. More importantly, it does not justify the stronger statement that arbitrary computational substrates with $\mu$-cost tracking automatically satisfy the full Einstein equations in complete generality. The honest status is narrower: the vacuum theorem is unconditional; the full-tensor bridge in \texttt{EinsteinEquationsFull.v} remains conditional; the active first-neighbor operator proves the uniform diagonal boundary witness and formally refutes the stronger non-uniform diagonal extension; the broader nonnegative local weighted family containing the simple symmetric average also fails on the same shell; and \texttt{SymmetricDerivative4D.v} proves that a concrete signed affine metric-scaled redesign rescues that non-uniform boundary witness with off-diagonal Ricci vanishing, Einstein tensor $G_{\mu\nu} = 3\,\delta_{\mu\nu}$ at the critical vertex, and, under \texttt{module\_structural\_mass s 1 = 1}, the aligned full-tensor witness $G_{\mu\nu} = 3\,T_{\mu\nu}$ there.
\end{remark}

\begin{corollary}[Discrete Vacuum Geometry is Emergent]
Discrete vacuum spacetime geometry is an emergent property of the machine's information-processing structure.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:einstein_main} constructs the discrete metric, curvature, and vacuum Einstein relation from computational primitives (VM states, operations, $\mu$-costs) with no physical axioms. Therefore, this model's discrete vacuum geometry is a derived concept rather than a primitive one.
\end{proof}
...
\begin{remark}
The 36 unconditional proofs cover opcodes through \texttt{SupportedOpcode}, plus CALL, RET, CHSH\_TRIAL (\texttt{EmbedStep\_WF.v}), TENSOR\_SET, TENSOR\_GET, LASSERT (\texttt{GraphReconstructionBridge.v}). The 10 conditional proofs cover the 4 partition opcodes and 7 morphism opcodes under \texttt{extended\_hw\_invariant} / \texttt{coupling\_wf}, which are proved inductive: they hold at initialization and are preserved by every instruction in the ISA.
\end{remark}
...
\begin{theorem}[Optimal Quartet {\normalfont (ArchTheorem.v --- reference-only)}]
...
Discrete Gaussian Curvature & \texttt{kernel/MetricFromMuCosts.v} \\
...
RTL Coverage Partition ($36 + 10 + 0 = 46$) & \texttt{kami\_hw/RTLGapRegistry.v} \\
\end{tabular}
\end{center}
...
Curvature from $\mu$-Gradients & \textbf{(S)} & Lemma~\ref{lem:curvature_mu} \\
Vacuum Implies Zero Stress-Energy & \textbf{(S)} & Lemma~\ref{lem:stress_conservation} \\
Einstein Equations (Vacuum) & \textbf{(S)} & Theorem~\ref{thm:einstein_main} \\
Einstein Equations (General full tensor) & \textbf{(C)} & Named off-diagonal Ricci premise in \texttt{EinsteinEquationsFull.v} \\
Affine Off-Diagonal Ricci Zero (vertex 1) & \textbf{(S)} & Non-uniform isotropic boundary 4-simplex; affine operator \\
Full Tensor EFE at Vertex 1 & \textbf{(S)} & \texttt{module\_structural\_mass s 1 = 1} \\
Outer Vertex Characterization & \textbf{(S)} & Same affine operator, outer vertices $\{0,2,3,4\}$ \\
Discrete Differential Geometry & \textbf{(S)} & Definitions only \\
