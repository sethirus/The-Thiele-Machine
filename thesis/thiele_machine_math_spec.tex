...
The core claims are structural (S): No Free Insight; $\mu$-conservation and initiality; $\mu$-hierarchy; $\mu$-ledger necessity; categorical separation; Turing strictness; the classical CHSH bound ($|S| \leq 2$); the algebraic Tsirelson bound ($|S| \leq 2\sqrt{2}$); and the zero-axiom biconditional \texttt{thiele\_honest\_iff\_quantum\_realizable}. The CHSH algebraic result is pure math. The Landauer connection is motivation. Geometry: only the Discrete Gauss-Bonnet theorem. That is the full extent of the physics content.

The document also covers, with explicit epistemological tags, conditional derivations under named physical axioms (Born rule, no-cloning, unitarity, Schr\"odinger analog) and reference explorations (discrete spacetime geometry, self-reference tower). These are marked (C) or (R) and do not form part of the primary claim ledger. There are exactly two named trust boundaries: \texttt{bsc\_kami\_compilation\_trusted} (Layer 2 Verilog semantics gap) and \texttt{A\_QM\_physical} (physical quantum axiom).
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Conditional Quantum Analogs}: Tsirelson bound (algebraic), CHSH classical bound, biconditional honesty theorem, and further results that are conditional on stated physical axioms (Born rule, no-cloning, unitarity, Schr\"odinger). These are marked \textbf{(C)} or \textbf{(R)} throughout. The CHSH algebraic result and the Tsirelson bound are pure math. The Landauer connection is motivation. That is the full extent of the physics content.
    \item \textbf{Discrete Geometry}: Gauss-Bonnet for partition graphs (unconditional); Landauer bridge; emergent spacetime as analogy (not derivation).
    \item \textbf{Meta-Theory}: Three-layer isomorphism, Curry--Howard--Thiele, categorical structure.
...
\begin{theorem}[No Free Lunch {\normalfont (NoFreeLunch.v --- reference-only)}]
...
\begin{theorem}[Abstract Simulation {\normalfont (Embedding\_TM.v --- reference-only, modular\_proofs/TM\_to\_Minsky.v --- reference-only)}]
...
\begin{theorem}[Semantic Strictness {\normalfont (ThieleFoundations.v --- reference-only)}]
...
\begin{theorem}[Diagonal Argument {\normalfont (kernel/OracleImpossibility.v --- reference-only)}]
...
\section{Confluence}
\begin{theorem}[Church--Rosser {\normalfont (Confluence.v --- reference-only)}]
...
\begin{theorem}[Module Independence {\normalfont (ThieleUnificationTensor.v --- reference-only)}]
...
\begin{theorem}[Amortization {\normalfont (AmortizedAnalysis.v --- reference-only)}]
...
\end{enumerate}
Formally (\texttt{mu\_hierarchy\_theorem}): for every $k \geq 1$, the witness trace $[\texttt{instr\_certify}(k{-}1)]$ (cost $= S(k{-}1) = k$) certifies \texttt{init\_state} at level $k$, while no trace of $\mu$-cost $< k$ can reach $\texttt{vm\_mu} \geq k$ from \texttt{init\_state} (since $\texttt{trace\_mu\_cost} = \texttt{vm\_mu\_final} - 0 \geq k$).
...
\begin{theorem}[Noether's Theorem {\normalfont (KernelNoether.v, RepresentationTheorem.v --- reference-only)}]
\leavevmode
\begin{enumerate}
...
The chapters in this part present results conditional on named physical axioms (primarily Axiom~\ref{ax:superposition}). They are marked \textbf{(C)} or \textbf{(R)} throughout. The primary unconditional quantum results — the classical CHSH bound, the algebraic Tsirelson bound, and the biconditional \texttt{thiele\_honest\_iff\_quantum\_realizable} — appear earlier (Chapters 12--13). Those are pure algebra, no physics axioms.
...
\chapter{Two-Dimensional Amplitude Space (Conditional)}
...
\begin{theorem}[1D Is Insufficient {\normalfont (TwoDimensionalNecessity.v --- reference-only)}]
...
\begin{theorem}[Rotation Group {\normalfont (ComplexNecessity.v --- reference-only)}]
...
Complex multiplication by $e^{i\theta}$ is exactly 2D rotation. The norm-preserving maps on $S^1$ are exactly complex multiplications: $\mathrm{SO}(2) \cong U(1)$. Hence complex amplitudes are the \emph{minimal} structure consistent with continuous 2D superposition; higher-dimensional extensions (quaternionic, etc.) are not ruled out by this argument alone.
\end{theorem}
...
\textbf{Note:} The Coq theorem also lists a \texttt{mu\_consistent} hypothesis, but it is \textbf{unused} in the proof. The hypothesis \texttt{mu\_consistent P} unfolds to ``measurement cost $\ge 0$ for valid Bloch vectors,'' which is directly from the definition true for all states and does not constrain $P$. The linearity assumption (item~3) does all the work---it assumes the functional form $P = az+b$ and solves a 2-equation system. This is less ``deriving the Born rule from $\mu$-accounting'' and more ``the Born rule is the unique linear rule consistent with eigenstates.'' The non-circular derivation from tensor consistency (\texttt{BornRuleFromSymmetry.v}, below) is the stronger result.
\end{theorem}

\begin{theorem}[Born Rule from Tensor Consistency {\normalfont (BornRuleFromSymmetry.v --- reference-only/replaced; active derivation in \texttt{kernel/BornRuleLinearity.v})}]
...
\begin{theorem}[No-Cloning from $\mu$-Monotonicity {\normalfont (NoCloningFromMuMonotonicity.v --- reference-only/replaced; active proof in \texttt{kernel/NoCloning.v})}]
...
\begin{theorem}[REVEAL Is Irreversible {\normalfont (ObservationIrreversibility.v --- reference-only)}]
...
\begin{remark}[Epistemological Status: \textbf{(C)} Conditional Derivation — Circularity Resolved]
The original Coq proof (\texttt{TsirelsonDerivation.v} --- reference-only/replaced) encoded $2\sqrt{2}$ in the definition of $\mu_{\mathrm{corr}}$, making its derivation status \textbf{(R)}.
...
\begin{theorem}[CHSH Separation {\normalfont (Deliverable\_CHSHSeparation.v --- reference-only)} --- \textbf{(C)}]
Strict numerical separation:
\[ 2 \quad < \quad 2\sqrt{2} \approx 2.828 \]
\begin{itemize}
    \item \textbf{Classical} (local strategies, locally factorizable): $|S| \le 2$ (classical CHSH bound, proved algebraically --- \textbf{(S)}).
    \item \textbf{Thiele-honest / zero-marginal NPA realizable}: $|S| \le 2\sqrt{2}$ (Tsirelson bound, proved algebraically --- \textbf{(S)}). The biconditional \texttt{thiele\_honest\_iff\_quantum\_realizable} is the kernel-tier characterization of this boundary.
    \item \textbf{Supra-quantum} ($|S| > 2\sqrt{2}$): not Thiele-honest (proved by contrapositive of Tsirelson boundary). The PR box achieves $|S|=4$ but fails column contractivity. No physical Thiele Machine experiment is claimed to have achieved $|S| > 2\sqrt{2}$; that would be a named A\_QM\_physical hypothesis.
\end{itemize}
The gap between 2 and $2\sqrt{2}$ is real. No locally factorizable strategy can bridge it. That is a theorem. Whether quantum hardware bridges it is an empirical question outside the scope of this document.
\end{theorem}
...
\begin{theorem}[Thiele Honesty $\iff$ NPA Realizability {\normalfont (kernel/ThieleQuantumEquivalence.v)} --- \textbf{(S)}]
\label{thm:thiele_honest_iff_qr}
\texttt{thiele\_honest\_iff\_quantum\_realizable}:
\[
...
\textbf{Forward direction} (\texttt{A\_QM\_thiele}): column-contractive $\Rightarrow$ NPA PSD. Proved by constructing the Cholesky decomposition of the $5\times5$ matrix from the three conditions.

\textbf{Reverse direction} (\texttt{quantum\_realizable\_implies\_thiele\_honest}): NPA PSD $\Rightarrow$ column-contractive. Proved by specializing the PSD condition at the vector $(0, -(E_{00}y_0+E_{01}y_1), -(E_{10}y_0+E_{11}y_1), y_0, y_1)$, obtaining a non-negative quadratic in $y_0, y_1$, then closing via \texttt{quadratic\_nonneg\_discriminant}.
...
Key consequences:
\begin{itemize}
\item \texttt{pr\_box\_not\_thiele\_honest}: PR box $(E_{00}=E_{01}=E_{10}=1, E_{11}=-1)$ fails condition 1 immediately ($1-1-1=-1<0$).
\item \texttt{tsirelson\_is\_thiele\_boundary}: Thiele-honest $\Rightarrow$ $|S| \leq 2\sqrt{2}$; and $|S| > 2\sqrt{2}$ $\Rightarrow$ not Thiele-honest. The converse is false and is not claimed.
\item \texttt{turing\_machine\_has\_free\_correlations}: $(1,0,1,0)$ satisfies $|S|=2$ but fails condition 1. A Turing machine can output this correlation; the Thiele Machine rejects it as dishonest.
\end{itemize}
Zero Admitted. Zero named hypotheses.
\end{theorem}

\chapter{The Schr\"odinger Equation (Conditional)}

\begin{theorem}[Emergent Schr\"odinger {\normalfont (EmergentSchrodinger.v --- reference-only)}]
...
\begin{theorem}[Planck Consistency Relation {\normalfont (PlanckDerivation.v --- reference-only)} --- \textbf{(R)}]
...
\begin{theorem}[Discrete Gauss-Bonnet {\normalfont (DiscreteCurvatureGaussBonnet.v)} --- \textbf{(S)}]
...
\]
where $\chi(g) = V - E + F$ is the Euler characteristic (vertices minus edges plus faces), and $\mathrm{angle\_defect}(g)$ is the sum over all vertices of $2\pi$ minus the total incident angle. This is a fully proven combinatorics result about the discrete triangulation model. It is not the continuum Gauss-Bonnet theorem and it is not a derivation of angles from $\mu$-cost. Zero Admitted. This is the full extent of the geometry result in the project.
\end{theorem}
...
\begin{theorem}[Vacuum Case: Both Sides Are Zero {\normalfont (EinsteinEquations4D.v, reference-only)} --- \textbf{(R)}]
...
The complete proof is formalized in \texttt{coq/kernel/EinsteinEquations4D.v} with:
\begin{itemize}
...
\begin{theorem}[Dissipative Model {\normalfont (DissipativeEmbedding.v --- reference-only)}]
...
\begin{theorem}[Reversible Physics {\normalfont (PhysicsEmbedding.v --- reference-only, WaveEmbedding.v --- reference-only)}]
...
\begin{theorem}[Genesis {\normalfont (Genesis.v --- reference-only)}]
...
\[ 30\ (\text{fully unconditional}) + 6\ (\text{argument-validity conditional}) + 7\ (\text{unconditional for KamiReachable}) + 3\ (\text{semantic-boundary}) + 0\ (\text{gaps}) = 46 \]
This is a Coq theorem, not a comment. The partition equation is machine-checked. Zero Admitted.
\end{theorem}

\begin{remark}
The 30 fully unconditional proofs cover the SupportedOpcode group. The 6 argument-validity conditionals are CALL, RET, CHSH\_TRIAL (\texttt{EmbedStep\_WF.v}), TENSOR\_SET, TENSOR\_GET, LASSERT (\texttt{GraphReconstructionBridge.v}). The 7 KamiReachable-unconditional proofs cover MORPH\_DELETE, MORPH\_ASSERT, MORPH\_GET, COMPOSE, MORPH\_TENSOR, MORPH, MORPH\_ID (\texttt{HardwareReachability.v}): these are unconditional for any state reachable from hardware reset. The 3 semantic-boundary opcodes are PNEW, PSPLIT, PMERGE, where hardware and VM differ on zero-size modules in ways no condition closes without changing semantics.
\end{remark}
...
\begin{description}
\item[\texttt{bsc\_kami\_compilation\_trusted} (Layer 2, Verilog semantics gap).] The Bluespec SystemVerilog compiler generates Verilog from the Kami hardware specification. The correctness of the BSC compilation step — that the generated Verilog implements exactly the Kami spec — is trusted, not proved in Coq. This is documented in \texttt{VerilogSemantics.v} under the \texttt{BSCTrustBoundary} section. The theorem \texttt{end\_to\_end\_rtl\_kernel\_correctness} holds for KamiReachable states under this named boundary. The boundary is empirically backed by 31/31 cosim tests and 11,049 fuzz tests.

\item[\texttt{A\_QM\_physical} (physical quantum axiom).] Whether a physical Thiele Machine can produce CHSH-violating statistics by exploiting quantum entanglement in hardware is an open empirical question. The \texttt{thiele\_honest\_iff\_quantum\_realizable} biconditional (Theorem~\ref{thm:thiele_honest_iff_qr}) requires no physical axiom — it is pure algebra. The physical quantum axiom A\_QM\_physical is the separate named hypothesis that the quantum experiments described by the NPA model are physically realizable. It does not appear in the kernel-tier proof of the biconditional.
\end{description}

\chapter{The Curry--Howard--Thiele Correspondence}

\begin{theorem}[Logic--Computation Isomorphism {\normalfont (LogicIsomorphism.v --- reference-only)}]
...
\begin{theorem}[Logic Is Physics {\normalfont (LogicToPhysics.v --- reference-only)}]
...
\section{The Conservation as Functor (Archived)}

\begin{theorem}[Conservation as Functor {\normalfont (Universe.v --- reference-only)}]
...
\begin{theorem}[CatNet Integrity {\normalfont (CatNet.v --- reference-only)}]
...
Exhibit $E_{00}, E_{01}, E_{10}, E_{11}$ satisfying the three column-contractivity conditions but with a non-PSD NPA moment matrix. The theorem \texttt{thiele\_honest\_iff\_quantum\_realizable} says no such example exists.
...
The table below maps each theorem to its Coq source file. Entries marked \emph{--- reference-only} are theoretical results that belong to an extended corpus; the corresponding Coq files are not present in the active kernel tree and their proofs are not machine-checked in the current build. Everything unmarked lives in the active corpus and compiles clean.
...
4D Simplicial Complex & \texttt{kernel/FourDSimplicialComplex.v} \\
Metric from $\mu$-Costs & \texttt{kernel/MetricFromMuCosts.v} \\
Riemann Curvature Tensor & \texttt{kernel/RiemannTensor4D.v} \\
Christoffel Symbols & \texttt{kernel/RiemannTensor4D.v} \\
Einstein Tensor & \texttt{kernel/RiemannTensor4D.v} \\
Stress-Energy Tensor & \texttt{kernel/EinsteinEquations4D.v} \\
Curvature from $\mu$-Gradients & \texttt{kernel/EinsteinEquations4D.v} \\
Stress-Energy Conservation & \texttt{kernel/EinsteinEquations4D.v} \\
Einstein Equations (Vacuum) & \texttt{kernel/EinsteinEquations4D.v} \\
Einstein Equations (General) & \texttt{kernel/EinsteinEquationsFull.v} \\
Discrete Gauss-Bonnet & \texttt{kernel/DiscreteCurvatureGaussBonnet.v} \\
Discrete Gaussian Curvature (reference) & \texttt{kernel/MetricFromMuCosts.v} \\
...
Full Isomorphism & \texttt{kernel/ThreeLayerIsomorphism.v} \\
...
\hline
\multicolumn{2}{l}{\emph{New results (active corpus, all Qed, zero Admitted)}} \\
\hline
Partition Ops Free, Certification Non-Free & \texttt{kernel/PartitionRefinementNoFI.v} \\
Partition Refinement Non-Free & \texttt{kernel/PartitionRefinementNoFI.v} \\
$\mu$-Ledger Necessity (5-way independence classification) & \texttt{NecessityOfMuLedger.v}, \texttt{kernel/NecessityAbstract.v} \\
$\mu$-Ledger Universal Necessity (from any VMState) & \texttt{NecessityOfMuLedger.v}: \texttt{mu\_ledger\_necessity\_universal} \\
$\mu$-Ledger Trace Closure (any prefix length) & \texttt{NecessityOfMuLedger.v}: \texttt{po1\_trace\_necessity} \\
Latent $\mu$ Intrinsic to Computation & \texttt{NecessityOfMuLedger.v}: \texttt{shadow\_mu\_is\_computation\_intrinsic} \\
$\mu$ Cost Universal Across Machines & \texttt{NecessityOfMuLedger.v}: \texttt{shadow\_mu\_delta\_universal} \\
$\mu$ Cost Unique Across Accounting Systems & \texttt{NecessityOfMuLedger.v}: \texttt{shadow\_mu\_unique\_accounting} \\
No Free Computation (inevitable $\mu$) & \texttt{NecessityOfMuLedger.v}: \texttt{shadow\_mu\_inevitable} \\
Graph Independence from $P_{\text{full}}$ & \texttt{kernel/NecessityAbstract.v}: \texttt{graph\_not\_recoverable\_from\_P\_full} \\
Three-Component Independence Classification & \texttt{kernel/NecessityAbstract.v}: \texttt{thiele\_state\_three\_component\_independence} \\
CERTIFY-$d$ Trace Cost Lower Bound ($\ge S(d)$) & \texttt{kernel/QuantitativeNoFI.v}: \texttt{certify\_d\_trace\_cost\_lower\_bound} \\
General Algebraic Tsirelson ($S^2 \le 8$, all coherent) & \texttt{kernel/AlgebraicCoherence.v}: \texttt{algebraically\_coherent\_tsirelson\_general} \\
$\mu$P / $\mu$NP definitions + Classical $\subseteq$ $\mu$P & \texttt{kernel/MuComplexity.v} \\
CHSH Trial Is Tier 0 & \texttt{kernel/WitnessInsightGeneral.v} \\
Three-Tier Observation Taxonomy & \texttt{kernel/WitnessInsightGeneral.v} \\
Locally Consistent $\Rightarrow$ Separable Coupling & \texttt{kernel/CHSHCouplingBridge.v} \\
Local Coupling Bound ($|S| \le 2$) & \texttt{kernel/CHSHCouplingBridge.v} \\
$S > 2$ Rules Out Local Couplings & \texttt{kernel/CHSHCouplingBridge.v} \\
Affine Off-Diagonal Ricci Zero (vertex 1) & \texttt{kernel/AffineEFEClosure.v} \\
Full Tensor EFE at Vertex 1 ($G = 3T$) & \texttt{kernel/AffineEFEClosure.v} \\
Outer Vertex Characterization (Ricci $= 3/32$) & \texttt{kernel/AffineEFEClosure.v} \\
Thiele Honest $\iff$ NPA Realizable (biconditional) & \texttt{kernel/ThieleQuantumEquivalence.v} \\
PR Box Not Thiele-Honest & \texttt{kernel/ThieleQuantumEquivalence.v} \\
Tsirelson Is Thiele Boundary & \texttt{kernel/ThieleQuantumEquivalence.v} \\
RTL Gap Count $= 0$ & \texttt{kami\_hw/RTLGapRegistry.v} \\
RTL Coverage Partition ($30 + 6 + 7 + 3 + 0 = 46$) & \texttt{kami\_hw/RTLGapRegistry.v} \\
\end{tabular}
\end{center}
...
Discrete Gauss-Bonnet & \textbf{(S)} & Well-formed triangulated partition graph; equilateral model \\
