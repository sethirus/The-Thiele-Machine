...
\begin{itemize}
    ...
    \item $\texttt{ADD}(\mathit{dst}, r_1, r_2, \Delta\mu)$: $\mathit{dst} \gets r_1 + r_2 \pmod{2^{32}}$.
    \item $\texttt{SUB}(\mathit{dst}, r_1, r_2, \Delta\mu)$: $\mathit{dst} \gets r_1 - r_2 \pmod{2^{32}}$.
...
\textbf{LASSERT}: Read formula from \texttt{vm\_mem}[\texttt{R[formula\_addr\_reg]}] (encoded-unit count \texttt{formula\_len}, eight concrete bits per unit) and witness package from \texttt{vm\_mem}[\texttt{R[cert\_addr\_reg]}]. Verify using \texttt{cert\_kind} (false = LRAT/UNSAT, true = satisfying model plus falsifying countermodel). If valid, append the formula to $A_m$; if invalid, set $\mathit{err} \gets \mathrm{true}$. Cost: $\mathtt{formula\_len} \times 8 + S(\mathtt{mu\_delta})$.
...
\begin{proof}
Theorem~\ref{thm:einstein_main} constructs the discrete metric, curvature, and vacuum Einstein relation from computational primitives (VM states, operations, $\mu$-costs) with no physical axioms. Therefore, the repo's discrete vacuum geometry is a derived concept rather than a primitive one.
\end{proof}
...
Any chain of MORPH + COMPOSE operations yields $\mu \geq \text{number of operations}$ — the NoFI policy applies to morphism construction.
\end{theorem}
...
Stress-Energy Conservation & \textbf{(S)} & Lemma~\ref{lem:stress_conservation} \\
