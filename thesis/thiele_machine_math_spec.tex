...
\begin{theorem}[Noether's Theorem {\normalfont (\texttt{coq/kernel/KernelNoether.v})}]
\leavevmode
\begin{enumerate}
...
\begin{theorem}[Thiele Honesty $\iff$ NPA Realizability {\normalfont (\texttt{coq/kernel/QuantumPartitionPSD.v}, corollary \texttt{column\_contractive\_iff\_quantum\_realizable})} --- \textbf{(S)}]
...
\textbf{Forward direction} (\texttt{zero\_marginal\_npa\_column\_contractive\_implies\_psd} in \texttt{coq/kernel/MuLedgerQuantumBridge.v}): column-contractive $\Rightarrow$ NPA PSD. Proved by constructing the Cholesky decomposition of the $5\times5$ matrix from the three conditions.

\textbf{Reverse direction} (\texttt{npa\_psd\_implies\_column\_contractive} in \texttt{coq/kernel/QuantumPartitionPSD.v}): NPA PSD $\Rightarrow$ column-contractive. Proved by specializing the PSD condition at the vector $(0, -(E_{00}y_0+E_{01}y_1), -(E_{10}y_0+E_{11}y_1), y_0, y_1)$, obtaining a non-negative quadratic in $y_0, y_1$, then closing via \texttt{quadratic\_nonneg\_discriminant}.
...
This is the full extent of the geometry result in the project.
\end{remark}
...
\begin{theorem}[Vacuum Case: Both Sides Are Zero {\normalfont (\texttt{coq/kernel/EinsteinEquations4D.v})} --- \textbf{(R)}]
...
$\mu$-Conservation & \texttt{coq/kernel/MuLedgerConservation.v} \\
Receipt Integrity & \texttt{coq/kernel/ReceiptIntegrity.v} \\
Noether / Gauge & \texttt{coq/kernel/KernelNoether.v} \\
Strict Subsumption & \texttt{coq/kernel/Subsumption.v} \\
Universal NFI (any substrate) & \texttt{coq/kernel/UniversalCertificationCost.v}: \texttt{universal\_nfi\_any\_substrate} \\
Quantitative NFI ($\ge S(d)$ trace bound) & \texttt{coq/kernel/QuantitativeNoFI.v} \\
$\mu$-Hierarchy & \texttt{coq/kernel/MuHierarchyTheorem.v}: \texttt{mu\_hierarchy\_theorem} \\
Partition Ops Free, Certification Non-Free & \texttt{coq/kernel/PartitionRefinementNoFI.v} \\
$\mu$-Ledger Necessity (5-way classification) & \texttt{coq/NecessityOfMuLedger.v}, \texttt{coq/kernel/NecessityAbstract.v} \\
...
$\mu$P / $\mu$NP definitions + Classical $\subseteq$ $\mu$P & \texttt{coq/kernel/MuComplexity.v} \\
\hline
\multicolumn{2}{l}{\emph{Categorical and shadow-projection layer}} \\
\hline
Categorical Separation / Category Laws & \texttt{coq/kernel/CategoryLaws.v}, \texttt{coq/kernel/CategoryBridge.v} \\
Shadow Strictly Lossy & \texttt{coq/kernel/ShadowProjection.v}: \texttt{shadow\_strictly\_lossy} \\
Turing Strictness ($D5$) & \texttt{coq/kernel/TuringStrictness.v} \\
CHSH Coupling Bridge ($S \le 2$ from local coupling, $S > 2$ rules it out) & \texttt{coq/kernel/CHSHCouplingBridge.v} \\
Witness Insight Three-Tier Taxonomy & \texttt{coq/kernel/WitnessInsightGeneral.v} \\
\hline
\multicolumn{2}{l}{\emph{Quantum / CHSH algebra (zero-axiom)}} \\
\hline
Thiele honest $\iff$ NPA realizable (biconditional) & \texttt{coq/kernel/QuantumPartitionPSD.v}: \texttt{column\_contractive\_iff\_quantum\_realizable} \\
Forward: column-contractive $\Rightarrow$ NPA PSD & \texttt{coq/kernel/MuLedgerQuantumBridge.v}: \texttt{zero\_marginal\_npa\_column\_contractive\_implies\_psd} \\
Reverse: NPA PSD $\Rightarrow$ column-contractive & \texttt{coq/kernel/QuantumPartitionPSD.v}: \texttt{npa\_psd\_implies\_column\_contractive} \\
Quadratic-form discriminant lemma & \texttt{coq/kernel/ConstructivePSD.v}: \texttt{quadratic\_nonneg\_discriminant} \\
Tsirelson from column contractivity ($|S| \le 2\sqrt{2}$) & \texttt{coq/kernel/MuLedgerQuantumBridge.v}: \texttt{state\_column\_contractive\_implies\_tsirelson} (\texttt{tsirelson\_from\_row\_bounds} in \texttt{TsirelsonGeneral.v} is the inner Cauchy--Schwarz step) \\
Tsirelson (algebraic, tight) & \texttt{coq/kernel/AlgebraicCoherence.v}: \texttt{tsirelson\_bound\_tight} \\
Tsirelson (algebraic from coherence) & \texttt{coq/kernel/TsirelsonFromAlgebra.v} \\
General Algebraic Tsirelson ($S^2 \le 8$) & \texttt{coq/kernel/AlgebraicCoherence.v}: \texttt{algebraically\_coherent\_tsirelson\_general} \\
\hline
\multicolumn{2}{l}{\emph{Conditional physics derivations (under named axioms; (C) tier)}} \\
\hline
Born Rule & \texttt{coq/kernel/BornRule.v} \\
Unitarity & \texttt{coq/kernel/Unitarity.v} \\
Purification & \texttt{coq/kernel/Purification.v} \\
No-Cloning & \texttt{coq/kernel/NoCloning.v} \\
Information Causality & \texttt{coq/kernel/InformationCausality.v} \\
Landauer derivation & \texttt{coq/thermodynamic/LandauerDerived.v} \\
Affine Off-Diagonal Ricci Zero (vertex 1) & \texttt{coq/kernel/AffineEFEClosure.v} \\
Full Tensor EFE at Vertex 1 ($G = 3T$) & \texttt{coq/kernel/AffineEFEClosure.v} \\
Outer Vertex Characterization (Ricci $= 3/32$) & \texttt{coq/kernel/AffineEFEClosure.v} \\
Spacetime Emergence (consistency check) & \texttt{coq/kernel/SpacetimeEmergence.v} \\
$\mu$-Geometry (pseudometric) & \texttt{coq/kernel/MuGeometry.v} \\
4D Simplicial Complex & \texttt{coq/kernel/FourDSimplicialComplex.v} \\
Metric from $\mu$-Costs & \texttt{coq/kernel/MetricFromMuCosts.v} \\
Riemann / Christoffel / Einstein tensors & \texttt{coq/kernel/RiemannTensor4D.v} \\
Stress-Energy Tensor / Conservation / Vacuum EFE & \texttt{coq/kernel/EinsteinEquations4D.v} \\
Einstein Equations (general, full diagonal-and-offdiag chain) & \texttt{coq/kernel/EinsteinEquationsFull.v} \\
...
Falsifiable Prediction record & \texttt{coq/kernel/FalsifiablePrediction.v} \\
Three-Layer (Coq $\leftrightarrow$ Python $\leftrightarrow$ RTL) Isomorphism & \texttt{coq/kernel/ThreeLayerIsomorphism.v} \\
\hline
...
RTL Trust Boundary (Section Variable) & \texttt{coq/kernel/VerilogRTLCorrespondence.v}: \texttt{rtl\_step\_correct} \\
BSC Compilation Trust Boundary & \texttt{coq/kernel/VerilogRTLCorrespondence.v}: \texttt{bsc\_kami\_compilation\_trusted} \\
...
OCaml extraction observable surface & \texttt{coq/kernel/OCamlExtractionBridge.v}: \texttt{ocaml\_bisimulation\_closure} \\
\end{tabular}
\end{center}
...
\hline
Born Rule Uniqueness & \textbf{(C)} & Linearity assumption (\texttt{coq/kernel/BornRule.v}, \texttt{BornRuleLinearity.v}) \\
