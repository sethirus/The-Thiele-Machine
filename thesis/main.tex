...
\begin{itemize}
    ...
    \item \textbf{Turing Subsumption}: A direct simulation proof shows Thiele strictly extends Turing, with cost certificates that Turing machines cannot produce---\path{coq/kernel/ProperSubsumption.v}.
\end{itemize}
...
\begin{itemize}
    ...
    \item Falsifiable predictions: linear $\mu$-cost scaling bounds for all 47 opcodes, plus the observation that the cost axiom re-derives known physics bounds (CHSH $\le 2$, Tsirelson $\le 2\sqrt{2}$, Landauer erasure)
\end{itemize}
