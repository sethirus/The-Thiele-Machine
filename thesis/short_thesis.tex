...
\begin{theorem}[$\mu$-ledger necessity, complete independence classification, \texttt{NecessityOfMuLedger.v} and \texttt{kernel/NecessityAbstract.v}, zero Admitted]
Five independence results hold simultaneously:
...
The three non-classical components---$\mu$, \texttt{vm\_certified}, \texttt{vm\_graph}---are each irrecoverable from any combination of the others plus classical state (\texttt{thiele\_state\_three\_component\_independence}). The projection $(\texttt{mem}, \texttt{regs}, \texttt{pc}, \mu, \texttt{certified})$ is the minimal complete extension for ledger semantics. The separation holds from \emph{every} reachable VMState (\texttt{mu\_ledger\_necessity\_universal}), and no common prefix program of any length closes it (\texttt{po1\_trace\_necessity}).
\end{theorem}

Plain version: the ledger is not just a counter someone forgot to derive. There are explicit witness states with identical Turing/RAM-visible state and incompatible receipts---not just from one starting point, but from every state the machine can reach. Cost, certification, and graph structure are three independent bookkeeping dimensions. A model that drops any of them cannot determine the full structural semantics internally.

\begin{theorem}[Latent $\mu$: the cost is intrinsic to computation, \texttt{NecessityOfMuLedger.v} §7, zero Admitted]
\label{thm:latent_mu}
...
\begin{theorem}[General algebraic Tsirelson bound, \texttt{AlgebraicCoherence.v}, zero Admitted]
\label{thm:general_algebraic_tsirelson}
For every algebraically coherent correlator --- one satisfying $|E_{xy}| \leq 1$ and four $3 \times 3$ minor constraints with witness parameters $t, s$ --- the CHSH value satisfies $S^2 \leq 8$, i.e., $|S| \leq 5657/2000 \approx 2\sqrt{2}$.
This is the \emph{general} case: all algebraically coherent correlators, not just the symmetric configuration. Proved via Positivstellensatz SOS certificate (\texttt{psatz Q 4}, CSDP solver). No physics premises. Pure polynomial arithmetic over $\mathbb{Q}$. The Tsirelson bound $2\sqrt{2}$ is computed from algebra, not assumed.
\end{theorem}
...
\item \textbf{mu\_ledger\_necessity}: no strict-classical oracle on $(\texttt{mem}, \texttt{regs}, \texttt{pc})$ recovers the pair $(\mu, \texttt{certified})$.
\item \textbf{vm\_mu\_not\_classically\_determined / vm\_certified\_not\_classically\_determined}: neither ledger balance nor certification status is a function of strict classical state.
\item \textbf{mu\_ledger\_mutual\_independence}: $\mu$ and \texttt{vm\_certified} are each independent of strict classical state and of each other under cost-only/cert-only projections.
\item \textbf{mu\_ledger\_minimality / P\_full\_is\_minimal\_complete\_extension}: the full $(\texttt{mem}, \texttt{regs}, \texttt{pc}, \mu, \texttt{certified})$ projection is complete, and dropping either $\mu$ or certification destroys the corresponding completeness.
