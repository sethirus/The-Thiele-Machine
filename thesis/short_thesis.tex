...
The central idea is simple. A Turing machine tracks registers, memory, and time. It cannot see whether a computation has become \emph{entitled} to use a structural shortcut --- a decomposition, a verified invariant, a certified fact about its own state. The Thiele Machine makes that entitlement first-class and cost-bearing. There is a global counter called $\mu$ that starts at zero and never goes down. Any computation that ends in a certified structural claim must have paid at least 1 into $\mu$. That is No Free Insight. It is a conservation law, proved in Coq from the machine's own step function.
...
\]
where $A = 1-E_{00}^2-E_{10}^2$, $B = E_{00}E_{01}+E_{10}E_{11}$, $C = 1-E_{01}^2-E_{11}^2$.
These are the three conditions of \texttt{zero\_marginal\_column\_contractive} (defined in \texttt{coq/kernel/MuLedgerQuantumBridge.v}): column 0 norm $\leq 1$, column 1 norm $\leq 1$, determinant $AC - B^2 \geq 0$.
...
The forward direction (\texttt{zero\_marginal\_npa\_column\_contractive\_implies\_psd} in \texttt{MuLedgerQuantumBridge.v}) was proved first. The reverse direction is new: given a PSD NPA matrix, specialize it at the vector $(0, -(E_{00}y_0 + E_{01}y_1), -(E_{10}y_0 + E_{11}y_1), y_0, y_1)$. The PSD condition gives a quadratic-in-$y_0/y_1$ that is everywhere non-negative. Then \texttt{quadratic\_nonneg\_discriminant} ($\forall t, a + 2bt + ct^2 \geq 0 \Rightarrow b^2 \leq ac$) closes the determinant condition. The diagonal conditions come from $y_0 = 1, y_1 = 0$ and $y_0 = 0, y_1 = 1$. Both directions are packaged as the biconditional \texttt{column\_contractive\_iff\_quantum\_realizable} in \texttt{coq/kernel/QuantumPartitionPSD.v}.
...
\item Tsirelson bound: \texttt{state\_column\_contractive\_implies\_tsirelson} (\texttt{coq/kernel/MuLedgerQuantumBridge.v}) proves Thiele-honest $\Rightarrow$ $|S|^2 \leq 8$ (i.e., $|S| \leq 2\sqrt{2}$) directly from column-contractivity. The chain factors through PSD: column-contractive $\Rightarrow$ NPA-PSD $\Rightarrow$ row bounds $\Rightarrow$ Tsirelson, and the row-bounds step is \texttt{tsirelson\_from\_row\_bounds} in \texttt{coq/kernel/TsirelsonGeneral.v}. The contrapositive ($|S| > 2\sqrt{2}$ $\Rightarrow$ not Thiele-honest) follows immediately. The converse is false and not proved.
\end{itemize}
...
Current Inquisitor status: zero HIGH/MEDIUM/LOW findings across all 205 Coq files in the active tree.
...
\item \textit{\small column\_contractive\_iff\_quantum\_realizable} (\texttt{coq/kernel/QuantumPartitionPSD.v}): Thiele honesty $\iff$ zero-marginal NPA realizability. A zero-axiom biconditional. Forward (\texttt{zero\_marginal\_npa\_column\_contractive\_implies\_psd} in \texttt{MuLedgerQuantumBridge.v}): column-contractive → NPA PSD. Reverse (\texttt{npa\_psd\_implies\_column\_contractive} in \texttt{QuantumPartitionPSD.v}): NPA PSD → column-contractive, proved by vector specialization plus the \texttt{quadratic\_nonneg\_discriminant} lemma from \texttt{ConstructivePSD.v}. Zero-marginal NPA realizability is the orthogonal-observables slice of the quantum elliptope ($\rho_{AA}=\rho_{BB}=0$), not the full elliptope. Classical correlations like $(1,0,1,0)$ have $|S| \leq 2$ but fail column contractivity — the classical polytope and the Thiele-honest set intersect but neither contains the other.
\item \textit{\small state\_column\_contractive\_implies\_tsirelson} (\texttt{coq/kernel/MuLedgerQuantumBridge.v}): Thiele-honest $\Rightarrow$ $|S|^2 \leq 8$, i.e., $|S| \leq 2\sqrt{2}$. The proof factors through quantum realizability: \texttt{state\_column\_contractive\_implies\_quantum\_gram} composes with \texttt{quantum\_realizable\_implies\_tsirelson\_bound} (in the same file). The row-bounds lemma \texttt{tsirelson\_from\_row\_bounds} in \texttt{coq/kernel/TsirelsonGeneral.v} is the algebraic Cauchy--Schwarz step inside this chain. The contrapositive ($|S| > 2\sqrt{2}$ $\Rightarrow$ not Thiele-honest) follows immediately. The converse ($|S| \leq 2\sqrt{2}$ $\Rightarrow$ Thiele-honest) is false and is not claimed.
