...
Beyond Coq's type checker, the project has a custom proof auditor called the Inquisitor (\texttt{scripts/inquisitor.py}). It runs on every commit and has 160 distinct rules. The main categories:
\begin{itemize}
...
\item \textit{\small feasible-subset-implies-strict-predicates}: feasible-set narrowing plus a distinguishing observation yields a strictly stronger predicate.
...
\item \textit{\small obs-partition-to-posterior-witness}: observation classes package into the posterior-representative witness for structural entitlement.
\item \textit{\small info-priced-posterior-reduction-bound}: with a decision-tree/representative witness, feasible-set compression is bounded by $\Delta\mu$.
\item \textit{\small info-priced-weighted-feasible-bound}: the $\Delta\mu$ lower bound extends to nat-weighted feasible sets.
