...
Here is the remarkable thing: this simple machine can compute \emph{anything that can be computed}. Every algorithm that has ever been written, in every programming language that has ever existed, can be translated into a Turing machine. This is the Church-Turing thesis, and it has never been falsified. Your laptop, your phone, every server in every data center --- all of them are, in the relevant theoretical sense, Turing machines. They can compute exactly the same class of things. The only difference is speed.
...
Reads a model description length from register $r$ and charges it as a $\mu$ cost. MDL stands for Minimum Description Length, an information-theoretic principle from Rissanen (1978): the best model for data is the one that minimizes the combined length of the model description and the data given the model. This opcode lets the machine explicitly account for MDL-based structural costs in its ledger.
...
TENSOR\_SET and TENSOR\_GET manage per-module mu-tensor entries, tracking cost-tensor relationships between modules for the physics bridge (Section~\ref{sec:physics}). The hardware contains dedicated hardware-resident tensor table registers (4$\times$4 entries per module), but these opcodes fall in the irreducible-gap category in \texttt{RTLGapRegistry.v}: formal bisimulation proofs require reasoning about variable-length tensor state that the current Kami RTL verification framework cannot mechanically discharge. For these opcodes the OCaml extracted runner provides the verified reference semantics.
...
A \emph{morphism} is a record saying: ``module $A$ is related to module $B$ in this specific, named way.'' Not just ``$A$ and $B$ exist.'' Not just ``$A$ and $B$ are connected by an edge.'' A morphism has a source, a target, a coupling type (a natural number encoding the kind of relationship), and an is-identity flag.

The coupling type is how you say: ``this morphism represents entanglement'' or ``this morphism represents a simulation'' or ``this morphism represents a composition.'' The machine does not interpret coupling types --- that is left to the programmer. But the machine tracks them, and you can read them back via MORPH\_GET.
...
These laws are proven in Coq in \texttt{CategoryLaws.v}. Zero Admitted. The Thiele Machine's categorical layer is not just called a category --- it satisfies the actual axioms.
...
A morphism between two modules says: ``there is a named, typed relationship between module $A$ and module $B$.'' This could mean:
\begin{itemize}
\item Module $B$ refines module $A$ (finer partition)
\item Module $A$ entangles with module $B$ (quantum coupling)
\item Module $A$ simulates module $B$ (bisimulation)
\item Whatever the programmer puts in the coupling field
\end{itemize}

The machine does not interpret coupling types. The programmer does. But the machine tracks them, and the machine can be asked to confirm or deny their existence.
...
The Kami framework lets you write hardware specifications in Coq and extract them to Bluespec SystemVerilog, which then compiles to Verilog RTL. The Thiele Machine hardware is defined in \texttt{ThieleCPUCore.v} (Kami module) and extracted through this pipeline.
...
The hardware implements 34 of 46 opcodes with verified proofs (30 unconditionally, 4 under explicit preconditions). The remaining 12 are documented in \texttt{RTLGapRegistry.v}:
\begin{itemize}
\item 9 irreducible gaps (morphism ops and tensor ops): the hardware contains hardware-resident tables for these (64 morphism descriptor slots, per-module tensor entries), and ISA v2 upper-lane opcodes drive real hardware state mutation via those tables. The irreducibility is in the \emph{proofs}: formal bisimulation for these opcodes requires reasoning about variable-length graph and tensor state that the current Kami RTL proof framework cannot mechanically discharge. For these opcodes the OCaml extracted runner provides the verified reference semantics.
\item 3 conditional gaps (CALL, RET, CHSH\_TRIAL): proofs exist in \texttt{EmbedStep\_WF.v} under explicit preconditions, but unconditional proofs are not provable without more specific hardware assumptions.
\end{itemize}

The 34/46 split is NOT a sign of incompleteness. It is the same relationship that exists between any ISA and its hardware implementation. Real CPUs delegate complex operations to microcode, firmware, or operating system libraries. The Intel x86 FPU uses microcode for some operations. The ARM Cortex-A series uses software emulation for some floating-point sequences. The abstract ISA is complete. The silicon implements the fast path. The software implements the rest.
...
The Thiele Machine has opcodes for recording CHSH trials. I need to explain what CHSH is.

\subsection{The Bell test setup, from scratch}

Imagine two players, Alice and Bob, in separate rooms. They cannot communicate once the game starts. A referee sends Alice one of two questions ($a = 0$ or $a = 1$). The referee sends Bob one of two questions ($b = 0$ or $b = 1$). Alice answers $x \in \{0,1\}$. Bob answers $y \in \{0,1\}$.

The game rule: they want $x \oplus y = a \land b$. In plain language: their answers should be different if and only if both questions were 1.

Classical strategy: Alice and Bob can agree on a strategy before the game. With the best classical strategy, they win at most 75\% of the time. This is a mathematical fact.
...
\]
where $\langle A_a B_b \rangle$ is the correlation between Alice's and Bob's answers when the questions are $a$ and $b$, with answers encoded as $+1$ and $-1$.

For any strategy where Alice's answer depends only on her question and some shared randomness $\lambda$ (and similarly Bob's answer depends only on his question and $\lambda$), we have $|S| \leq 2$. This is the classical CHSH bound.

Quantum mechanics allows $|S| \leq 2\sqrt{2} \approx 2.83$. This is the Tsirelson bound.

The gap between 2 and $2\sqrt{2}$ is the signature of quantum non-locality: correlations that cannot be explained by any shared classical randomness.
...
Getting $S > 2\sqrt{2}$ in any computational context requires revelation: at some point, Alice-side and Bob-side information must be shared to compute the correlation. That sharing is a communication event, and it has a cost. The REVEAL opcode is the Thiele Machine's way of making that cost explicit. Trying to get non-local statistics above the quantum bound without paying for revelation will fail. This is not enforced as a policy. It is a structural consequence of the step semantics.
...
Rolf Landauer proved in 1961 that erasing one bit of information from a physical system must release at least $kT \ln 2$ joules of heat into the environment, where $k$ is Boltzmann's constant and $T$ is the temperature of the environment.

This is a theorem about physics, not just computers. Information is physical. Forgetting costs energy. This is Landauer's principle.

\subsection{What the Thiele Machine proves}

The $\mu$ ledger is consistent with Landauer's principle in the following sense:
...
The Thiele Machine's $\mu$ costs define a geometry. Here is what that means.

Each module carries a local $\mu$ cost. The partition graph gives a graph structure. The morphisms give relationships between modules. Together, these define a discrete metric: the cost of going from one module to an adjacent one in the partition graph.

A discrete metric defines curvature. Curvature means the space is not flat: triangles have angles that don't add up to 180 degrees. The Gauss-Bonnet theorem relates the total curvature of a surface to its topology.
...
The chain from this to Einstein's field equations goes through:
\begin{enumerate}
\item Gauss-Bonnet (proven)
\item Clausius entropy-area relation (proven in \texttt{ClausiusFromEntropyArea.v})
\item Raychaudhuri equation (proven conditionally in \texttt{DiscreteRaychaudhuri.v})
\item Jacobson derivation of Einstein equations from thermodynamics (chain assembled in \texttt{EinsteinEmergence.v})
\item Full tensor Einstein equations $G_{\mu\nu} = \kappa T_{\mu\nu}$ (proven in Section in \texttt{EinsteinEquationsFull.v})
\end{enumerate}
...
\item It does not mean the Coq axioms themselves are consistent. Coq is based on the Calculus of Constructions, which is believed to be consistent but this has not been proven within Coq (Gödel's incompleteness theorem prevents it).
\item It does not mean the hardware implements exactly the Coq semantics. The hardware covers 34/46 opcodes with proofs; 12 gaps are documented in \texttt{RTLGapRegistry.v}.
...
\begin{itemize}
\item \textbf{Hardware bisimulation}: 34/46 with proofs (30 unconditional, 4 conditional); 12 gaps in \texttt{RTLGapRegistry.v}.
...
A Turing machine is blind to global structure. It can compute anything but it pays in time whenever it needs to discover structure that is already there. The Thiele Machine makes structural knowledge visible, first-class, and cost-bearing. The discipline that governs it is No Free Insight: you cannot certify stronger claims than you started with without paying in a monotone ledger $\mu$ that records every structural commitment.
...
The hardware implements 34 of 46 opcodes in silicon with proofs and the rest in software. This is not incompleteness. It is the normal relationship between an abstract machine model and its physical instantiation. The abstract model is mathematically complete. The hardware is a finite physical approximation. Every physical computer in the world has this relationship with its mathematical model. The Thiele Machine is no different.
