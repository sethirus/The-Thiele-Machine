...
This appendix contains the auto-generated Coq proof verifying that the Thiele Machine has successfully rediscovered the Schrödinger equation from raw data. The proof establishes structural equivalence between the discovered update rules and the finite-difference discretization of the Schrödinger equation.

\begin{lstlisting}[
  language=Caml,
  caption={Emergent Schrödinger Equation Proof (Artifact)},
  keywords={Definition, Theorem, Lemma, Proof, Qed, Require, Import, Open, Scope, forall, exists, fun, match, with, end, if, then, else, let, in, Prop, Type, Set, nat, bool, list, option, string, Q, check_lrat, check_model, apply_cost, normalize_region, Observable, ObservableRegion, strictly_stronger, stronger, run_vm, ledger_entries, ledger_sum, step_vm, extracted_mass, extracted_inv_2m, extracted_dt, coef_a_a, coef_a_b, coef_a_lap_b, coef_a_Vb, coef_b_b, coef_b_a, coef_b_lap_a, coef_b_Va, schrodinger_update_a, schrodinger_update_b, target_update_a, target_update_b},
  literate={->}{{$\to$}}1 {=>}{{$\Rightarrow$}}1 {/\}{{$\land$}}1 {\/}{{$\lor$}}1 {ö}{{\"o}}1 {Σ}{{$\Sigma$}}1
]
...
Theorem structural_equivalence :
  forall (a b lap_a lap_b Va Vb : Q),
    Qeq (schrodinger_update_a a b lap_b Vb) (target_update_a a lap_b Vb) /    Qeq (schrodinger_update_b b a lap_a Va) (target_update_b b lap_a Va).
Proof.
  intros.
