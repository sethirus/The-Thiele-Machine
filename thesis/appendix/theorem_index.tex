...
\textbf{Note}: This index lists selected highlights from each proof domain. The complete corpus contains over 4,700 formal declarations across the active source tree. Every theorem can be located by grepping the \texttt{coq/} directory.
...
\begin{itemize}
    ...
    \item \texttt{vm\_is\_a\_correct\_refinement\_of\_kernel} (SimulationProof.v): VM implementation refines kernel spec (witness construction: proves existence of a related kernel state but does not show the kernel TM actually reaches it via \texttt{step\_thiele})
...
Key theorems include:
\begin{itemize}
    \item \texttt{mu\_monotonic}, \texttt{mu\_never\_decreases} (MuComplexity.v, reference-only): $\mu$-cost monotonicity
    ...
    \item \texttt{mu\_is\_landauer\_valid} (MuNecessity.v, reference-only): $\mu$-cost satisfies Landauer's principle
    \item \texttt{mu\_distance\_triangle} (MuGeometry.v): $\mu$-distance metric inequality
    \item \texttt{mu\_ledger\_minimality} (NecessityAbstract.v): named projection classification for $\mu$ and certification completeness
    \item \texttt{P\_full\_is\_minimal\_complete\_extension} (NecessityAbstract.v): the full ledger shadow is complete, while projections that forget $\mu$ or certification lose the corresponding completeness
\end{itemize}
...
Key theorems include:
\begin{itemize}
    \item \texttt{mu\_ledger\_necessity} (NecessityOfMuLedger.v): no strict-classical function recovers the pair $(\mu,\texttt{certified})$
    \item \texttt{vm\_mu\_not\_classically\_determined} (NecessityOfMuLedger.v): $\mu$ is not a function of strict classical state
    \item \texttt{vm\_certified\_not\_classically\_determined} (NecessityOfMuLedger.v): certification is not a function of strict classical state
    \item \texttt{mu\_ledger\_necessity\_universal} (NecessityOfMuLedger.v): from \emph{any} VMState, CERTIFY and PNEW produce the same strict shadow but different $\mu$ and certification---the separation holds universally, not only from crafted witnesses
    \item \texttt{po1\_trace\_necessity} (NecessityOfMuLedger.v): for any common prefix program of any length, the two witness extensions reach equal strict shadows but strictly different $\mu$---no classical computation accumulation closes the gap
    \item \texttt{shadow\_mu\_is\_computation\_intrinsic} (NecessityOfMuLedger.v): the total $\mu$ accumulated by any program equals starting $\mu$ plus sum of instruction costs---determined entirely by the instruction sequence, independent of vm\_graph, vm\_regs, vm\_mem, or any other state
    \item \texttt{shadow\_mu\_delta\_universal} (NecessityOfMuLedger.v): two machines running the same program from any two starting states accumulate the same additional $\mu$---the cost is written in the program before execution begins
    \item \texttt{shadow\_mu\_unique\_accounting} (NecessityOfMuLedger.v): any instruction-consistent accounting system assigning zero to the starting state assigns exactly \texttt{trace\_total\_cost} to the result---the cost is forced, not chosen; the Turing machine paid it silently on every execution
    \item \texttt{shadow\_mu\_inevitable} (NecessityOfMuLedger.v): any program containing an instruction with positive cost accumulates strictly positive $\mu$---there is no free computation
    \item \texttt{turing\_ram\_mu\_necessity}, \texttt{turing\_ram\_cert\_necessity}, \texttt{turing\_ram\_pair\_necessity} (NecessityAbstract.v): Turing/RAM projections cannot determine ledger receipts
    \item \texttt{cost\_model\_cert\_necessity} and \texttt{cert\_model\_mu\_necessity} (NecessityAbstract.v): cost-only and certification-only projections each lose the other half of the receipt
    \item \texttt{graph\_not\_recoverable\_from\_P\_full} (NecessityAbstract.v): no function of the full $\mu$-ledger shadow (mem, regs, pc, $\mu$, certified) can recover \texttt{vm\_graph}---the categorical layer is a third independent non-classical component
    \item \texttt{thiele\_state\_three\_component\_independence} (NecessityAbstract.v): complete independence classification---$\mu$, certified, and graph are each provably irrecoverable from any combination of the others together with classical state
    \item \texttt{certify\_d\_trace\_cost\_lower\_bound} (QuantitativeNoFI.v): any trace containing \texttt{instr\_certify d} accumulates total $\mu$-cost $\ge S(d)$---connects the abstract threshold to the declared ISA cost
    \item \texttt{algebraically\_coherent\_tsirelson\_general} (AlgebraicCoherence.v): $S^2 \le 8$ for \emph{every} algebraically coherent correlator (general case, not just symmetric)---closes the gap noted in the file header; proved via Positivstellensatz SOS certificate (CSDP)
    \item \texttt{algebraically\_coherent\_tsirelson\_abs} (AlgebraicCoherence.v): $|S| \le 5657/2000 \approx 2\sqrt{2}$ for every algebraically coherent correlator
...
Key theorems include:
\begin{itemize}
    \item \texttt{Physics\_Requires\_Extra\_Structure} (TOEDecision.v, reference-only): Theory-of-everything impossibility
...
Key theorems include:
\begin{itemize}
    \item \texttt{main\_subsumption} (Subsumption.v): Main subsumption theorem (no project-local axioms)
    ...
    \item \texttt{D2\_faithfulness} (TuringClassicalEmbedding.v): Classical programs run faithfully inside Thiele
    \item \texttt{D5\_thiele\_strictly\_extends\_classical} (TuringStrictness.v): Definitive KERNEL-tier strictness theorem
    \item \texttt{degenerate\_projection\_theorem} (TuringClassicalEmbedding.v): Shadow projection = classical equivalence quotient
...
\section{Kernel TOE Theorems (reference-only)}

\textbf{Note}: The kernel\_toe/ files are maintained as reference-only at \texttt{archive/coq\_unused/kernel\_toe/}.
...
\section{ThieleMachine Theorems (reference-only)}

\textbf{Note}: The \texttt{coq/thielemachine/} files are maintained as reference-only at \texttt{archive/coq\_unused/thielemachine/}.
...
\begin{itemize}
    ...
    \item \texttt{born\_rule\_from\_tensor\_consistency} (BornRuleFromSymmetry.v, reference-only; active replacement: BornRuleLinearity.v): Born rule from tensor product consistency (non-circular)
    \item \texttt{no\_cloning\_from\_mu\_monotonicity} (NoCloningFromMuMonotonicity.v, reference-only; active replacement: NoCloning.v): Machine-native no-cloning via integer arithmetic
...
\subsection{Oracle Impossibility (reference-only)}

\textbf{Note}: OracleImpossibility.v is maintained as reference-only at \texttt{archive/kernel\_exploratory/dead\_end\_chains/}.

Key theorems include:
\begin{itemize}
    \item \texttt{halting\_undecidable} (OracleImpossibility.v, reference-only): Halting problem undecidability via diagonalization
    \item \texttt{oracle\_halts\_costs\_mu} (OracleImpossibility.v, reference-only): Any halting oracle incurs $\mu$-cost
    \item \texttt{hypercomputation\_bounded} (OracleImpossibility.v, reference-only): Hypercomputation is bounded by the Thiele framework
    \item \texttt{always\_halts\_undecidable} (OracleImpossibility.v, reference-only): Rice's theorem---no non-trivial semantic property is decidable
    \item \texttt{oracle\_cost\_linear} (OracleImpossibility.v, reference-only): Oracle cost grows linearly
\end{itemize}

\subsection{Oracle Accounting (reference-only)}

\textbf{Note}: Oracle.v and HyperThiele*.v are maintained as reference-only at \texttt{archive/coq\_unused/thielemachine/}.
...
\begin{itemize}
    ...
    \item \texttt{ic\_mu\_record\_projection} (InformationCausality.v): IC--$\mu$ record-equivalence projection
...
\section{Bridge Theorems (reference-only)}

\textbf{Note}: All bridge/ files are maintained as reference-only at \texttt{archive/coq\_unused/bridge/}.
...
\begin{itemize}
    ...
    \item \texttt{python\_mu\_ledger\_isomorphism} (PythonMuLedgerBisimulation.v): Python--Coq ledger simulation (forward refinement)
...
\section{Shor Primitives Theorems (reference-only)}

\textbf{Note}: All shor\_primitives/ files are maintained as reference-only at \texttt{archive/coq\_unused/shor\_primitives/}.
...
\section{Modular Proofs Theorems (reference-only)}

\textbf{Note}: All \texttt{coq/modular\_proofs/} files are maintained as reference-only at \texttt{archive/coq\_unused/modular\_proofs/}.
...
\section{ThieleUniversal Theorems (reference-only)}

\textbf{Note}: All \texttt{coq/thieleuniversal/} files are maintained as reference-only at \texttt{archive/coq\_unused/thieleuniversal/}.
...
See also \texttt{non\_circularity\_verified} (NonCircularityAudit.v), \texttt{three\_layer\_semantic\_isomorphism} (SemanticComplexityIsomorphism.v, reference-only), and \texttt{complete\_verification\_chain} (HardwareBisimulation.v).

\section{Quantum-Information Facts (proved inline)}

\textbf{Note}: The six quantum-information facts that anchor the Tsirelson bound derivation were originally proved in standalone files (\texttt{HardMathFactsProven.v} and \texttt{AssumptionBundle.v}), which are removed. The corresponding results are now proved directly in the active kernel files: \texttt{ClassicalBound.v} ($S \leq 2$ for local strategies), \texttt{BoxCHSH.v} ($S \leq 4$ for valid strategies), \texttt{AlgebraicCoherence.v} (symmetric coherence bound), \texttt{MinorConstraints.v} (Tsirelson from algebraic coherence), and \texttt{TsirelsonFromAlgebra.v} (tight Tsirelson bound). No standalone bundle or archive exists.
...
All files in the active proof tree pass the zero-admit check: there are no \texttt{Admitted}, \texttt{admit.}, or \texttt{Axiom} declarations beyond foundational logic. The only axioms are standard Coq library axioms (\texttt{functional\_extensionality\_dep}, \texttt{sig\_forall\_dec}, \texttt{sig\_not\_dec}). The six quantum-information facts originally parameterized in a standalone bundle are now proved directly in the active kernel files (see above).

\section{Compilation Status}

Compilation of the formal development serves as the definitive check that every theorem in this index is valid. The full build currently produces \texttt{.vo} targets with zero errors across the active source tree (counts reduced from original 286 targets and 292 files due to archival of exploratory modules), and the Inquisitor report scans the active Coq files with zero HIGH/MEDIUM/LOW findings.
