...
def _encode(opcode: int, op_a: int = 0, op_b: int = 0, cost: int = 0) -> int:
    return ((opcode & 0xFF) << 24) | ((op_a & 0xFF) << 16) | ((op_b & 0xFF) << 8) | (cost & 0xFF)
...
def to_binary(instructions: list[int]) -> bytes:
    """Pack instructions as little-endian 32-bit words."""
    return b"".join(struct.pack("<I", w) for w in instructions)


def to_hex(instructions: list[int]) -> str:
    """One 8-char uppercase hex word per line."""
    return "\n".join(f"{w:08X}" for w in instructions) + "\n"
