...
# WHY THIS EXISTS
# ---------------
# artifacts/print_assumptions_all_proofs.{json,csv,txt} is the repository's
# headline honesty artifact: it is what backs the "0 project-local axioms"
# badge and the stdlib-axiom counts quoted in the README. Until now it was
# regenerated by nothing. `coq/AssumptionsProbeAll.v` is deliberately excluded
# from `_CoqProject`, no Makefile target invoked the probe scripts, and both
# generators hardcoded an absolute path to one developer machine -- so the
# receipt could not be re-derived on any other checkout, and the test that
# "checked" it compared committed JSON against committed README prose (two
# files written together, not a re-derivation).
#
# This script closes that loop: probe generator -> coqtop over the whole
# corpus -> aggregator -> artifacts/. Run it via `make assumption-receipt`.
#
# Requires: coqc/coqtop on PATH and a fully built corpus (`make -C coq`),
# because Print Assumptions loads compiled .vo files.
set -euo pipefail
