...
- **Initiality** ([`coq/kernel/MuInitiality.v`](coq/kernel/MuInitiality.v)). μ is the *unique* canonical cost measure inside Thiele. Any other instruction-consistent, zero-starting, monotone cost functional over the Thiele ISA equals μ on every reachable state.
...
- **LASSERT and CERTIFY costs.** Forced by Shannon entropy reduction + description complexity (LASSERT) and A2 directly (CERTIFY). Both are substantive lower-bound proofs in [`coq/kernel/MuCostDerivation.v`](coq/kernel/MuCostDerivation.v) and [`coq/kernel/AbstractNoFI.v`](coq/kernel/AbstractNoFI.v).
- **PNEW / PSPLIT / PMERGE = 0.** A model-design choice consistent with **Bennett's reversible-computing principle** (Bennett 1973): reversible operations cost no energy, only time and trace-tape length. The Coq theorem [`partition_ops_zero_cost`](coq/kernel/MuCostDerivation.v) confirms internal consistency. It is essentially a definitional unfold. Whether the operations are physically reversible is the model assumption, not a Coq derivation.
- **Numerical calibration to physical units (Joules, kelvin).** A named bridge hypothesis (`mu_landauer_unruh_calibrated` in [`coq/kernel/NoFIToEinstein.v`](coq/kernel/NoFIToEinstein.v)). Not an axiom, not hidden.
...
The classical compute layer is preserved fully; structural operations are *added*, not *replaced*. Every classical program embeds directly into the Thiele VM ([`coq/kernel/TuringClassicalEmbedding.v`](coq/kernel/TuringClassicalEmbedding.v)) and leaves the structural layer untouched ([`coq/kernel/ClassicalConservativity.v`](coq/kernel/ClassicalConservativity.v)).
...
Any trace beginning with `vm_certified = false` and ending with `vm_certified = true` has strictly increased μ. ([`AbstractNoFI.v`](coq/kernel/AbstractNoFI.v), [`NoFreeInsight.v`](coq/kernel/NoFreeInsight.v))
...
There is no total function from `(mem, regs, pc)` that recovers `vm_mu` for every reachable VM state. ([`NecessityOfMuLedger.v`](coq/NecessityOfMuLedger.v), [`NecessityAbstract.v`](coq/kernel/NecessityAbstract.v))
...
Any instruction-consistent, zero-starting, monotone cost functional equals μ on every reachable state. ([`MuInitiality.v`](coq/kernel/MuInitiality.v))
...
There exist VM states with identical `(mem, regs, mu, pc)` that differ in ways no classical function can detect. ([`PartitionSeparation.v`](coq/kernel/PartitionSeparation.v))
...
Every classical/Turing program embeds into the Thiele VM ([`TuringClassicalEmbedding.v`](coq/kernel/TuringClassicalEmbedding.v), [`ClassicalConservativity.v`](coq/kernel/ClassicalConservativity.v)). And: there exist Thiele states reachable by structural instructions that no classical trace can reach ([`TuringStrictness.v`](coq/kernel/TuringStrictness.v), [`ShadowProjection.v`](coq/kernel/ShadowProjection.v)).
...
[`UniversalCertificationCost.v`](coq/kernel/UniversalCertificationCost.v) abstracts over everything (state type, instruction type, step function, cost function) and proves: if the cert-flip step costs ≥ 1 (A2), then any uncertified-to-certified trace pays ≥ 1 in total. Theorem: [`universal_nfi_any_substrate`](coq/kernel/UniversalCertificationCost.v#L108).
...
Formally: `column_contractive_iff_quantum_realizable` ([`QuantumPartitionPSD.v:227`](coq/kernel/QuantumPartitionPSD.v)), a biconditional with zero project-local axioms. The Tsirelson bound is then derived two independent ways:

1. From PSD of the zero-marginal NPA moment matrix via `npa_psd_iff_column_contractive` ([`QuantumPartitionPSD.v:183`](coq/kernel/QuantumPartitionPSD.v)) and the μ-ledger ↔ quantum bridge ([`MuLedgerQuantumBridge.v`](coq/kernel/MuLedgerQuantumBridge.v)). Uses quantum-information machinery, but no physics axioms.
2. From algebraic coherence alone via a Positivstellensatz Sum-of-Squares certificate ([`AlgebraicCoherence.v`](coq/kernel/AlgebraicCoherence.v)). Pure polynomial arithmetic. No quantum-information machinery either.
...
```
coq/kernel/VMStep.v                       (one normative semantics, in Coq)
   │
...
If you are reading this and you are equipped to be that reader, the load-bearing argument fits in three sections of the short thesis ([`thesis/short_thesis.pdf`](thesis/short_thesis.pdf), §1, §7, §19) plus the math spec abstract ([`thesis/thiele_machine_math_spec.pdf`](thesis/thiele_machine_math_spec.pdf)). The complete claim ledger is in [`coq/kernel/MasterSummary.v`](coq/kernel/MasterSummary.v). Falsification conditions are at the top of [`README.md`](README.md). The audit receipt is in [`artifacts/`](artifacts/).
...
| The long-form 13-chapter thesis | [`thesis/main.pdf`](thesis/main.pdf) |
| The audited claim ledger | [`coq/kernel/MasterSummary.v`](coq/kernel/MasterSummary.v) |
