...
Tests edge cases in mathematical computation:
...
# =============================================================================
# MATHEMATICAL ALGORITHMS (identical in both environments)
# =============================================================================

def safe_multiply(a: int, b: int) -> Tuple[int, int]:
    """Multiply large integers (Python handles arbitrary precision)."""
    ops = 1
    return a * b, ops
...
def binomial(n: int, k: int) -> Tuple[int, int]:
    """Compute n choose k using Pascal's identity."""
    if k > n or k < 0:
        return 0, 0
    if k == 0 or k == n:
        return 1, 1
...
def modular_inverse(a: int, mod: int) -> Tuple[int, int]:
    ...
    (g, x, y), ops = gcd_extended(a, mod)
    if g != 1:
        return -1, ops  # No inverse exists
    return x % mod, ops
...
def run_standard_python() -> Dict[str, Any]:
    """Run all math tests with standard Python."""
    results = {'runtime': 'Standard Python', 'tests': []}
    
    test_cases = [
        # Large integer multiplication (Python handles arbitrary precision)
        ...
        ("ModInv no inverse", lambda: modular_inverse(6, 9)),  # gcd != 1
    ]
...
def run_thiele_vm() -> Dict[str, Any]:
    """Run all math tests through Thiele VM."""
...
def compare_and_report() -> Dict[str, Any]:
    ...
    print(f"\n{'Test Name':<25} {'Std Result':<20} {'VM Result':<20} {'Match':<6}")
    print("-" * 75)
    ...
    for std_test, vm_test in zip(std_results['tests'], vm_results['tests']):
        ...
        std_res = str(std_test['result'])[:18]
        vm_res = str(vm_test['result'])[:18]
        status = "✓" if match else "✗"
        
        print(f"{std_test['name']:<25} {std_res:<20} {vm_res:<20} {status:<6}")
