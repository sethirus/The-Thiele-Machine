...
def run_verilog_trace():
    ...
    return run_verilog(program_text)
...
def run_vm_trace(program_path: Path) -> dict:
    ...
    return run_vm(instructions, fuel=10000)
...
class TestVerilogTraceAlignment:
    ...
    def test_verilog_compiles(self):
        """Kami RTL should compile via cosim gateway."""
        from thielecpu.hardware.cosim import run_verilog
        result = run_verilog("HALT 0")
        assert result is not None, "cosim.run_verilog should return a result on HALT"
