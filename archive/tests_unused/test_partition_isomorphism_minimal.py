...
def _rtl_regions_after_words(program_words: list[int]) -> list[list[int]]:
    ...
    for word in program_words:
        ...
        c = word & 0xFF
        if op == 0x00:
            lines.append(f"PNEW {{{a}}} {c}")
        elif op == 0x01:
            lines.append(f"PSPLIT {a} {b} {c}")
        elif op == 0x02:
            lines.append(f"PMERGE {a} {b} {c}")
        elif op == 0xFF:
            lines.append(f"HALT {c}")
    program_text = "\n".join(lines)

    payload = run_verilog(program_text)
    if payload is None:
        raise RuntimeError("RTL simulation unavailable")
