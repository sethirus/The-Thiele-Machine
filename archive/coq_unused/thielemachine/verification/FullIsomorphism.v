(** * Full Three-Layer Comparison Contract
(* INQUISITOR NOTE: proof-connectivity -- bridged to Thiele machine foundations. *)

    
  STATUS: active contract/specification layer; not a standalone proof that
  every Python and RTL artifact is unconditionally equal to Coq in CI.
    
  This module packages the observable-state contract used to compare three
  computational layers:
    ...
    COMPARISON PROPERTY (when backend-specific decoders satisfy the contract):
    For any instruction trace τ, the chosen observable decoders agree on the
    projected state tracked in this file.
    
    Here decode extracts a reduced observable state (μ, partitions, pc, halt).
    
    VERIFICATION STRATEGY:
    - Coq proves abstract properties (monotonicity, no-signaling)
    - Python tests provide evidence for Coq alignment
    - Verilog tests provide evidence for Python/RTL alignment when available
    - Transitivity is expressed here as an equality fact over observable states
    
    This file contains the Coq-side observable encoding that Python and Verilog
    comparison layers are expected to match.
*)
...
(** =========================================================================
  PART 5: OBSERVABLE EQUALITY LEMMAS
    =========================================================================*)
...
(** These definitions specify what Python and Verilog decoders must satisfy.
  Backend validation lives outside this file in tests and build artifacts. *)
...
(** Full comparison relation: all three decoders agree on the reduced
  observable state tracked here. *)
...
(** Transitivity: equality of observable projections composes. *)
...
(** This module provides:
    ...
    VERIFICATION:
    - This file: Coq proofs over the reduced observable-state contract
    - Python / RTL alignment evidence is external to this file
    
    This module completes the formal projection layer used for three-way comparison.
*)