...
Module ThieleKernelCausality.
  ...
  Definition instr_targets (s : VMState.VMState) (instr : VMStep.vm_instruction)
    ...
    | VMStep.instr_tensor_set mid _ _ _ _ => [mid]
    | _ => []
    end.
    ...
    - destruct (VMState.graph_lookup g_wo_both existing) as [existing_mod|] eqn:Hex in Hpm; try discriminate.
      ...
      rewrite (ThieleUnificationProtocol.graph_lookup_update_other
                ...
                {| VMState.module_region := VMState.module_region existing_mod;
                   ...
                   VMState.module_mu_tensor := VMState.module_mu_tensor existing_mod |}
                Hneq).
  ...
  Proof.
    ...
    - (* certify *)
      reflexivity.
    - (* tensor_set *)
      assert (other <> module) as Hneq.
      { intro Heq; subst. apply Hnotin. left. reflexivity. }
      destruct (Nat.ltb i 4); destruct (Nat.ltb j 4);
        simpl; try reflexivity.
      unfold VMState.graph_update_module_tensor.
      destruct (VMState.graph_lookup (VMState.vm_graph s) module) eqn:Hlookup.
      + apply VMState.graph_update_preserves_unrelated. exact Hneq.
      + reflexivity.
    - (* tensor_get *)
      destruct (Nat.ltb i 4); destruct (Nat.ltb j 4); simpl; reflexivity.
  Qed.
