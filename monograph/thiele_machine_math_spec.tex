...
\begin{itemize}
    ...
    \item $\texttt{MORPH\_ASSERT}(f, \mathit{prop}, \mathit{cert}, \Delta\mu)$: \textbf{Cert-setter.} Charges $S(\Delta\mu) \geq 1$ unconditionally. Even failed attempts (morphism does not exist) incur cost $1$. This is the strongest form of the NoFI law: the machine refuses to let even a doomed attempt at certifying a relational claim slip through for free.
...
\begin{itemize}
    \item A decision problem $f$ is \textbf{\texttt{mu\_budget\_decidable}} (with respect to $p_{\mathrm{fuel}}$ and $p_{\mu}$) iff a solver trace exists for every input: a trace of length $\le p_{\mathrm{fuel}}(\mathrm{size}(s))$ using at most $p_{\mu}(\mathrm{size}(s))$ units of $\mu$-cost that runs without error.
    \item A decision problem $f$ is \textbf{\texttt{mu\_budget\_verifiable}} iff every positive instance has a certificate trace verifiable within the same fuel and $\mu$-cost bounds, ending with \texttt{vm\_certified = true}.
\end{itemize}
\end{definition}

\begin{theorem}[Zero-$\mu$ classical solvers are $\mu$-budget decidable {\normalfont (kernel/MuComplexity.v)} --- \textbf{(S)}]
\texttt{classical\_mu\_budget\_decidable}: any decision problem solved by a zero-$\mu$ classical program within polynomial fuel is \texttt{mu\_budget\_decidable} with $\mu$-bound 0. Classical computation is a degenerate case of $\mu$-computation. Zero Admitted.
\end{theorem}

\begin{remark}
\texttt{mu\_budget\_decidable} and \texttt{mu\_budget\_verifiable} are predicates classifying decision problems by their polynomial-$\mu$ envelope, not classical complexity classes. The Coq file proves the basic inclusion (every decidable problem is verifiable, by re-running the solver and certifying). It does not prove a class separation. The factored-search witness in \texttt{StructuralAdvantage.v} gives a concrete program-level $\mu$-cost advantage (paying $\mu = 18$ to certify a decomposition and reduce search from $O(N^2)$ to $O(2N)$), but that is not a class separation. Earlier versions of this section named these predicates \texttt{in\_muP} and \texttt{in\_muNP}; the names were misleading because they evoked the classical P-versus-NP question, which this file does not address.
\end{remark}
...
\begin{enumerate}
  \item \textbf{Achievability}: there exists a trace from \texttt{init\_state} with $\mu$-cost exactly $k$ that is \emph{level-$k$ certified} --- the run executes a \texttt{CERTIFY}~$\delta$ instruction with $S(\delta) \geq k$ and ends with \texttt{vm\_certified = true}.
  \item \textbf{Lower bound}: any level-$k$ certified trace from \texttt{init\_state} must have spent at least $k$ units of $\mu$-cost.
\end{enumerate}
Formally (\texttt{mu\_hierarchy\_theorem}): for every $k \geq 1$, the witness trace $[\texttt{instr\_certify}(k{-}1)]$ (cost $= S(k{-}1) = k$) executes a level-$k$ \texttt{CERTIFY} from \texttt{init\_state}; conversely, the trace-side predicate \texttt{level\_k\_certified} forces $\texttt{trace\_mu\_cost} \geq k$ via \texttt{level\_k\_certification\_cost\_floor}, since the \texttt{CERTIFY}'s declared cost appears in the executed-instruction ledger and the ledger sum equals the trace cost (\texttt{run\_vm\_mu\_conservation}).
...
Theorem~\ref{thm:mu-hierarchy} is the $\mu$-analogue of the classical time hierarchy theorem. It is not a statement about classical P versus NP, which this project does not touch. In the certified-structure setting it makes one thing precise: \emph{finding} a certificate of structural complexity $k$ costs $\mu \geq k$, while \emph{verifying} it (given the trace) costs $0$ extra $\mu$. The hierarchy is strict and unbounded, with zero \texttt{Admitted}, no oracle, and no diagonalization. The lower-bound proof is trace-side: a level-$k$ certified run executed a specific \texttt{CERTIFY}~$\delta$ with $S(\delta) \geq k$, and \texttt{executed\_instruction\_cost\_recorded} plus \texttt{ledger\_sum\_contains\_lower\_bound} place that cost in the ledger sum, which \texttt{run\_vm\_mu\_conservation} equates with the trace's $\mu$-cost.
\end{remark}

\section{Intrinsic level hierarchy}

\begin{definition}[Intrinsic level {\normalfont (coq/IntrinsicLevelHierarchy.v)}]
A final state $s$ is \emph{intrinsically certifiable at level $\geq k$}, written $\texttt{level\_intrinsic\_at\_least}(k, s)$, iff every $\texttt{run\_vm}$ trace from \texttt{init\_state} that ends at $s$ with \texttt{vm\_certified = true} executes at least $k$ cert-setter instructions:
\[
\forall \mathit{fuel}\, \mathit{trace}.\;
\texttt{run\_vm}(\mathit{fuel}, \mathit{trace}, \texttt{init\_state}) = s \;\Rightarrow\;
s.\texttt{vm\_certified} = \texttt{true} \;\Rightarrow\;
\texttt{cert\_setter\_executions}(\mathit{fuel}, \mathit{trace}, \texttt{init\_state}) \geq k.
\]
The level is a property of the \emph{final state}, not of any particular trace reaching it.
\end{definition}

\begin{theorem}[Intrinsic level forces $\mu$ {\normalfont (coq/IntrinsicLevelHierarchy.v::level\_intrinsic\_forces\_mu)}]
If $\texttt{level\_intrinsic\_at\_least}(k, s)$ holds and $s.\texttt{vm\_certified} = \texttt{true}$, then for every trace reaching $s$ from \texttt{init\_state}, $s.\texttt{vm\_mu} \geq k$. Zero Admitted.
\end{theorem}

\begin{theorem}[Strict separation {\normalfont (coq/IntrinsicLevelHierarchy.v::level\_strict\_separation)}]
\label{thm:intrinsic-strict-separation}
For every $k \geq 0$, if $\texttt{level\_intrinsic\_at\_least}(k+1, s)$ holds and $s.\texttt{vm\_certified} = \texttt{true}$, then every trace reaching $s$ executes strictly more than $k$ cert-setter instructions. Zero Admitted.
\end{theorem}

\begin{remark}[Relation to Theorem~\ref{thm:mu-hierarchy}]
Theorem~\ref{thm:intrinsic-strict-separation} is the state-level counterpart of the trace-level $\mu$-hierarchy. The trace-level version (\texttt{mu\_hierarchy\_theorem}, \texttt{level\_k\_certification\_cost\_floor}) defines level on the trace --- a trace is level-$k$ certified iff it executes a \texttt{CERTIFY}~$\delta$ with $S(\delta) \geq k$. The state-level version defines level on the final state --- every trace reaching it requires at least $k$ cert-setter executions. The two are complementary entry points to the same hierarchy: the trace-level theorem gives the achievability witness ($[\texttt{instr\_certify}(k{-}1)]$ from \texttt{init\_state}); the state-level theorem gives the lower bound on $\mu$ for any state intrinsically requiring $k$ cert-events. Both are present-tense in the kernel; both close under the global context.
\end{remark}
...
\begin{itemize}
    \item \texttt{bare\_honest\_claim} : $\texttt{VMState} \to \texttt{Prop}$;
    \item \texttt{bare\_explains} : $\texttt{VMState} \to \texttt{BareTranscript} \to \texttt{Prop}$ (the transcript-honestly-witnessed-by-state relation);
    \item \texttt{bare\_cheap\_budget}, \texttt{bare\_recompute\_cost} : $\N$.
\end{itemize}
A bare verifier is a record \texttt{BareVerifier} with $\texttt{bv\_decide} : \texttt{BareTranscript} \to \texttt{bool}$ and $\texttt{bv\_cost} : \texttt{BareTranscript} \to \N$.
\end{definition}

\begin{definition}[Soundness, completeness, cheapness {\normalfont (coq/VerifierModel.v)}]
For a problem $P$ and verifier $V$:
\begin{itemize}
    \item \texttt{bare\_sound}$(P, V) := \forall t.\; V(t) = \texttt{true} \Rightarrow \forall s.\; \texttt{bare\_explains}(P, s, t) \Rightarrow \texttt{bare\_honest\_claim}(P, s)$.
    \item \texttt{bare\_complete}$(P, V) := \forall s\, t.\; \texttt{bare\_honest\_claim}(P, s) \Rightarrow \texttt{bare\_explains}(P, s, t) \Rightarrow V(t) = \texttt{true}$.
    \item \texttt{bare\_cheap}$(P, V) := \forall t.\; \texttt{bv\_cost}(V, t) \leq \texttt{bare\_cheap\_budget}(P)$.
\end{itemize}
The soundness condition is \emph{strong}: the claim must hold for every state explaining the transcript, not merely for some.
\end{definition}

\begin{theorem}[Bare-setting impossibility {\normalfont (coq/VerifierImpossibility.v::bare\_setting\_no\_sound\_complete\_verifier)}]
\label{thm:bare-verifier-impossibility}
There is no bare verifier simultaneously \texttt{bare\_sound} and \texttt{bare\_complete} on the verification problem $\texttt{mu\_eq\_one\_problem}$ whose claim is $\texttt{vm\_mu}(s) = 1$ and whose \texttt{bare\_explains} relation holds for the two $\texttt{NecessityOfMuLedger}$ witness states against the shared strict-shadow trace.
\end{theorem}

\begin{proof}
$\texttt{po1\_state\_A}$ (the cost-1 \texttt{CERTIFY 0} result, $\mu = 1$) and $\texttt{po1\_state\_B}$ (the cost-0 \texttt{PNEW [] 0} result, $\mu = 0$) project to identical strict-shadow traces. Completeness applied to $A$'s honest run forces $V$ to accept the shared transcript. Strong soundness applied to the same accepted transcript at the explanation by $B$ forces $\texttt{vm\_mu}(B) = 1$, contradicting $\texttt{vm\_mu}(B) = 0$. \texttt{Print Assumptions} returns \texttt{Closed under the global context}.
\end{proof}

\begin{theorem}[Three sufficient escapes {\normalfont (coq/VerifierEscape\_\{Substrate,Hardness,Interaction\}.v)}]
...
\begin{itemize}
    \item \texttt{substrate\_escape\_succeeds} --- transcript is $\texttt{list VMState}$; verifier reads $\mu$ directly.
    \item \texttt{hardness\_escape\_succeeds} --- transcript is $\texttt{BareTranscript} \times \texttt{bool}$ (commitment bit); verifier accepts the commitment under a parameterised \texttt{HardnessHypothesis} record.
    \item \texttt{interactive\_escape\_succeeds} --- transcript is $\texttt{BareTranscript} \times \N$ (response); verifier checks the response against the claim.
\end{itemize}
Each closes under the global context with zero \texttt{Admitted}.
\end{theorem}

\begin{theorem}[Factorisation impossibility {\normalfont (coq/VerifierExhaustiveness.v::V\_does\_not\_factor\_through\_classical)}]
...
A correlation tuple $(E_{00}, E_{01}, E_{10}, E_{11})$ is \textbf{slice-coherent} (column-contractive; called ``Thiele-honest'' in earlier versions, renamed because the predicate tests slice membership, not truthfulness) if:
\begin{align}
...
\[
\text{slice\_coherent}(\mathbf{E}) \iff \text{quantum\_realizable}(\mathbf{E})
\]
where \emph{slice\_coherent} means the three column-contractivity conditions hold, and \emph{quantum\_realizable} means the $5\times5$ zero-marginal NPA moment matrix is positive semidefinite.
...
\item Turing-classical correlations $(1,0,1,0)$ satisfy $|S|=2$ but fail condition 1: $1-1-1=-1<0$. A Turing machine can output this correlation; the slice gate rejects it as slice-incoherent (the full-elliptope gate of the next section accepts it --- the point is inside the elliptope, outside the zero-marginal slice). Direct arithmetic from \texttt{column\_contractive\_iff\_quantum\_realizable}.
\end{itemize}
...
\begin{itemize}
\item \texttt{deterministic\_strategy\_elliptope}: every deterministic sign strategy is elliptope-realizable, witnessed by its own cross moments (rank-one Gram argument).
\item \texttt{elliptope\_finite\_mixture} / \texttt{lhv\_mixture\_elliptope}: the set is closed under arbitrary finite sub-convex mixtures; every LHV correlator is inside.
\item \texttt{elliptope\_tsirelson}: every member satisfies $S^2 \leq 8$, via Cauchy--Schwarz inside the PSD form (\texttt{psd\_cauchy\_schwarz}); row-norm bounds provably fail on the full set.
\item \texttt{pr\_box\_not\_elliptope}: the PR box admits no completion.
\item \texttt{beyond\_classical\_elliptope}: $S = 12/5 > 2$ is attained inside, so the classical inclusion is strict.
\end{itemize}
\end{theorem}

The decidable gate (\texttt{coq/kernel/quantum/ElliptopeGate.v}) is the $\mathbb{Z}$-arithmetic check an opcode can branch on: a fraction-free Sylvester branch over supplied completion buckets (strict interior) and a rational $LDL^{\mathsf T}$-certificate branch reaching singular and boundary completions, with soundness (\texttt{elliptope\_check\_full\_sound}) into \texttt{elliptope\_realizable}, computed acceptances of the $\mu = 0$ tightness witness, $(1,0,1,0)$, and the on-Tsirelson-curve point $(3/5,4/5,4/5,-3/5)$ (\texttt{gate\_accepts\_pythagorean\_boundary}), and the universal refusal \texttt{elliptope\_full\_gate\_never\_accepts\_pr\_box}. Scope: correlator level; the only points beyond any exact integer check are those whose every PSD completion is irrational; the runtime opcode binding is future engineering.

\section{The Pointer-Observable Criterion}

\texttt{coq/kernel/frontier/PointerObservable.v} formalizes the schema behind the metering conjecture. An \textbf{ecosystem} (\texttt{Ecosystem}) is a state type carrying $n$ observers, each holding a fragment map over states; an event $E$ \textbf{redundantly proliferates} (\texttt{redundantly\_proliferating}) iff every observer's fragment records $E$; and $E$ is the \textbf{unique pointer among} a designated list of rival events (\texttt{unique\_pointer\_among}) iff $E$ proliferates and no listed rival does. Uniqueness is relative to the named rivals on purpose --- absolute uniqueness is false for free. A toy replicated-ledger instance shows the definitions are satisfiable and discriminating: the certification event is the unique pointer among the toy's two meterable events (\texttt{toy\_cert\_unique\_pointer}), a sanity instance only.

\texttt{PointerObservableReductions.v} instantiates the schema at the five deployed metering disciplines of the reductions tier --- proof-of-stake finality, gas metering, TEE attestation, certificate transparency, and proof-carrying verification. Each discipline's metered event is proved the unique pointer among its named rivals (\texttt{PoS\_unique\_pointer}, \texttt{Gas\_unique\_pointer}, \texttt{TEE\_unique\_pointer}, \texttt{CT\_unique\_pointer}, \texttt{PCC\_unique\_pointer}), and the conjunction is exported as \texttt{five\_disciplines\_are\_pointers}, which closes under the global Coq context --- no axioms at all, stdlib or otherwise.
...
\texttt{rtl\_coverage\_partition}: The 47 synth-realized opcodes (of the 51-opcode ISA) partition as:
\[ 37\ (\text{unconditional Qed}) + 10\ (\text{Qed under inductive invariants } \texttt{coupling\_wf},~\texttt{morph\_table\_wf}) + 0\ (\text{gaps}) = 47 \]
...
MORPH and COMPOSE can carry $\Delta\mu = 0$ and are not in the mandatory-cost class. The NoFI policy enters through $\texttt{MORPH\_ASSERT}$: asserting that a built morphism chain exists and activating the cert channel always costs $S(\Delta\mu) \geq 1$. Building the chain is free; certifying it never is.
\end{theorem}
...
$\mu$-budget predicates + zero-$\mu$ inclusion & \texttt{coq/kernel/mu\_calculus/MuComplexity.v} \\
\hline
...
\hline
Slice-coherent $\iff$ NPA realizable (biconditional) & \texttt{coq/kernel/quantum/QuantumPartitionPSD.v}: \texttt{column\_contractive\_iff\_quantum\_realizable} \\
...
Elliptope realizability (existential completion) & \texttt{coq/kernel/quantum/ElliptopeCompletion.v}: \texttt{elliptope\_realizable} (definition); \texttt{deterministic\_strategy\_elliptope}, \texttt{lhv\_mixture\_elliptope} (every LHV correlator inside) \\
Tsirelson for the full elliptope ($S^2 \le 8$) & \texttt{coq/kernel/quantum/ElliptopeCompletion.v}: \texttt{elliptope\_tsirelson} (inner step \texttt{psd\_cauchy\_schwarz}) \\
PR box excluded; classical $\subset$ elliptope strict & \texttt{coq/kernel/quantum/ElliptopeCompletion.v}: \texttt{pr\_box\_not\_elliptope}, \texttt{beyond\_classical\_elliptope} \\
Decidable elliptope gate (soundness) & \texttt{coq/kernel/quantum/ElliptopeGate.v}: \texttt{elliptope\_check\_full\_sound}; boundary acceptance \texttt{gate\_accepts\_pythagorean\_boundary}; universal refusal \texttt{elliptope\_full\_gate\_never\_accepts\_pr\_box} \\
\hline
\multicolumn{2}{l}{\emph{Frontier: pointer-observable criterion}} \\
\hline
Proliferation / unique-pointer schema & \texttt{coq/kernel/frontier/PointerObservable.v}: \texttt{redundantly\_proliferating}, \texttt{unique\_pointer\_among} (definitions); \texttt{toy\_cert\_unique\_pointer} (sanity instance) \\
Five deployed disciplines are unique pointers & \texttt{coq/kernel/frontier/PointerObservableReductions.v}: \texttt{PoS\_unique\_pointer}, \texttt{Gas\_unique\_pointer}, \texttt{TEE\_unique\_pointer}, \texttt{CT\_unique\_pointer}, \texttt{PCC\_unique\_pointer}; conjunction \texttt{five\_disciplines\_are\_pointers} (closed under the global context) \\
\hline
...
RTL Coverage Partition ($37 + 10 + 0 = 47$) & \texttt{coq/kami\_hw/RTLGapRegistry.v}: \texttt{rtl\_coverage\_partition} \\
...
Slice-Coherent $\iff$ NPA Realizable & \textbf{(S)} & Pure polynomial arithmetic; zero axioms \\
...
Five Disciplines Are Unique Pointers & \textbf{(S)} & Closed under the global context; no axioms \\
