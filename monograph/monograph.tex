...
\texttt{MORPH\_ASSERT} deserves special attention. It charges $S(\delta) \geq 1$ regardless of whether the assertion succeeds. This is the sharpest instance of No Free Insight: the \emph{attempt to claim structural knowledge costs something}, even when the claim is false. A failed assertion is not free. A machine without a structural ledger has no way to express this constraint. In the 51-opcode realisation it is enforced by the step semantics and tied back to the abstract theorem in \texttt{AbstractNoFI.v}.
...
\item Witness and certification (general + Q\textsubscript{1+AB} family): 7 ops (\texttt{CHSH\_TRIAL}, \texttt{CERTIFY}, \texttt{REVEAL}, \texttt{CHSH\_LASSERT\_1AB}, \texttt{CHSH\_LASSERT\_1AB\_G5}, \texttt{CHSH\_LASSERT\_1AB\_G345}, \texttt{CHSH\_LASSERT\_1AB\_G12345})
...
\item CHSH-aware certification: 1 op (\texttt{CHSH\_LASSERT})
\end{itemize}
...
\medskip
\noindent\textbf{Why identity?} Every object needs a self-loop, even the boring ones. A module with no connections to anything else still has to exist somewhere in the framework. \texttt{MORPH\_ID} is the instruction for that: ``$A$ to itself, coupling empty.'' Drop it and isolated objects fall outside the framework entirely. The category does not see them.
...
A morphism between two modules says: ``there is a named relationship between $A$ and $B$, with explicit coupling data.'' The coupling data is a list of (source-cell, target-cell) pairs. \texttt{COMPOSE} computes the relational composition: pair $(a, c)$ lands in the composed coupling exactly when there exists $b$ with $(a, b)$ in the first and $(b, c)$ in the second. Proven. Tracked. Via \texttt{MORPH\_GET} (selector 2), you can read back the coupling-pair count of any morphism.
...
Extra registers do not have provenance. You can write anything into a register. You can write 42 into register 5 and claim it is a morphism ID. But the VM checks: did a \texttt{MORPH} instruction actually run that created morphism 42 with the source and target you claim? If not, \texttt{MORPH\_ASSERT} fails. And \texttt{MORPH\_ASSERT} costs $S(\delta) \geq 1$ even if it fails.
...
This is why the morphism layer is not ``extra state.'' It is \emph{provenance-bearing state}. The trace records where every asserted morphism came from, and the substrate has a place to check that provenance. A classical machine cannot fake that at the same cost. That is the Categorical Separation Theorem.
...
\medskip
\noindent\textbf{Falsifier.} If you produce a function on the six shadow fields $(\texttt{vm\_regs}, \texttt{vm\_mem}, \texttt{vm\_mu}, \texttt{vm\_pc}, \texttt{vm\_err}, \texttt{vm\_certified})$ that returns different values on $s_1$ and $s_2$ above, \texttt{shadow\_strictly\_lossy} is wrong and the Coq proof in \texttt{coq/kernel/witness/ShadowProjection.v} won't compile. That is what falsifies this section.
...
Run \texttt{MORPH\_ASSERT}($h$, ``composed'', ``'', 0). Morphism $h$ exists and the property string ``composed'' is non-empty. The assertion succeeds. \texttt{csr\_cert\_addr} goes to the ASCII checksum of ``composed'' (nonzero). Cert-address channel activated. $\mu$ goes up by $S(0) = 1$.

(Why must the property string be non-empty? Because \texttt{csr\_cert\_addr} stores the ASCII checksum of the property string. If property = ``'', checksum = 0, and the channel stays at 0. Not activated. Any non-empty string works. ``composed'' is just an example label.)
...
Snapshot both programs at the end of Act 2, before Act 3's \texttt{MORPH\_ASSERT}. Program B takes a shortcut: it loads $r_0 = A$ and $r_1 = C$ via \texttt{LOAD\_IMM}. The six-field shadow is identical: same registers, $\mu = 0$ on both sides (Program A has not asserted yet either). No morphism $h$ was ever created in Program B.

Apply \texttt{MORPH\_DELETE}($h$): Program A (Act 2) succeeds. Program B fails and sets err.
