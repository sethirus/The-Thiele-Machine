...
At the kernel level, \texttt{LOAD} and \texttt{STORE} are register-indirect memory operations over \texttt{vm\_mem}. At the RTL/cosimulation layer, the partition wall adds active-module locality checks. \texttt{INIT\_PT} and \texttt{INIT\_ACTIVE\_MODULE} are harness setup commands for that finite hardware table, not VM opcodes and not fields of \texttt{VMState}. This is the honest split: the abstract ISA defines memory semantics; the bounded hardware layer enforces locality through its own table state.
...
\texttt{EMIT} and \texttt{READ\_PORT} live in the cert/revelation class but pay more than the $S(\delta)$ floor: they pay for every bit they touch. \texttt{EMIT} unfolds the payload into actual bits, charges those bits directly, then adds at least 1 for the revelation-class step. A one-byte ASCII payload (8 bits, $\delta = 0$) costs 9. Ten bytes (80 bits, $\delta = 0$) costs 81. You cannot emit more for the same price as less. \texttt{READ\_PORT} is the same shape on the way in: bits read, plus at least 1. The floor is still there. Zero-cost certification is still impossible. What this layer adds is that the cost tracks the actual bit content, not just the fact of the step.

\texttt{TENSOR\_SET} and \texttt{TENSOR\_GET} manage per-module metric tensor entries, storing the 4$\times$4 geometric weight matrix used by the morphism algebra. The kernel tensor is a flattened 4$\times$4 vector. The hardware has dedicated tensor state. Hardware and Coq agree step-for-step on both success and error paths: bad tensor indices latch error in both semantics; the success-path bisimulation requires $i, j < 4$ (and, for \texttt{TENSOR\_GET}, the module to exist in the graph), both of which are direct argument-validity checks. Covered in \texttt{GraphReconstructionBridge.v} and recorded in \texttt{RTLGapRegistry.v}.
