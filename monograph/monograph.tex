...
\smallskip
\noindent What I'm pointing at: flipping a certification bit from false to true should cost at least one. A classical step rule has nowhere to carry that as a rule of its transition, and nothing Turing-equivalent does either: equivalence is about which functions you compute, not about what the step is allowed to read, so the whole class is missing the field, not one machine at a time. It can only run a checker beside the program, and a checker can be skipped. If that's right, classical computation is the forced, lossy shadow of something that charges the flip: the projection down is clean, and getting back up needs an outside choice of the state that got dropped.

\smallskip
\noindent The part I lean on hardest is that the charging rule, A2, isn't something I picked. I argue it's the only rule that prices certification exactly, and two one-step traces are the whole argument. A rule that ever lets a certification flip go uncharged underprices it: run the trace that does exactly that flip, and it reports zero where the floor is one. A rule that ever charges a step that certifies nothing overprices it: run the trace that does exactly that step, and it bills for a certification that never happened. The only rule left fires on exactly the certification flips, and I think that's A2. If you can name a different one, a rule that prices certification exactly and isn't A2, then I'm wrong, and those two traces are why I don't think you'll find it.
...
\begin{abstract}
The Thiele Machine is a computational substrate where certification cost ($\mu$) lives in the step rule. The step rule is a small bureaucrat that refuses to advance the state until the cost has been counted. The program does not get a vote. The principal structural result is asymmetric, Coq-verified, zero axioms. Every Thiele state has a canonical forgetful projection onto a classical-equivalent state. Every classical state has multiple Thiele preimages and no canonical lift back. The projection is canonical. The lift is not. The fields the projection drops (\texttt{vm\_certified}, \texttt{csr\_cert\_addr}, $\mu$ relative to the bare three-field observable that drops it) are provably not functions of the classical projection (\code{cert\_not\_function\_of\_forget}, \code{cert\_addr\_not\_function\_of\_forget}, \code{mu\_not\_function\_of\_bare\_observable} in \code{coq/kernel/witness/ProjectionNonExistence.v}, all closed under the global context). Every Thiele machine has a Turing projection. No Turing machine has a canonical Thiele lift. The directionality is proved, not stipulated.

The categorical universal property is closed. The Thiele substrate is the initial object in the category of A2-respecting substrates over the \texttt{vm\_instruction} signature (\code{thiele\_is\_initial\_a2\_substrate} in \code{coq/kernel/nfi/ThieleInitiality.v}, zero axioms). That same universal property makes $\mathbb{Z}$ the initial ring, which is exactly why being initial settles nothing on its own: $\mathbb{Z}$ is initial too, and nobody thinks $\mathbb{Z}$ is the shadow of a deeper thing. The priority gets decided lower down, by the proofs. Hold the trust fixed and A2 is the only rule that prices certification exactly. Miss a cert-flip and the floor fails on a one-step trace. Charge a step that certifies nothing and you have overpaid. The one rule left fires on exactly the cert-flips, and that is A2 (\code{a2\_equal\_trust\_substitution\_payoff}). The state it runs on, $(\mu, \texttt{cert})$, is the proved-minimal extension over (mem, regs, pc): cert is not a function of the rest, $\mu$ is not a function of the rest (\code{P\_full\_is\_minimal\_complete\_extension}). So a step rule has a cert to read and a $\mu$ to charge, or it has no way to say A2 at all. A law that lives in the step is not a checker you run next to the program and hope. That is what makes classical computation the derivative thing, and it is the proofs that say so, not me. Whether the A2 category is the chosen one among every cost-tracking category you could write down is a question the proofs ask of nobody, and I am not asking it either.
...
\textbf{1. A2 constrains state fields the bare classical signatures do not have.} A2 is the rule. Every step that flips a certification predicate from false to true forces the executed instruction's cost $\geq 1$. The certification predicate and the cost function are not in the bare Turing-machine signature (state, alphabet, transition table). They are not in register machines. They are not in lambda calculi. Extend a classical signature with cert and cost fields plus a step rule that respects A2, and you have moved to a different substrate: the \code{CertificationSystem} record in \code{coq/kernel/nfi/UniversalCertificationCost.v}, over which \code{universal\_nfi\_any\_substrate} concludes a cost floor of $\geq 1$ on the certifying trace of \emph{any} substrate carrying the cert predicate. The floor is not a function of the substrate; it holds for every inhabitant of the record, which is what makes it universal rather than a one-off arithmetic bound. \code{cert\_not\_function\_of\_forget} (\code{ProjectionNonExistence.v}) sharpens the boundary. Thiele's certification flag is not a derived function of the bare classical projection. No clever predicate over the classical fields recovers A2's cert-predicate; the information just isn't down there to recover. The claim is not that A2 is unwritable in the absolute sense. The claim is that A2 constrains state fields the bare classical signatures lack, so any model carrying A2 is no longer the bare classical model. The migration is one-way.

\textbf{2. Therefore the substrate is forced.} Any model that formalises certification cost at the step rule must carry state that classical models do not have. There is no ``TM plus A2''. There is no A2 on a TM, by \code{cert\_not\_function\_of\_forget}. Write the cert flag onto the tape and you have a Thiele substrate hiding inside a TM, like a person crouched inside a piano. The piano is still a piano. The person is still a person. The substrate distinction does not care what you encoded it onto. The structural axis $(\mu, \texttt{cert})$ is the proved-minimal extension over (mem, regs, pc): neither field is a function of the rest (\code{P\_full\_is\_minimal\_complete\_extension}). That is the state A2 needs to be a sentence at all. Something to read the cert from. Something to charge the $\mu$ to. Take either away and the rule has nothing left to say. The substrate is not an extension of classical computation. It is what classical computation needed in order for the silent omission to be visible at all. The cost-ledger separation in Section~\ref{sec:categorical} depends on $\mu$ being a state field. \code{mu\_not\_function\_of\_bare\_observable} proves that separation has no analog in the bare classical signature.

\textbf{3. Classical computation is the projection, and the projection is not invertible.} \texttt{lift\_config} sends every Turing-machine configuration to a Thiele configuration. \code{thiele\_simulates\_turing} runs every Turing-machine trace inside the substrate, same tape, same state. \code{degenerate\_projection\_theorem} closes the loop in one direction. Classical computation is exactly the image of substrate computation under the projection that forgets the structural axis. \code{fiber\_has\_two\_preimages} (\code{coq/kernel/witness/BlindnessRepresentation.v}:238) closes the other direction. Every classical state has multiple Thiele preimages. Any lift back is non-canonical and requires external choice of cost schedule, graph state, and certification flag. \texttt{D4\_strictness} witnesses substrate states with no classical preimage. Same computable functions on both sides. Classical substrates are not a parallel option. They are the projection. Every Turing machine, every register machine, every lambda-calculus reducer, every CPU on every desk: classical state is a forgetful image of the Thiele fields. The claim is what survives the forgetting: the cost-on-certification rule lives in the step relation, and no classical signature has a field to carry it as a law. Hold the trust fixed and A2 is the only rule that prices the certification flip exactly (\code{a2\_equal\_trust\_substitution\_payoff}). The rule is forced; you don't pick it. The projection-and-lift asymmetry is real, but it is generic. Drop a field from any model and the same shadow falls. So it colors the picture without doing the work. The TM case is the one I proved (\code{thiele\_simulates\_turing}, \code{degenerate\_projection\_theorem}); the rest go the same way, and the shape is what carries the weight.

\textbf{4. What holds each step up.} Step 1 is a sentence about what a classical step relation can carry, witnessed by \code{cert\_not\_function\_of\_forget}. Step 2 follows from step 1, witnessed by \code{mu\_not\_function\_of\_bare\_observable}. Step 3 is mechanised in Coq, theorem names above. The asymmetry of projection versus lift is witnessed by \code{fiber\_has\_two\_preimages}. If a step reads like metaphor to you, the metaphor has to be living inside one of these witnesses, and every one of them is a Coq name you can open and check against what it says. Nothing in the chain does its work on a handshake. The names are where the weight sits.

\textbf{The one that got me too.} The instinct everyone has, me included: A2 can be enforced in software on a TM, so the substrate distinction is a hardware/software boundary, not a fundamental one. Here's the one sentence that pulled me out of it. A Turing machine cannot refuse to execute a buggy A2-simulator. A Thiele substrate cannot execute one. Load a Thiele simulator onto a TM with a bug, a program that certifies without incrementing $\mu$. The TM runs it faithfully because its step rule has no field for A2 and cannot detect the bug. Load the same buggy program onto a Thiele substrate. The step rule traps. It isn't that A2 sits inside the simulator as an interpreted step that could be skipped. A2 is the transition law itself. Subsumption is a step-rule claim, not a software-layer claim. ``Thiele is simulable on a TM'' is true and is not the question. ``Thiele's step rule can be written down on a TM'' is the question. The answer is no.
...
A1 through A6 are independent of any specific instruction set. They are conditions on Coq record types; supplying instances of those record types is the only way to invoke the corresponding theorems. The 51-opcode VM supplies them by construction: \code{coq/kernel/nfi/UniversalCertificationCost.v} builds two concrete \code{CertificationSystem} values for the VM (one for the \texttt{csr\_cert\_addr} channel, one for the \texttt{vm\_certified} channel), each discharging A2 from the existing \code{no\_free\_certification} kernel theorems. A separate substrate-level abstraction (the \texttt{Substrate} typeclass in \code{coq/kernel/foundation/Substrate.v}) carries a related but distinct field \texttt{mu\_monotone} stating that the cost ledger never decreases on a step. \texttt{mu\_monotone} is a strictly weaker general-purpose monotonicity property than A2: A2 specifically asserts the cert transition costs $\geq 1$, whereas \texttt{mu\_monotone} only asserts the ledger does not go down. The two coincide on the 51-opcode VM, where every instruction adds non-negative cost and the cert transition adds at least 1. Two locks on the same door, by different keys.
...
\end{tikzpicture}
\caption{The directional structural theorem. Every Thiele state has a single canonical projection down via \code{shadow\_proj} (\code{degenerate\_projection\_theorem} in \code{coq/kernel/foundation/TuringClassicalEmbedding.v}). The classical state has multiple Thiele preimages and no canonical lift back (\code{fiber\_has\_two\_preimages} in \code{coq/kernel/witness/BlindnessRepresentation.v}). One direction is a function; the other is not.}
...
Here is what that means in the kernel. \code{CertificationSystem} (defined in \code{coq/kernel/nfi/UniversalCertificationCost.v}) bundles a state, an instruction set, a step rule, a cost function, a certification predicate, and a proof of A2. That proof is not commentary sitting next to the type. It is one of the fields. Here is the record body, copied verbatim:
...
\begin{theorem}[D2 faithfulness, \code{coq/kernel/foundation/TuringClassicalEmbedding.v:106}]
...
\begin{theorem}[D3 conservativity, \code{coq/kernel/foundation/ClassicalConservativity.v:204}]
...
\begin{theorem}[Thiele simulates Turing, \code{coq/kernel/foundation/ProperSubsumption.v:172}]
...
\begin{theorem}[Degenerate projection theorem (four-part), \code{coq/kernel/foundation/TuringClassicalEmbedding.v:524}]
...
The subsumption theorems above establish what the Thiele substrate carries over and above its classical projection: fields the projection drops. The natural next question is whether a classical observer could reconstruct those fields by some clever derived predicate on the surviving shadow. The answer is no, and I want to be honest about what kind of no it is: a structural no, the sort you can't argue your way around, not me waving my hands. \code{coq/kernel/witness/ProjectionNonExistence.v} proves three irrecoverability theorems (each one a non-existence of a function from the bare classical projection back to a Thiele-only field) and packages the corollaries the subsumption argument relies on. \texttt{Print Assumptions} on every theorem in the file returns \texttt{Closed under the global context}: zero axioms, zero admits, constructive proofs.

The projection in this subsection is the four-field map $\texttt{forget} : \texttt{VMState} \to \texttt{TMSnapshot}$ from \code{BlindnessRepresentation.v}, keeping $(\texttt{pc}, \mu, \texttt{regs}, \texttt{mem})$. It is already forgetful, but it still retains $\mu$. For the cost-ledger separation, the stricter three-field projection $\texttt{bare\_observable}$ defined in \code{ProjectionNonExistence.v} drops $\mu$ as well, keeping only $(\texttt{pc}, \texttt{regs}, \texttt{mem})$: the observable the standard complexity-theory cost model sees.
...
\begin{proof}
The witnesses \code{mu\_separation\_witness\_zero} and \code{mu\_separation\_witness\_one} (defined in \code{ProjectionNonExistence.v}) differ only in their $\mu$ field (0 vs 1) and have identical bare observables. A $\phi$ satisfying the equation would force $0 = 1$. Contradiction. \qed
\end{proof}
...
\begin{proof}
The witnesses \code{cert\_addr\_witness\_zero} and \code{cert\_addr\_witness\_one} (defined in \code{ProjectionNonExistence.v}) differ only in their \texttt{csr\_cert\_addr} field (0 vs 1) and have identical \texttt{forget} images. A $\phi$ satisfying the equation would force $0 = 1$. Contradiction. \qed
\end{proof}
...
\begin{enumerate}
\item \emph{Substrate-trust} (\code{coq/VerifierEscape\_Substrate.v}). The transcript carries the full \texttt{VMState} and the verifier reads $\mu$ directly. \code{substrate\_escape\_succeeds} exhibits a verifier with constant cost that is both sound and complete on $\texttt{vm\_mu}\ s = 1$. The verifier's decision function is \texttt{Nat.eqb (last t).vm\_mu 1}; the proof is one inhabitation per quantifier.

\item \emph{Hardness} (\code{coq/VerifierEscape\_Hardness.v}). The transcript carries an unforgeable commitment; the verifier accepts the commitment under a hardness hypothesis. \code{hardness\_escape\_succeeds} exhibits a constant-cost weak-sound verifier conditional on a \texttt{HardnessHypothesis} record. The record is a parameter the verifier is constructed over, not an axiom; whether the hypothesis is realised in nature is a question for cryptography, not this kernel.

\item \emph{Interaction} (\code{coq/VerifierEscape\_Interaction.v}). The verifier elicits a response from the prover that pins the claim. \code{interactive\_escape\_succeeds} exhibits a constant-cost sound and complete verifier when the response set is rich enough to report $\mu$. This is the simplest possible interactive model --- one round, one fixed query --- and that is all the corollary needs.
\end{enumerate}
...
\begin{proof}
If $V$ factored through $\pi$, then $V(t_1) = V(t_2)$ whenever $\pi(t_1) = \pi(t_2)$. The bare-setting witness-state collision lifts to $T$ as a pair with this property. Running the bare-setting argument on the factored $V$ reproduces the contradiction. The Coq source is \code{coq/VerifierExhaustiveness.v}; \texttt{Print Assumptions} returns \texttt{Closed under the global context}. \qed
\end{proof}
...
\emph{(1) $\mu \perp (\texttt{mem}, \texttt{regs}, \texttt{pc})$.} Same visible state, different ledger. One branch runs a $\delta = 0$ no-op like $\texttt{ADD}\,0\,0\,0\,0$. The other uses $\texttt{EMIT}$ with $\delta = 0$. Registers, memory, and pc match. $\mu$ does not: $9$ versus $0$. The Coq witness in \code{coq/kernel/nfi/NecessityAbstract.v} constructs it.
...
\medskip
\noindent\textbf{What the package looks like in the concrete \texttt{n=1} witness.} Two kernel files do the instantiation: \code{coq/kernel/nfi/StructuralAdvantageObservedShortcut.v} and \code{coq/kernel/nfi/StructuralAdvantageCertifiedShortcut.v}. Both target the smallest factored-search case. The record \code{SoundStructuralShortcut} lives at \code{coq/kernel/nfi/HonestNoFI\_TheoremsWithoutAssumptions.v:501--546}. It carries fifteen fields. Seven are data: decoder, observation fn, representative observation fn, observation equality, prior set, posterior set, decision tree. Eight are proof obligations on those data. The nine semantic ingredients below cover all seven data fields plus two of the proof obligations: strict narrowing and initial-uncertified state. The remaining six obligations are discharged inline by the wiring in \code{StructuralAdvantageCertifiedShortcut.v}. Fifteen fields, nine ingredients, six wiring obligations. Every part accounted for.
...
\item \textbf{Decision tree.} \texttt{sighted\_n1\_tree := dt\_branch dt\_leaf dt\_leaf} (defined in \code{coq/kernel/nfi/StructuralAdvantageObservedShortcut.v:53--54} and reused by the certified-shortcut file). A two-leaf branching tree, one branch per factor.
...
The Coq lemma \code{sound\_shortcut\_from\_components} lives at \code{coq/kernel/nfi/HonestNoFI\_TheoremsWithoutAssumptions.v}, lines 637--679. It constructively builds a \code{SoundStructuralShortcut} record from seven data parameters (the seven data fields above) plus eight proof hypotheses discharging the eight obligations. The closed term \code{factored\_n1\_shortcut : SoundStructuralShortcut 33 sighted\_n1\_supra\_trace init\_state} in \code{coq/kernel/nfi/StructuralAdvantageCertifiedShortcut.v} wires through \code{sound\_shortcut\_from\_components}. It provides the seven data parameters above. It discharges each of the eight hypotheses by a named lemma. The downstream theorem \code{factored\_n1\_shortcut\_lands\_in\_representation} in the same file applies \code{every\_sound\_structural\_shortcut\_lands\_here} to the closed term. It reads off the conclusion: strictly stronger posterior predicate, structure-addition event, and the $\log_2^{\uparrow}$ narrowing bounded by $\Delta\mu$. The class is constructively non-empty.
...
Three tiers, three prices. The kernel proves the split. File: \code{WitnessInsightGeneral.v}.
...
For \textbf{pure compute programs}, meaning programs that do arithmetic, load and store values, and jump, every instruction carries $\delta = 0$. The program I use as the concrete comparison case for the Turing Strictness theorem (\texttt{blind\_program} in \code{StructuralAdvantage.v}) has this property: every instruction sits at $\delta = 0$. The proof that its total $\mu$-cost is zero is a direct calculation. For these programs, $\mu$ stays at 0 unless a positive-cost instruction fires.
...
Why $+1$ and not $+2$ or no floor? Because $+1$ is the minimum that makes certification impossible for free. $\delta + 0$ would allow $\delta = 0$ to certify for free. $\delta + 2$ would be overcharging. $\delta + 1$ is the unique honest minimum: you can always choose $\delta = 0$ and pay exactly 1. You can choose $\delta > 0$ and pay more. The proof that $S(\delta)$ is the only consistent cost floor is in \code{MuCostDerivation.v} (\texttt{cost\_necessity} + \texttt{cost\_uniqueness}).
...
This is forced, not chosen. There's no way to validate a 1000-bit formula on a 10-bit budget; the cost counts the formula you actually present, byte for byte, not the formula you wished you'd encoded. The proof that this is the unique minimum cost satisfying that constraint is in \code{MuCostDerivation.v} (\texttt{cost\_necessity} + \texttt{cost\_uniqueness}).
...
\texttt{TENSOR\_SET} and \texttt{TENSOR\_GET} manage per-module metric tensor entries, storing the 4$\times$4 geometric weight matrix used by the morphism algebra. The kernel tensor is a flattened 4$\times$4 vector. The hardware has dedicated tensor state. Hardware and Coq agree step-for-step on both success and error paths: bad tensor indices latch error in both semantics; the success-path bisimulation requires $i, j < 4$ (and, for \texttt{TENSOR\_GET}, the module to exist in the graph), both of which are direct argument-validity checks. Covered in \code{GraphReconstructionBridge.v} and recorded in \texttt{RTLGapRegistry.v}.
...
The base opcode \texttt{CHSH\_LASSERT mu\_delta} reads the eight buckets of \texttt{vm\_witness} directly, computes the integer-arithmetic check \code{column\_contractive\_check\_witness} (defined in \code{coq/kernel/foundation/VMStep.v} using only $\mathbb{Z}$ arithmetic on the same/diff counts), and either advances the program counter on success or traps to \texttt{LASSERT\_TRAP\_PC} and latches \texttt{vm\_err} on failure. The cost is $S(\mathit{mu\_delta}) \geq 1$ regardless of outcome. Mandatory-floor discipline: you pay to check, even to fail. The bridge theorem \code{chsh\_lassert\_no\_trap\_implies\_quantum\_realizable} (\code{coq/kernel/quantum/QuantumPartitionPSD.v}) proves that any no-trap, no-err-flip execution of \texttt{CHSH\_LASSERT} implies the witness-derived $5\times5$ NPA moment matrix is PSD (i.e., quantum-realizable at NPA level 1). The chain in plain order: the integer check on the witness counters passes; that fact lifts to real-valued column-contractivity by \code{column\_contractive\_check\_witness\_sound} in \code{coq/kernel/nfi/MuLedgerQuantumBridge.v}; column-contractivity is biconditional with NPA-PSD by \code{column\_contractive\_iff\_quantum\_realizable}. Three theorems. No physics input. The opcode is the place in the kernel where the algebra of Section~\ref{sec:chsh} becomes something the VM actually refuses to do, not just declines to recommend.

The four Q\textsubscript{1+AB} extensions \texttt{CHSH\_LASSERT\_1AB} (tag 0x2F, $\gamma = 0$), \code{CHSH\_LASSERT\_1AB\_G5} (tag 0x30), \code{CHSH\_LASSERT\_1AB\_G345} (tag 0x31), and \code{CHSH\_LASSERT\_1AB\_G12345} (tag 0x32) lift the same discipline from the level-1 $5\times5$ matrix to the level-1+AB $9\times9$ NPA moment matrix. Each runs its own integer-arithmetic Schur-cascade check on \texttt{vm\_witness} plus the supplied $\gamma$-bucket pairs, advances on pass or traps on fail, charges $S(\mathit{mu\_delta}) \geq 1$. The four bridge theorems (one per slice, named \code{chsh\_lassert\_1ab\_*\_no\_trap\_implies\_quantum\_realizable\_q1ab} in \code{coq/kernel/quantum/QuantumPartitionPSD\_1AB.v}) deliver \code{quantum\_realizable\_q1ab} = symmetric9 $\wedge$ PSD9 on the witness-derived rationals. The mathematical detail of the four-slice Schur cascade is in \S\ref{sec:chsh}'s Q\textsubscript{1+AB} subsection below.
...
\medskip
\noindent\textbf{Why identity?} Every object needs a self-loop, even the boring ones. A module with no connections to anything else still has to exist somewhere in the framework. \texttt{MORPH\_ID} is the instruction for that: it builds an $A \to A$ morphism, stores an empty coupling, and flags it \texttt{is\_identity}. That flag is load-bearing, and here is why it has to be. An empty coupling would \emph{annihilate} under relational composition --- compose anything with the empty relation and you get the empty relation back, not the thing you started with --- so the identity cannot be carried by the coupling data. It is carried by the flag, which \texttt{COMPOSE} honors: composing an identity-flagged arrow with $f$ returns $f$'s coupling unchanged, so $\text{id}_A \mathbin{;} f = f$ and $f \mathbin{;} \text{id}_B = f$ hold at the coupling level (\code{graph\_compose\_morphisms\_coupling} in \code{coq/kernel/category/CategoryBridge.v}; the matching hardware path is \code{driven\_step\_compose} in \code{coq/kami\_hw/GraphReconstructionBridge.v}). Drop it and isolated objects fall outside the framework entirely. The category does not see them.
...
The probe used in \code{ShadowProjection.v} is deliberately boring: \texttt{ADD 0 0 0 0}. It is not a morphism operation. That is the point. The theorem is not relying on a failing probe to manufacture a difference. The difference is already in the structural state, and ordinary classical projection cannot see it.

\begin{corollary}[Classical observer is blind, \code{ShadowProjection.v}]
...
\begin{corollary}[Thiele strictly extends classical, \code{TuringStrictness.v}]
\label{cor:strict}
...
The trust boundary at this layer is named explicitly. \code{coq/kernel/hardware\_bridge/OCamlExtractionBridge.v} proves formal facts about the Coq-side observable (\texttt{shadow\_to\_eo (vm\_apply s i)} strips a state down to its observable fields after one instruction): exact $\mu$ accounting, $\mu$ never decreases, the function is total. The file also names the external runner boundary. The Coq lemma \code{ocaml\_runner\_agrees} is a Coq-side reflexive witness (a proof that one Coq expression equals itself, useful as a placeholder), not an unproved \texttt{Hypothesis} (Section~\ref{sec:vocab}). The cross-language claim, that the built OCaml binary returns the same observable as the Coq semantics on every input, is checked by the parity test suite (Section~\ref{sec:vocab}: run both on the same battery, verify they agree). I do not count that as a kernel theorem. It is an audited boundary, backed by tests, named in the source.
...
\item \textbf{6 opcodes require valid instruction arguments.} The hardware and Coq model AGREE on the error path for invalid arguments, so no semantic disagreement exists, but the success-path proof requires the argument to be in range:
 ...
 \end{itemize}
 Covered in \texttt{EmbedStep\_WF.v} (\texttt{CALL}, \texttt{RET}, \texttt{CHSH\_TRIAL}) and \code{GraphReconstructionBridge.v} (\texttt{TENSOR\_SET}, \texttt{TENSOR\_GET}, \texttt{LASSERT}).
...
So: maximum classical win rate = 3/4 = 75\%. Proven by exhaustion in \code{coq/kernel/foundation/ClassicalBound.v:165} (zero Admitted): an executable trace using only \texttt{PNEW}, \texttt{PSPLIT}, and \texttt{CHSH\_TRIAL} with $\mu = 0$ achieves exactly this, and \code{coq/kernel/quantum/MinorConstraints.v:822} (\code{local\_box\_CHSH\_bound}) proves no $\mu = 0$ program can exceed it.
...
This is proven in Coq in \code{coq/kernel/quantum/MinorConstraints.v:822} (zero Admitted, \code{local\_box\_CHSH\_bound}). The formal statement: for any factorizable box B, $|S(B)| \leq 2$. \code{coq/kernel/foundation/ClassicalBound.v:165} (\code{classical\_bound\_achieved}) gives the matching constructive witness: an executable trace at $\mu = 0$ reaching $|S| = 2$. Combined, the two say the classical bound is tight: $\max\{|S| : \mu = 0\} = 2$ exactly.
...
Tightness: the strategy $A_x \equiv 1$, $B_y \equiv 1$ gives $\langle A_0 B_0 \rangle = \langle A_0 B_1 \rangle = \langle A_1 B_0 \rangle = \langle A_1 B_1 \rangle = 1$, so $S = 1 + 1 + 1 - 1 = 2$. This is achievable at $\mu = 0$ by a program with all $\delta = 0$ instructions accumulating one trial per setting pair in \texttt{vm\_witness}. The Coq witness is \code{classical\_bound\_achieved} at \code{coq/kernel/foundation/ClassicalBound.v:165}. \qed
\end{proof}
...
These are the three conditions of \code{zero\_marginal\_column\_contractive} (defined in \code{coq/kernel/nfi/MuLedgerQuantumBridge.v}): column 0 norm $\leq 1$, column 1 norm $\leq 1$, determinant $AC - B^2 \geq 0$.
...
The forward direction (\code{zero\_marginal\_npa\_column\_contractive\_implies\_psd} in \code{coq/kernel/nfi/MuLedgerQuantumBridge.v}) was proved first. The reverse direction: given a PSD NPA matrix, specialize it at the vector $(0, -(E_{00}y_0 + E_{01}y_1), -(E_{10}y_0 + E_{11}y_1), y_0, y_1)$. The PSD condition gives a quadratic-in-$y_0/y_1$ that is everywhere non-negative. Then \code{quadratic\_nonneg\_discriminant} ($\forall t, a + 2bt + ct^2 \geq 0 \Rightarrow b^2 \leq ac$) closes the determinant condition. The diagonal conditions come from $y_0 = 1, y_1 = 0$ and $y_0 = 0, y_1 = 1$. Both directions are packaged as the biconditional \code{column\_contractive\_iff\_quantum\_realizable} in \code{coq/kernel/quantum/QuantumPartitionPSD.v}.
...
\item Tsirelson bound: \code{state\_column\_contractive\_implies\_tsirelson} (\code{coq/kernel/nfi/MuLedgerQuantumBridge.v}) proves Thiele-honest $\Rightarrow$ $|S|^2 \leq 8$ (i.e., $|S| \leq 2\sqrt{2}$) directly from column-contractivity. The chain factors through PSD: column-contractive $\Rightarrow$ NPA-PSD $\Rightarrow$ row bounds $\Rightarrow$ Tsirelson, and the row-bounds step is \code{tsirelson\_from\_row\_bounds} in \code{coq/kernel/quantum/TsirelsonGeneral.v}. The contrapositive ($|S| > 2\sqrt{2}$ $\Rightarrow$ not Thiele-honest) follows immediately. The converse is false and not proved.
\end{itemize}
...
\code{coq/kernel/foundation/VMStep.v}) is the kernel-level enforcement
...
\begin{theorem}[Integer-check soundness, \code{column\_contractive\_check\_witness\_sound} in \code{coq/kernel/nfi/MuLedgerQuantumBridge.v}]
...
\begin{theorem}[CHSH\_LASSERT $\Rightarrow$ NPA-PSD, \code{chsh\_lassert\_no\_trap\_implies\_quantum\_realizable} in \code{coq/kernel/quantum/QuantumPartitionPSD.v}]
...
\end{tikzpicture}
\caption{The Schur cascade lifts the base-case sym4 PD-interior fact from Slice C all the way up to PSD$_9(M)$. Each arrow is one Schur-complement reduction, formally a chain of polynomial identities provable by \code{ring}. Bridge counts beside each arrow are the number of kernel theorems that compose to lift integer Schur entries into the real-valued $q_{12345}$ polynomial.}
...
\begin{itemize}
\item 21 \emph{H bridges} \code{cleared\_g12345\_HXX\_Z\_bridge}, one per upper-triangle entry of $M_{6\times6}$. Each lifts a cleared integer numerator (a polynomial in the witness bucket counts and $\gamma$-bucket pairs, cleared by the common denominator
$\text{KZ} = \bigl(N_{00} N_{01} N_{10} N_{11} \cdot D_{g_1} D_{g_2} D_{g_3} D_{g_4} D_{g_5}\bigr)^2$)
to the real-valued $q_{12345}$ polynomial in $(E_{ij}, \gamma_k)$.
\item 15 \emph{S6 cascade bridges} \code{cleared\_g12345\_S6\_IJ\_Z\_bridge} for $I,J \in \{2,\ldots,6\}, I \leq J$. Each lifts the integer Schur step on cleared-H entries to the real Schur-complement entry on $q_{12345}$ polynomials.
\item 10 \emph{S5 cascade bridges} \code{cleared\_g12345\_S5\_IJ\_Z\_bridge} for $I,J \in \{2,\ldots,5\}, I \leq J$. Each composes two S6 bridges via \code{schur\_step\_Z\_IZR} to reach the level-2 Schur complement.
...
\end{itemize}
The headline corollary \code{q1ab\_g12345\_caller\_witness\_z\_abs\_implies\_psd9} composes the four bridge layers; the no-trap wrapper \code{chsh\_lassert\_1ab\_g12345\_no\_trap\_implies\_quantum\_realizable\_q1ab} packages the result as \code{quantum\_realizable\_q1ab} (= symmetric9 $\wedge$ PSD9). \texttt{Print Assumptions} on either reports only the two standard-library axioms \code{ClassicalDedekindReals.sig\_forall\_dec} and \code{FunctionalExtensionality.functional\_extensionality\_dep} the rest of the Q\textsubscript{1+AB} kernel uses: no project-local axiom, no \texttt{Hypothesis}, no \texttt{Parameter}.
...
\]
The Coq witness is \code{total\_irreversible\_bits\_le\_cost} in \code{coq/kernel/nfi/LandauerDerivation.v:241}.
\end{theorem}
...
\begin{theorem}[$\mu$-Hierarchy (\code{mu\_hierarchy\_theorem} in \code{coq/kernel/mu\_calculus/MuHierarchyTheorem.v:162})]
For every $k \geq 1$:
\begin{enumerate}
...
\]
From \code{well\_formed\_triangulated}: the structural invariants $E = I + B$ and $3F = 2I + B$ combine to give $3V = 5E - 6F$ (the triangulation-combinatorial identity at \code{DiscreteTopology.v:651}). Solving for $V$: $V = (5E - 6F)/3$. Substituting:
\[
...
The file \code{coq/kernel/foundation/Substrate.v} defines a Coq typeclass capturing exactly what an A2-respecting computational substrate is, divorced from any particular instruction set. The typeclass exposes:
...
\item A cost-ledger monotonicity field \texttt{mu\_monotone}: \texttt{forall p s s', step p s s' $\to$ mu s $\le$ mu s'}. This is a strictly weaker general-purpose monotonicity property than the kernel's A2 (which specifically asserts that a step flipping certification false-to-true costs $\geq 1$, and which is carried by the \code{CertificationSystem} record in \code{coq/kernel/nfi/UniversalCertificationCost.v}, not by this typeclass). The two coincide on the 51-opcode VM where all instruction costs are non-negative and the cert transition costs $\geq 1$. See Section~\ref{sec:axioms} for the A2 statement and the related axioms A3 through A6.
...
The minimal nat-coded substrate \texttt{nat\_substrate} (in \code{coq/kernel/foundation/NatSubstrateInstance.v}) discharges every typeclass field unconditionally, no \texttt{Hypothesis} and no \texttt{Axiom}. Its programs are natural numbers; three program codes have meaning (\texttt{0}~=~yes, \texttt{1}~=~no, \texttt{2}~=~the self-aware flip fixed point); \texttt{run} is a Coq \texttt{Fixpoint}; \texttt{Representable f} is the predicate ``\texttt{f} is the diagonal flip transformer for the substrate's own decide function''; \texttt{recursion\_theorem} is discharged by exhibiting code \texttt{2} as the fixed point of any such transformer. \code{Print Assumptions nat\_recursion\_theorem} is \texttt{Closed under the global context}. The substrate-level limitative theorem fires unconditionally for this substrate.

The 51-opcode VM substrate \texttt{vm\_substrate} (in \code{coq/kernel/foundation/VMSubstrateInstance.v}) is parameterized over the VM-specific Gödel encoding (\texttt{vm\_encode}, \texttt{vm\_decode\_safe}, the round-trip), the VM's internal-representability predicate (\texttt{vm\_representable}), and the VM's internal recursion theorem restricted to representable transformers. The Coq Section discipline is identical to \texttt{BekensteinBound.v}'s use for physical-constant positivity: the parameters are not global axioms; closing the surrounding Section discharges them as explicit \texttt{forall} premises on every consumer. The encoding piece is a closed lemma (\code{coq/kernel/foundation/VMInstructionEncoding.v} provides \texttt{program\_to\_nat}, \texttt{nat\_to\_program}, and the round-trip with no Hypothesis). The VM's internal-representability predicate and internal recursion theorem are the VM-instance-specific Section parameters; the substrate-level result does not depend on them, because \texttt{nat\_substrate} discharges the limitative theorem unconditionally at the substrate level.

The VM-side \texttt{AdmitsShortcut} is the extensional predicate ``this VM program's bounded run from \texttt{init\_state} coincides with the bounded run of \code{simple\_morph\_trace}.'' \texttt{yes\_program} is \code{simple\_morph\_trace} itself (defined at \code{coq/kernel/nfi/SimpleMorphShortcut.v:32}); \texttt{no\_program} is the empty trace (which produces \texttt{init\_state}, demonstrably distinct from \code{simple\_morph\_final} at \code{coq/kernel/nfi/SimpleMorphShortcut.v:38} since the latter has fired the supra-cert channel and the former has not). The extensionality field is the standard ``equal \texttt{run} on every state implies the predicates agree.'' The instantiation that assembles these into the \code{WithShortcutPredicate} interface lives in \code{coq/kernel/nfi/StructuralUndecidability.v}.

\subsection{The substrate-level limitative theorem}

\begin{theorem}[Structural-axis undecidability, substrate-level (\code{coq/kernel/nfi/StructuralUndecidability.v:127--164})]
...
\begin{theorem}[Structural-axis undecidability for the nat-coded substrate (\code{coq/kernel/foundation/NatSubstrateInstance.v})]
...
\begin{theorem}[Structural-axis undecidability for the 51-opcode VM (\code{coq/kernel/nfi/StructuralUndecidability.v})]
...
\item Exhibit a substrate that has both an internal recursion-theorem witness for a representable transformer and a non-trivial extensional predicate, but for which the diagonal construction does not produce a contradiction. The proof body is about 38 lines (\code{coq/kernel/nfi/StructuralUndecidability.v:127--164}), roughly 16 of them actual tactic steps; finding such a substrate means finding an error in those tactic steps.
\end{enumerate}
...
Current Inquisitor status (250 Coq files scanned): zero HIGH findings, zero MEDIUM, and exactly one LOW. The LOW is a vacuity-score warning on \code{coq/kami\_hw/F4\_VerilogEvaluator.v}, flagged because the file matches a heuristic for ``constant function'' patterns. That file is the F4 reference Verilog evaluator surface (a small implementation of the reference 4-element field $F_4$, deliberately defined in terms of small constant predicates and enumerated boolean functions, which is what triggers the heuristic). The LOW is documented and audited. It does not correspond to a missing proof step.
...
\item It does not mean the hardware implements exactly the Coq semantics in every case. All 47 synth-realised opcodes have Qed bisimulation proofs: 37 unconditional, 10 under structural invariants \texttt{coupling\_wf} and \texttt{morph\_table\_wf} that hold inductively from hardware reset. Zero structural gaps within the synth-realised subset. Documented in \texttt{RTLGapRegistry.v} and \code{CloseoutVerification.v}. The four Q\textsubscript{1+AB} cert-opcodes live in the Kami HW abstraction (with kernel-equivalence proven) but outside the synthesized-Verilog surface, by silicon budget, not by semantics gap.
\item It does not mean the OCaml runner binary is proven inside Coq to be bitwise identical to the Coq semantics. \code{OCamlExtractionBridge.v} proves Coq-side observable facts. The parity suite checks the extracted binary empirically. One is a theorem. The other is a test.
\end{itemize}
...
\end{itemize}
If you find such a trace, \code{certification\_requires\_positive\_mu} is wrong. Add the counterexample as a theorem. Coq refuses the compile.
...
\item Exhibit a substrate that has both an internal recursion-theorem witness for a representable transformer and a non-trivial extensional predicate, but for which the diagonal construction does not produce a contradiction. The proof body is about 38 lines in \code{coq/kernel/nfi/StructuralUndecidability.v:127--164}, roughly 16 of them actual tactic steps; finding such a substrate means finding an error in those tactic steps.
\end{itemize}
...
\item \textit{\small structural\_shortcut\_undecidable} (\code{coq/kernel/nfi/StructuralUndecidability.v}): for every Substrate plus WithShortcutPredicate instance, no Coq function whose diagonal flip transformer is internally representable in the substrate decides the structural-shortcut admittance predicate. The substrate's own halting-shaped wall, by Kleene 1938 diagonal restricted to the substrate's internal expressive power.
\item \textit{\small nat\_recursion\_theorem / nat\_structural\_shortcut\_undecidable / nat\_self\_undecidable} (\code{coq/kernel/foundation/NatSubstrateInstance.v}): the minimal nat-coded substrate discharges every \texttt{Substrate} typeclass field unconditionally (no Hypothesis, no Axiom). The recursion theorem is proved by exhibiting a self-aware fixed-point program (code \texttt{2}). The substrate-level limitative theorem fires unconditionally, and the corollary \code{nat\_self\_undecidable} reads off: the substrate cannot internally decide its own shortcut-admittance predicate (the abstract, A2-free version; the certification-axis-specific form is the conditional VM corollary). \texttt{Print Assumptions} on all three returns \texttt{Closed under the global context}.
\item \textit{\small vm\_structural\_shortcut\_undecidable} (\code{coq/kernel/nfi/StructuralUndecidability.v}): substrate-level theorem instantiated at the 51-opcode VM. Carries the VM-side Section parameters (Gödel encoding, internal-representability predicate, internal recursion theorem) as explicit \texttt{forall} premises in the Bekenstein-bound style. The substrate-level content is unconditional at \texttt{nat\_substrate}; the VM corollary presents the result at the VM scope.
\item \textit{\small program\_to\_nat / nat\_to\_program / nat\_to\_program\_program\_to\_nat} (\code{coq/kernel/foundation/VMInstructionEncoding.v}): closed Gödel encoding of \texttt{list vm\_instruction} as a \texttt{nat} with a proven left inverse, all 51 opcode constructors handled, no Hypothesis. Composes the existing binary encoders from \texttt{VMEncoding.v} with a \texttt{list bool $\leftrightarrow$ nat} bijection through \texttt{positive}.
...
\item \textit{\small CHSH coupling bridge} (\code{CHSHCouplingBridge.v}): Bell's theorem in morphism coupling language. A locally consistent strategy produces a separable coupling and has $|S| \leq 2$. $S > 2$ rules out every locally factorizable morphism coupling.
...
\item \textit{\small column\_contractive\_iff\_quantum\_realizable} (\code{coq/kernel/quantum/QuantumPartitionPSD.v}): Thiele honesty $\iff$ zero-marginal NPA realizability. A biconditional with no project-local axioms (its \texttt{Print Assumptions} reports only Coq's standard classical-reals and functional-extensionality stdlib axioms). Forward (\code{zero\_marginal\_npa\_column\_contractive\_implies\_psd} in \code{coq/kernel/nfi/MuLedgerQuantumBridge.v}): column-contractive → NPA PSD. Reverse (\code{npa\_psd\_implies\_column\_contractive} in \code{coq/kernel/quantum/QuantumPartitionPSD.v}): NPA PSD → column-contractive, proved by vector specialization plus the \code{quadratic\_nonneg\_discriminant} lemma from \code{coq/kernel/quantum/ConstructivePSD.v}. Zero-marginal NPA realizability is the orthogonal-observables slice of the quantum elliptope ($\rho_{AA}=\rho_{BB}=0$), not the full elliptope. Classical correlations like $(1,0,1,0)$ have $|S| \leq 2$ but fail column contractivity. The classical polytope and the Thiele-honest set intersect but neither contains the other.
\item \textit{\small state\_column\_contractive\_implies\_tsirelson} (\code{coq/kernel/nfi/MuLedgerQuantumBridge.v}): Thiele-honest $\Rightarrow$ $|S|^2 \leq 8$, i.e., $|S| \leq 2\sqrt{2}$. The proof factors through quantum realizability: \code{state\_column\_contractive\_implies\_quantum\_gram} composes with \code{quantum\_realizable\_implies\_tsirelson\_bound} (in the same file). The row-bounds lemma \code{tsirelson\_from\_row\_bounds} in \code{coq/kernel/quantum/TsirelsonGeneral.v} is the algebraic Cauchy--Schwarz step inside this chain. The contrapositive ($|S| > 2\sqrt{2}$ $\Rightarrow$ not Thiele-honest) follows immediately. The converse ($|S| \leq 2\sqrt{2}$ $\Rightarrow$ Thiele-honest) is false and is not claimed.
...
\item \textbf{Structural advantage}: \code{StructuralAdvantage.v} proves exact $\mu$ costs and the arithmetic time-tax facts for the factored-search witness; executable tests check the measured VM behavior.
...
\textbf{Categorical foundationality vs.\ metaphysical foundationality.} The theorem \code{thiele\_is\_initial\_a2\_substrate} (\code{coq/kernel/nfi/ThieleInitiality.v}, zero axioms) establishes Thiele as the initial/universal object in the category of A2-respecting substrates over \texttt{vm\_instruction}: given any A2-respecting substrate $M$ over the same instruction signature and any chosen basepoint $s_0^M$ in $M$, the canonical trace-fold simulation from Thiele to $M$ is the unique morphism preserving the basepoint and commuting with step-extension. This is initiality in the precise categorical sense, in the same way that $\mathbb{Z}$ is the initial ring or the free group is initial in the category of groups with a fixed generating set. Whether the A2-respecting category is itself foundationally privileged over alternative cost-tracking categories is a meta-mathematical judgment about which signatures matter, of the same shape as ``why study groups instead of semigroups.'' Such questions are not settled by any theorem inside any category; they are settled by which questions the structure helps the reader answer. The proofs in this corpus establish the categorical universal property, and they also establish the priority claim: classical computation is derivative, on substitution-adequacy and lattice-scoped minimality plus the law-versus-checker fact. What they do not establish, and do not claim to, is the cosmic primacy of the A2 category over alternative cost-tracking categories --- the same shape as ``why study groups instead of semigroups,'' a choice of which signatures to care about. That category-selection question is the reader's call, on grounds outside the Coq corpus. Derivativeness is not; it is decided inside.

\textbf{Where priority lives, and where it does not.} Here is what does the work. Hold the trust fixed and A2 is the \emph{exact least} local predicate that prices certification. Miss a cert-flip and the floor fails on a one-step trace. Charge a non-cert-flip and you overpay. So the only exact, non-overcharging substitute already \emph{is} A2. There is even a concrete trusted rival in the running, erasure accounting, and it provably loses (\code{a2\_equal\_trust\_substitution\_payoff} in \code{coq/kernel/nfi/A2Payoff.v}, sharp forms \code{substitution\_test\_exact\_substitute\_is\_a2} and \code{substitution\_test\_rejects\_non\_a2\_exact\_substitute} in \code{coq/kernel/nfi/CommitmentPredicateAdequacy.v}). The structural axis $(\mu, \texttt{cert})$ is the proved-minimal extension over (mem, regs, pc): cert is not a function of the rest, $\mu$ is not a function of the rest (\code{P\_full\_is\_minimal\_complete\_extension} in \code{coq/kernel/nfi/NecessityAbstract.v}). Drop either field and certification semantics stop being determinate. And a classical step rule cannot carry A2 as a law. It can only run a fallible checker of it, after the fact, from outside the step. That is what makes classical computation \emph{derivative} and not merely projected: it is the state left over when you forget the law, and you cannot put the law back without rebuilding the substrate. None of that rides on initiality or on the projection/lift asymmetry, and here is the one-line reason to keep your eye on both. Initiality is the freest-object property. $\mathbb{Z}$ is the initial ring, and a $\mathbb{Z}$ with an inert tag bolted on is initial among tagged structures by the identical proof. The projection/lift asymmetry is generic to any field-drop, the tagged $\mathbb{Z}$ included. Neither prices anything, so neither buys priority. The directional claim is true: classical is the canonical lossy projection, and Thiele strictly extends it. But it stands on the substitution and minimality results above, not on the asymmetry. Bolt A2 into a classical step rule and you have not extended classical computation. You have built the Thiele Machine. Read ``initial'' as freest. Never as deeper. The word that survives the proofs is ``derivative,'' and it survives on substitution-adequacy and minimality plus the law-versus-checker fact, not on the projection asymmetry.
...
\begin{itemize}
\item \textbf{Irrecoverability theorems} (\code{cert\_not\_function\_of\_forget}, \code{mu\_not\_function\_of\_bare\_observable}, \code{cert\_addr\_not\_function\_of\_forget} in \code{coq/kernel/witness/ProjectionNonExistence.v}). Standard parent: the expressibility limitation of a smaller signature: no derived predicate on the bare classical projection captures the dropped fields. Thiele-specific content: the \emph{choice} of dropped fields (\texttt{vm\_certified}, \texttt{vm\_mu}, \texttt{csr\_cert\_addr}) and the explicit reachable-state witnesses (\texttt{trace\_witness\_A}, \code{forget\_witness\_uncert}, the bare-observable and cert-addr witness pairs defined in \code{ProjectionNonExistence.v}). The witnesses are not abstract; they are concrete computational states with named identifiers and verified \texttt{forget} equalities. The corollaries (\code{no\_classical\_a2\_cert\_predicate} etc.) are statements about what can be expressed on the bare classical signature, not just about what has been forgotten in the projection.
...
At the instance level, the strongest single result is \code{chsh\_lassert\_no\_trap\_implies\_quantum\_realizable} in \code{coq/kernel/quantum/QuantumPartitionPSD.v}: a kernel-level Z-arithmetic check on CHSH outcome counters entails PSD of the NPA moment matrix, by a chain of Coq proofs that carries no physics premises. The algebraic bridge from a step-level certification check to a quantum realizability condition, with nothing in the dependency tree project-local. \texttt{Print Assumptions} on it returns \texttt{Closed under the global context}.

At the substrate level, the strongest single result is \code{nat\_structural\_shortcut\_undecidable} in \code{coq/kernel/foundation/NatSubstrateInstance.v}, together with the corollary \code{nat\_self\_undecidable}. A concrete substrate with every typeclass field discharged constructively, no Hypothesis and no Axiom, that cannot internally decide its own shortcut-admittance predicate. A substrate's own halting-shaped wall, by Kleene 1938 diagonal restricted to the substrate's internal expressive power, internal to the substrate's own typeclass machinery --- the unconditional, axiom-free floor, the generic family Rice's theorem also inhabits. The certification-axis-specific form, where the predicate is tied to the supra-cert channel, is the VM corollary --- conditional on its internal recursion theorem, a routine condition rather than a live doubt (Section~\ref{sec:substrate-undecidability} sets out why). \texttt{Print Assumptions} returns \texttt{Closed under the global context}.

CHSH-NPA at the instance level. The substrate-level structural-axis undecidability at the substrate level. What makes this document foundational is the universal A2 cost floor: \code{universal\_nfi\_any\_substrate} holds on \emph{any} substrate that carries a certification predicate and charges A2 on the cert-flip as a law of its step rule, and \code{P\_full\_is\_minimal\_complete\_extension} pins $(\mu, \texttt{cert})$ as the proved-minimal extension over (mem, regs, pc), the state a step rule needs to read the cert and charge the $\mu$. The CHSH-NPA and undecidability pillars are strong, but they are companion facts; the foundational weight rides on universality, scoped-minimality, and the law-versus-checker fact. That is the priority claim, and it rides on universality, minimality, and the law-versus-checker fact. Not on a physics-free Bell bridge, not on a Rice-family diagonal. The CHSH-NPA equivalence and the substrate-axis undecidability are strong companion facts: an algebraic bridge with no physics premises on one side, and a limitative result internal to the substrate's own typeclass machinery on the other. The 51 opcodes, the OCaml runtime, the Bluespec hardware, the parity tests, the simulator harness are scaffolding for one realization of the substrate; the universal cost floor and the lattice-scoped irredundancy of $(\mu, \texttt{cert})$ over (mem, regs, pc) are facts about substrates, and the two theorems above are strong instances of how those facts cash out.

Each of these results carries an explicit falsification condition, and they are gathered in one place (Section~\ref{sec:falsification}): a zero-$\mu$ trace from uncertified to certified, a column-contractive correlator with a non-PSD NPA matrix, a cost-rule-respecting \code{CertificationSystem} that certifies at total cost zero, a \texttt{nat} function that decides \texttt{nat\_admits} for its own substrate. Any one of them collapses the matching theorem. They held up to the checker.
