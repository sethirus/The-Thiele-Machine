...
\fancyhead[R]{\small\itshape Devon Thiele \textbullet\ July~2026}
...
\smallskip
\noindent There is a machine underneath every computer that has ever been built. It has a part our machines can't see, whether the thing they are holding was actually earned or is just sitting there. Every computer we have ever made is that machine with that part switched off. Not broken. Blind. A shadow of the whole thing.

\smallskip
\noindent I built the one with the part switched on. And then, because I don't trust my own eye, I didn't just claim it, I proved the shadow is a shadow. The blindness is real. What gets dropped can't be gotten back. You cannot climb from the shadow up to the machine without making something up out of thin air. The Turing machine was never the thing itself. It is what is left of the thing after you forget the one law.

\smallskip
\noindent And it is not the chip that makes it real, and not the OCaml. You could burn every line of the OCaml and every gate of the RTL and lose nothing, because the machine is the law, not the machinery. I think it is real the way a law is real, that it holds whether or not anyone ever built it, and I will be straight that this part is what I believe, not what I proved. I did not bolt a counter onto a Turing machine. I built the machine that keeps the coordinate every other machine drops.

\smallskip
\noindent That is the machine, and that is the whole claim. Every computer is its blind, lossy shadow, and that I proved: you cannot recover the coordinate it drops, and you cannot even verify the claim from the shadow. Certification, the move from unverified to verified, is not the point of any of it. It is the one thing I could pin to the new part to prove the part is there. Everything after this, the opcodes and the OCaml and the chip and the Coq, is me showing the machine can be built and cannot be waved away. None of it is what makes it real. \textbf{The law is what makes it real.}
...
\begin{abstract}
The Thiele Machine is a substrate: a state, a step rule, and a structural cost ledger $\mu$ that the step rule charges as it runs. One law governs the charging. A step that turns a result from unverified to verified, the move I call certification, costs at least one, and the price is paid in the step itself, not in a checker bolted on beside the program, because a program can skip a checker and cannot skip its own step rule. Any substrate that carries the certificate and the law pays the same floor, whatever its instruction set.

The result is an asymmetry. Every Thiele state forgets down onto a classical one by a single canonical projection that drops the cost ledger and the certification bit; the lift back is not canonical, because a classical state sits under many substrate states and choosing one means supplying the dropped fields from outside. Forgetting is forced and unique. Reconstructing is a guess.

What makes that more than a field you happened to drop: $(\mu, \mathrm{cert})$ is the smallest extension of memory, registers, and program counter on which certification is even sayable, with neither field recoverable from the rest, and A2, with trust held fixed, is the only rule that prices the certification step exactly, charging that step and nothing else. So classical computation is not a parallel model that records less. It is what is left of the substrate once you forget the law, the exact sense in which it is the derivative thing. The same functions compute on both sides; what is new is where the law lives.

Everything after this page is that claim unfolded, walked one step at a time, and handed a falsification condition for every load-bearing piece. The open pieces are drawn there too, not hidden, the largest of them being whether certification had to be the event on the meter at all.
\end{abstract}
...
\noindent\textbf{One thing before the story, because people get it backwards.} I used language models to write the code and the proofs in this project, and I trust language models about as far as I can throw them, which is exactly why I ran every line through a proof assistant that does not care who or what wrote it. Coq rejects a wrong proof the same whether a tenured professor or a model hands it over, and it accepted these, with none of my own axioms anywhere in them, re-checkable by anyone in one command. Being self-taught and using modern tools did not make this looser. It is the reason I made a machine check all of it, me included. The long version of that is a few pages down, under how the work actually got made; I am putting the short version up here so it lands before anything else does.
...
Then within weeks I found out the engine I had built to draw pictures was general enough to run a physics simulation on the exact same primitives, without my rewriting any of it. I wrote Newton's laws as a program in it, half expecting nothing, and it ran. I want to be careful about what that was and what it was not, because this is exactly the kind of thing that gets misread. It was not me deriving physics, and I am not claiming the machine explains gravity. It was the compositional structure being general enough to express a physics simulation the same way it had expressed a renderer, and that generality is what told me the structure was the point and the pixels never were. Somewhere in those weeks I crossed from debugging why one pixel would not show up to understanding that I had quietly built a categorical computing engine, and that the rendering had only ever been the first thing I aimed it at. The pixel did eventually show up, for what it is worth.
...
\textbf{A note before the technical content, on what I do and do not know about prior work.} I started this with no background in computer science, math, physics, quantum mechanics, hardware, or category theory. None. I didn't know what a Turing machine was. I didn't know what Coq was. I built this alone. Nobody mentored me or walked me through the field. The only person who has touched this work in any way is a friend who taught me how to use GitHub and set up an IDE. Every name, every comparison, every paper I went looking for, came from me searching on my own. So treat what follows as a list of labels I reached for, not a literature review; where I have mismatched one against the real thing, tell me, because that is a correction I can use.
...
\textbf{The priority claim, in three lines.} Each line is a thing I am willing to be wrong about by name. Wrong conclusion means wrong premise: the law you learned in tenth grade and have not been asked to unlearn since. There is no third move.
...
\item Then classical machines are a structurally blind, strictly lossy projection of a substrate that does---a shadow of it.
\end{enumerate}

\noindent That is the priority claim, the sharpest cut through the axis. A napkin holds it. The rest of this document is the same argument unfolded with kernel citations and a worked instruction set: same content, more words.

Where to look. The first line is A2, and A2 is two clauses and an integer. A step that flips cert costs at least one, the rule fits on the napkin with the conclusion, and the check is finite, public, and yours to run. The second line is the kind of fact you check by writing down a different Turing machine, which is what I did. Add a state field the step rule reads to see whether the last move flipped cert, and you've left the classical world, which was the claim. Keep it classical and there is no field that does the job. There aren't any that do. That is the signature the field itself carries the moment it is written down.
...
An embedding is a functor that throws nothing away. That is the whole point of putting the word here: the classical projection of a Thiele state loses things on the way out, and an embedding is the move that does not. Faithful is the precise version of ``loses nothing on arrows'': if two arrows between the same pair of objects in $\mathcal{C}$ land on the same arrow under $F$, they were already the same arrow, so $F$ never quietly fuses two distinct things into one. Conservative is the version for invertibility: if $F(f)$ has an inverse over in $\mathcal{D}$, then $f$ already had one back in $\mathcal{C}$, so $F$ cannot manufacture an isomorphism that was not there. (An isomorphism is just an arrow with an inverse.) Both words are saying the same thing from two angles. The map is honest. It does not get to invent structure or hide it.
...
\end{tikzpicture}
\caption{The directional structural theorem. Every Thiele state has a single canonical projection down via \code{shadow\_proj} (\code{degenerate\_projection\_theorem}). The classical state has multiple Thiele preimages and no canonical lift back (\code{fiber\_has\_two\_preimages}). One direction is a function; the other is not. The three states above agree on every field the projection keeps, $\mu$ and the cert bit included, and differ only in the morphism graph, the field it drops; the projected node shows the three classical fields for legibility.}
...
The three structural components ($\mu$, \texttt{vm\_certified}, \texttt{vm\_graph}) do not fall out of the classical projected fields. Nor does any one of them fall out of the others. The projection $(\texttt{mem}, \texttt{regs}, \texttt{pc}, \mu, \texttt{certified})$ is the minimal complete extension for ledger semantics: drop $\mu$ or \texttt{vm\_certified} from it and the dropped field is gone for good, since nothing left in the projection determines it, and results (3) and (4) say the other one of the pair never determines it either. The $\mu$ separations hold from every reachable substrate state; no program prefix, however long, collapses them.
\end{theorem}
...
\emph{(5) $\texttt{vm\_graph} \perp (\texttt{mem}, \texttt{regs}, \texttt{pc}, \mu, \texttt{certified})$.} The witness is Categorical Separation: same projected fields, different graph. $s_1$ carries $\texttt{pg\_morphisms} = [(0, \mathrm{id})]$; $s_2$ carries $\texttt{pg\_morphisms} = []$. The pair is written down directly, field for field, because the claim needs nothing more: ``not a function of the projection'' is settled by any two states the projection cannot tell apart, however they came to be.

\emph{(1) $\mu \perp (\texttt{mem}, \texttt{regs}, \texttt{pc})$.} Same visible state, different ledger. One branch runs $\texttt{CERTIFY}\,0$, the other $\texttt{PNEW}\,[\,]\,0$, each one step from the clean start. Registers, memory, and pc match. $\mu$ does not: $1$ versus $0$. The Coq witnesses are \code{abs\_A\_strict} and \code{abs\_B\_strict}.

\emph{(2) $\texttt{certified} \perp (\texttt{mem}, \texttt{regs}, \texttt{pc})$.} The same pair, read on a different coordinate: $\texttt{CERTIFY}\,0$ certifies and $\texttt{PNEW}\,[\,]\,0$ does not, while the visible tuple matches. ($\mu$ happens to differ here too; (3) is where that gets tied down.)

\emph{(3) $\texttt{certified} \perp (\texttt{mem}, \texttt{regs}, \texttt{pc}, \mu)$.} Case (2) with the ledger tied down: $\texttt{CERTIFY}\,2$ against $\texttt{PNEW}\,[\,]\,3$. Both charge $\mu = 3$, both leave the visible tuple identical, and they still disagree on \texttt{vm\_certified}. The Coq witnesses are \code{abs\_A\_cost} and \code{abs\_B\_cost}.

\emph{(4) $\mu \perp (\texttt{mem}, \texttt{regs}, \texttt{pc}, \texttt{certified})$.} Same visible tuple, same certification flag, different ledger: $\texttt{CERTIFY}\,0$ against $\texttt{CERTIFY}\,1$. Both certify; the declared costs differ, so $\mu$ comes out $1$ versus $2$. The Coq witnesses are \code{abs\_A\_cert} and \code{abs\_B\_cert}.

The parametric rider belongs to the $\mu$ results, and there it is not decoration: from any reachable state the certify-versus-build split is still available, so no prefix, however long, prevents the $\mu$ witness pair (\code{mu\_ledger\_necessity\_universal}). The certified results run from any uncertified state, and that is as far as they can go, for the honest reason that certification is permanent: once a prefix has certified, nothing clears the flag, so both branches of any extension agree on it. The theorem's content never needed more; one witness pair is exactly what ``not a function of'' takes.
...
\end{tikzpicture}
\caption{Feasible-set narrowing and the entropy lower bound. A strict subset plus a distinguishing observation yields a strictly stronger posterior predicate (one fewer admissible state, witnessed by $s_w$). The number of yes/no bits the narrowing buys ($\log_2^{\uparrow}$ is the base-2 log rounded up, built in full at the representation theorem below) is at most the $\mu$-cost of the trace that produced it. Same event, two sides of one equation.}
...
On this substrate the word \emph{insight} stops being a mood and starts having a meter reading. It comes in three tiers, three genuinely different things that happen when a computation learns, and they sort themselves by price: the first is free, the second never is, and the third is where the price tag has its hand on something no classical account of correlation can reach at all.
...
Why $+1$ and not $+2$ or no floor? Because $+1$ is the minimum that makes certification impossible for free. $\delta + 0$ would allow $\delta = 0$ to certify for free. $\delta + 2$ would be overcharging. $\delta + 1$ is the unique honest minimum: you can always choose $\delta = 0$ and pay exactly 1. You can choose $\delta > 0$ and pay more. The two-sided version of that contest, underpricing against overpricing, is the substitution test: with trust held fixed, the only exact, non-overcharging pricer of the certification flip is A2 itself (\code{a2\_equal\_trust\_substitution\_payoff}; the sharp forms are \code{substitution\_test\_exact\_substitute\_is\_a2} and \code{substitution\_test\_rejects\_non\_a2\_exact\_substitute}).
...
This is why the morphism layer is not ``extra state.'' It is \emph{provenance-bearing state}. The trace records where every asserted morphism came from, and the substrate has a place to check that provenance. A classical machine cannot fake that for free: the assertion flips the cert-address channel, and that flip costs $S(\delta) \geq 1$ by No Free Insight (Section~\ref{sec:nofi}), and a classical step rule has no metered field of its own to carry that charge as law. The companion fact, that the classical shadow cannot even tell two different morphism graphs apart, is the Categorical Separation Theorem (Section~\ref{sec:categorical}).
...
\node[branchlabel, above=0.5em of kami.north]
  {\textit{Kami spec}\,$\cdot$\,\code{kami\_refines\_vm\_step}};
...
So the classical ceiling is 3/4, seventy-five percent, and not a hair more. And I do not want you to take that on the strength of an XOR trick on a page; the reference kernel pins it down both ways and leaves nothing Admitted. One executable trace, built from nothing but \texttt{PNEW}, \texttt{PSPLIT}, and \texttt{CHSH\_TRIAL} and never spending a cent of the cost ledger ($\mu = 0$), walks right up to 75\% and touches it. Then \code{local\_box\_CHSH\_bound} closes the lid from the other side: no local strategy, however randomized, no factorizable box anywhere, gets over the line. A trace can of course just \emph{write} any tallies it likes into the counters at $\mu = 0$, the way anyone can write a fake scoreboard; the theorem is about what local play can earn, and the certification step that would stand behind a scoreboard is the priced one. The achievable and the unbeatable land on the same number, which is the only kind of ceiling I trust.
...
For every algebraically coherent correlator (one satisfying $|E_{xy}| \leq 1$ along with four constraints derived from $3 \times 3$ submatrices of the NPA moment matrix; the submatrix determinants, called minors, must be non-negative, which is part of the requirement that the full matrix be positive-semidefinite; both terms are built in full a few pages below, in the ``Algebraic characterization'' subsection), and given some witness parameters $t$ and $s$ that the kernel builds explicitly, the CHSH value satisfies $S^2 \leq 8$, that is, $|S| \leq 2\sqrt{2}$.
...
PSD is the mathematical condition that a matrix can represent genuine inner products, covariances, or correlations from a quantum state. The NPA framework uses PSD of a specific $5 \times 5$ matrix (the ``moment matrix'') as its level-1 condition for quantum realizability. You build that $5 \times 5$ matrix from the four CHSH correlators, and if it is PSD, the correlators are consistent with some quantum state measured with orthogonal observables and zero marginals. Two scope words in that sentence are load-bearing: for this pinned zero-marginal matrix the PSD test is exact for its slice, while the general level-1 NPA test, marginals free, is a relaxation of the quantum set that the hierarchy tightens at higher levels (Section~\ref{sec:q1ab}).
...
A correlation $(E_{00}, E_{01}, E_{10}, E_{11})$ is \emph{slice-coherent} if and only if it is zero-marginal NPA realizable. Exactly. Not approximately. Not ``consistent with some physics axiom.'' A naming confession before the theorem, because the old name was a trap I set for myself and walked into. Earlier drafts called this property \emph{Thiele-honest}, and that word claims too much: it reads as if the gate can tell true tallies from invented ones, and it cannot. No arithmetic on four ratios can. What the three conditions test is membership in the zero-marginal slice, coherence with one specific cone, and nothing else, so the name now says that and only that. The biconditional is pure polynomial arithmetic with no project-local axioms (\texttt{Print Assumptions} on \code{column\_contractive\_iff\_quantum\_realizable} reports only Coq's standard classical-reals and functional-extensionality stdlib axioms, namely \texttt{sig\_not\_dec}, \texttt{sig\_forall\_dec}, and \texttt{functional\_extensionality\_dep}, the same stdlib families Section~\ref{sec:q1ab} discloses for the Q\textsubscript{1+AB} theorems; ``zero axioms'' in this section means zero project-local, in the sense of Section~\ref{sec:falsification}):

\begin{theorem}[Slice coherence $\iff$ NPA-PSD]
\[
\text{coherent}(\mathbf{E}) \iff \text{psd-npa}(\mathbf{E})
\]
where \emph{coherent} means the three column-contractivity conditions hold, and \emph{psd-npa} means the $5\times5$ zero-marginal NPA matrix is positive semidefinite.
\end{theorem}
...
\item Slice-coherent = zero-marginal NPA realizable (biconditional, both directions, no project-local axioms). Every slice-coherent correlation satisfies $|S| \leq 2\sqrt{2}$, but NOT every correlation with $|S| \leq 2\sqrt{2}$ is slice-coherent: $(1,0,1,0)$ has $|S|=2$ but fails condition 1.
...
Now the paragraph I owe you, because an earlier draft of this passage implied its opposite and I would rather correct myself in print than be corrected. The gate is a membership test, not a lie detector. Run the sixteen deterministic local strategies through it: every one of them has all four correlators at $\pm 1$, so condition 1 reads $1 - 1 - 1 = -1 < 0$ and \texttt{CHSH\_LASSERT} traps every single one of them, including the all-ones strategy that this section's own tightness theorem (\code{classical\_bound\_achieved}) runs at $\mu = 0$ to touch the classical bound. Those tallies are as true as tallies get, the trace really did accumulate those counts, and the gate rejects them anyway, because deterministic play sits outside the zero-marginal slice: its marginals are anything but zero. In the other direction, a program can \texttt{LOAD\_IMM} a fabricated set of counters that happens to land inside the slice, and the gate waves it through; whether counts were earned by play or written by fiat is the kind of fact the receipt discipline of Section~\ref{sec:receipt} meters, not this opcode. So the correct reading of the boundary is set-theoretic, not moral. The classical polytope and the slice-coherent set intersect, uniform noise $(0,0,0,0)$ sits comfortably in both, but neither contains the other: $(1,0,1,0)$, reachable by a classical mixture and carrying a legal $|S| = 2$, fails condition 1 ($1 - 1^2 - 1^2 = -1 < 0$, closed by \texttt{lra} once \code{column\_contractive\_iff\_quantum\_realizable} is destructed), while every quantum point at zero marginals passes. What a successful \texttt{CHSH\_LASSERT} certifies is that the counters cohere with the zero-marginal slice, no more and no less, and the Tsirelson corollary is inherited from that membership, not from any power to sniff out forged data.
...
Section~\ref{sec:chsh} above derives the level-1 bridge: CHSH\_LASSERT $\Rightarrow$ PSD of the $5\times5$ zero-marginal NPA matrix. The level-1 cone contains the Tsirelson set $Q$ (every quantum-realisable correlator is PSD at level 1), but it strictly contains $Q$: the level-1 cone is bigger than $Q$ for CHSH, so a level-1 certificate does not on its own pin the correlator down to physically quantum. The next NPA level that does coincide with $Q$ for CHSH, at the correlator level, is level 1+AB: the $9\times9$ moment matrix indexed by the basis $\{I, A_0, A_1, B_0, B_1, A_0 B_0, A_0 B_1, A_1 B_0, A_1 B_1\}$. That $Q_{1+AB} = Q$ for the CHSH correlator set is Ishizaka 2025's result (\emph{Entropy} 27(2):182, arXiv:2502.10746), not mine, and I cite it instead of re-deriving it because re-deriving someone else's NPA-hierarchy theorem from scratch is a fight nobody asked me to pick. The correlator-level scoping is load-bearing, not cosmetic: for the \emph{complete} behavior set, marginals included, no finite NPA level is exact in this same scenario (Chaturvedi 2026, arXiv:2607.14569), so an unscoped ``$Q_{1+AB} = Q$'' would be quoting the theorem into a claim it does not make, exactly the failure mode this document keeps fencing against. What the substrate actually nails down is the certification end: a successful step of one of the four \texttt{CHSH\_LASSERT\_1AB} cert-opcodes is a proof that the $9\times9$ Q\textsubscript{1+AB} moment matrix is PSD at the witness-derived rationals and the bucket-derived $\gamma$ values. The opcode steps; the matrix is in the cone. That is the whole bridge.
...
\textbf{Why A2 is load-bearing.} Each of the four cert-opcodes is a cert-setter; A2 forces its instruction cost to be at least 1. Without A2 the cert-flip would be free, and a substrate of the unconstrained class could stamp its attestation without any priced event binding the claim to the check that earned it; the decider itself would still reject a non-PSD matrix, but the stamp would cost nothing to counterfeit around it. With A2 every successful Q\textsubscript{1+AB} certification commits $S(\mathit{mu\_delta}) \geq 1$ units of $\mu$ to the ledger. This is the same A2 application that powers the cost-ledger separation, the $n$-way bound, and the LASSERT polynomial exceedance.
...
Everything in this section so far characterizes slices: cross moments pinned to zero at level 1, then progressively freed $\gamma$ buckets at level 1+AB, always with the orthogonality assumptions carried along. The naming confession earlier in this section admitted what the pinning costs: the slice-coherence gate rejects every deterministic local strategy, including the $\mu = 0$ trace that this section's own tightness theorem runs to touch the classical bound. An earlier revision of this document listed the full-elliptope connection as an open algebraic gap. At the correlator level, it is open no longer, and this subsection says exactly what closed and exactly what did not.
...
So the three-way geometry the naming confession promised is now theorem-shaped at the correlator level: the classical polytope and the zero-marginal slice intersect without containment, and both sit inside the elliptope, which admits every honest classical point the slice refuses while still refusing everything past $2\sqrt{2}$.

And the gate itself is no longer prose. \code{ElliptopeGate.v} builds the decidable check an opcode can branch on (\code{elliptope\_check\_full}), in the same integer-only discipline as \code{column\_contractive\_check\_witness}: no real and no rational evaluated at runtime, every denominator cleared into $\mathbb{Z}$. The check has two branches. The PD branch is the cheap path for a strictly-interior point: it takes completion buckets for $(x, y)$, exactly the bucket-pair idiom the 1+AB opcodes use for their $\gamma$ moments, and runs the fraction-free Sylvester test on the denominator-cleared completed matrix, whose soundness is the same \code{sym4\_qf\_nonneg\_from\_pd} LDLT lemma the Slice-C development proves. The LDL branch reaches everything else, boundary included: it takes a rational $LDL^{\mathsf T}$ certificate, six subdiagonal entries and four pivots over common positive denominators, and verifies the ten entry equations of $M = LDL^{\mathsf T}$ in cross-multiplied $\mathbb{Z}$-arithmetic. Its soundness (\code{elliptope\_ldl\_check\_sound}) is a sum-of-weighted-squares identity: given the equations, the quadratic form \emph{is} $d_1 P_1^2 + d_2 P_2^2 + d_3 P_3^2 + d_4 P_4^2$. Because a zero pivot in a PSD matrix forces its whole column to vanish, every rational PSD matrix has a rational $LDL^{\mathsf T}$ without pivoting, so the LDL branch accepts every point admitting \emph{any} rational PSD completion, singular ones included; it subsumes the PD branch, and it subsumes what a sixteen-weight LHV-mixture decomposition could reach (an earlier draft carried such a branch and it is removed as redundant and as the file's one performance liability). The load-bearing theorem is \code{elliptope\_check\_full\_sound}: a passing check entails elliptope membership of the witness-derived correlators, with the check's own passing as its only hypothesis. And the gate computes, not just proves, each by \texttt{reflexivity}: \code{gate\_accepts\_all\_ones} runs the $\mu = 0$ tightness witness through the LDL branch (its all-ones completion is rank-one singular) and it passes; \code{gate\_accepts\_turing\_point} does the same for $(1,0,1,0)$; \code{gate\_accepts\_beyond\_classical} passes the interior $S = 12/5$ point through the PD branch; \code{gate\_accepts\_pythagorean\_boundary} certifies $(3/5, 4/5, 4/5, -3/5)$, whose arcsine sum is exactly $\pi$ by the 3-4-5 triangle so it sits exactly \emph{on} the Tsirelson curve with $S = 14/5 > 2$ --- a point invisible to the strict PD branch because its completion is singular, reached by the LDL branch with pivots $(1, 1, 0, 0)$ at $x = y = 0$; and \code{elliptope\_full\_gate\_never\_accepts\_pr\_box} proves no completion buckets and no certificate ever get the PR box through, not by enumeration but by soundness plus the Tsirelson theorem closing both branches at once. What remains outside any exact integer check, provably and not by omission: tuples whose \emph{every} PSD completion is irrational, for which a rational certificate does not exist to be supplied. That residue is the arithmetic of the boundary, not an engineering gap, and no finite rational gate, this one or any other, can cross it.

What this does not deliver, said with the same care as everywhere else. First, the kernel's runtime opcode is unchanged: \texttt{CHSH\_LASSERT} still checks slice coherence, and binding \code{elliptope\_check} into the step relation (opcode constructor, cost schedule under A2, Kami mirror, on the pattern of the four 1+AB cert-opcodes) is plumbing that costs silicon and is not done here; the decider and its soundness, the mathematical content of that opcode, are. Second, this is the correlator level, and after Chaturvedi 2026 that scoping is not pedantry: for the complete behavior set, marginals included, no finite NPA level is exact, so ``the elliptope'' here means the projection onto correlators, exactly as stated. Third, nothing physical moved: the identification of PSD completions with quantum-realizable correlators is Tsirelson's theorem, cited externally, and the kernel's contribution remains what it always was, polynomial arithmetic under audit. To falsify: a deterministic sign pattern whose completed Gram matrix has a negative quadratic form kills \code{deterministic\_strategy\_elliptope}; an elliptope-realizable tuple with $S^2 > 8$ kills \code{elliptope\_tsirelson}; a PSD completion for the PR box kills \code{pr\_box\_not\_elliptope} and Tsirelson with it.
...
\emph{Part (2): cost lower bound.} The level-$k$ hypothesis hands over an executed \texttt{CERTIFY} with $S(\delta) \geq k$; the kernel proof finds it by scanning the executed-instruction log. That one step charged at least $k$ to the ledger, every other step charged at least $0$, and by Latent $\mu$ the total cost is the sum of the step charges, so the total is at least $k$. The scan is the work: an earlier draft of this bound collapsed to reading $\mu_{\text{final}} \geq k$ off the hypothesis, which proves nothing, and the kernel's comment says so out loud. The present form earns the floor from the certifying step itself.
...
A discrete triangulation defines a curvature-like angle defect. As I understand it (I researched this for the project, so check me on the geometry): on a flat surface, if you draw a triangle, the interior angles sum to $180^{\circ}$. On a curved surface, the angles can sum to more or less than $180^{\circ}$. Curvature is the amount by which triangles deviate from flat. In the equilateral discrete model, curvature at a vertex is the defect left after subtracting the angles of the incident triangles (the triangles meeting at that vertex) from $2\pi$ (a full turn).
...
\]
From \code{well\_formed\_triangulated}: the structural invariants $E = I + B$ and $3F = 2I + B$, together with the boundary-Euler relation $B = 3\chi$ the predicate also carries, combine to give $3V = 5E - 6F$ (the triangulation-combinatorial identity). Solving for $V$: $V = (5E - 6F)/3$. Substituting:
\[
...
Now the same thing in the formal voice. \code{well\_formed\_triangulated} is a planar-triangulation predicate; its combinatorial invariants are packaged into the theorem \code{triangulation\_combinatorial\_identity}. Its structural invariants are $E = I + B$ and $3F = 2I + B$, where I count the edges in two flavours: $I$ for interior edges (edges shared by two triangles) and $B$ for boundary edges (edges with a triangle on one side only). These invariants, joined to the boundary-Euler relation $B = 3\chi$, give $\chi(g) = (2E - 3F)/3$ and $3V = 5E - 6F$. In a closed surface like a sphere the corresponding identity is $2E = 3F$, but this predicate is for planar regions with a boundary, not closed surfaces.
...
Current Inquisitor status (280 Coq files scanned): zero HIGH findings, zero MEDIUM, zero LOW. The one LOW the report used to carry, a vacuity-score warning on the F4 reference Verilog evaluator (a small implementation of the reference 4-element field $F_4$, deliberately defined in terms of small constant predicates and enumerated boolean functions, which is what trips the ``constant function'' heuristic), was documented, audited, and has since been cleared. It never corresponded to a missing proof step.
...
Now the honest limitation. The sources I read on Coq's foundation say it is believed to be logically consistent (meaning you cannot derive a contradiction from it), but this has not been proven within Coq itself. The way I understand it, that is not a weakness specific to Coq. The label I dug up is Gödel's incompleteness theorem from 1931: any consistent formal system powerful enough to do arithmetic cannot prove its own consistency from within itself. A system that could prove its own consistency would already be inconsistent. And an inconsistent system proves everything, its own consistency included. So Coq's foundation is trusted, not proved-from-within. ``Trusted'' has substance here. The same foundation has verified the CompCert C compiler and the Four Color Theorem (Gonthier 2005, the first fully machine-checked proof of a major mathematical theorem, one first settled on paper-plus-computer in 1976), among other artifacts. Large-scale inconsistencies would have surfaced through those uses if they were there. I am relaying what I read. I have not personally audited CompCert or Gonthier's proof. ``Zero Admitted'' means every theorem follows from those axioms. It does not mean those axioms are themselves in-system guaranteed. It means they are the same axioms a substantial body of verified software has relied on without breaking.
...
The bridge tier involves named hypotheses and documented trust boundaries. Four of them. First, the BSC (Bluespec compiler) trust boundary for hardware: Section~\ref{sec:hardware} introduces Bluespec; the BSC compiler is the tool that translates Bluespec into Verilog; the proof assumes this tool's output faithfully implements its input. Second, the A\_QM physical assumption for the quantum connection: a named hypothesis I use sparingly, marking the place where the Coq proof would need a physics input it does not derive. Third, the external OCaml parity boundary, described in Section~\ref{sec:hardware}: the OCaml binary is checked against the Coq semantics by tests, not by a Coq theorem. Fourth, the Landauer--Unruh calibration (\code{mu\_landauer\_unruh\_calibrated}), the named hypothesis that prices $\mu$ in physical units; every physical-interpretation claim is conditional on it, and Section~\ref{sec:landauer} treats it. These are explicitly labelled and auditable. They are not Admitted proofs. They are documented limits of the formal claim.
...
\item \textit{\small vm\_structural\_shortcut\_undecidable}: substrate-level theorem instantiated at the 51-opcode VM. Carries the VM-side Section parameters (Gödel encoding, internal-representability predicate, internal recursion theorem) as explicit \texttt{forall} premises, in the named-premise style the physical bridges use. The substrate-level content is unconditional at \texttt{nat\_substrate}; the VM corollary presents the result at the VM scope.
...
\textbf{Full-elliptope quantum mechanics connection.} The subsumption claim establishes the algebraic equivalence between slice coherence and zero-marginal NPA realizability (\code{column\_contractive\_iff\_quantum\_realizable}, no project-local axioms). The zero-marginal framework is the orthogonal-observables slice ($\rho_{AA} = \rho_{BB} = 0$): a proper subset of the full quantum elliptope. The correlator-level algebraic gap this item used to name is now closed: Section~\ref{sec:elliptope}'s \code{ElliptopeCompletion.v} characterizes elliptope membership as existential completion of the cross moments, proves every LHV correlator inside (deterministic strategies plus convexity), keeps Tsirelson ($S^2 \leq 8$) and the PR-box exclusion, and does it under the same zero-project-local-axiom discipline. What remains open, exactly: the kernel's runtime gate still checks the slice, not the completion (the witness-carrying opcode is undone engineering); the \emph{behavior}-level set with marginals is not characterized at any finite NPA level (Chaturvedi 2026), so no finite-level gate can ever close that form of the gap, and this document does not pretend otherwise; and the claim still does not establish that a physical Thiele Machine can produce CHSH-violating statistics by exploiting quantum entanglement in hardware. Closing the empirical gap would require an experiment.
...
%===========================================================================
\section{The pointer-observable conjecture: a name for the open question's successor}
\label{sec:pointer}
%===========================================================================

Twice now I have walked up to the same fence and stopped. The preface stops at it: whether the universe is forced to put certification on the meter, rather than head-reversals or multiplications, is a question I said I have not answered. Section~\ref{sec:unknown} stops at it again, slower: the substitution contest ranks chargers of a \emph{fixed} event, so what it settles is which charger, never which event, and that certification is the event on the meter was written into the setup, not won there. I am not going to answer it here either. What I can do, and what this section is for, is name the question's successor: replace ``is certification the forced event?'' with a sharper question that has a criterion inside it, so the next round of work has something to aim at instead of a shrug.
...
The evidence, such as it is, is Section~\ref{sec:reductions}, and the reason it counts as evidence is that I did not build any of it. Five metering disciplines, evolved by five different communities for five different reasons, none of them with this document in view. Proof-of-stake finality meters the finalization event, and a finalized block is copied by every full node on the network. Gas metering charges the committed state transition, and the committed transition is what every replica re-executes and stores. Trusted-execution attestation prices the attestation, and the attestation is what every relying party verifies and archives. Certificate transparency meters log inclusion, and the log is mirrored, gossiped, and audited by design. Proof-carrying verification prices the checked commitment, and the carried certificate is what every consumer of the code re-checks. In every case, the event that ended up on the meter is the event whose record proliferates across parties that share nothing else; and in no deployed discipline I can find does anyone meter head-reversals, multiplications, or comparisons, the rival events whose records nobody replicates. Five independent convergences on the same shape are not a theorem. They are, though, exactly what you would expect to see if the conjecture were true, and what you would not expect if the choice of metered event were arbitrary.
...
The criterion now has a formal skeleton, so the door has hinges and not just a handle. \code{PointerObservable.v} defines the objects: an \code{Ecosystem} of independent observers each holding a boolean fragment of the state; \code{records}, one observer's fragment deciding an event; \code{redundantly\_proliferating}, every observer recording it independently; and \code{unique\_pointer\_among}, proliferating while no named rival event does, with uniqueness deliberately relative to the rival family because absolute uniqueness is false for free (an event's complement proliferates whenever it does). The conjecture itself is stated as a schema (\code{pointer\_criterion\_holds}): over any class of ecosystems with designated metered events and rivals, every metered event is the unique pointer among its rivals.

And the five instantiations Section~\ref{sec:reductions} argues informally are now machine-checked. \code{PointerObservableReductions.v} builds a faithful ecosystem projection of each deployed discipline: proof-of-stake finality (the finalized block, mirrored by every full node; rival, the proposer's work, which no node stores), gas metering (the committed transition versus scratch state), trusted-execution attestation (the attestation versus the enclave's measurement noise), certificate transparency (log inclusion versus submission latency), and proof-carrying verification (the checked certificate versus the prover's effort). Each is proved a \code{unique\_pointer\_among} its rival, and the aggregate \code{five\_disciplines\_are\_pointers} is the indexed schema instantiated at all five, each theorem closing under the global context with not one axiom of any kind. Five disciplines that owe each other nothing, and the criterion discriminates the metered event from the rival at every one of them: convergence made formal.

What this does not do, fenced exactly as before: it does not prove the conjecture. Which event is ``metered'' and which is ``rival'' is the modeler's designation in each of the five, precisely as the open-problem fence in Section~\ref{sec:unknown} says; the ecosystems are minimal projections, not full protocol models. What is now checked, rather than asserted, is that under the faithful modeling the criterion picks out the certification-flavored event and rejects the rival at five independently evolved designs. That is evidence for the conjecture strengthened from five hand-waved instantiations to five \texttt{Qed}s. It is not, and cannot be from inside the system, a proof that record proliferation is the \emph{right} criterion or that certification is the event a step rule is \emph{forced} to price. Those stay open, and they stay the kind of question settled by adoption and further theory rather than by a proof.

This section does not upgrade any theorem in this document. The universal cost floor, the minimality of $(\mu, \texttt{cert})$, the substitution contest, the CHSH bridge, the substrate diagonal all stand exactly where they stood, scoped exactly as Section~\ref{sec:unknown} scopes them, with the forced-event question still open. What it does is turn the fence's blank face into a door, hinges, a handle, and now five machine-checked worked examples of what lies on the far side of it.
...
Those are the two pillars, and neither is the foundation. What makes this document foundational is the universal A2 cost floor: \code{universal\_nfi\_any\_substrate} holds on \emph{any} substrate that carries a certification predicate and charges A2 on the cert-flip as a law of its step rule (its only premises the two trace endpoints, given a run that starts uncertified and ends certified), and \code{P\_full\_is\_minimal\_complete\_extension} pins $(\mu, \texttt{cert})$ as the proved-minimal extension over (mem, regs, pc), the state a step rule needs to read the cert and charge the $\mu$. The foundational weight is the priority claim, and it rides on universality, scoped-minimality conjoined with the substitution-adequacy only A2 has, and the law-versus-checker fact. Not on a physics-free Bell bridge, not on a Rice-family diagonal, and not on any claim that certification is the event a step rule is forced to price; that last question stays open, exactly as Section~\ref{sec:unknown} draws it and Section~\ref{sec:pointer} names its successor. The CHSH-NPA equivalence and the substrate-axis undecidability are strong companion facts, no more: an algebraic bridge with no physics premises on one side, and a limitative result internal to the substrate's own typeclass machinery on the other. The 51 opcodes, the OCaml runtime, the Bluespec hardware, the parity tests, the simulator harness are scaffolding for one realization of the substrate; the universal cost floor and the lattice-scoped irredundancy of $(\mu, \texttt{cert})$ over (mem, regs, pc) are facts about substrates, and the two theorems above are strong instances of how those facts cash out.
