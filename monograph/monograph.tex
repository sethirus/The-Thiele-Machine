...
The comparison with earlier work concerns the object each result studies. Landauer's erasure bound concerns physical dissipation under a thermodynamic model. No-free-lunch results concern averages over specified problem distributions. A2 is a lower-bound contract on designated certification transitions. Exact event pricing additionally imposes a no-overcharge condition. These comparisons distinguish the stated contracts; they do not establish historical priority or a unique choice of certification as the event to price. Primary sources for these limited comparisons are Landauer, \href{https://doi.org/10.1147/rd.53.0183}{``Irreversibility and Heat Generation in the Computing Process''} (1961), and Wolpert--Macready, \href{https://doi.org/10.1109/4235.585893}{``No Free Lunch Theorems for Optimization''} (1997).
...
\textbf{Substrate vs scaffolding.} Two layers run through the rest of this document, and I kept conflating them myself until the proofs forced the line; confuse them and the theorems quietly stop meaning what they say. The \emph{substrate} is the general structural layer: a state type and a step relation, extended by different typeclasses for different theorems, and the extensions are not all the same requirement. The A2-respecting extension, a certification indicator plus the rule that flipping it false-to-true costs at least one, is what No Free Insight needs; the ledger-initiality theorem separately fixes an initial value and increment schedule. These theorems hold for any state type and step relation carrying that extension, no matter what instruction set realizes it. The \code{WithShortcutPredicate} extension, a monotone-enough state type plus a shortcut-admittance predicate with two canonical witnesses and an internal recursion theorem, is what the structural-axis undecidability theorem in Section~\ref{sec:substrate-undecidability} needs instead, and that extension does not require A2 at all: the candidate-indexed \texttt{nat\_substrate} family that supplies its restricted fixed points has no certification content whatsoever, by construction. ``Substrate-level'' does not mean one fixed interface every theorem shares; it means each result is stated over the particular typeclass extension it actually uses, not folded into the 51-opcode VM. The \emph{scaffolding} is everything that makes a substrate runnable, verifiable, and synthesizable in practice: the 51 opcodes, the Python reference kernel, the extracted OCaml runtime, the Bluespec hardware, the simulator harness. The scaffolding-level constructions (the opcode semantics, the bisimulation between Coq and Verilog, the synthesised gate count) are how this particular substrate is made concrete enough that you can boot it, audit it, and run it on silicon. The 51-opcode VM realizes several of these typeclass extensions at once, an A2-respecting \texttt{CertCostMachine} and a \code{WithShortcutPredicate} instance among them, but a given substrate-level result borrows only the extension its own theorem is stated over. That is the distinction to hold onto: a substrate-level result is about the typeclass it names, and the 51-opcode VM is one instance realizing several such typeclasses simultaneously, not a single flat interface called ``the substrate.'' Let the distinction collapse and a theorem stated over one extension reads as though it carried premises it does not use.
...
\textbf{What is claimed unique, and what is not.} A conversational shorthand for this project says there is only one possible substrate. That shorthand covers three different mathematical claims, and this document does not establish all three.

The first is uniqueness of a law, given the A2 cost rule, is there essentially one way to price certification honestly. Section~\ref{sec:mu} answers a version of this for a fixed schedule: \code{instruction\_consistent\_measure\_equals\_mu} shows any measure sharing the kernel's own per-instruction schedule and a zero start is forced to equal $\mu$ on every reachable state. It does not show the schedule itself is forced. A different schedule prices differently and is a different, equally honest, accounting.

The second is uniqueness of an abstract structural core up to some stated equivalence: do all A2-respecting systems collapse, under the right notion of sameness, to one shared mathematical object. This is the strongest reading of ``only one substrate,'' and it is not a theorem here. It is a proposal, stated so it can be attacked, not a claim this document proves. Call a system \emph{adequate} if it carries an A2-respecting certification indicator, a step relation, and an observation interface rich enough to state No Free Insight, ledger initiality, and the diagonal. The proposed theorem form is
\[
\mathrm{Adequate}(M) \;\Longrightarrow\; \mathrm{StructuralCore}(M) \;\simeq\; \mathrm{ThieleCore},
\]
where $\mathrm{StructuralCore}$ strips a system down to the state, step, and certification data those theorems actually use, and $\simeq$ is an equivalence still to be pinned down, certainly not literal identity of opcodes, storage layout, implementation language, or clock cycles, since none of those enter any premise of any theorem in this document.

The history-retaining target is the test case that shows why $\simeq$ is hard to pin down and why this stays a proposal rather than a theorem. A target construction discussed later in this document (Section~\ref{sec:projection-nonexistence}; the Coq witness is \code{certification\_agreement\_does\_not\_imply\_descent}) retains the VM's full instruction history, charges the same schedule, and reads certification the same way the VM does. It agrees with the Thiele VM on every certification and every charge. It still distinguishes the empty trace from \texttt{JUMP 0 0}, two traces the VM identifies, because it keeps the history and the VM does not. A2 and certification agreement alone do not force that extra history scaffolding to collapse away. Any adequacy condition strong enough to make $\mathrm{StructuralCore}$ identify this target with the VM has to say, explicitly, what makes the retained history inessential to the equivalence, and that is exactly the premise the proposal above leaves open.

The third claim, uniqueness of a particular state-transition object, this exact VM state space up to renaming, is not made anywhere in this document and is not what any theorem needs. The separating witnesses (Section~\ref{sec:categorical}'s graph pair, the irrecoverability theorems above) stand on their own regardless of how the second claim resolves. They do not require it, and it does not require them.

So: the first claim is proved, for a fixed schedule. The third claim is not made. The second is the open one, offered here as $\mathrm{Adequate}(M) \Rightarrow \mathrm{StructuralCore}(M) \simeq \mathrm{ThieleCore}$, a conjecture with a stated test case it has to survive, not a theorem this document proves.
...
\midrule
File-local TM lift & \texttt{th\_tm\_config} & \code{thiele\_run}/\code{tm\_run}, fuel-indexed TM step & TM configuration, $\texttt{th\_mu}=0$ & \code{thiele\_simulates\_turing} \\
VM instruction-list fold & \texttt{VMState} & \texttt{run}, folds a fixed instruction list once, in order & full \texttt{VMState} & D2/D3, degenerate projection theorem \\
PC-indexed runner & \texttt{VMState} & \texttt{vm\_apply}, single-step transition driven by \texttt{vm\_pc} & full \texttt{VMState} & $\mu$-conservation, \texttt{instruction\_cost} agreement \\
Bounded evaluator & \texttt{VMState} & \texttt{vm\_run}, fixed 1000-step fuel, always returns \texttt{Some} & full \texttt{VMState}, fuel-truncated & structural-axis undecidability, VM instance (conditional, Section~\ref{sec:substrate-undecidability}) \\
Hardware retirement model & Kami/RTL state & \texttt{WFDrivenRun}, clock-driven, multi-cycle per instruction & full \texttt{VMState} at instruction retirement & \code{rtl\_contract\_trace\_agreement} \\
...
\texttt{forget} & 4-tuple & (a projection) & adds $\mu$ to the above & \code{cert\_not\_function\_of\_forget} \\
\texttt{shadow\_proj} & 6-tuple & (a projection) & adds \texttt{vm\_err}, \texttt{vm\_certified}; still drops the graph & degenerate projection theorem, \code{shadow\_strictly\_lossy} \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
Let $T$ be a transcript type with a projection $\pi : T \to \texttt{list StrictClassicalState}$ and a relation $\texttt{explains\_T} : \texttt{VMState} \to T \to \mathrm{Prop}$ connecting VM states to the transcripts that could have produced them. Suppose the bare-setting witness pair (states $s_A, s_B$ with identical bare projections, $\texttt{vm\_mu}\ s_A = 1$, $\texttt{vm\_mu}\ s_B = 0$) lifts to $T$: there exist $t_A, t_B : T$ with $\pi(t_A) = \pi(t_B)$, $\texttt{explains\_T}\ s_A\ t_A$, and $\texttt{explains\_T}\ s_B\ t_B$. Then no verifier $V : T \to \texttt{bool}$ that is sound (every transcript $V$ accepts explains only $\texttt{vm\_mu} = 1$ states) and complete (every transcript explaining a $\texttt{vm\_mu} = 1$ state is accepted) factors through $\pi$.
\end{theorem}
...
The parametric rider belongs to the $\mu$ results, and there it is not decoration: from any reachable state the certify-versus-jump split is still available, so no prefix, however long, prevents the $\mu$ witness pair. The certified results run from any uncertified state, and that is as far as they can go, for the honest reason that certification is permanent: once a prefix has certified, nothing clears the flag, so both branches of any extension agree on it. The theorem's content never needed more; one witness pair is exactly what ``not a function of'' takes, and swapping \texttt{PNEW} for a graph-inert instruction of matching cost is what lets the same style of witness carry the graph-fixed hypothesis too, closing the gap an earlier pass of this document left open. \qed
\end{proof}
...
Run any trace. The $\mu$ it accumulates is just the sum of the step costs, charged by the kernel's own fixed per-instruction schedule \texttt{canonical\_cost} (the schedule \texttt{vm\_apply} itself charges, not a stand-in for some other pricing choice). Partition graph, registers, memory, program counter, certified flag: none of them changes that sum. Formally:
\[
\mu_{\text{final}} = \mu_{\text{initial}} + \textstyle\sum_{i \in \textit{prog}} \texttt{instruction\_cost}(i).
\]
The right side uses the summation symbol from Section~\ref{sec:vocab}. It reads: add up \texttt{instruction\_cost}(i) for every program element $i$ in \textit{prog}. The result does not depend on the starting state's structural fields. Two runs of the same program from any two starting states with equal $\mu_{\text{initial}}$ accumulate the same additional $\mu$. Any measure that charges exactly \texttt{canonical\_cost} at every step, the identical schedule, and assigns $0$ to \texttt{init\_state}, agrees with \texttt{vm\_mu} on every reachable state (\code{instruction\_consistent\_measure\_equals\_mu}). This fixes the accumulated total once the schedule and the zero start are fixed. It is not a claim that \texttt{canonical\_cost} is the only schedule a system could honestly adopt, and it says nothing about a measure charging a different per-instruction price; Section~\ref{sec:mu} below keeps that separate question, exact-event pricing among possible schedules, apart from this one. If any step in \textit{prog} has positive \texttt{canonical\_cost}, then $\Delta\mu$ is strictly positive: this is the theorem's positive-cost-step guarantee, not a claim that no program or no instruction can cost zero.
\end{theorem}
...
\end{tikzpicture}
\caption{The decision tree witness. Each internal node is a certified observation; each leaf is one posterior class. The realised-by-the-trace condition bounds the tree's depth by the count of cert-setter steps the trace executes, a numeric payment bound, not a claim that each node is individually matched to a specific step. The tree's depth is what bounds the entropy gain on the right, and the $\mu$ paid for the trace is what bounds the depth on the left.}
...
What is reachable at runtime is narrower than that type permits. \texttt{MORPH} always constructs its morphism with an empty coupling list, regardless of the \texttt{coupling\_idx} argument the opcode is given; \texttt{MORPH\_ID} also starts empty. Composing or concatenating empty coupling lists returns an empty coupling list. So in the current reference semantics, every coupling list reachable from \texttt{init\_state} through the seven opcodes of this group is empty, and \texttt{MORPH\_GET} selector 2 reads back zero on any reachable state; the kernel does contain a decoder, \texttt{load\_coupling\_from\_mem}, meant to populate a morphism's coupling from a serialized block in VM memory, but nothing in the current opcode semantics calls it, so it is unused code rather than an active loading path. The relational-composition and concatenation proofs above hold at the level of the \texttt{CouplingData} type for arbitrary coupling lists, including states a caller could construct directly in Coq; they are not, at present, proofs about data any reachable VM trace produces. The graph-presence separating witness used elsewhere in this section (morphism existence, not coupling content) does not depend on nonempty coupling data and is unaffected by this gap.
...
The CI (Full) workflow defines synthesis, simulation, and FPGA-generation jobs. Historical resource, timing, and bitstream reports describe their recorded RTL and tool versions. They do not measure a changed Kami source merely because its Coq proofs compile. For this revision, regenerated RTL, synthesis resources, timing, and bitstream claims require fresh pipeline artifacts linked to the source identity in Section~\ref{sec:reproduction}.
...
The gas example defines a unit commitment counter. \code{gas\_schedule\_exactness} characterizes exact pricing relative to its designated commitment predicate: floor and no-overcharge together identify the charging predicate and unit price. A deployed gas schedule also prices resources such as computation and storage. Its full fee is not identified with this counter.
...
\begin{plainreading}
The formal fixed-completion predicate is equivalent to column contractivity. The resulting row bounds give the CHSH inequality by Cauchy--Schwarz. Arbitrary quantum behaviors and empirical local samples have separate definitions.
\end{plainreading}
...
\]
This is a total Coq function from \texttt{Program} to \texttt{Program}. The theorem also requires that this particular \texttt{flip} satisfy \texttt{Representable}; total Coq definability alone does not establish that premise. With that premise, apply the substrate's recursion-theorem field to \texttt{flip}: there exists a program \texttt{p} with \texttt{prog\_equiv p (flip p)}.
...
Repository: \url{https://github.com/sethirus/The-Thiele-Machine}. The review workspace starts from commit \code{69b968906eeb88622f6480cd65a011a7d021b890}. The changes described here are supplied by the companion \texttt{artifacts/review\_revision/source.patch}; \texttt{source\_manifest.json} identifies its bytes and the reviewed source files. The base commit alone does not contain these changes. Publication of a final release commit remains separate from this local review artifact.

The verified local toolchain is Coq 8.18.0, OCaml 4.14.1, and Python 3.12.1, with the vendored Kami and bbv dependencies built. From a checkout of that base commit, apply the companion patch with \texttt{git apply source.patch}. Then run the following commands from the repository root in the prepared toolchain:
\begin{verbatim}
...
make -j2
cd ..
python3 scripts/check_review_contracts.py \
  > artifacts/review_revision/contract_report.txt
bash monograph/build_monograph.sh
\end{verbatim}

The Coq build and contract probe must exit zero. Inspect the report's theorem types as well as its global dependencies; a closed report does not discharge explicit premises. The monograph build prints \texttt{Build Successful} and regenerates \texttt{monograph.pdf} and \texttt{monograph.txt}. The full-corpus reports remain in \texttt{artifacts/print\_assumptions\_all\_proofs.*}; their stored counts belong to the source version recorded by that audit, not automatically to every later edit.

RTL generation is a separate command, \texttt{bash scripts/kami\_extract.sh}, requiring the Bluespec compiler and its standard library in addition to the Coq/OCaml toolchain. Its logs, generated Verilog, synthesis report, and any bitstream must carry the source identity they actually used. A successful Coq build supplies no fresh synthesis or timing measurement. The saved Inquisitor report likewise retains its generation date and waiver census.
...
The saved \texttt{INQUISITOR\_REPORT.md}, generated 11 September 2026, records 297 scanned Coq files, zero HIGH, MEDIUM, and LOW unsuppressed findings, and 341 suppression markers across 140 files. This is a historical report for its recorded source state. Regenerate it for a changed tree. The rule checks and waiver census are heuristic evidence; they do not prove semantic applicability or absence of every vacuous premise.
...
Each row identifies the result's domain, conclusion, load-bearing premises, and instance status. A symbol prefixed by \texttt{Kernel} is in its namesake file under \texttt{coq/kernel/}; \texttt{KamiHW} names files under \texttt{coq/kami\_hw/}. The companion \code{artifacts/review\_revision/contract\_sources.json} maps every row to its source file and line. The \texttt{contract\_probe.v} file in that directory loads the exact modules. Its report combines \texttt{Check}, expanded records and definitions via \texttt{Print}, and \texttt{Print Assumptions}. The source declaration and its imported definitions remain the complete mathematical contract.
