...
\item \textbf{6 opcodes require valid instruction arguments.} The hardware and Coq model AGREE on the error path for invalid arguments, so no semantic disagreement exists, but the success-path proof requires the argument to be in range:
 \begin{itemize}
 \item \texttt{CALL}: stack pointer must be in bounds (\texttt{WellFormedSnapshot}), and $\texttt{pc} < \texttt{MEM\_SIZE}$.
 \item \texttt{RET}: stack pointer must be in bounds (\texttt{WellFormedSnapshot}).
 ...
 \end{itemize}
 Covered in \texttt{EmbedStep\_WF.v} (\texttt{CALL}, \texttt{RET}, \texttt{CHSH\_TRIAL}) and \texttt{GraphReconstructionBridge.v} (\texttt{TENSOR\_SET}, \texttt{TENSOR\_GET}, \texttt{LASSERT}).
...
So: maximum classical win rate = 3/4 = 75\%. Proven by exhaustion in \texttt{coq/kernel/foundation/ClassicalBound.v:165} (zero Admitted): an executable trace using only \texttt{PNEW}, \texttt{PSPLIT}, and \texttt{CHSH\_TRIAL} with $\mu = 0$ achieves exactly this, and \texttt{coq/kernel/quantum/MinorConstraints.v:822} (\texttt{local\_box\_CHSH\_bound}) proves no $\mu = 0$ program can exceed it.
