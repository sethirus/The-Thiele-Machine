...
\begin{abstract}
The Thiele Machine is a computational substrate where certification cost ($\mu$) is tracked at the step relation rather than at the program layer. The principal structural result, Coq-verified with zero axioms, is asymmetric: every Thiele state has a canonical forgetful projection onto a classical-equivalent state, and every classical state has multiple Thiele preimages with no canonical lift between them. The projection is the canonical map; the lift is not. The fields the projection drops --- \texttt{vm\_certified}, \texttt{csr\_cert\_addr}, $\mu$ relative to the bare three-field observable that drops it --- are provably not functions of the classical projection (\texttt{cert\_not\_function\_of\_forget}, \texttt{cert\_addr\_not\_function\_of\_forget}, \texttt{mu\_not\_function\_of\_bare\_observable} in \texttt{coq/kernel/witness/ProjectionNonExistence.v}, all closed under the global context). Every Thiele machine has a Turing projection; no Turing machine has a canonical Thiele lift; and the directionality is provable, not stipulated. The categorical universal property is closed: Thiele is the initial object in the category of A2-respecting substrates over the Thiele instruction signature (\texttt{thiele\_is\_initial\_a2\_substrate} in \texttt{coq/kernel/nfi/ThieleInitiality.v}, zero axioms), in the same precise sense that $\mathbb{Z}$ is the initial ring. Whether the A2 category is itself foundationally privileged over alternative cost-tracking categories is a meta-mathematical judgment outside what the proofs decide; the proofs deliver the categorical universal property, not metaphysical primacy. The structural placement of each load-bearing result against its standard parent --- the irrecoverability theorems against signature-expressibility, the categorical initiality against the universal property of the term algebra, the substrate-axis undecidability against Kleene's 1938 diagonal pattern, the $\mu$-uniqueness against natural-number recursion --- is enumerated in Section~\ref{sec:unknown}, so a reader pattern-matching to ``this is just X'' can see which X is being matched and what the substrate-specific delta is before building an objection that is already named. The argument is four steps. To dismiss it, name the step you reject. A wave-off is not a response.

\textbf{1. A2 constrains state fields the bare classical signatures do not have.} A2 is the rule: every step that flips a certification predicate from false to true forces the executed instruction's cost $\geq 1$. The certification predicate and the cost function are not in the bare Turing-machine signature (state, alphabet, transition table), nor in register machines or lambda calculi. If you extend a classical signature with cert and cost fields and a step rule that respects A2, you have moved to a different substrate --- the \texttt{CertificationSystem} record in \texttt{coq/kernel/nfi/UniversalCertificationCost.v}, over which \texttt{universal\_nfi\_any\_substrate} concludes a trace-level cost floor of $\geq 1$. \texttt{cert\_not\_function\_of\_forget} (\texttt{ProjectionNonExistence.v}) sharpens the boundary: Thiele's certification flag is not a derived function of the bare classical projection, so A2's cert-predicate cannot be recovered from classical fields by any clever predicate. The claim is not that A2 is unwritable in the absolute sense; it is that A2 is a constraint on state fields the bare classical signatures lack, so any model carrying A2 is no longer the bare classical model.

\textbf{2. Therefore the substrate is forced.} Any model that formalises certification cost at the step rule must carry state that classical models do not have. There is no ``TM plus A2'' --- there is no A2 on a TM, by \texttt{cert\_not\_function\_of\_forget}. The structural axis (\texttt{vm\_mu}, \texttt{vm\_certified}, \texttt{csr\_cert\_addr}) is the minimum state for A2 to be a sentence in the first place. The substrate is not an extension of classical computation; it is what is required for the classical models' silent omission to be representable at all. The cost-ledger separation in Section~\ref{sec:categorical} below depends on $\mu$ being a state field; \texttt{mu\_not\_function\_of\_bare\_observable} proves that separation has no analog in the bare classical signature.

\textbf{3. Classical computation is the projection, and the projection is not invertible.} \texttt{lift\_config} sends every Turing-machine configuration to a Thiele configuration. \texttt{thiele\_simulates\_turing} runs every Turing-machine trace inside the substrate, same tape, same state. \texttt{degenerate\_projection\_theorem} closes the loop in one direction: classical computation is exactly the image of substrate computation under the projection that forgets the structural axis. \texttt{fiber\_has\_two\_preimages} (\texttt{coq/kernel/witness/BlindnessRepresentation.v}:238) closes the other direction: every classical state has multiple Thiele preimages, so any lift back is non-canonical and requires external choice of cost schedule, graph state, and certification flag. \texttt{D4\_strictness} witnesses substrate states with no classical preimage. Same computable functions on both sides; classical substrates are not a parallel option, they are the projection. Every Turing machine, every register machine, every lambda-calculus reducer, every CPU on every desk: a Thiele Machine running with the structural axis sidelined, and the path back from the classical side requires data the classical side does not provide.

\textbf{4. Name the step you reject.} Step 1 is a sentence about what a classical step relation can carry, witnessed by \texttt{cert\_not\_function\_of\_forget}. Step 2 follows from step 1, witnessed by \texttt{mu\_not\_function\_of\_bare\_observable}. Step 3 is mechanised in Coq, with theorem names above; the asymmetry of projection vs lift is witnessed by \texttt{fiber\_has\_two\_preimages}. ``This sounds like a metaphor'' is not a rejection; it is a refusal to read.
...
An ISA (instruction set architecture) is the list of all instructions the CPU recognises, plus their formats and costs. Each instruction is identified by an opcode (a numeric tag the hardware decodes to choose which behaviour to run). This machine has 51 opcodes.
...
A1 through A6 are independent of any specific instruction set. They are conditions on Coq record types; supplying instances of those record types is the only way to invoke the corresponding theorems. The 51-opcode VM supplies them by construction: \texttt{coq/kernel/nfi/UniversalCertificationCost.v} builds two concrete \texttt{CertificationSystem} values for the VM (one for the \texttt{csr\_cert\_addr} channel, one for the \texttt{vm\_certified} channel), each discharging A2 from the existing \texttt{no\_free\_certification} kernel theorems. A separate substrate-level abstraction (the \texttt{Substrate} typeclass in \texttt{coq/kernel/foundation/Substrate.v}) carries a related but distinct field \texttt{mu\_monotone} stating that the cost ledger never decreases on a step. \texttt{mu\_monotone} is a strictly weaker general-purpose monotonicity property than A2: A2 specifically asserts the cert transition costs $\geq 1$, whereas \texttt{mu\_monotone} only asserts the ledger does not go down. The two coincide on the 51-opcode VM, where every instruction adds non-negative cost and the cert transition adds at least 1.
...
Can a Turing machine track something equivalent to $\mu$ on its tape and produce traces whose tape encoding satisfies A2? Yes, but A2 then lives in the program running on the Turing machine, not in the Turing machine itself. Here is what that means. The Coq record \texttt{CertificationSystem} (defined in \texttt{coq/kernel/nfi/UniversalCertificationCost.v}) is a Coq type that bundles a state, an instruction set, a step rule, a cost function, and a proof of A2 as one of its fields. Here is the actual record body, copied verbatim from the kernel file:
...
The subsumption theorems above establish what Thiele does over and above its classical projection: it carries fields the projection drops. The natural next question is whether a classical observer could nonetheless reconstruct those fields by some clever derived predicate on the surviving shadow. The answer is no, and the no is structural rather than rhetorical. \texttt{coq/kernel/witness/ProjectionNonExistence.v} proves three irrecoverability theorems --- each one a non-existence of a function from the bare classical projection back to a Thiele-only field --- and packages the corollaries the subsumption argument relies on. \texttt{Print Assumptions} on every theorem in the file returns \texttt{Closed under the global context}: zero axioms, zero admits, constructive proofs.

The bare classical projection in this subsection is the four-field map $\texttt{forget} : \texttt{VMState} \to \texttt{TMSnapshot}$ from \texttt{BlindnessRepresentation.v}, keeping $(\texttt{pc}, \mu, \texttt{regs}, \texttt{mem})$. For the cost-ledger separation, a stricter three-field projection $\texttt{bare\_observable}$ defined in \texttt{ProjectionNonExistence.v} drops $\mu$ as well, keeping only $(\texttt{pc}, \texttt{regs}, \texttt{mem})$: the observable the standard complexity-theory cost model sees.
...
\medskip
\noindent\textbf{Plain reading.} The Thiele \texttt{vm\_certified} flag is not a function of the four classical fields. Two Thiele states can agree on $(\texttt{pc}, \mu, \texttt{regs}, \texttt{mem})$ and still differ on whether they are certified. No formula computed from the four-tuple alone can decide certification, because the certification bit lives outside the four-tuple.
...
\medskip
\noindent\textbf{Plain reading.} The Thiele $\mu$ counter is not a function of the three-field classical observable $(\texttt{pc}, \texttt{regs}, \texttt{mem})$. Two Thiele states can agree on the classical observable and still have arbitrarily different $\mu$ values. The cost-ledger separation between Thiele states that the classical observable cannot tell apart is not a derived fact that a clever classical observer could compute; it is a state distinction the classical observer's signature is structurally blind to.
...
\medskip
\noindent\textbf{Plain reading.} The Thiele \texttt{csr\_cert\_addr} channel is not a function of the four classical fields. The structural-axis diagonal in Section~\ref{sec:substrate-undecidability} below builds an undecidable predicate over \texttt{csr\_cert\_addr} reachability; that predicate cannot be re-expressed as a predicate over the four classical fields because the field it references is not visible in the classical signature.
...
\begin{enumerate}
\item \emph{A2 cannot be stated as a constraint on the bare projection alone.} A2 (\texttt{cs\_cert\_costs}, a typed field on \texttt{CertificationSystem}) is the constraint that a cert-predicate flip from \texttt{false} to \texttt{true} forces instruction cost $\geq 1$. By cert irrecoverability, any cert-predicate phrased as a function of the bare \texttt{TMSnapshot} cannot agree with Thiele's \texttt{vm\_certified} on every Thiele state. A2 phrased on the bare classical signature is therefore not Thiele's A2: it constrains a different predicate. The Coq corollary is \texttt{no\_classical\_a2\_cert\_predicate}.

\item \emph{The cost-ledger observability separation has no analog in the bare projection.} The separation in Section~\ref{sec:categorical} below names two Thiele states with identical $(\texttt{pc}, \texttt{regs}, \texttt{mem})$ but different $\mu$. The separation as a predicate on the bare classical observable requires reading $\mu$ off that observable, which is impossible by mu-irrecoverability. The separation lives at the Thiele level by structural necessity, not by drafting choice. The Coq corollary is \texttt{no\_classical\_mu\_separation\_predicate}.

\item \emph{The structural-axis diagonal has no analog in the bare projection.} The undecidable predicate constructed in Section~\ref{sec:substrate-undecidability} branches on \texttt{csr\_cert\_addr} reachability. By cert-addr irrecoverability, no function on the bare \texttt{TMSnapshot} can recover \texttt{csr\_cert\_addr}. The diagonal predicate, when lifted from Thiele programs to programs interpreted on the bare projection alone, references a field that the projection erases; the lift is therefore ill-defined. The Thiele-axis undecidability is not the halting problem in disguise; it is a different limitative result, structurally distinct because the predicate that participates in the diagonal references a Thiele-only field. The Coq corollary is \texttt{no\_classical\_cert\_addr\_predicate}.
\end{enumerate}
...
This is the structural asymmetry the subsumption argument names. The projection $\texttt{Thiele} \to \texttt{Turing}$ is canonical: it is the unique forgetful map onto the four classical fields, and it is a structure-preserving quotient (parts 2 and 3 of the degenerate projection theorem). The lift $\texttt{Turing} \to \texttt{Thiele}$ is not canonical: every \texttt{lift\_config}-style definition fixes a choice of $\mu = 0$, empty graph, and \texttt{vm\_certified = false}, but those choices are not extracted from the Turing data; they are appended. Every Thiele machine has a Turing projection; no Turing machine has a canonical Thiele lift.
...
The asymmetry pinned down above has a categorical counterpart: Thiele is not merely structurally richer than its classical projection; it is initial in the category of A2-respecting substrates over the Thiele instruction signature. The theorem \texttt{thiele\_is\_initial\_a2\_substrate} (\texttt{coq/kernel/nfi/ThieleInitiality.v}, \texttt{Print Assumptions} returns \texttt{Closed under the global context}) packages this universal property: for any A2-respecting substrate $M$ over \texttt{vm\_instruction} and any chosen basepoint $s_0^M$ in $M$'s state space, the canonical trace-fold simulation $\texttt{trace} \mapsto \texttt{fold\_left}\ M.\texttt{ccm\_step}\ \texttt{trace}\ s_0^M$ is the unique morphism preserving the basepoint and commuting with step-extension. Any other function $g$ satisfying the same two conditions agrees with the canonical trace-fold on every trace; the proof is a one-line induction on traces using the step-commutation hypothesis.

The state-level corollary \texttt{thiele\_morphism\_unique\_on\_reachable} packages the consequence: any two \texttt{CertCostMorphism} maps from Thiele to $M$ agreeing on \texttt{init\_state} agree on every state reachable from \texttt{init\_state}. States unreachable from \texttt{init\_state} are not constrained by the universal property; they live outside the categorical reach of Thiele.

This is initiality in the precise categorical sense: in the category of A2-respecting substrates over \texttt{vm\_instruction} with trace-fold morphisms restricted to reachable states from a chosen basepoint, Thiele is the initial object. The technical content is the universal property of the term algebra of an essentially-algebraic theory: the free model is initial in the category of models over its own signature, and receives a unique morphism into every other model in that category. The free group on a fixed generating set has the same shape of universal property within group theory. The $\mathbb{Z}$-as-initial-ring framing used in the abstract is the accessible analogy for a reader meeting initiality for the first time; the precise statement as named here is the term-algebra universal property, which is the form the Coq theorem actually delivers. The proof of \texttt{thiele\_is\_initial\_a2\_substrate} is short (~30 lines, all by list-induction); the work was in the surrounding machinery (\texttt{CertCostMachine}, \texttt{CertCostMorphism}, \texttt{thiele\_morphism\_exists}, \texttt{thiele\_morphism\_unique\_on\_traces}) that this theorem packages into one named universal property.

The scope of the categorical universal property is named precisely in Section~\ref{sec:unknown}: the initiality is relative to the A2-respecting category. Whether that category is itself foundationally privileged over alternative cost-tracking categories is a meta-mathematical judgment outside what the proofs decide.
...
Here is the chain, written out, because this is the place readers most often collapse two distinct steps into one. Suppose register \texttt{r0} holds the address of a four-byte formula in memory and \texttt{r1} holds the address of a dual-witness certificate (a satisfying assignment together with a falsifying assignment, the non-triviality witness from Section~\ref{sec:isa}). Step one: \texttt{LASSERT freg=r0 creg=r1 kind=SAT flen=4 delta=0} reads the formula header, walks the formula and the two witnesses through the on-chip Logic Engine, charges $4 \times 8 + 1 = 33$ to $\mu$, and either advances the PC (validation passed) or traps to \texttt{LASSERT\_TRAP\_PC} and latches \texttt{vm\_err = true} (validation failed). The $\mu$ cost is charged either way. Crucially, both \texttt{csr\_cert\_addr} and \texttt{vm\_certified} are unchanged by this step regardless of whether validation passed. Step two, conditional on step one passing: \texttt{MORPH\_ASSERT mid=m property="validated\_constraint" cert="" delta=0} charges $S(0) = 1$ to $\mu$ and flips \texttt{csr\_cert\_addr} to the ASCII checksum of \texttt{"validated\_constraint"}, which is nonzero by construction because the property string is non-empty. The supra-cert channel is now active.

Total cost across the two steps: 34 $\mu$. LASSERT paid for the validation work. MORPH\_ASSERT paid for the channel flip. The cert channel only crossed from 0 to nonzero on the second step, not the first. The two costs are not redundant: one is the cost of checking the constraint package against its dual witness, the other is the cost of committing to the result of that check by activating the cert channel. A downstream certified predicate that references \texttt{csr\_cert\_addr} sees the channel-active state produced by the MORPH\_ASSERT step and is allowed to rely on the validated constraint produced by the LASSERT step. Skip either step and the chain breaks: a LASSERT alone leaves the cert channel cold, and a MORPH\_ASSERT without a preceding LASSERT activates the channel without an underlying validated constraint --- you have paid for a commitment to nothing.
...
\begin{proof}
Case analysis over the 51 opcodes shows that \texttt{vm\_certified} flips from false to true only when $i = \texttt{instr\_certify}$; every other opcode leaves it unchanged. The CERTIFY apply rule charges $S(\delta) \geq 1$. So the single-step instance of A2 holds for this channel. Instantiate the universal theorem with $\mathrm{cert}(s) := s.\texttt{vm\_certified}$, otherwise as above. \qed
\end{proof}
...
\item $s.\texttt{vm\_csrs}$: control-status registers. In hardware design, there are two kinds of registers. Data registers (like $s.\texttt{vm\_regs}$) hold computation values. Control-status registers hold machine configuration and status. This record has four fields:
 ...
 \item \texttt{csr\_status}: general status code. Informational; the kernel does not read it to gate opcodes.
...
The Thiele Machine has 51 opcodes (the term ``opcode'' is in Section~\ref{sec:vocab}: the numeric tag the hardware decodes to choose which behaviour to run). Every one carries an explicit \texttt{mu\_delta} parameter. Cost is part of the instruction. The machine reads it, the machine charges it.
...
The number 51 comes from what the machine needs to do:
\begin{itemize}
...
Coq can automatically extract executable code from a Coq development. The language it targets here is OCaml, a general-purpose programming language. ``Extraction'' means: Coq keeps the computational content of the definitions and throws away the proof terms. A function like \texttt{vm\_apply} (which defines what every opcode does) extracts to an OCaml function that computes the same thing. The extracted result is not a translation of the proofs, the proofs are erased. What remains is the algorithm. The extracted runner executes all 51 opcodes from the Coq semantics, including the morphism operations and CHSH\_LASSERT.
...
47 of the 51 opcodes are implemented in the RTL design: \texttt{ThieleCPUCore.v} has hardware logic for those 47. The remaining four --- the Q\textsubscript{1+AB} cert-opcode family \texttt{instr\_chsh\_lassert\_1ab}, \texttt{instr\_chsh\_lassert\_1ab\_g5}, \texttt{instr\_chsh\_lassert\_1ab\_g345}, \texttt{instr\_chsh\_lassert\_1ab\_g12345} --- are VM-only by design, executing in the OCaml runner and Python harness but absent from the RTL CPU. The OCaml/RTL opcode-set parity test tolerates exactly these four. The question is not which opcodes are in hardware, but which opcodes have a completed formal \emph{bisimulation proof}.
...
Formal bisimulation proofs exist for all 47 RTL-realised opcodes (zero Admitted, all Qed). The four VM-only Q\textsubscript{1+AB} cert-opcodes have no RTL counterpart, so the question of RTL bisimulation does not arise for them; they are covered instead by the OCaml-runner / Python-harness layer plus the substrate-level Coq theorems of \S\ref{sec:chsh}. Here is the precise breakdown for the 47 RTL-realised opcodes:
\begin{itemize}
...
The official classification (\texttt{coq/kami\_hw/RTLGapRegistry.v}, theorem \texttt{rtl\_coverage\_partition}) records the partition $37 + 10 + 0 = 47$ for the RTL-realised subset (all Qed, zero structural gaps within that subset). The four VM-only Q\textsubscript{1+AB} cert-opcodes sit outside the RTL surface; the OCaml/RTL opcode-set slack of 4 between the 51-opcode abstract ISA and the 47-opcode RTL partition is by design.

The 47/47 RTL-bisimulation coverage (all Qed) is the correct relationship between the RTL-realised subset and its physical instantiation: the abstract model defines what is correct, the hardware implements 47 of the 51 opcodes, and all 47 of those have formal bisimulation proofs. The four VM-only Q\textsubscript{1+AB} cert-opcodes are software-tier cert checks that the Coq kernel proves sound at the substrate level (see \S\ref{sec:chsh}). This is honest proof accounting.
...
\item It does not mean the hardware implements exactly the Coq semantics in all cases. All 47 RTL-realised opcodes have Qed bisimulation proofs (37 unconditional, 10 under structural invariants \texttt{coupling\_wf} and \texttt{morph\_table\_wf} that hold inductively from hardware reset, zero structural gaps within the RTL-realised subset); documented in \texttt{RTLGapRegistry.v} and \texttt{CloseoutVerification.v}. The four VM-only Q\textsubscript{1+AB} cert-opcodes are outside the RTL surface by design.
...
\begin{itemize}
\item \textbf{Hardware bisimulation}: 47/47 RTL-realised opcodes with Qed proofs (37 unconditional + 10 under structural invariants \texttt{coupling\_wf} / \texttt{morph\_table\_wf}, 0 structural gaps in the RTL-realised subset); documented in \texttt{RTLGapRegistry.v} (\texttt{rtl\_coverage\_partition} witnesses $37 + 10 + 0 = 47$). The four VM-only Q\textsubscript{1+AB} cert-opcodes (slack 4 in the OCaml/RTL opcode-set parity test) are software-tier cert checks documented alongside their substrate-level Coq soundness theorems.
