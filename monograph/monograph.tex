...
builds a fresh source snapshot and never relies on a review-session patch or
precompiled proof objects.
...
The committed \texttt{INQUISITOR\_REPORT.md} records the result for the
source state from which it was generated: zero unsuppressed HIGH, MEDIUM, and
LOW findings. Regenerate it whenever the source tree changes. The rule checks
and waiver census are heuristic evidence; they do not prove semantic
applicability or absence of every vacuous premise.
...
Each row identifies the result's domain, conclusion, load-bearing premises, and instance status. A symbol prefixed by \texttt{Kernel} is in its namesake file under \texttt{coq/kernel/}; \texttt{KamiHW} names files under \texttt{coq/kami\_hw/}. The stable contract probes under \texttt{tests/coq\_probes/} load the exact modules. Their reports combine \texttt{Check}, expanded records and definitions via \texttt{Print}, and \texttt{Print Assumptions}. The source declaration and its imported definitions remain the complete mathematical contract.
