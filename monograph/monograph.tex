...
\begin{abstract}
The Thiele Machine is a computational substrate where certification cost ($\mu$) lives in the step rule. The step rule is a small bureaucrat that refuses to advance the state until the cost has been counted. The program does not get a vote. The principal structural result is asymmetric, Coq-verified, zero axioms. Every Thiele state has a canonical forgetful projection onto a classical-equivalent state. Every classical state has multiple Thiele preimages and no canonical lift back. The projection is canonical. The lift is not. The fields the projection drops (\texttt{vm\_certified}, \texttt{csr\_cert\_addr}, $\mu$ relative to the bare three-field observable that drops it) are provably not functions of the classical projection (\texttt{cert\_not\_function\_of\_forget}, \texttt{cert\_addr\_not\_function\_of\_forget}, \texttt{mu\_not\_function\_of\_bare\_observable} in \texttt{coq/kernel/witness/ProjectionNonExistence.v}, all closed under the global context). Every Thiele machine has a Turing projection. No Turing machine has a canonical Thiele lift. The directionality is proved, not stipulated.

The categorical universal property is closed. The Thiele substrate is the initial object in the category of A2-respecting substrates over the \texttt{vm\_instruction} signature (\texttt{thiele\_is\_initial\_a2\_substrate} in \texttt{coq/kernel/nfi/ThieleInitiality.v}, zero axioms). Same precise sense in which $\mathbb{Z}$ is the initial ring. Whether the A2 category is itself foundationally privileged over other cost-tracking categories is a meta-mathematical judgment the proofs do not decide. The proofs deliver the universal property. Metaphysical primacy is a separate question, asked of nobody by the theorems and answered by nobody in them.

Each load-bearing result has a standard parent in the literature: the irrecoverability theorems against signature-expressibility, the categorical initiality against the universal property of the term algebra, the substrate-axis undecidability against Kleene's 1938 diagonal pattern, the $\mu$-uniqueness against natural-number recursion. Section~\ref{sec:unknown} enumerates each placement, so a reader pattern-matching to ``this is just X'' can see which X is being matched and what the substrate-specific delta is before building an objection that is already named. The argument is four steps. To dismiss it, name the step you reject. A wave-off is not a response.

\textbf{1. A2 constrains state fields the bare classical signatures do not have.} A2 is the rule. Every step that flips a certification predicate from false to true forces the executed instruction's cost $\geq 1$. The certification predicate and the cost function are not in the bare Turing-machine signature (state, alphabet, transition table). They are not in register machines. They are not in lambda calculi. Extend a classical signature with cert and cost fields plus a step rule that respects A2, and you have moved to a different substrate: the \texttt{CertificationSystem} record in \texttt{coq/kernel/nfi/UniversalCertificationCost.v}, over which \texttt{universal\_nfi\_any\_substrate} concludes a trace-level cost floor of $\geq 1$. \texttt{cert\_not\_function\_of\_forget} (\texttt{ProjectionNonExistence.v}) sharpens the boundary. Thiele's certification flag is not a derived function of the bare classical projection. A2's cert-predicate cannot be recovered from classical fields by any clever predicate. The claim is not that A2 is unwritable in the absolute sense. The claim is that A2 constrains state fields the bare classical signatures lack, so any model carrying A2 is no longer the bare classical model. The migration is one-way.

\textbf{2. Therefore the substrate is forced.} Any model that formalises certification cost at the step rule must carry state that classical models do not have. There is no ``TM plus A2''. There is no A2 on a TM, by \texttt{cert\_not\_function\_of\_forget}. Write the cert flag onto the tape and you have a Thiele substrate hiding inside a TM, like a person crouched inside a piano. The piano is still a piano. The person is still a person. The substrate distinction does not care what you encoded it onto. The structural axis (\texttt{vm\_mu}, \texttt{vm\_certified}, \texttt{csr\_cert\_addr}) is the minimum state for A2 to be a sentence in the first place. The substrate is not an extension of classical computation. It is what classical computation needed in order for the silent omission to be visible at all. The cost-ledger separation in Section~\ref{sec:categorical} depends on $\mu$ being a state field. \texttt{mu\_not\_function\_of\_bare\_observable} proves that separation has no analog in the bare classical signature.

\textbf{3. Classical computation is the projection, and the projection is not invertible.} \texttt{lift\_config} sends every Turing-machine configuration to a Thiele configuration. \texttt{thiele\_simulates\_turing} runs every Turing-machine trace inside the substrate, same tape, same state. \texttt{degenerate\_projection\_theorem} closes the loop in one direction. Classical computation is exactly the image of substrate computation under the projection that forgets the structural axis. \texttt{fiber\_has\_two\_preimages} (\texttt{coq/kernel/witness/BlindnessRepresentation.v}:238) closes the other direction. Every classical state has multiple Thiele preimages. Any lift back is non-canonical and requires external choice of cost schedule, graph state, and certification flag. \texttt{D4\_strictness} witnesses substrate states with no classical preimage. Same computable functions on both sides. Classical substrates are not a parallel option. They are the projection. Every Turing machine, every register machine, every lambda-calculus reducer, every CPU on every desk: classical state is a forgetful image of the Thiele fields. The projection is canonical. The lift is not. The TM case is the one proved (\texttt{thiele\_simulates\_turing}, \texttt{degenerate\_projection\_theorem}). The others follow the same shape. If one doesn't, name it.

\textbf{4. Name the step you reject.} Step 1 is a sentence about what a classical step relation can carry, witnessed by \texttt{cert\_not\_function\_of\_forget}. Step 2 follows from step 1, witnessed by \texttt{mu\_not\_function\_of\_bare\_observable}. Step 3 is mechanised in Coq, theorem names above. The asymmetry of projection versus lift is witnessed by \texttt{fiber\_has\_two\_preimages}. ``This sounds like a metaphor'' is not a rejection. It is a refusal to read.

\textbf{The standard misread.} The trained reflex: A2 can be enforced in software on a TM, so the substrate distinction is a hardware/software boundary, not a fundamental one. The falsifier is one sentence. A Turing machine cannot refuse to execute a buggy A2-simulator. A Thiele substrate cannot execute one. Load a Thiele simulator onto a TM with a bug, a program that certifies without incrementing $\mu$. The TM runs it faithfully because its step rule has no field for A2 and cannot detect the bug. Load the same buggy program onto a Thiele substrate. The step rule traps. A2 is not interpreted by the simulator and could be skipped. A2 is the transition law itself. Subsumption is a step-rule claim, not a software-layer claim. ``Thiele is simulable on a TM'' is true and is not the question. ``Thiele's step rule can be written down on a TM'' is the question. The answer is no.

This is the standard design pattern of encoding invariants in dependent types. Instances of a record (\texttt{CertificationSystem}) cannot be built without satisfying the typed obligation (\texttt{cs\_cert\_costs}). CHERI uses the pattern for capability discipline. TPMs use it for hardware-rooted key invariants. Thiele uses it for cost-on-certification. The pattern is generic. What is specific to Thiele is which invariant is being enforced: A2, the cost-on-certification rule, with $\mu$ as the unforgeable receipt. The substrate-versus-program distinction is real and shared with these other systems. The choice of invariant is what is Thiele's own.

These four steps are the entire foundational claim. They do not require 51 opcodes. They do not require an FPGA. They do not require the CHSH result. Constructively witnessing the substrate needs only two opcodes from the scaffolding: any classical compute primitive, so subsumption has something to project to; and one opcode that flips certification, so A2 has something to enforce. In the 51-opcode realisation, \texttt{instr\_certify} is the only instruction that flips the cert-flag (\texttt{vm\_apply\_certified} in \texttt{coq/kernel/nfi/PrimeAxiom.v}). The cost-ledger and cert-flag together are the minimal state extension over (mem, regs, pc) that makes certification semantics determinate (\texttt{P\_full\_is\_minimal\_complete\_extension} in \texttt{coq/kernel/nfi/NecessityAbstract.v}). Forty-nine of the fifty-one scaffolding opcodes are exploration of what the substrate can express. Two are required.

The rest of the document is downstream of the four steps. The CHSH$\leftrightarrow$NPA equivalence (Section~\ref{sec:chsh}) bakes the quantum-realisability condition into the step rule that the \texttt{CHSH\_LASSERT} opcode realises. A chain of Coq theorems (\texttt{column\_contractive\_check\_witness\_sound}, \texttt{column\_contractive\_iff\_quantum\_realizable}, \texttt{chsh\_lassert\_no\_trap\_implies\_quantum\_realizable}, all in \texttt{coq/kernel/quantum/} and \texttt{coq/kernel/nfi/}) proves the integer-arithmetic check is equivalent to the Q\textsubscript{1} PSD conditions on the NPA moment matrix. Section~\ref{sec:chsh} lifts that to Q\textsubscript{1+AB} via a four-slice Schur cascade. Two levels of NPA. Not the whole tower. Don't read it as the whole tower. The proof tree contains no physics premises. The three-layer realisation (Section~\ref{sec:hardware}) is a Coq kernel with zero Admitted in the active proof tree, an OCaml runner extracted from that kernel, and a Kami RTL design synthesised through Bluespec and yosys to a Xilinx Kintex-7 FPGA. Bisimulation proofs cover the 47 synth-realised opcodes. The four Q\textsubscript{1+AB} cert-opcodes live in the Kami HW abstraction with kernel-equivalence proven but not in the synthesized Verilog (silicon-budget, not semantics), contributing the OCaml/RTL opcode-set slack of 4 the parity tests tolerate. None of the 51 opcodes is foundationally required. The 51 is engineering. The substrate is the thing.
...
\item Bennett's reversibility argument. What I read: Charles Bennett (1973) showed that you can in principle organise any computation so it does not erase information mid-stream. Operations that throw information away are the irreversible ones; everything else can be undone. My reading: in the 51-opcode realisation, the positive-cost instructions are exactly the operations that commit information irreversibly (CERTIFY, MORPH\_ASSERT, EMIT, REVEAL); the rest are designed to be free at $\delta = 0$ for the same reason Bennett identified. The shape matches. The dollars-and-joules calibration is a separate named hypothesis.
...
The formal version of that shape is a directional structural theorem. \texttt{thiele\_simulates\_turing} (\texttt{coq/kernel/foundation/ProperSubsumption.v:172}) lifts every Turing-machine trace into the substrate via \texttt{lift\_config} (same file, line 153): take any TM config, set $\mu = 0$, you have a Thiele config running the same tape. \texttt{degenerate\_projection\_theorem} (\texttt{coq/kernel/foundation/TuringClassicalEmbedding.v:524}) closes the loop in the other direction: the shadow projection from Thiele to classical is the classical-equivalence quotient map, classical traces respect the quotient, and the projection is strictly lossy. So one direction is canonical (Thiele $\to$ classical: there is exactly one forgetful map onto the classical fields), and the other direction is not (classical $\to$ Thiele: every classical state has multiple Thiele preimages, witnessed by \texttt{fiber\_has\_two\_preimages} in \texttt{coq/kernel/witness/BlindnessRepresentation.v}:238). Any lift back requires picking a value for the dropped fields. That pick is external data. Not extracted from the classical state. Appended to it.

The asymmetry is sharpened by the three irrecoverability theorems in \texttt{coq/kernel/witness/ProjectionNonExistence.v}: \texttt{vm\_certified}, \texttt{csr\_cert\_addr}, and $\mu$ relative to the bare three-field classical observable are provably not functions of the classical projection. Every Thiele machine has a Turing projection. No Turing machine has a canonical Thiele lift. The structural axis is not derivable from the classical projection by any clever predicate. The classical fragment is a proper sub-thing of the Thiele world (\texttt{D4\_strictness} and \texttt{D5\_thiele\_strictly\_extends\_classical} in \texttt{coq/kernel/foundation/TuringStrictness.v}), not a parallel world that happens to compute the same functions. The proper-sub-thing relationship is proved by named theorems, not asserted by framing. If any of that is wrong, name the theorem to inspect.
...
A1 through A6 are independent of any specific instruction set. They are conditions on Coq record types; supplying instances of those record types is the only way to invoke the corresponding theorems. The 51-opcode VM supplies them by construction: \texttt{coq/kernel/nfi/UniversalCertificationCost.v} builds two concrete \texttt{CertificationSystem} values for the VM (one for the \texttt{csr\_cert\_addr} channel, one for the \texttt{vm\_certified} channel), each discharging A2 from the existing \texttt{no\_free\_certification} kernel theorems. A separate substrate-level abstraction (the \texttt{Substrate} typeclass in \texttt{coq/kernel/foundation/Substrate.v}) carries a related but distinct field \texttt{mu\_monotone} stating that the cost ledger never decreases on a step. \texttt{mu\_monotone} is a strictly weaker general-purpose monotonicity property than A2: A2 specifically asserts the cert transition costs $\geq 1$, whereas \texttt{mu\_monotone} only asserts the ledger does not go down. The two coincide on the 51-opcode VM, where every instruction adds non-negative cost and the cert transition adds at least 1. Two locks on the same door, by different keys.
...
Here is what that means in the kernel. \texttt{CertificationSystem} (defined in \texttt{coq/kernel/nfi/UniversalCertificationCost.v}) bundles a state, an instruction set, a step rule, a cost function, a certification predicate, and a proof of A2. That proof is not commentary sitting next to the type. It is one of the fields. Here is the record body, copied verbatim:
...
The last field, \texttt{cs\_cert\_costs}, is not a comment on the record. It is a typed field whose type is the literal statement of A2. The colon between the field name and the \texttt{forall} is doing real work: any value of type \texttt{CertificationSystem} must carry, at that position, a term whose type is the proposition printed there. Coq refuses to build the record otherwise.

That is the precise sense in which A2 is built into \texttt{CertificationSystem}, not asserted beside it. To make a value of that type (the Coq word is an inhabitant) you have to supply a proof of A2. In Coq, ``supplying a proof'' means handing over a term whose type is the statement you are proving; Section~\ref{sec:vocab} covers this. Build the record without that proof and the type checker rejects you. Cold. Unmoved. Correct.

So \texttt{CertificationSystem}, by the shape of the type, names substrates whose primitive step enforces cost-on-certification. A Turing machine step rule over state and tape symbol is not one of them. You can package ``Turing machine plus a simulator program that maintains an A2-respecting tape encoding'' into a \texttt{CertificationSystem} value by exhibiting a step rule for the package and proving A2 of it. But then the package is the simulator, not the Turing substrate. The Turing machine is indifferent. Run a buggy simulator that skips the cost increment and the Turing machine keeps running. There is no level at which the Turing machine knows the simulation is dishonest. There is no level at which the Turing machine knows anything about simulations.

The Coq theorems do not blur this line. \texttt{universal\_nfi\_any\_substrate} takes a \texttt{CertificationSystem} as input, so its premise is already step-level A2. \texttt{thiele\_represents\_simulating\_cert\_system} takes a \texttt{SimulatingCertificationSystem} (a \texttt{CertificationSystem} plus a structure-preserving translation into Thiele) and proves that the simulation's certifications transport into Thiele certifications. Neither theorem makes Turing machines members of the class. They make A2-enforcing systems members of the class.

Turing machines can simulate members of the class. That still makes simulation a property of the running program, not a substrate law. \texttt{coq/kernel/nfi/HonestCostTracking.v} makes the negative side explicit: \texttt{CostBearingSystem} is the unconstrained record (same shape as \texttt{CertificationSystem}, but without the A2 field), and \texttt{dishonest\_forge\_system} is a concrete \texttt{CostBearingSystem} where a single instruction sets the certified flag at total cost zero. \texttt{universal\_nfi\_any\_substrate} forbids that in the \texttt{CertificationSystem} world by type-checking. A bare Turing substrate belongs on the unconstrained side by default. Nothing in its step rule excludes the forge.

The actual separation is between substrates whose step rule enforces A2 and substrates where A2 can only be encoded into the running program's behaviour. The Thiele substrate sits in the first class. The bare Turing substrate sits in the second. This is not a claim about a new class of computable functions. The 51-opcode realisation demonstrates the shared-computation side: its structural opcodes can be set to do no structural work (the cost parameter $\delta$ can be 0 on every instruction, in which case the cost-on-cert floor is just $S(0) = 1$ at the moment of certification, and that floor matches what an honest simulator would charge anyway). A Turing machine can simulate honest traces of that realisation. But simulation is not instantiation. Putting cost-tracking on the tape is not putting cost-tracking in the step.
...
\medskip
\noindent\textbf{Plain reading.} Classical programs leave the structural axis alone. The shadow you see equals the full state's projected fields. The graph and cert channels stay at whatever they were at the start.

\begin{theorem}[D3 conservativity, \texttt{coq/kernel/foundation/ClassicalConservativity.v:204}]
...
\item Part (2) is definitional. \texttt{shadow\_proj} extracts its six projected fields. Equal projections mean those six fields agree. The Coq witness is \texttt{shadow\_proj\_kernel\_is\_eq\_on\_classical\_shadow}.
\item Part (3) follows from D2 and D3. Run a classical program: the output shadow depends only on the input shadow. Equal shadows in, equal shadows out. The Coq witness is \texttt{D2\_classical\_shadow\_preserved}.
\item Part (4) is \texttt{shadow\_strictly\_lossy}. The Categorical Separation construction gives the witness: $s_1$ has one identity morphism in \texttt{pg\_morphisms}; $s_2$ has none. The six projected fields agree. The morphism lists do not.
\end{itemize}
...
The subsumption theorems above establish what the Thiele substrate carries over and above its classical projection: fields the projection drops. The natural next question is whether a classical observer could reconstruct those fields by some clever derived predicate on the surviving shadow. The answer is no, and the no is structural rather than rhetorical. \texttt{coq/kernel/witness/ProjectionNonExistence.v} proves three irrecoverability theorems (each one a non-existence of a function from the bare classical projection back to a Thiele-only field) and packages the corollaries the subsumption argument relies on. \texttt{Print Assumptions} on every theorem in the file returns \texttt{Closed under the global context}: zero axioms, zero admits, constructive proofs.

The projection in this subsection is the four-field map $\texttt{forget} : \texttt{VMState} \to \texttt{TMSnapshot}$ from \texttt{BlindnessRepresentation.v}, keeping $(\texttt{pc}, \mu, \texttt{regs}, \texttt{mem})$. It is already forgetful, but it still retains $\mu$. For the cost-ledger separation, the stricter three-field projection $\texttt{bare\_observable}$ defined in \texttt{ProjectionNonExistence.v} drops $\mu$ as well, keeping only $(\texttt{pc}, \texttt{regs}, \texttt{mem})$: the observable the standard complexity-theory cost model sees.

\begin{theorem}[Cert irrecoverability, \texttt{cert\_not\_function\_of\_forget}]
...
\begin{proof}
The witnesses are \texttt{trace\_witness\_A} (certified, defined in \texttt{coq/kernel/witness/ThieleTraceProjection.v}:128) and \texttt{forget\_witness\_uncert} (not certified, defined in \texttt{coq/kernel/witness/BlindnessRepresentation.v}:143). Direct computation shows the two states have equal \texttt{forget} images. Suppose $\phi$ existed: then $\phi(\texttt{forget}\ \texttt{trace\_witness\_A}) = \texttt{true}$ and $\phi(\texttt{forget}\ \texttt{forget\_witness\_uncert}) = \texttt{false}$. But the inputs are equal, forcing the outputs to be equal too. Contradiction. \qed
\end{proof}

\medskip
\noindent\textbf{Plain reading.} The Thiele \texttt{vm\_certified} flag is not a function of the four fields kept by \texttt{forget}. Two Thiele states can agree on $(\texttt{pc}, \mu, \texttt{regs}, \texttt{mem})$ and still differ on whether they are certified. No formula computed from that four-tuple alone can decide certification. The certification bit lives outside the four-tuple.

\begin{theorem}[Mu irrecoverability from bare observable, \texttt{mu\_not\_function\_of\_bare\_observable}]
...
\begin{proof}
The witnesses \texttt{mu\_separation\_witness\_zero} and \texttt{mu\_separation\_witness\_one} (defined in \texttt{ProjectionNonExistence.v}) differ only in their $\mu$ field (0 vs 1) and have identical bare observables. A $\phi$ satisfying the equation would force $0 = 1$. Contradiction. \qed
\end{proof}

\medskip
\noindent\textbf{Plain reading.} The Thiele $\mu$ counter is not a function of the three-field classical observable $(\texttt{pc}, \texttt{regs}, \texttt{mem})$. Two Thiele states can agree on the classical observable and still have different $\mu$ values. The cost-ledger separation between Thiele states that the classical observable cannot tell apart is not a derived fact that a clever classical observer could compute; it is a state distinction the classical observer's signature is structurally blind to.

\begin{theorem}[Cert-addr irrecoverability, \texttt{cert\_addr\_not\_function\_of\_forget}]
...
\begin{proof}
The witnesses \texttt{cert\_addr\_witness\_zero} and \texttt{cert\_addr\_witness\_one} (defined in \texttt{ProjectionNonExistence.v}) differ only in their \texttt{csr\_cert\_addr} field (0 vs 1) and have identical \texttt{forget} images. A $\phi$ satisfying the equation would force $0 = 1$. Contradiction. \qed
\end{proof}

\medskip
\noindent\textbf{Plain reading.} The Thiele \texttt{csr\_cert\_addr} channel is not a function of the four fields kept by \texttt{forget}. The structural-axis diagonal in Section~\ref{sec:substrate-undecidability} below builds an undecidable predicate over \texttt{csr\_cert\_addr} reachability; that predicate cannot be re-expressed as a predicate over those four fields because the field it references is not visible in the classical signature.
...
\begin{enumerate}
\item \emph{A2 cannot be stated as a constraint on the bare projection alone.} A2 (\texttt{cs\_cert\_costs}, a typed field on \texttt{CertificationSystem}) says that a cert-predicate flip from \texttt{false} to \texttt{true} forces instruction cost $\geq 1$. By cert irrecoverability, any cert-predicate phrased as a function of the bare \texttt{TMSnapshot} cannot agree with Thiele's \texttt{vm\_certified} on every Thiele state. So A2 on the bare classical signature is not Thiele's A2. It constrains a different predicate. The Coq corollary is \texttt{no\_classical\_a2\_cert\_predicate}.
...
The center of the project is not just ``certification has a price.'' That is true. It is the first conservation law, not the whole picture.
...
Every term below is an exact Coq definition, not a metaphor. Every theorem that follows has been checked by Coq. I am not stamping ``machine-checked'' on each one. Assume all of them are.
...
A distinguishing observation is a function that separates an eliminated state from the posterior. Formally: there exists at least one eliminated state $s_w \in \Omega \setminus \Omega'$ such that the observation function outputs a value on $s_w$ that no state in $\Omega'$ produces. The observation is the ``test'' that reveals the structure. If you cannot name the test, you cannot show that the posterior genuinely excludes something the prior allowed.
...
\emph{(1) $\mu \perp (\texttt{mem}, \texttt{regs}, \texttt{pc})$.} Same visible state, different ledger. One branch runs a $\delta = 0$ no-op like $\texttt{ADD}\,0\,0\,0\,0$. The other uses $\texttt{EMIT}$ with $\delta = 0$. Registers, memory, and pc match. $\mu$ does not: $9$ versus $0$. The Coq witness in \texttt{coq/kernel/nfi/NecessityAbstract.v} constructs it.
...
The ledger is not just a counter someone forgot to derive. There are explicit witness states with identical Turing/RAM-visible state and incompatible receipts. Not just from one starting point. From every reachable substrate state. Cost, certification, and graph structure are three independent bookkeeping dimensions. A model that drops any of them cannot determine the full structural semantics internally. The dropped-field model is what classical computation has been all along.
...
\medskip
\noindent\textbf{Plain reading.} Same theorem as the NoFI section, named at this layer because structural entitlement needs it under that label. The certification cost is forced wherever A2 is accepted. A2 acceptance is the entry fee.

This theorem matters because it is not about the 51-opcode ISA. The conservation law is forced for any system the moment that system has a certification predicate and a cost rule that charges for certification transitions. Build the predicate so it can flip at zero cost and you have free certification by construction. Refuse to represent certification at all and you cannot talk about structural entitlement internally. You can only smuggle it in from outside and put your name on it. Smuggled entitlement is not entitlement.
...
\]
Here $|\Omega|$ means the number of states in the prior feasible set, using the $|\cdot|$ notation from Section~\ref{sec:vocab}. $|\Omega'|$ is the posterior size. $\log_2^{\uparrow}$ means: take the base-2 logarithm and round up. It counts yes-or-no questions. A 1000-state set needs 10 binary questions because $2^{10}=1024$, the first power of two that can cover 1000 items. The ceiling makes the bound conservative. Rounding up never makes the inequality easier to satisfy. $\Delta\mu$ is final $\mu$ minus initial $\mu$ across the trace.
\end{theorem}
...
\emph{(iii) The $\log_2^\uparrow$ bound.} The realized decision tree $T$ is binary, with leaves tagged by posterior states. Each prior state in $\Omega$ reaches a leaf by following the trace's actual observations. If all of $\Omega$ lands in at most $|\Omega'|$ leaves, then $T$ needs at least $|\Omega|/|\Omega'|$ effective leaf-classes at the root level, forcing depth
\[
...
\medskip
\noindent\textbf{Plain reading.} Three conclusions, three steps. Conclusion 1 is just the predicate-strengthening theorem, applied to the strict-narrowing and distinguishing-observation hypotheses. Conclusion 2 is just the structure-addition theorem, applied to the trace's start-uncertified, end-certified condition. Conclusion 3 is the entropy bound. A binary tree that compresses $|\Omega|$ states into $|\Omega'|$ leaf-classes has depth at least $\lceil \log_2(|\Omega|/|\Omega'|) \rceil$, bounded by $\log_2^\uparrow|\Omega| - \log_2^\uparrow|\Omega'|$ on the underestimate side. Each tree node is a certified observation. Each certified observation pays at least $1$ in $\mu$. The tree depth is bounded by $\Delta\mu$. You don't get to compress for less than you paid. The bound is not a price you can negotiate.
...
\begin{proof}
By construction, a \texttt{SoundStructuralShortcut} Coq record (defined at \texttt{coq/kernel/nfi/HonestNoFI\_TheoremsWithoutAssumptions.v:501--546}) contains: $\Omega$, $\Omega'$ with $\Omega' \subsetneq \Omega$, an observation function $f_{\mathrm{obs}}$ with a distinguishing-witness proof, a decision tree $T$ with a tree-realized-by-the-trace proof, a posterior-representative reduction with a representatives proof, and an initial-uncertified proof on the starting state. These fields and obligations are exactly the hypotheses of the Structural-Entitlement-Representation theorem. Project the seven data fields and eight proof obligations from the record into the theorem's hypotheses. Apply the theorem. The three conclusions (strictly stronger predicate, structure-addition event, $\log_2^\uparrow$-narrowing $\leq \Delta\mu$) follow. \qed
\end{proof}

\medskip
\noindent\textbf{Plain reading.} The record is exactly what the representation theorem needs as input. Unpack the record. Plug the fields into the theorem. Read off the three conclusions. The class is non-empty because \texttt{factored\_n1\_shortcut} in \texttt{coq/kernel/nfi/StructuralAdvantageCertifiedShortcut.v} builds a member of the class on the page, by name. Non-empty by exhibition, not by reasoning.

This is the formal version of ``it has to.'' Once a shortcut stops being a human story and becomes a sound object the substrate may use, it has to carry the data that makes it admissible. In this framework, that data is exactly the structural-entitlement witness. If someone says symmetry, factorization, modularity, independence, or decomposition gave them the shortcut, the next question is no longer mystical. Show me the prior set, the posterior set, the observation that rules something out, and the representative witness and tree witness. Show me, or the structure is still outside the computation.

\subsection{What narrowing buys}

There is also a concrete structural-advantage demonstration in the kernel. \texttt{StructuralAdvantage.v} studies a factored search problem. The blind program runs with every instruction cost set to 0 and searches the flat space. The sighted program pays two receipt-visible events, each \texttt{EMIT "." 0}, for a total receipt cost of 18. It then searches two factors separately. The file proves the exact cost formulas. It also proves the arithmetic fact that the $N^2$ versus $2N$ gap eventually dwarfs the constant $\mu$ cost by orders of magnitude. The executable tests check the measured VM behavior on representative sizes. The receipt costs 18 once. The exponent saves you forever.
...
\medskip
\noindent\textbf{What the package looks like in the concrete \texttt{n=1} witness.} Two kernel files do the instantiation: \texttt{coq/kernel/nfi/StructuralAdvantageObservedShortcut.v} and \texttt{coq/kernel/nfi/StructuralAdvantageCertifiedShortcut.v}. Both target the smallest factored-search case. The record \texttt{SoundStructuralShortcut} lives at \texttt{coq/kernel/nfi/HonestNoFI\_TheoremsWithoutAssumptions.v:501--546}. It carries fifteen fields. Seven are data: decoder, observation fn, representative observation fn, observation equality, prior set, posterior set, decision tree. Eight are proof obligations on those data. The nine semantic ingredients below cover all seven data fields plus two of the proof obligations: strict narrowing and initial-uncertified state. The remaining six obligations are discharged inline by the wiring in \texttt{StructuralAdvantageCertifiedShortcut.v}. Fifteen fields, nine ingredients, six wiring obligations. Every part accounted for.
...
\item \textbf{Distinguishing observation.} \texttt{sighted\_n1\_supra\_obs\_fn}. Returns \texttt{sighted\_n1\_supra\_trace} when \texttt{vm\_mu = 19}, else \texttt{[]}. The post-\texttt{MORPH\_ASSERT} state has \texttt{vm\_mu = 19}. The initial state has \texttt{vm\_mu = 0}. The observation separates them.
...
Three tiers, three prices. The kernel proves the split. File: \texttt{WitnessInsightGeneral.v}.
...
For \textbf{pure compute programs}, meaning programs that do arithmetic, load and store values, and jump, every instruction carries $\delta = 0$. The program I use as the concrete comparison case for the Turing Strictness theorem (\texttt{blind\_program} in \texttt{StructuralAdvantage.v}) has this property: every instruction sits at $\delta = 0$. The proof that its total $\mu$-cost is zero is a direct calculation. For these programs, $\mu$ stays at 0 unless a positive-cost instruction fires.
...
Throughout this document, $S(n)$ means $n + 1$. The $S$ stands for ``successor'' in the basic math sense: the successor of $n$ is the next natural number after $n$. So $S(0) = 1$, $S(1) = 2$, and so on. $S(\delta) = \delta + 1$ is always at least 1. \texttt{EMIT}, \texttt{REVEAL}, and \texttt{READ\_PORT} go further: their cost also grows with the information content they carry. An \texttt{EMIT} of one ASCII character (which is 8 bits, see Section~\ref{sec:vocab}) with $\delta = 0$ costs $8 + 1 = 9$. Ten characters costs $80 + 1 = 81$. You cannot emit more for the same price as less.
...
\medskip
\noindent\textbf{Plain reading.} $M$ agrees with $\mu$ at the start (both zero). The step rule says $M$ and $\mu$ both update by the same amount on every step ($M$ by hypothesis (b), $\mu$ by $\mu$-Conservation). Induction closes it. There is no alternative accounting that agrees with the cost schedule at every step, starts at zero, and disagrees with $\mu$ on a reachable VM state. The cost is forced.

In plain language: given the cost schedule, there is no alternative accounting. If you charge $S(\delta)$ for certification-floor opcodes, $|\text{payload}|_{\text{bits}} + S(\delta)$ for \texttt{EMIT}, and $\delta$ for everything else, then $\mu$ is the only function on VM states that starts at zero and tracks the schedule. What was a design choice is the schedule itself: the decision that \texttt{CERTIFY} costs $S(\delta)$ and \texttt{ADD} costs $\delta$. Once those choices are in place, the initiality theorem closes the question of whether $\mu$ is the right thing to be tracking. You cannot swap in a different count that agrees on the schedule and reaches a different total. That freedom is closed.
...
This is forced, not chosen. You cannot validate a 1000-bit formula by paying for a 10-bit one; the cost counts the formula you actually present, byte for byte, not the formula you wished you'd encoded. The proof that this is the unique minimum cost satisfying that constraint is in \texttt{MuCostDerivation.v} (\texttt{cost\_necessity} + \texttt{cost\_uniqueness}).
...
\medskip
\noindent\textbf{Plain reading.} This is just the abstract universal NoFI theorem instantiated for the 51-opcode realisation and read back through the Thiele substrate interface. The two concrete certification channels (\texttt{csr\_cert\_addr} and \texttt{vm\_certified}) are both cost-positive at the step level, and the predicate-strengthening reading of NoFI falls out of the structure-addition theorem.

This is the VM-level version. \texttt{AbstractNoFI.v} proves the two concrete cert channels cannot be activated for free, by case analysis over the 51 opcodes. \texttt{NoFreeInsight.v} adds the predicate-strengthening layer: once a stronger accepted predicate is tied to \texttt{has\_supra\_cert}, the trace must contain a structure-addition event somewhere in it.
...
\begin{proof}
Case analysis over the 51 opcode constructors. The Coq proof uses \texttt{cert\_addr\_setterb}: if \texttt{csr\_cert\_addr} changes from 0 to nonzero, the instruction must fall inside that cost-positive class. The class includes \texttt{instr\_morph\_assert}, \texttt{instr\_emit}, \texttt{instr\_reveal}, \texttt{instr\_ljoin}, and \texttt{instr\_lassert}. It is a deliberately broad guardrail class, not a list of literal CSR writers. In the current \texttt{vm\_apply} semantics, the direct CSR-writing success case is \texttt{instr\_morph\_assert}. For the theorem, the required fact is cost: every member of \texttt{cert\_addr\_setterb} has \texttt{instruction\_cost} $\geq 1$, with \texttt{instr\_emit} and \texttt{instr\_reveal} higher because they also charge payload bits. \qed
\end{proof}

\medskip
\noindent\textbf{Plain reading.} If you see \texttt{csr\_cert\_addr} move from zero to nonzero, the proof catches that step inside a cost-positive guardrail class. That class is broader than the direct CSR writers, but broad is fine here: every member costs at least 1. The field cannot flip for free.
...
\medskip
\noindent\textbf{Plain reading.} Same shape as the other channel. Only one opcode (\texttt{CERTIFY}) can flip the top-level flag. It charges $\geq 1$. The universal theorem lifts that to the trace.
...
\item $s.\texttt{vm\_csrs}$: control-status registers. The hardware split: data registers hold values (like $s.\texttt{vm\_regs}$); CSRs hold configuration and status. Four fields:
 \begin{itemize}
 \item \texttt{csr\_cert\_addr}: 0 until something is asserted. Set to the ASCII checksum of the property string on a successful \texttt{MORPH\_ASSERT} (nonzero for any non-empty property string). The name says address; the contents say fingerprint. Nonzero here means a structural property was certified and paid for.
...
These opcodes modify the partition graph. They are how the kernel builds structural knowledge: one module, one morphism, one accounting entry at a time.
...
Merges modules $m_1$ and $m_2$ and charges $\delta$. The hardware-aligned step concatenates the module sizes through \texttt{graph\_hw\_pmerge}; module IDs wrap modulo 64. It does not perform a rich invariant-compatibility check in this step. Like PNEW and PSPLIT, the VM does not enforce a minimum cost here. The mandatory cost comes later, on the certification step where the structure has to pay for being trusted.
...
This is the most important logic opcode in the VM. It is where a constraint package is checked before any later certification step can lean on it. You cannot fake the pricing anymore by declaring a tiny \textit{flen} for a large in-memory formula: if the header and the instruction disagree, the VM traps. You also cannot fake semantic narrowing with a tautology: the dual-witness requirement closes the tautology-inflation hole because there is no falsifying witness to show the formula excluded any state. The cost formula is still a spend ledger. The honest-length check and the non-triviality witness are what tie that spend to the real checked object, not to a thing the programmer wished they had checked.
...
Why does the ISA have all these? Because the kernel realises a complete computing machine, not just a structural accounting system. Programs need to compute intermediate values, manage data, and control flow. The compute group provides all of that. The key property: none of these opcodes touch the certification channels. They cannot create certified structural knowledge, no matter how much arithmetic they do.
...
\subsection{Group 6: Witness and certification (7 opcodes)}
...
This is a Tier 0 operation: it records data but does not activate any certification channel. It is provably free in the sense that CHSH\_TRIAL does not and cannot trigger a cert-insight event (proven in \texttt{WitnessInsightGeneral.v}, \texttt{chsh\_trial\_is\_tier0}). One validation: if any of x, y, a, b is not in \{0, 1\}, for example, passing x=5, the VM fires a bad-bits error step: the error flag latches, no counter is incremented, but the cost is still charged. The VM does not silently count invalid inputs as valid trials, and there is no path through the kernel that lets you pretend it did.
...
\medskip
\noindent\textbf{CHSH\_LASSERT\_1AB($\delta$)}: Q\textsubscript{1+AB}-aware certification at $\gamma = 0$.\\
Extends CHSH\_LASSERT with the level-1+AB NPA condition. Reads the 8 \texttt{vm\_witness} buckets, runs an integer-arithmetic check (Q\textsubscript{1} column-contractivity plus the SOS sum-of-squares condition $E_{00}^2 + E_{01}^2 + E_{10}^2 + E_{11}^2 \leq 1$ at $\gamma_1 = \gamma_2 = \gamma_3 = \gamma_4 = \gamma_5 = 0$), advances PC on pass or traps to \texttt{LASSERT\_TRAP\_PC} on fail. Charges $S(\delta) \geq 1$ either way. The bridge theorem \texttt{chsh\_lassert\_1ab\_no\_trap\_implies\_quantum\_realizable\_q1ab} proves a non-trapping step entails \texttt{quantum\_realizable\_q1ab} = symmetric9 $\wedge$ PSD9 of the $9 \times 9$ NPA Q\textsubscript{1+AB} moment matrix at the witness-derived rationals.

\medskip
\noindent\textbf{CHSH\_LASSERT\_1AB\_G5($\delta$, $\mathit{same\_g5}$, $\mathit{diff\_g5}$)}: Q\textsubscript{1+AB} with the four-body moment $\gamma_5$ free.\\
...
\medskip
\noindent\textbf{CHSH\_LASSERT\_1AB\_G345($\delta$, $\mathit{same\_g3}$, $\mathit{diff\_g3}$, $\mathit{same\_g4}$, $\mathit{diff\_g4}$, $\mathit{same\_g5}$, $\mathit{diff\_g5}$)}: Q\textsubscript{1+AB} with three-body and four-body moments $\gamma_3$, $\gamma_4$, $\gamma_5$ free.\\
...
\medskip
\noindent\textbf{CHSH\_LASSERT\_1AB\_G12345($\delta$, $\mathit{same\_g_1}$, $\mathit{diff\_g_1}$, \ldots, $\mathit{same\_g_5}$, $\mathit{diff\_g_5}$)}: full Q\textsubscript{1+AB} with all five $\gamma$ moments free.\\
Carries five $\gamma$-bucket pairs. Runs the compositional sym6 $\to$ sym5 $\to$ sym4 Schur-cascade check (21 cleared H-entry numerators, 15 cleared S6 Schur entries, 10 cleared S5 Schur entries, then four sym4-determinant minors of the cleared 4$\times$4). On pass, the bridge gives PSD9 at $(E, \gamma_1, \gamma_2, \gamma_3, \gamma_4, \gamma_5)$ for the bucket-derived rationals: the full Q\textsubscript{1+AB} cone the substrate certifies. By Ishizaka 2025 (cited externally), Q\textsubscript{1+AB} = Q in the CHSH scenario, so the certified region coincides with the Tsirelson set, though the equivalence is not formalised in Coq. The four cert-opcodes in this family are defined in the Kami HW abstraction with kernel-equivalence proven (\texttt{coq/kami\_hw/Abstraction.v}, \texttt{EmbedStep.v}), but are not synthesized into the Genesys 2 bitstream. The reason is silicon, not semantics: the base \texttt{CHSH\_LASSERT} 23-phase FSM already burns roughly 74\% of the K325T's LUTs after time-sharing a 384$\times$384 multiplier across phases, and the Q\textsubscript{1+AB} variants do strictly more wide arithmetic per check (\texttt{\_g12345} runs a 50-minor sym6 $\to$ sym5 $\to$ sym4 Schur cascade). The Kami proof chain covers all 51 opcodes; the bitstream covers the 47 simpler ones plus the base CHSH-LASSERT.
...
\item Witness and certification (general + Q\textsubscript{1+AB} family): 7 ops (CHSH\_TRIAL, CERTIFY, REVEAL, CHSH\_LASSERT\_1AB, CHSH\_LASSERT\_1AB\_G5, CHSH\_LASSERT\_1AB\_G345, CHSH\_LASSERT\_1AB\_G12345)
...
$14 + 8 + 7 + 4 + 3 + 7 + 7 + 1 = 51$. Of the 51, 47 are synthesized into the FPGA bitstream; the four Q\textsubscript{1+AB} cert-opcodes (CHSH\_LASSERT\_1AB and its three $\gamma$-extended variants) live in the Kami HW abstraction with kernel-equivalence proven, but not in the synthesized bitstream (see the CHSH cert family note above for the silicon-budget reason).

\medskip
\noindent\textbf{The CHSH cert family: CHSH\_LASSERT plus four Q\textsubscript{1+AB} extensions.}
The base opcode \texttt{CHSH\_LASSERT mu\_delta} reads the eight buckets of \texttt{vm\_witness} directly, computes the integer-arithmetic check \texttt{column\_contractive\_check\_witness} (defined in \texttt{coq/kernel/foundation/VMStep.v} using only $\mathbb{Z}$ arithmetic on the same/diff counts), and either advances the program counter on success or traps to \texttt{LASSERT\_TRAP\_PC} and latches \texttt{vm\_err} on failure. The cost is $S(\mathit{mu\_delta}) \geq 1$ regardless of outcome (cert-setter discipline; you pay to check, even to fail). The bridge theorem \texttt{chsh\_lassert\_no\_trap\_implies\_quantum\_realizable} (\texttt{coq/kernel/quantum/QuantumPartitionPSD.v}) proves that any no-trap, no-err-flip execution of CHSH\_LASSERT implies the witness-derived $5\times5$ NPA moment matrix is PSD (i.e., quantum-realizable at NPA level 1). The chain in plain order: the integer check on the witness counters passes; that fact lifts to real-valued column-contractivity by \texttt{column\_contractive\_check\_witness\_sound} in \texttt{coq/kernel/nfi/MuLedgerQuantumBridge.v}; column-contractivity is biconditional with NPA-PSD by \texttt{column\_contractive\_iff\_quantum\_realizable}. Three theorems. No physics input. The opcode is the place in the kernel where the algebra of Section~\ref{sec:chsh} becomes something the VM actually refuses to do, not just declines to recommend.

The four Q\textsubscript{1+AB} extensions \texttt{CHSH\_LASSERT\_1AB} (tag 0x2F, $\gamma = 0$), \texttt{CHSH\_LASSERT\_1AB\_G5} (tag 0x30), \texttt{CHSH\_LASSERT\_1AB\_G345} (tag 0x31), and \texttt{CHSH\_LASSERT\_1AB\_G12345} (tag 0x32) lift the same discipline from the level-1 $5\times5$ matrix to the level-1+AB $9\times9$ NPA moment matrix. Each runs its own integer-arithmetic Schur-cascade check on \texttt{vm\_witness} plus the supplied $\gamma$-bucket pairs, advances on pass or traps on fail, charges $S(\mathit{mu\_delta}) \geq 1$. The four bridge theorems (one per slice, named \texttt{chsh\_lassert\_1ab\_*\_no\_trap\_implies\_quantum\_realizable\_q1ab} in \texttt{coq/kernel/quantum/QuantumPartitionPSD\_1AB.v}) deliver \texttt{quantum\_realizable\_q1ab} = symmetric9 $\wedge$ PSD9 on the witness-derived rationals. The mathematical detail of the four-slice Schur cascade is in \S\ref{sec:chsh}'s Q\textsubscript{1+AB} subsection below.

Every opcode exists for a reason. None are redundant. If you removed CERTIFY, you could not activate \texttt{vm\_certified}. If you removed MORPH, you could not build morphism relationships. If you removed CHSH\_TRIAL, you could not track 2-player binary game statistics. If you removed CHSH\_LASSERT, the kernel would record CHSH trials but could not enforce that certified outcomes lie inside the level-1 NPA-PSD boundary. If you removed the Q\textsubscript{1+AB} family, the kernel could not certify the level-1+AB cone the Tsirelson set lives in (per Ishizaka 2025, Q\textsubscript{1+AB} = Q in CHSH). The ISA is designed, not grown.
...
Objects can be anything you want. Sets. Vector spaces. Topological spaces. In the Thiele Machine they are modules: partitions of state, named pieces of the computation.

Arrows have a second name: morphisms. An arrow goes from object to object. $f : A \to B$ means ``arrow $f$ from $A$ to $B$.'' It does not have to be a function. People forget this. In the Thiele Machine, a morphism is a claim: these two modules are related, here is the type of the relationship.
...
\medskip
\noindent\textbf{Why identity?} Every object needs a self-loop, even the boring ones. A module with no connections to anything else still has to exist somewhere in the framework. MORPH\_ID is the instruction for that: ``$A$ to itself, coupling empty.'' Drop it and isolated objects fall outside the framework entirely. The category does not see them.
...
These laws are proven in Coq in \texttt{CategoryLaws.v}. Concretely: morphisms carry coupling data, a list of (source-cell, target-cell) pairs plus a label. A ``cell'' is a natural number indexing into a module's memory region. A coupling pair $(a, c)$ says: cell $a$ in the source module is related to cell $c$ in the target module. The coupling is the \emph{relational content} of the morphism, the actual evidence of which parts of one module correspond to which parts of another. COMPOSE produces a new morphism whose coupling pairs are the relational composition of the two inputs: $(a, c)$ is included when there exists an intermediate cell $b$ such that $(a, b)$ is in the first coupling and $(b, c)$ is in the second. Standard relational composition. Matrix multiplication for sets of pairs. This is proven in \texttt{CategoryBridge.v} (\texttt{graph\_compose\_morphisms\_coupling}). The Thiele Machine's categorical layer is not just called a category. It satisfies the actual axioms on the actual coupling data, with Coq watching.

\subsection{Why modules as objects}

In the Thiele Machine, the objects of the category are the modules of the partition graph. A module is a named piece of the computation. It owns a region of memory or state. It may carry certified axioms. It contributes a local $\mu$ cost. Modules are the bookkeeping unit. Everything structural hangs off them.

A morphism between two modules says: ``there is a named relationship between $A$ and $B$, with explicit coupling data.'' The coupling data is a list of (source-cell, target-cell) pairs. COMPOSE computes the relational composition: pair $(a, c)$ lands in the composed coupling exactly when there exists $b$ with $(a, b)$ in the first and $(b, c)$ in the second. Proven. Tracked. Via MORPH\_GET (selector 2), you can read back the coupling-pair count of any morphism.
...
Extra registers do not have provenance. You can write anything into a register. You can write 42 into register 5 and claim it is a morphism ID. But the VM checks: did a MORPH instruction actually run that created morphism 42 with the source and target you claim? If not, MORPH\_ASSERT fails. And MORPH\_ASSERT costs $S(\delta) \geq 1$ even if it fails.
...
This is why the morphism layer is not ``extra state.'' It is \emph{provenance-bearing state}. The substrate knows where every morphism came from. A classical machine cannot fake that at the same cost. That is the Categorical Separation Theorem.
...
\medskip
\noindent\textbf{Two witness states, side by side.} What the classical observer sees. What the substrate actually holds.
...
The classical machine simulates inside the Thiele substrate faithfully. Every classical program runs correctly. No side effects on the structural layer. But the substrate has transitions the classical fragment cannot reach. The 51-opcode realisation surfaces them as PNEW, MORPH, MORPH\_ASSERT. Proven. Not claimed.
...
The VM paid for a failed assertion. A machine without a structural ledger has no way to express this, let alone enforce it. In the Thiele Machine, the cost of claiming knowledge is charged whether or not the claim is true.
...
Three scaffolding layers wrap the substrate: a Coq kernel, an OCaml runner extracted from it, and Kami RTL extracted from it. None of them IS the substrate. They are how the substrate gets to live on a chip, on a laptop, and inside a proof checker. The kernel-to-RTL seam is closed by formal bisimulation proofs (Section~\ref{sec:vocab}: same input, same output, field by field). The kernel-to-OCaml seam is closed by audited parity tests (Section~\ref{sec:vocab}: run both on a battery of inputs, verify they agree). Proof on the hardware side. Tested correspondence on the runner side. Both seams are named in the source. Neither is a wave-off.
...
47 of the 51 opcodes are implemented in the synthesized RTL: \texttt{ThieleCPUCore.v} has hardware logic for those 47, and the Verilog at \texttt{thielecpu/hardware/rtl/thiele\_cpu\_kami.v} carries them through to the FPGA bitstream. The remaining four (the Q\textsubscript{1+AB} cert-opcode family \texttt{instr\_chsh\_lassert\_1ab}, \texttt{instr\_chsh\_lassert\_1ab\_g5}, \texttt{instr\_chsh\_lassert\_1ab\_g345}, \texttt{instr\_chsh\_lassert\_1ab\_g12345}) are defined in the Kami HW abstraction with kernel-equivalence proven (\texttt{coq/kami\_hw/Abstraction.v}, \texttt{EmbedStep.v}), and they execute in the OCaml runner and Python harness, but they are not synthesized into the FPGA bitstream because the silicon-budget for additional wide-arithmetic FSMs is exhausted by the base \texttt{CHSH\_LASSERT} check. The OCaml/RTL opcode-set parity test tolerates exactly these four. The question is not which opcodes are in hardware, but which opcodes have a completed formal \emph{bisimulation proof}.
...
The official classification (\texttt{coq/kami\_hw/RTLGapRegistry.v}, theorem \texttt{rtl\_coverage\_partition}) records the partition $37 + 10 + 0 = 47$ for the synth-realised subset: all Qed, zero structural gaps within that subset. The four Q\textsubscript{1+AB} cert-opcodes sit in the Kami HW abstraction (with kernel-equivalence proven) but outside the synthesized-Verilog surface. The slack of 4 between the 51-opcode VM ISA and the 47-opcode synth partition is a silicon-budget choice, not a semantics gap.

The 47/47 RTL-bisimulation coverage (all Qed) is the correct relationship between the synth-realised subset and its physical instantiation: the Coq kernel defines what is correct, the hardware implements 47 of the 51 opcodes, and all 47 of those have formal bisimulation proofs. The four Q\textsubscript{1+AB} cert-opcodes live in the Kami HW abstraction with kernel-equivalence proven (sound at the substrate level, see \S\ref{sec:chsh}), and run in the OCaml runner and Python harness, but the silicon budget excludes them from the synthesized Verilog. This is honest proof accounting.
...
The substrate has a CHSH step. The VM realises it with two opcodes. CHSH\_TRIAL records outcomes of a 2-player binary game into hardware witness counters. CHSH\_LASSERT refuses to certify the trace unless those counters satisfy a Z-arithmetic check. I used the pair to stress-test the $\mu$-ledger's cost algebra against a known algebraic boundary: the NPA moment matrix conditions for bounding what correlations are achievable under polynomial constraints.

I did not set out to find what I found. The chain is concrete. The Z-arithmetic check the kernel runs on the witness counters (decidable, integer-only) is sound with respect to the real-valued column-contractive predicate by \texttt{column\_contractive\_check\_witness\_sound} (188 lines, closing with \texttt{nra} after IZR distribution and field simplification, in \texttt{coq/kernel/nfi/MuLedgerQuantumBridge.v}). That real-valued predicate is biconditional with PSD of the zero-marginal NPA moment matrix by \texttt{column\_contractive\_iff\_quantum\_realizable} (in \texttt{coq/kernel/quantum/QuantumPartitionPSD.v}). End-to-end: a successful CHSH\_LASSERT step entails NPA-PSD on the witness-derived correlators, by \texttt{chsh\_lassert\_no\_trap\_implies\_quantum\_realizable} in the same file. No quantum-mechanical premises live in the Coq dependency tree: no Hilbert space, no operators, no observables. The identification of NPA-PSD with quantum-realizable correlations lives in the NPA framework itself, cited externally (Navascu\'es--Pironio--Ac\'in 2007, 2008) as standard. The kernel's content is the algebraic bridge between an integer-arithmetic check inside an opcode and PSD of a specific matrix; calling that condition ``quantum-realizable'' uses NPA's own terminology, not a physics derivation inside the kernel.

\medskip
\noindent\textbf{Direction of derivation: substrate first, NPA second.}
This section is presented in two passes, and the order matters for what the result is claiming. The first pass is substrate-side: the CHSH\_TRIAL opcode, its witness-counter cost schedule, the integer Z-arithmetic predicate \texttt{column\_contractive\_check} that CHSH\_LASSERT runs on those counters, and the soundness of that integer check against its real-valued lift. None of this references NPA. The honesty condition the $\mu$-ledger enforces at CHSH\_LASSERT (``a successful certification step requires the witness counters to satisfy three explicit polynomial inequalities on cleared integer ratios'') is a substrate-native predicate, defined inside the cost algebra and verified by the kernel without consulting the quantum-mechanics literature. The second pass is the structural agreement: that substrate-native predicate, lifted to the reals via IZR and the witness-count denominators, is the column-contractive predicate; and the column-contractive predicate is biconditional with PSD of the zero-marginal NPA moment matrix. The biconditional is a structural agreement between two independently-defined objects (the cost algebra's honesty condition and NPA's quantum-realizability condition), proven in pure polynomial arithmetic with zero axioms. The order in which the section presents the material reflects this: §\ref{sec:chsh}'s ``Algebraic characterization of the $\mu$-ledger honesty condition'' subsection is the load-bearing result; the earlier subsections (Bell test setup, classical bound, quantum bound) are pedagogical context, not the formal content. If you have time for one subsection, that is the one. The CHSH game is built from scratch in the next subsection so the result makes sense to readers who have not seen Bell tests before; readers who have can skip ahead.
...
Section~\ref{sec:chsh} above derives the level-1 bridge: CHSH\_LASSERT $\Rightarrow$ PSD of the $5\times5$ zero-marginal NPA matrix. The level-1 cone contains the Tsirelson set $Q$ (every quantum-realisable correlator is PSD at level 1), but it strictly contains $Q$: the level-1 cone is bigger than $Q$ for CHSH, so a level-1 certificate does not on its own pin the correlator down to physically quantum. The next NPA level that does coincide with $Q$ for CHSH is level 1+AB: the $9\times9$ moment matrix indexed by the basis $\{I, A_0, A_1, B_0, B_1, A_0 B_0, A_0 B_1, A_1 B_0, A_1 B_1\}$. The result that $Q_{1+AB} = Q$ in the CHSH scenario is due to Ishizaka 2025; the substrate cites it externally rather than re-formalising it (per standard NPA-hierarchy practice). What the substrate formalises is the certification end: a successful step of one of the four \texttt{CHSH\_LASSERT\_1AB} cert-opcodes implies PSD of the $9\times9$ Q\textsubscript{1+AB} moment matrix at the witness-derived rationals and the bucket-derived $\gamma$ values. The Coq machinery for this lives in \texttt{coq/kernel/quantum/QuantumPartitionPSD\_1AB.v}.
...
\[
\gamma_1 = \langle A_0 A_1 B_0\rangle,\quad
\gamma_2 = \langle A_0 A_1 B_1\rangle,\quad
\gamma_3 = \langle A_0 B_0 B_1\rangle,\quad
\gamma_4 = \langle A_1 B_0 B_1\rangle,\quad
\gamma_5 = \langle A_0 A_1 B_0 B_1\rangle.
\]
$\gamma_1, \gamma_2$ are the two three-body $A$-$A$-$B$ moments. $\gamma_3, \gamma_4$ are the two three-body $A$-$B$-$B$ moments. $\gamma_5$ is the four-body moment. Each $\gamma_k$ lives in $[-1, 1]$; on a quantum state with zero marginals, each $\gamma_k$ is the corresponding expectation. The $9\times9$ matrix entries are polynomials of degree $\leq 2$ in $(E_{ij}, \gamma_k)$, given explicitly by the products of basis operators (e.g.\ entry $(I, A_0 B_0) = E_{00}$, entry $(A_0, A_1 B_0) = \gamma_1$, entry $(A_0 B_0, A_0 B_0) = 1$, etc.). PSD of this matrix is the level-1+AB NPA condition.
...
\item \textbf{Slice B} ($\gamma_5$ free, tag 0x30): the four-body moment $\gamma_5$ is admitted as a bucket-pair argument; the other four $\gamma$ are zero. The check adds a weighted 4-D Cauchy-Schwarz identity that handles the $2\gamma_5(v_{11}v_{22} + v_{12}v_{21})$ cross term arising in the residual.
...
\[
\text{PSD9}(M) \;\Leftarrow\; \text{sym6 PD-interior on the 21 entries of } M_{6\times6} \;\Leftarrow\; \text{sym5 PD-interior on the } 5\times5 \text{ Schur complement of row 1}
\]
\[
\;\Leftarrow\; \text{sym4 PD-interior on the } 4\times4 \text{ Schur complement of row 2}
\]
where $M_{6\times6}$ is the residual matrix (the $9\times9$ NPA matrix's projection onto the basis $\{v_{B1}, v_{B2}, v_{11}, v_{12}, v_{21}, v_{22}\}$ after the Q\textsubscript{1+AB} subsumption identities are applied). Each Schur step is a single polynomial identity provable by Coq's \texttt{ring} tactic; the base case is the sym4 PD-interior lemma from Slice C. The substrate proves the lifting by
\begin{itemize}
\item 21 \emph{H bridges} \texttt{cleared\_g12345\_HXX\_Z\_bridge}, one per upper-triangle entry of $M_{6\times6}$. Each lifts a cleared integer numerator (a polynomial in the witness bucket counts and $\gamma$-bucket pairs, cleared by the common denominator $\text{KZ} = (N_{00} N_{01} N_{10} N_{11} D_{g_1} D_{g_2} D_{g_3} D_{g_4} D_{g_5})^2$) to the real-valued $q_{12345}$ polynomial in $(E_{ij}, \gamma_k)$.
\item 15 \emph{S6 cascade bridges} \texttt{cleared\_g12345\_S6\_IJ\_Z\_bridge} for $I,J \in \{2,\ldots,6\}, I \leq J$. Each lifts the integer Schur step on cleared-H entries to the real Schur-complement entry on $q_{12345}$ polynomials.
\item 10 \emph{S5 cascade bridges} \texttt{cleared\_g12345\_S5\_IJ\_Z\_bridge} for $I,J \in \{2,\ldots,5\}, I \leq J$. Each composes two S6 bridges via \texttt{schur\_step\_Z\_IZR} to reach the level-2 Schur complement.
\item Four \emph{sym4 leading-minor bridges} (inlined in the soundness theorem) lifting Sylvester's PD criterion through the cascade, yielding PSD9 at the witness-derived rationals when the bool decider passes.
\end{itemize}
The headline corollary \texttt{q1ab\_g12345\_caller\_witness\_z\_abs\_implies\_psd9} composes the four bridge layers; the no-trap wrapper \texttt{chsh\_lassert\_1ab\_g12345\_no\_trap\_implies\_quantum\_realizable\_q1ab} packages the result as \texttt{quantum\_realizable\_q1ab} (= symmetric9 $\wedge$ PSD9). \texttt{Print Assumptions} on either reports only the two standard-library axioms \texttt{ClassicalDedekindReals.sig\_forall\_dec} and \texttt{FunctionalExtensionality.functional\_extensionality\_dep} the rest of the Q\textsubscript{1+AB} kernel uses: no project-local axiom, no \texttt{Hypothesis}, no \texttt{Parameter}.
...
\medskip
\noindent\textbf{Plain reading.} Each cost-positive instruction contributes at least $1$ to total cost and at most $1$ to the irreversible-bit count. Summing across the trace, the count is bounded by the cost. Shape-Landauer in the sense that irreversible operations are charged. No physics in the proof. A $0/1$ indicator bounded by what it indicates. No microscopic erasure model. No physical entropy. No Boltzmann constant. The conversion of $\mu$ counts to joules is the calibration hypothesis (\texttt{mu\_landauer\_unruh\_calibrated}, Section~\ref{sec:unknown}). No kernel theorem uses it.
...
\medskip
\noindent\textbf{Relationship to Rice's theorem.} The result that follows is Kleene's 1938 diagonal pattern instantiated at the substrate level. Rice's 1953 theorem applies the same diagonal to non-trivial extensional properties of recursive functions computed by Turing machines; the substrate-level version below applies it to non-trivial extensional predicates of substrate programs in any model that exposes an internal recursion theorem and an internal representability predicate. The formal content is the diagonal construction; what the substrate-level framing adds is the parameterization (any substrate satisfying the typeclass interface, not just the standard Turing model) and the choice of extensional predicate (\texttt{AdmitsShortcut}, branching on \texttt{csr\_cert\_addr} reachability). The result is therefore a substrate-level instance of the pattern Rice used; the result is not derivable as a corollary of Rice's theorem in its original setting, because Rice's theorem's premises are about partial recursive functions in the Turing-machine model, not about arbitrary substrates with their own recursion theorems and representability predicates. Both theorems are sibling instances of Kleene's diagonal; neither is a corollary of the other.
...
\item A cost-ledger monotonicity field \texttt{mu\_monotone}: \texttt{forall p s s', step p s s' $\to$ mu s $\le$ mu s'}. This is a strictly weaker general-purpose monotonicity property than the kernel's A2 (which specifically asserts that a step flipping certification false-to-true costs $\geq 1$, and which is carried by the \texttt{CertificationSystem} record in \texttt{coq/kernel/nfi/UniversalCertificationCost.v}, not by this typeclass). The two coincide on the 51-opcode VM where all instruction costs are non-negative and the cert transition costs $\geq 1$. See Section~\ref{sec:axioms} for the A2 statement and the related axioms A3 through A6.
...
\begin{theorem}[Structural-axis undecidability for the 51-opcode VM (\texttt{coq/kernel/nfi/StructuralUndecidability.v})]
For the 51-opcode VM, there is no Coq function \texttt{decide : list vm\_instruction $\to$ bool} whose diagonal flip transformer is internally representable in the VM language and that for all programs \texttt{p} satisfies: \texttt{decide p = true} iff \texttt{vm\_admits\_shortcut\_extensional p}.
\end{theorem}
...
``Zero Admitted'' is literal. Every theorem in the proof tree was derived. No step asserted without a derivation. Coq checked the whole chain, end to end, with no ``and the rest is obvious.''
...
\end{itemize}
If you find such a trace, \texttt{certification\_requires\_positive\_mu} is wrong. Add the counterexample as a theorem. Coq refuses the compile.
...
Prove that for ALL states $s_1, s_2$ with identical classical shadow, ALL instructions produce the same error behavior. That kills \texttt{categorical\_separation}.

\medskip
\noindent\textbf{Falsify Turing strictness.}\\
Prove that every Thiele VM program can be simulated by a classical program with the same observable behavior (registers, memory, $\mu$) without ever touching the partition graph or morphism map. \texttt{D5\_thiele\_strictly\_extends\_classical} falls.

\medskip
\noindent\textbf{Falsify structural entitlement.}\\
Find two Thiele VM states with the same classical shadow where no probe can distinguish their structural fields. \texttt{shadow\_strictly\_lossy} falls. Or produce a strict feasible-set subset with a distinguishing observation that does not yield stronger membership predicates. The feasible-subset theorem falls with it.

\medskip
\noindent\textbf{Falsify universal certification cost.}\\
Build a \texttt{CertificationSystem} satisfying the one-step rule \texttt{cs\_cert\_costs}, then give a trace from uncertified to certified with total cost 0. \texttt{universal\_nfi\_any\_substrate} falls. If you instead drop the one-step rule, you have found a free-certification system, not a counterexample.
...
Exhibit a locally factorizable strategy that achieves $|S| > 2$. \texttt{chsh\_stat\_violation\_not\_local} falls. So does known mathematics.
...
\item \textit{\small vm\_structural\_shortcut\_undecidable} (\texttt{coq/kernel/nfi/StructuralUndecidability.v}): substrate-level theorem instantiated at the 51-opcode VM. Carries the VM-side Section parameters (Gödel encoding, internal-representability predicate, internal recursion theorem) as explicit \texttt{forall} premises in the Bekenstein-bound style. The substrate-level content is unconditional at \texttt{nat\_substrate}; the VM corollary presents the result at the VM scope.
...
\textbf{Physical interpretation of $\mu$ as thermodynamic entropy.} The subsumption claim establishes that $\mu$ is the unique cost coordinate up to the local conditions in \texttt{mu\_is\_initial\_monotone} (Section~\ref{sec:mu}) and that classical substrates project onto Thiele substrates in the cost-tracking-blind direction. It does not establish that the $\mu$-ledger corresponds to physical thermodynamic entropy in joules. The Landauer connection in Section~\ref{sec:landauer} is a named hypothesis (\texttt{landauer\_unruh\_calibration}); the formal development is consistent with it but does not depend on it. Closing this gap would require a physical experiment, not a proof.

\textbf{Class separation.} The subsumption claim establishes a cost-tracking separation at the substrate level: classical substrates do not carry A2 in their step rule, and the cost-ledger separation between Thiele states with identical bare-classical observables has no analog on the classical signature (\texttt{no\_classical\_mu\_separation\_predicate} in Section~\ref{sec:projection-nonexistence}). It does not establish a complexity-class separation (P vs NP or any analog). \texttt{MuComplexity.v} defines \texttt{mu\_budget\_decidable} and \texttt{mu\_budget\_verifiable} and proves only the basic implication that every decidable problem is verifiable. The factored-search witness ($\mu = 18$ pays for one structural shortcut, \texttt{StructuralAdvantage.v}) is a witness-level cost gap, not a class separation. There is no concrete problem placed in \texttt{mu\_budget\_verifiable} but provably not in \texttt{mu\_budget\_decidable}. Closing this gap would require constructing such a problem.

\textbf{Privileged choice of extensional predicate for the substrate-axis diagonal.} The subsumption claim establishes that the structural-axis diagonal lives on \texttt{csr\_cert\_addr} reachability and that the diagonal's content cannot be re-expressed on the bare classical projection (\texttt{cert\_addr\_not\_function\_of\_forget} in Section~\ref{sec:projection-nonexistence}). It establishes the substrate-level diagonal for an entire family of non-trivial extensional predicates (\texttt{structural\_shortcut\_undecidable} in \texttt{coq/kernel/nfi/StructuralUndecidability.v}). The VM-instance corollary picks one specific predicate in the family: \texttt{simple\_morph\_trace}-coincidence. Whether that is the privileged choice, versus a predicate built directly from \texttt{has\_supra\_cert} reachability, versus receipt-list equality at a specific fuel bound, is open. The substrate-level result is the family statement; the VM corollary is one tile. Closing this gap would require an argument that one tile is canonical.

\textbf{Unconditional Shannon-style bound on every deterministic single trace.} The subsumption claim establishes the entropy-bounded $\mu$-ledger theorem for sound structural shortcuts: any trace equipped with a realized-by-the-trace decision tree pays at least $\log_2^{\uparrow}|\Omega| - \log_2^{\uparrow}|\Omega'|$ in $\mu$ for narrowing $\Omega$ to $\Omega'$ (Section~\ref{sec:entitlement}; \texttt{MuShannonBridge.v}). It does not establish the same bound for an arbitrary deterministic single trace without the tree as witness. The naive single-trace claim is false in general and the file records that explicitly (\texttt{monograph.tex}:848). The remaining open step: derive the realization condition (\texttt{decision\_tree\_realized\_by\_trace}) from VM trace behavior without requiring the user to verify the leaf count. Closing this gap would require automating the tree extraction from a trace, which the kernel does not currently do.

\textbf{Full-elliptope quantum mechanics connection.} The subsumption claim establishes the algebraic equivalence between Thiele honesty and zero-marginal NPA realizability (\texttt{column\_contractive\_iff\_quantum\_realizable} in \texttt{coq/kernel/quantum/QuantumPartitionPSD.v}, zero axioms). The zero-marginal framework is the orthogonal-observables slice ($\rho_{AA} = \rho_{BB} = 0$): a proper subset of the full quantum elliptope. The claim does not establish that the full elliptope corresponds to a structural-shortcut class in the kernel, nor that a physical Thiele Machine can produce CHSH-violating statistics by exploiting quantum entanglement in hardware. Closing the algebraic gap would require extending the column-contractivity characterisation; closing the empirical gap would require an experiment.

The boundary above is the perimeter of the positive claim. Inside the perimeter: the directional subsumption (Section~\ref{sec:turing}), the three irrecoverability theorems (Section~\ref{sec:projection-nonexistence}), the cost-ledger observability separation (Section~\ref{sec:categorical}), the entropy-bounded $\mu$-ledger theorem (Section~\ref{sec:entitlement}), the substrate-axis undecidability (Section~\ref{sec:substrate-undecidability}), the CHSH-NPA equivalence on the orthogonal slice (Section~\ref{sec:chsh}), and the categorical universal property of Thiele as the initial A2-respecting substrate (\texttt{thiele\_is\_initial\_a2\_substrate} in \texttt{coq/kernel/nfi/ThieleInitiality.v}, Print Assumptions: Closed under the global context). Outside the perimeter: the items above, each with the formal reason the existing argument does not extend to it. There is no rhetorical claim that crosses the perimeter; if a reader's objection lives on the outside, the objection is about a claim the document is not making.

\textbf{Categorical foundationality vs.\ metaphysical foundationality.} The theorem \texttt{thiele\_is\_initial\_a2\_substrate} (\texttt{coq/kernel/nfi/ThieleInitiality.v}, zero axioms) establishes Thiele as the initial/universal object in the category of A2-respecting substrates over \texttt{vm\_instruction}: given any A2-respecting substrate $M$ over the same instruction signature and any chosen basepoint $s_0^M$ in $M$, the canonical trace-fold simulation from Thiele to $M$ is the unique morphism preserving the basepoint and commuting with step-extension. This is initiality in the precise categorical sense, in the same way that $\mathbb{Z}$ is the initial ring or the free group is initial in the category of groups with a fixed generating set. Whether the A2-respecting category is itself foundationally privileged over alternative cost-tracking categories is a meta-mathematical judgment about which signatures matter, of the same shape as ``why study groups instead of semigroups.'' Such questions are not settled by any theorem inside any category; they are settled by which questions the structure helps the reader answer. The proofs in this corpus establish the categorical universal property; they do not establish, and do not claim to establish, the meta-mathematical primacy of the A2 category over alternatives. Foundationality of the asymmetry is the reader's judgment to make, on grounds outside the Coq corpus.
