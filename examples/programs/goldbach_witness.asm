...
#   - The sum is exactly 1000 (r1=0 in certified state)
...
#   r1 = sum error          (P+Q-N, 0 = correct)
#   r7 = combined proof     (r14+r15+r1 = 0 iff all checks pass)
#   r3 = MOD working register (dividend, in/out)
#   r2  = MOD divisor
...
CHECKPOINT 1 0 1
    ...
    XFER  r3 r10 0             # r3 = P = 3
    LOAD_IMM r2 2 1             # divisor = 2
    CALL MOD 1                  # r3 = 3 mod 2 = 1
    JNEZ  r3 P_PRIME 0         # if remainder != 0, not divisible → prime
...
#   Each call: reload r3=997, set r2=d, CALL MOD
...
CHECKPOINT 3 0 1
    ...
    SUB r1 r6  r12 1           # r1 = (P+Q) - N = 1000 - 1000 = 0
...
CHECKPOINT 4 1 1
    ...
    ADD r7 r14 r15 1           # r7 = P_error + Q_error
    ADD r7 r7 r1 1           # r7 += sum_error  →  should be 0
...
# ════════════════════════════════════════════════════════════════════════════
# Subroutine MOD: r3 = r3 mod r2
#   Uses r6 as scratch. Preserves r2.
#   Repeated subtraction: while r3 >= r2, r3 -= r2
# ════════════════════════════════════════════════════════════════════════════
MOD:
MOD_LOOP:
    SUB  r6 r3 r2 0            # r6 = r3 - r2  (wraps if r3 < r2)
    SHR  r6 r6  r9 0            # sign bit: 1 iff r3 < r2
    JNEZ r6 MOD_RET 0           # if r3 < r2, done
    SUB  r3 r3 r2 0           # r3 -= r2
    JUMP MOD_LOOP 0
MOD_RET:
    RET 1
