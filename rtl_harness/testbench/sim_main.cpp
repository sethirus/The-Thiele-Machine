...
BridgeResp call_z3_bridge(const std::string& script, uint8_t opcode, uint32_t payload) {
    ...
    cmd << "python3 " << script << " --request " << req.string() << " --response " << rsp.string();
...
int main(int argc, char** argv, char**) {
    ...
    const std::string bridge_script = env_script ? env_script : "rtl_harness/logic_z3_bridge.py";
    ...
    while (!contextp->gotFinish()) {
        if (have_pending_resp) {
            const uint64_t packed = (1ULL << 33) | (static_cast<uint64_t>(pending.error ? 1 : 0) << 32) | static_cast<uint64_t>(pending.value);
