"""Dispatch fetch and tensor/CSR behavior validation.  Every command runs under a time limit and
...
MODS = ['ReadFreeObservation', 'DecodedReadFree', 'DispatchFetch',
        'HWBoundaryReads', 'BoundaryDecoded', 'TensorDispatch']
...
commands = [('generator', ['python3', str(ROOT / 'scripts/generate_hw_boundary.py'), '--check']),
            ('integration', ['make', '-j1', 'COQEXTRAFLAGS=-time', 'all']),
            ('contracts', ['coqc', *FLAGS, '-time', str(HERE / 'Contracts.v')]),
            ('factoring', ['coqc', *FLAGS, '-time', str(HERE / 'FetchFactoring.v')]),
