...
def evaluate_primitive_novelty(
    strategy_code: str,
    strategy_name: str,
    parameters: Dict[str, Any]
) -> float:
    """
    Evaluate a strategy's primitive novelty.
    
    Higher scores for strategies that use primitive sequences that have appeared
    less frequently in the evolutionary history (ascension_ledger).
    
    Args:
        strategy_code: The Python code of the strategy
        strategy_name: Name of the strategy
        parameters: Parameters including history_file path
    
    Returns:
        Fitness score (0.0 to 1.0), where 1.0 = maximally novel
    """
    import json
    from collections import Counter
    
    history_file = parameters.get('history_file', 'ascension_ledger.json')
    
    # Extract primitives from current strategy
    current_primitives = []
    for line in strategy_code.split('\n'):
        line = line.strip()
        if line:
            # Look for primitive calls
            words = line.split()
            for word in words:
                if word.isupper() or word.startswith('prim_'):
                    current_primitives.append(word)
    
    if not current_primitives:
        return 0.5  # Neutral score for strategies with no primitives
    
    # Load historical primitive usage
    try:
        with open(history_file, 'r') as f:
            ledger = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        # If no history exists, all primitives are novel
        return 1.0
    
    # Count historical primitive usage
    ...
    for entry in ledger:
        for step in entry.get('strategy_dna', []):
            ...
            for word in words:
                if word.isupper() or word.startswith('prim_'):
                    historical_counts[word] += 1
    
    # Calculate novelty score
    # Novel primitives have low historical counts
    novelty_scores = []
    for prim in current_primitives:
        count = historical_counts.get(prim, 0)
        # Novelty decreases logarithmically with usage count
        # New primitive (count=0) gets score 1.0
        # Primitive used once (count=1) gets score ~0.7
        # Primitive used 10 times gets score ~0.3
        if count == 0:
            novelty_scores.append(1.0)
        else:
            novelty_scores.append(1.0 / (1.0 + np.log(count + 1)))
    
    # Return average novelty across all primitives in strategy
...
EVALUATION_FUNCTIONS: Dict[str, Callable] = {
    ...
    'evaluate_primitive_novelty': evaluate_primitive_novelty,
