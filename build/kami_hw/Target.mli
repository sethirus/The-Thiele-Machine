...
val projT1 : ('a, 'b) sigT -> 'a
val pred : int -> int
val add : int -> int -> int
val eqb : bool -> bool -> bool
module Nat : sig val div2 : int -> int end
val map : ('a -> 'b) -> 'a list -> 'b list
type t = F1 of int | FS of int * t
type 'a t0 = Nil | Cons of 'a * int * 'a t0
val nth : int -> 'a t0 -> t -> 'a
val map0 : ('a -> 'b) -> int -> 'a t0 -> 'b t0
val ascii_eq : char -> char -> bool
val string_eq : char list -> char list -> bool
val mod2 : int -> bool
type word = WO | WS of bool * int * word
val natToWord : int -> int -> word
val wzero : int -> word
type 'a attribute = { attrName : char list; attrType : 'a; }
val vector_find' : ('a -> bool) -> 'b -> 'a t0 -> 'c
val vector_find : 'a -> int -> 'b -> 'c
type ('a, 'b) ilist = Inil | Icons of 'a * int * 'a t0 * 'b * ('a, 'b) ilist
type 'a vec = Vec0 of 'a | VecNext of int * 'a vec * 'a vec
val replicate : 'a -> int -> 'a vec
type kind =
    Bool
  | Bit of int
  | Vector of kind * int
  ...
  | Array of kind * int
type fullKind = SyntaxKind of kind | NativeKind of __
type constT =
    ConstBool of bool
...
type constFullT = SyntaxConst of kind * constT | NativeConst of __ * __
val vector_repeat : int -> 'a -> 'a t0
val getDefaultConst : kind -> constT
type signatureT = { arg : kind; ret : kind; }
type uniBoolOp = NegB
type binBoolOp = AndB | OrB
type uniBitOp =
    Inv of int
  | Neg of int
  ...
  | TruncLsb of int * int
type binSign = SignSS | SignSU | SignUU
type binBitOp =
    Add of int
  | Sub of int
  | Mul of int * binSign
  ...
  | Sra of int * int
  | Concat of int * int
type binBitBoolOp = Lt of int | Slt of int
type 'ty fullType = __
val fieldKind : int -> 'a attribute t0 -> t -> 'a
type 'ty expr =
...
type bitFormat = Binary | Decimal | Hex
...
val fieldAccessor : char list -> 'a attribute -> bool
type moduleElt =
    MERegister of regInitT
  | MERule of action attribute
  | MEMeth of defMethT
type inModule = NilInModule | ConsInModule of moduleElt * inModule
val makeModule' :
...
type actionS =
  ...
  | ReturnS of exprS
val getActionS : tyS -> kind -> tyS actionT -> tyS * actionS
...
val getMethS : (signatureT, 'a) sigT -> (signatureT, actionS) sigT
val getModuleS : modules -> modulesS
val mapVec : ('a -> 'b) -> int -> 'a vec -> 'b vec
type bExpr =
    BVar of int
...
val bindVec : int -> 'a option vec -> 'a vec option
val bindVector : int -> 'a option t0 -> 'a t0 option
val exprSToBExpr : fullKind -> 'a expr -> bExpr option
val actionSToBAction : kind -> actionS -> bAction list option
val rulesToBRules :
  actionS attribute list -> bAction list attribute list option
val methsToBMethods :
  (signatureT, actionS) sigT attribute list ->
  (signatureT * bAction list) attribute list option
...
val sP_IDX : word
val check_bounds : 'a expr -> 'a expr -> 'a expr
val read_mem : 'a expr -> 'a expr -> 'a expr
val write_mem : 'a expr -> 'a expr -> 'a expr -> 'a expr
...
val targetB : 'a -> bModule list option
