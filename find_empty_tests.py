#!/usr/bin/env python3
"""Find test files with no pytest tests."""
import subprocess
import sys
...
for test_file in test_files:
    ...
    if "no tests ran" in result.stdout or "collected 0 items" in result.stdout or "no tests collected" in result.stdout:
...
if empty_tests:
    print(f"\n{'='*80}")
    print(f"Found {len(empty_tests)} test files with no tests:")
    for test_file in empty_tests:
        print(f"  - {test_file}")
    sys.exit(1)
else:
    print(f"\n{'='*80}")
    print("All test files have tests!")
