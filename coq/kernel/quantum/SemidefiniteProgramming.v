...
(** Diagonal elements of PSD matrices are non-negative.
    This is a per-size case-projection from the PSD definition: for each
    n ∈ [1, 5] the corresponding PSD_k unfolds into a conjunction whose
    first n components are the diagonal-non-negativity assertions; for
    n ≥ 6 the PSD body is itself the universal diagonal-non-negativity
    predicate. So this is a structural projection across the size cases.
    Definition-with-ltac-body form. *)
Definition PSD_diagonal_nonneg : forall (n : nat) (M : Matrix n) (i : nat),
  (i < n)%nat ->
  PSD M ->
  M i i >= 0 := ltac:(
