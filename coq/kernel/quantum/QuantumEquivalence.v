...
(** Field-projection definitions on [is_quantum_correlation] and
    [certifiable_with_mu_zero]. Expressed as [Definition] with explicit
    proof terms — they are not derivations, they are the canonical
    record-projection API for the conjunction / existential structure
    of those predicates. *)

Definition quantum_implies_mu_zero
  (ac : AbstractCorrelation) (Hq : is_quantum_correlation ac) :
  ac.(chsh_value) <= tsirelson_bound :=
  match Hq with conj _ Hbound => Hbound end.

Definition mu_zero_implies_quantum
...
Definition quantum_requires_no_revelation
  (ac : AbstractCorrelation) (Hq : is_quantum_correlation ac) :
  ac.(chsh_value) <= tsirelson_bound :=
  quantum_implies_mu_zero ac Hq.
