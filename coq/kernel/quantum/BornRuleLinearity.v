...
(** No-signaling and mixture compatibility.
    ...
    Layer 3: mathematical bridge, defined below:
      ...
      In the definitional bridge, this is already the hypothesis type.
    ...
    This last implication is physical content. In this file it is
    either the thin definitional bridge or an explicit Hardy-style
    hypothesis. *)
...
(** Summary of the algebraic layer.
    ...
    The thin no-signaling bridge is definitional. The Hardy bridge
    names the physical hypotheses needed for a non-definitional route
    to [mixture_compatible].
...
(** Physical assumption C1: encoding completeness, or "H_universal".
    ...
    Classification: representation assumption dischargeable for the
    exact-encoding two-point support. Universal coverage would
    require an approximate-encoding redesign that is not provided
    here. *)
...
(** Physical assumption C2: grounding, or "H_grounded".
    ...
    Classification: grounding assumption constructively dischargeable
    when [outcome] is fixed to [born_outcome]. The [forall r]
    quantifier is stronger than the Hardy bridge proof requires; the
    direct algebraic path [born_rule_capstone] proves the uniqueness
    theorem without the Hardy bridge hypotheses. *)
