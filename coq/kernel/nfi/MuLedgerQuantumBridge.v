...
(** ** State-based quantum bridge

    The trace-based bridge above extracts correlators by scanning
    [list vm_instruction]. This section provides the equivalent
    state-based bridge that reads correlators directly from the final
    [VMState]'s [vm_witness] field ([WitnessCounts] buckets). The
    quantum object is determined by final machine state alone, not
    reconstructed from an external execution trace. *)
