(** CHSHStatisticalBridge: count the CHSH evidence, then ask what it rules out
   ...
   [proved here through local_bound_for_wc]
    ...
    6. hitting a trial-count threshold still costs that many CHSH_TRIAL steps
        ...
        There is no Hoeffding theorem here. There is no confidence theorem here.
    Finite-sample certification still needs a real probability library and
    explicit sampling assumptions.

    I also removed the old hoeffding_chsh_concentration placeholder because it
    concluded True and carried no load. The deterministic chain stands without it.
*)
...
(** Summary.
  ...
  The probability side is still outside the file. I removed the old vacuous
  placeholder, and I am not upgrading that boundary until there is a real
  probability formalization behind it.
*)
