...
(** ** A2 no-go: uniform positive mass cannot satisfy the non-vacuum EFE

    Combining [uniform_module_tensor s] (which forces the global
    Einstein tensor to zero) with uniform positive structural mass
    (which makes [T_00 = INR m > 0]) produces a contradiction under
    the unit convention [8πG = 1]: the 00 component would require
    [0 = INR m]. The theorem below records that contradiction
    explicitly, so the infeasible case is closed as a checked no-go
    rather than a prose note. The non-vacuum route must use
    non-uniform mass/metric data, as in
