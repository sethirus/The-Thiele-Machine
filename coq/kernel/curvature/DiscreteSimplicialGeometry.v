...
(** Definitional wrapper. combinatorially_orthogonal IS the
    off-diagonal-Ricci-zero predicate; this is the canonical instance
    invocation. Definition-with-proof-term form. *)
Definition combinatorially_orthogonal_implies_off_diagonal_ricci
  (sc : SimplicialComplex4D) (s : VMState) (v _kappa : ModuleID)
  (Hco : combinatorially_orthogonal sc s v) :
  forall mu nu : nat,
    (mu < 4)%nat -> (nu < 4)%nat -> mu <> nu ->
    curved_ricci s sc mu nu v = 0%R :=
  Hco.
