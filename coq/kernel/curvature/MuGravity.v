(** MuGravity: discrete geometry built on top of μ and module structure

   This file defines a geometric layer over the module graph and μ bookkeeping:
   module distances, angle-style quantities, curvature proxies, and the
   Gauss-Bonnet-style identities used later in the gravity pipeline.

   The scope is explicit. The file sets up discrete geometric structure and
...
(** Get triangles containing a module (m, n1, n2) where n1, n2 are neighbors of m
...
Definition module_triangles (s : VMState) (m : ModuleID) : list (ModuleID * ModuleID) :=
  ...
  flat_map (fun n1 =>
    map (pair n1)
      (filter (fun n2 =>
         andb (andb (negb (n1 =? n2)) (n1 <? n2))
              (modules_adjacent_by_region s n1 n2))
         neighbors))
    neighbors.
...
(** Encoding length of a single axiom, measured as concrete payload bits. *)
...
(* definitional lemma *)
Lemma modules_adjacent_by_region_vm_graph_invariant : forall s1 s2 m1 m2,
  vm_graph s1 = vm_graph s2 ->
  modules_adjacent_by_region s1 m1 m2 = modules_adjacent_by_region s2 m1 m2.
Proof.
  intros s1 s2 m1 m2 Hgraph.
...
Proof.
  ...
  apply filter_ext.
  intros n2.
  rewrite (modules_adjacent_by_region_vm_graph_invariant s1 s2 n1 n2 Hgraph).
  reflexivity.
Qed.
...
(** Generic helper: pointwise extensionality on [In] for [flat_map]. *)
Lemma flat_map_ext_in_local :
  forall (A B : Type) (f g : A -> list B) (l : list A),
  (forall a, In a l -> f a = g a) -> flat_map f l = flat_map g l.
Proof.
  intros A B f g l Hfg.
  induction l as [|a rest IH]; cbn [flat_map]; [reflexivity|].
  rewrite (Hfg a (or_introl eq_refl)).
  f_equal. apply IH. intros b Hin. apply Hfg. right. exact Hin.
Qed.
...
Proof.
  ...
  set (ns := module_neighbors s m).
  assert (Hbnd : forall n, In n ns -> n < pg_next_id (vm_graph s)).
  { intros n Hin. unfold ns, module_neighbors, module_neighbors_physical,
                         module_neighbors_adjacent in Hin.
    ...
    - exact Hin. }
  apply flat_map_ext_in_local.
  intros n1 Hn1.
  pose proof (Hbnd n1 Hn1) as Hn1_lt.
  f_equal.
  apply filter_ext_in.
  intros n2 Hn2.
  pose proof (Hbnd n2 Hn2) as Hn2_lt.
  rewrite (modules_adjacent_by_region_pnew_preserved s region n1 n2 Hn1_lt Hn2_lt).
  reflexivity.
Qed.
