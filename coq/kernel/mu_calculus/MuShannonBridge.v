(** MuShannonBridge: connecting mu to Shannon-style reasoning.
    ...
    The proven part is modest. The file defines what it means to
    distinguish states, keeps the ledger identity [δ-μ = declared cost
    sum], and proves bounds that go through once the pricing policy is
    assumed. The unproven part is also stated plainly: there is no
    first-principles proof here that every n-way distinction requires
