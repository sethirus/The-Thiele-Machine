(** MuInitiality: mu as the universal monotone.

    The claim here is stronger than "mu is a useful cost measure." I am saying
    that once you fix the local consistency law and require the initial state to
    start at zero, you do not have freedom left. Any other cost functional with
    those same rules has to agree with vm_mu on every reachable state.

    That is what the initiality language is doing. It is a precise way to say
    mu is forced by the laws rather than chosen by taste. If someone can build a
    different instruction-consistent zero-based functional that disagrees with
    mu on a reachable state, this file falls apart. *)
...
(** init_graph: the empty partition graph at startup.

  This starts with no allocated partitions and no hidden structure. That is
  why mu = 0 is coherent at initialization: nothing has been built yet. If an
  initial state carried pre-existing partition structure for free, the whole
  cost story would already be compromised before execution began. *)
...
(** init_csrs: initial control/status registers.
  ...
  All the metadata starts at zero because the machine has not done anything
  yet. No certificate has been set, no status flags are raised, and no error
  has occurred. That matches the same zero-information story as init_graph.

  If initialization secretly required nonzero metadata to function, then the
  machine would already contain hidden information before any execution step.
  That would break the mu = 0 starting story this file relies on. *)
...
(** ** Breaking the apparent circularity
    ...
    [canonical_cost] is just [instruction_cost], which extracts the
    [mu_delta] parameters carried by individual instructions. Read
    naively, that makes the cost law look like an arbitrary design
    choice rather than a derived quantity.

    The resolution lives in [MuCostDerivation.v]: instruction costs
    are uniquely determined by information theory and thermodynamics:
...
(** mu_is_initial_monotone: THE FLAGSHIP INITIALITY THEOREM
    ...
    μ is the UNIQUE monotone satisfying:
    ...
    FORMAL ∀M. instruction_consistent(M, canonical_cost) → M(init_state) = 0 →
...
(** UNIVERSALITY μ is the unique CostFunctional *)
...
(** Summary.
    ...
    By this point the file has proved the full initiality story: traces produce
    reachable states, mu accumulates canonical cost along those traces, and any
    other zero-based instruction-consistent cost functional collapses to the
    same value on reachable states.

    That is the categorical punchline too. In the intended category, vm_mu is
    initial because every other admissible cost functional agrees with it by the
    only morphism available: identity on reachable states. *)
