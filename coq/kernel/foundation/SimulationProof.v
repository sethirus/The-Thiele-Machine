...
(** Basic projections from the states relation.
    ...
    them via [apply]/[exact] exactly as before. *)

Definition states_related_implies_encoding
  (s_vm : VMState) (s_kernel : state)
  (H : states_related s_vm s_kernel) :
  decode_vm_state s_kernel.(tape) = Some (s_vm, []) :=
  match H with conj _ (conj _ Htape) => Htape end.

Definition states_related_implies_pc
  (s_vm : VMState) (s_kernel : state)
  (H : states_related s_vm s_kernel) :
  s_vm.(vm_pc) = s_kernel.(tm_state) :=
  match H with conj Hpc _ => Hpc end.

Definition states_related_implies_mu
  (s_vm : VMState) (s_kernel : state)
  (H : states_related s_vm s_kernel) :
  s_vm.(vm_mu) = s_kernel.(mu_cost) :=
  match H with conj _ (conj Hmu _) => Hmu end.
...
(** Compile one VM instruction into a kernel program sequence: update
    PC, charge cost to μ, then apply the VM operation. *)
