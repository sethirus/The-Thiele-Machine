          ...
          | None ->
            (match parse_int tok with
             | Some lit ->
               if Z.eqb lit Z0
               then None
               else Some ((Z.to_nat (Z.abs lit)),(Z.gtb lit Z0))
             | None -> None))
...
let graph_add_axiom g mid ax =
  ...
  | Some m ->
    let updated = { module_region = m.module_region; module_axioms =
      (app m.module_axioms (ax::[])); module_mu_tensor = m.module_mu_tensor }
    in
  ...
  N.to_nat
    (N.coq_land
      (N.add (N.of_nat (word32 a))
        (N.add (N.coq_lxor (N.of_nat (word32 b)) word32_mask) (Npos XH)))
      word32_mask)
...
let word32_and = (fun a b -> a land b land 0xFFFFFFFF)

(** val word32_or : int -> int -> int **)

let word32_or = (fun a b -> (a lor b) land 0xFFFFFFFF)

(** val word32_shl : int -> int -> int **)

let word32_shl = (fun a b -> (a lsl (b mod 32)) land 0xFFFFFFFF)

(** val word32_shr : int -> int -> int **)

let word32_shr = (fun a b -> (a land 0xFFFFFFFF) lsr (b mod 32))

(** val word32_mul : int -> int -> int **)

let word32_mul = (fun a b -> (a * b) land 0xFFFFFFFF)
...
| Instr_lassert (module0, formula, cert, cost) ->
  (match cert with
   | Lassert_cert_unsat proof ->
     advance_state s (Instr_lassert (module0, formula, (Lassert_cert_unsat
       proof), cost)) s.vm_graph (csr_set_err s.vm_csrs ((fun x -> x + 1) 0))
       (latch_err s true)
...
| Instr_load_imm (dst, imm, cost) ->
  advance_state_rm s (Instr_load_imm (dst, imm, cost)) s.vm_graph s.vm_csrs
    (write_reg s dst (word32 imm)) s.vm_mem s.vm_err
...
| Instr_add (dst, rs1, rs2, cost) ->
  advance_state_rm s (Instr_add (dst, rs1, rs2, cost)) s.vm_graph s.vm_csrs
    (write_reg s dst (word32_add (read_reg s rs1) (read_reg s rs2))) s.vm_mem
    s.vm_err
| Instr_sub (dst, rs1, rs2, cost) ->
  advance_state_rm s (Instr_sub (dst, rs1, rs2, cost)) s.vm_graph s.vm_csrs
    (write_reg s dst (word32_sub (read_reg s rs1) (read_reg s rs2))) s.vm_mem
    s.vm_err
...
| Instr_xor_add (dst, src, cost) ->
  advance_state_rm s (Instr_xor_add (dst, src, cost)) s.vm_graph s.vm_csrs
    (write_reg s dst (word32_xor (read_reg s dst) (read_reg s src))) s.vm_mem
    s.vm_err
...
| Instr_xor_rank (dst, src, cost) ->
  advance_state_rm s (Instr_xor_rank (dst, src, cost)) s.vm_graph s.vm_csrs
    (write_reg s dst (word32_popcount (read_reg s src))) s.vm_mem s.vm_err
...
(** val run_vm : int -> vm_instruction list -> vMState -> vMState **)
