...
REPORT=ADMIT_REPORT.txt
: > "$REPORT"

echo "Admitted occurrences (file:line):" | tee -a "$REPORT"
grep -R -n "\bAdmitted\b" . --include="*.v" || true | tee -a "$REPORT"

echo "" | tee -a "$REPORT"
echo "Axiom declarations (file:line):" | tee -a "$REPORT"
grep -R -n "^Axiom " . --include="*.v" || true | tee -a "$REPORT"

echo "" | tee -a "$REPORT"
echo "Summary:" | tee -a "$REPORT"
num_admits=$(grep -R -n "\bAdmitted\b" . --include="*.v" | wc -l || true)
num_axioms=$(grep -R -n "^Axiom " . --include="*.v" | wc -l || true)
