(** ThieleProc: Thiele programs as a process category.
  ...
  This file packages Thiele programs with sequential composition into a small
  categorical wrapper. Objects describe interfaces, morphisms are programs,
  composition is concatenation, and the identity morphism is the empty
  program. The receipt-trace lemmas are there so later tensor and functor
  constructions have concrete execution data to point at.

  The scope is modest: this is categorical bookkeeping over the existing
  machine model, not a claim that every higher categorical structure has been
  settled here.
 *)
...
(* Tensor on objects is addition on partition counts. *)
...
(** Split morphisms: one morphism per subsystem after a split A⊗B. *)
...
(** Pointwise composition on split morphisms. *)
...
(** Tensor on morphisms: parallel pair constructor. *)
Definition tensor_morphism (f g : Prog) : SplitMorphism :=
  {| morph_left := f; morph_right := g |}.

(** Bifunctor law on morphisms (pointwise composition). *)
...
(** PSPLIT decomposition/recomposition at morphism level. *)
...
(* The lemmas above give the concrete footholds required by
   tensor structure (running programs in parallel) and
   embeddings from logic/computation/physics.  Later files build on
