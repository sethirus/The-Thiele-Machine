 ...
 *
 * Thiele programs with sequential composition form a category.
 * Objects describe partition interfaces (number of Π-components);
 * morphisms are programs whose receipts run on those interfaces.
 * Composition is program concatenation and identities are empty
 * programs.  Helper lemmas about receipt traces generated by composed
 * programs support tensor structure and functors.
 *)
...
(* Tensor on objects is addition on partition counts; we record the
   intended geometry even though only the plain category structure is
   used here. *)
