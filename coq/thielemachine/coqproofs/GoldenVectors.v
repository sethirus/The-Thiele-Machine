...
Definition golden2_state : State := {|
  ...
  |};
  mu_ledger := {| mu_operational := 8; mu_information := 0; mu_total := 8 |};
  pc := 1%nat;
  halted := false;
  result := None;
...
Definition golden3_state : State := {|
  partition := {| modules := []; next_module_id := 0%nat |};
  mu_ledger := {| mu_operational := 1; mu_information := 0; mu_total := 1 |};
  pc := 1%nat;
  halted := true;
  result := Some 42%nat;
  program := [EMIT 42%nat]
...
Definition golden4_state : State := {|
  ...
  |};
  mu_ledger := {| mu_operational := 24; mu_information := 0; mu_total := 24 |};
  pc := 3%nat;
  halted := false;
  result := None;
  program := [
