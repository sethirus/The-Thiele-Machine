...
Proof.
  intros x y. destruct x, y; cbn; reflexivity.
Qed.
...
Proof.
  intros x y1 y2 a.
  destruct x, y1, y2, a; cbn; reflexivity.
Qed.
...
Proof.
  intros y x1 x2 b.
  destruct y, x1, x2, b; cbn; reflexivity.
Qed.
...
Proof.
  unfold E, PR, PR_p, sum_bit2, sum_bit; cbn; reflexivity.
Qed.
...
Proof.
  unfold S, E, PR, PR_p, sum_bit2, sum_bit; cbn; reflexivity.
Qed.

Lemma PR_Qabs : Qabs (S PR) == inject_Z 4.
Proof.
  assert (HS : S PR == inject_Z 4).
  { unfold S, E, PR, PR_p, sum_bit2, sum_bit; cbn; reflexivity. }
  assert (Hpos : 0#1 <= inject_Z 4).
  { unfold Qle; simpl; lia. }
  apply Qabs_of_nonneg in Hpos.
  setoid_rewrite HS.
  exact Hpos.
Qed.
...
Proof.
  assert (HS : S TsirelsonApprox == (4#1) * tsirelson_gamma) by apply TsirelsonApprox_S.
  assert (Hpos : 0#1 <= (4#1) * tsirelson_gamma).
  { unfold Qle; simpl; lia. }
  apply Qabs_of_nonneg in Hpos.
  setoid_rewrite HS.
  exact Hpos.
...
Definition pr_alice_cert : TM.ConcreteCert :=
  tsirelson_cert pr_alice_setting pr_alice_outcome.

Definition pr_bob_cert : TM.ConcreteCert :=
  tsirelson_cert pr_bob_setting pr_bob_outcome.

Definition pr_alice_receipt : TM.ConcreteReceipt :=
  {| TM.receipt_instr := TM.PYEXEC "alice_measurement"%string;
     TM.receipt_pre := tsirelson_state 2%nat;
     TM.receipt_post := tsirelson_state 3%nat;
     TM.receipt_obs :=
       {| TM.ev := Some (TM.PolicyCheck "alice_measurement"%string);
          TM.mu_delta := 0%Z;
          TM.cert := pr_alice_cert |} |}.

Definition pr_bob_receipt : TM.ConcreteReceipt :=
  {| TM.receipt_instr := TM.PYEXEC "bob_measurement"%string;
     TM.receipt_pre := tsirelson_state 3%nat;
     TM.receipt_post := tsirelson_state 4%nat;
     TM.receipt_obs :=
...
Definition pr_alice_frame :
  BridgeReceiptFrame TM.ThieleInstr TM.ConcreteState TM.StepObs :=
  concrete_receipt_frame pr_alice_receipt.

Definition pr_bob_frame :
  BridgeReceiptFrame TM.ThieleInstr TM.ConcreteState TM.StepObs :=
  concrete_receipt_frame pr_bob_receipt.

Lemma pr_alice_frame_cert :
  TM.cert (brf_obs pr_alice_frame) = pr_alice_cert.
Proof.
  reflexivity.
Qed.

Lemma pr_bob_frame_cert :
  TM.cert (brf_obs pr_bob_frame) = pr_bob_cert.
Proof.
  reflexivity.
Qed.
...
(* ------------------------------------------------------------------------- *)
(*  PR receipt trace scaffolding                                             *)
(* ------------------------------------------------------------------------- *)

Definition pr_program : list TM.ThieleInstr := tsirelson_program. (* Same program *)

Definition pr_start : TM.ConcreteState := TM.default_concrete_state.

Definition pr_receipts : list TM.ConcreteReceipt :=
  TM.concrete_receipts_of pr_start pr_program.

Definition pr_frames :
  list (BridgeReceiptFrame TM.ThieleInstr TM.ConcreteState TM.StepObs) :=
  List.map concrete_receipt_frame pr_receipts.

Lemma pr_receipts_sound :
  @receipts_sound _ _ _ concrete_step_frame pr_start pr_frames.
Proof.
  unfold pr_frames, pr_receipts.
  apply concrete_receipts_sound.
Qed.



Lemma pr_alice_cert_timestamp :
  cert_timestamp_bit pr_alice_cert = pr_alice_setting.
Proof.
  unfold pr_alice_cert, tsirelson_cert.
  apply cert_timestamp_bit_bit_to_Z.
Qed.

Lemma pr_alice_cert_sequence :
  cert_sequence_bit pr_alice_cert = pr_alice_outcome.
Proof.
  unfold pr_alice_cert, tsirelson_cert.
  apply cert_sequence_bit_bit_to_nat.
Qed.

Lemma pr_bob_cert_timestamp :
  cert_timestamp_bit pr_bob_cert = pr_bob_setting.
Proof.
  unfold pr_bob_cert, tsirelson_cert.
  apply cert_timestamp_bit_bit_to_Z.
Qed.

Lemma pr_bob_cert_sequence :
  cert_sequence_bit pr_bob_cert = pr_bob_outcome.
Proof.
  unfold pr_bob_cert, tsirelson_cert.
  apply cert_sequence_bit_bit_to_nat.
Qed.

Lemma pr_alice_frame_input_bit :
  frame_input_bit pr_alice_frame = pr_alice_setting.
Proof.
  unfold frame_input_bit.
  rewrite pr_alice_cert_timestamp.
  reflexivity.
Qed.

Lemma pr_bob_frame_input_bit :
  frame_input_bit pr_bob_frame = pr_bob_setting.
Proof.
  unfold frame_input_bit.
  rewrite pr_bob_cert_timestamp.
  reflexivity.
Qed.

Lemma pr_bob_frame_output_bit :
  frame_output_bit pr_bob_frame = pr_bob_outcome.
Proof.
  unfold frame_output_bit.
  rewrite pr_bob_cert_sequence.
  reflexivity.
Qed.

Lemma pr_alice_frame_valid :
  frame_valid concrete_step_frame pr_alice_frame.
Proof.
  unfold frame_valid, frame_result, concrete_step_frame,
    concrete_receipt_frame, pr_alice_frame,
    pr_alice_receipt.
  simpl.
  reflexivity.
Qed.

Lemma pr_bob_frame_valid :
  frame_valid concrete_step_frame pr_bob_frame.
Proof.
  unfold frame_valid, frame_result, concrete_step_frame,
    concrete_receipt_frame, pr_bob_frame,
    pr_bob_receipt.
  simpl.
  reflexivity.
Qed.

Lemma pr_measurements_sound :
  @receipts_sound _ _ _ concrete_step_frame (tsirelson_state 2%nat)
    [pr_alice_frame; pr_bob_frame].
Proof.
  simpl.
  split; [reflexivity|].
  split; [apply pr_alice_frame_valid|].
  split; [reflexivity|].
  split; [apply pr_bob_frame_valid|].
  exact I.
Qed.

Definition pr_measurement_pair :
  BridgeReceiptFrame TM.ThieleInstr TM.ConcreteState TM.StepObs *
  BridgeReceiptFrame TM.ThieleInstr TM.ConcreteState TM.StepObs :=
  (pr_alice_frame, pr_bob_frame).

Definition pr_trial_of (interp : FrameTrialInterpreter) : CHSHTrial :=
  interpret_trial interp pr_alice_frame pr_bob_frame.

Lemma pr_trial_E_frame_interpreter :
  trial_E (pr_trial_of tsirelson_frame_interpreter) == -1#1.
Proof.
  unfold trial_E.
  rewrite pr_alice_frame_output_bit.
  rewrite pr_bob_frame_output_bit.
  unfold pr_alice_outcome, pr_bob_outcome.
  simpl.
  reflexivity.
Qed.

Lemma pr_trial_S_frame_interpreter :
  trial_S (pr_trial_of tsirelson_frame_interpreter) == 1#1.
Proof.
  unfold trial_S.
  rewrite pr_alice_frame_input_bit.
  rewrite pr_bob_frame_input_bit.
  simpl.
  apply pr_trial_E_frame_interpreter.
Qed.

(* The machine can produce PR-like correlations *) 

Theorem thiele_machine_realizes_pr_correlations :
  exists frames,
    @receipts_sound _ _ _ concrete_step_frame (tsirelson_state 2%nat) frames /\
    let trial := interpret_trial tsirelson_frame_interpreter
                   (fst (hd (chunk_pairs (filter_measurement_frames frames))))
                   (snd (hd (chunk_pairs (filter_measurement_frames frames)))) in
    trial_S trial == 1#1 /\ trial_E trial == -1#1.
Proof.
  exists [pr_alice_frame; pr_bob_frame].
  split.
  - apply pr_measurements_sound.
  - simpl.
    split.
    + apply pr_trial_S_frame_interpreter.
    + apply pr_trial_E_frame_interpreter.
Qed.
