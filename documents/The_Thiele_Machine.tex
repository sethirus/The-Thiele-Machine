...
\end{abstract}
\section*{Mechanization and Proof Audit Status (2025-09-12)}
\textbf{All Coq proofs in the repository were compiled and checked as of September 12, 2025.}
\begin{itemize}
  \item \textbf{All core theorems and invariants referenced in this document are fully mechanized and verified in the Coq development.}
  \item \textbf{Exceptions:} Two files, \texttt{ListModules.v} (stub, not referenced in main text) and \texttt{ThieleMachineConcretePack.v} (import path error), failed to compile. These do not affect the main mechanized results.
  \item \textbf{Admitted theorems:} Some advanced lemmas in amortization and partition logic are admitted and marked as such in the code; all main soundness, confluence, and paradox theorems are fully proved.
  \item \textbf{Empirical receipts, cost formulas, and canonical encodings are up to date and match the referenced scripts and schema.}
\end{itemize}
This document is thus fully auditable: every claim is either mechanized, empirically supported, or explicitly marked as pending.
...
\begin{enumerate}
  ...
  \item The logic engine $L$ returns one of three results:
    \begin{itemize}
      ...
      \item \texttt{unknown (timeout or incomplete)}: execution halts, the state is marked as unresolved, and a finite but high cost is logged for the failed query.
    \end{itemize}
...
\begin{itemize}
  \item \textbf{Theorem (Consistency or Death):} At every step, either the state is consistent with all axioms, or the paradox flag is set and cost is infinite. \emph{(Mechanized: If paradox is detected, cost is infinite.)}
  \item \textbf{Theorem (No Free Sight):} Every refinement of partition, every new axiom, every shortcut to structure is paid for in $\mu$-bits. No exceptions. \emph{(Mechanized: Ledger tracks all costs, no unpaid transitions.)}
  \item \textbf{Theorem (Auditability):} Every transition is justified by a machine-verifiable certificate. No hand-waving, no “trust me, bro.” \emph{(Mechanized: Each transition is accompanied by a certificate and hash chain.)}
  \item \textbf{Theorem (Order-Invariance / Church-Rosser):} The outcome depends on the structure, not the sequence. If your answer changes with order, you’re paying for blindness. \emph{(Mechanized: The transition system is confluent; see Church-Rosser property below.)}
\end{itemize}
...
The formal proof is mechanized in Coq (see Appendix~\ref{appendix:coq}, [`AmortizedAnalysis.v`](coq/thielemachine/coqproofs/AmortizedAnalysis.v:1)). The key insight is that the discovery cost is paid once per batch, while the operational cost is paid per instance. As the number of instances increases, the per-instance share of the discovery cost vanishes.
...
\textbf{Note on Proof Status (2025-09-12):}
\begin{itemize}
