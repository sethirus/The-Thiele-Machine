...
The Thiele Machine extends classical computational models such as the Turing machine and RAM model by integrating logical auditing and partition-aware transitions as first-class components of the computation. Unlike the Turing machine, which operates on a linear tape with fixed transition rules independent of external verification, or the RAM model, which assumes unit-time random access without explicit consistency checks, the Thiele Machine conditions state transitions on verifiable logical consistency checks performed by an external logic engine. The results of these checks—termed receipts—are incorporated directly into the machine's state and ledger, making the history of logical queries an auditable, integral part of the computation itself.

Formally, the Thiele Machine can be situated as an augmented Turing machine $(Q, \Sigma, \delta, q_0, q_{acc}, q_{rej}, C)$, where $C$ is an auxiliary certificate tape that records logical witnesses (models or unsat cores) and $\mu$-bit costs, and the transition function $\delta$ is extended to include a satisfiability query step before applying any state update. This augmentation addresses a fundamental limitation in classical models: the absence of mechanisms for explicitly measuring and accounting for the information-theoretic cost of structural hypotheses (partitions) and logical justifications during computation.

In contrast to RAM models, which focus on arithmetic and memory operations without built-in support for modular decomposition, the Thiele Machine's partitions $\Pi$ enable localized reasoning over disjoint modules, with composition verified through explicit interface axioms. This design supports ``sighted'' computation—where hidden structure is hypothesized, verified, and amortized—while classical models remain ``blind,'' paying exponential time penalties for undiscovered structure without any formal accounting.

This formal extension reframes potential critiques (e.g., ``The Thiele Machine merely wraps an SMT solver'') as the core innovation: by elevating logical auditing and partition refinement to primitive operations within the computational model, the Thiele Machine provides a rigorous framework for verifiable, cost-aware computation that classical models do not natively incorporate.
...
\begin{enumerate}
  ...
  \item Step 3 — STORE $m_2$ with policy enforcement:
    \begin{itemize}
      ...
      \item receipt appended.
    \end{itemize}
  \item Finalization (example script output):
    \begin{itemize}
      \item total mu-bits paid (as produced by the example script): 2 (see \texttt{receipts/xor_tseitin.json}).
      \item global certificate: composition of step certificates (hash chain). The example script produced a canonical receipt file \texttt{receipts/xor_tseitin.json} whose certificate hash for the single step record is \texttt{ac11ddfe190d547340608979d3cfe010a973401a13dc3fd5a9b8b61f73708b8f}.
      \item archive: store receipts in an immutable, auditable ledger. The example script also emits a PNG plot at \texttt{examples/xor_tseitin_plot.png}.
    \end{itemize}
\end{enumerate}
...
The following reported result is the verified output for the small Tseitin instance reproduced in the artifact. Full benchmark sweeps are available in the archival receipts (see \texttt{tseitin_receipts.json} and the summary file produced by the benchmark script).
\begin{table}[h!]
\centering
...
While this paper uses IIT as its test case due to its mathematical rigor, the framework is designed to be modular. The consciousness_axiom.smt2 file can be replaced with axioms formalizing alternative theories (e.g., Global Workspace Theory, Higher-Order Thought Theory), allowing for a comparative, formal verification of competing models of consciousness. This elevates our work from a paper about IIT to a paper that introduces a universal platform for computational metaphysics.
