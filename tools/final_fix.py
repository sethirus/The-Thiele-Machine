...
Proof.
  ...
  simpl in Hminor.
  rewrite E00, E01, rho_BB in Hminor.
  lra.
Qed.
...
Proof.
  ...
  unfold principal_minor3, det3_matrix, npa_to_matrix in Hminor.
  simpl in Hminor.
  rewrite E10, E11, rho_BB in Hminor.
  lra.
Qed.
...
Proof.
  ...
  assert (Hquad: (y - (x*b - sqrt((1-x*x)*(1-b*b)))) * (y - (x*b + sqrt((1-x*x)*(1-b*b)))) <= 0).
  { admit. }
  admit.
Admitted.
...
Proof.
  ...
  { assert (Hsq2: sqrt2 * sqrt2 = 2) by apply sqrt2_squared.
    assert (Hsqrt2_pos: sqrt2 > 0) by apply sqrt2_positive.
    assert (Hinv2: (1/sqrt2)*(1/sqrt2) = 1/2).
    { rewrite <- Rinv_mult_distr; try lra. rewrite Hsq2. reflexivity. }
    nra.
  }
  rewrite <- Hexpr.
