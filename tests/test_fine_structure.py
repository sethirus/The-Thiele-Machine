...
def test_self_reference_density_not_trivial(n_bits: int) -> None:
    ...
    if valid == 0:
        # Fallback: if a curated corpus exists (generated by tools/generate_valid_programs.py),
        # use it instead of random sampling. This ensures the test can exercise the
        # self-reference density check in constrained environments.
        from pathlib import Path

        corpus_path = Path(__file__).parent / "data" / f"valid_programs_{n_bits}.txt"
        if corpus_path.exists():
            ...
            # Propagate fallback counts into the test variables
            self_ref = self_referential
            ratio = self_referential / valid if valid else 0.0
        else:
            pytest.skip(f"no valid programs encountered at length {n_bits} bits")
