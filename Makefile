...
.PHONY: assumption-receipt assumption-receipt-check
...
canonical-extract: install-hooks canonical-source-gate
	...
	@echo "[canonical-extract] Verifying byte-identical VM extraction..."
	@cmp build/thiele_core.ml build/thiele_core_complete.ml
	@echo "[canonical-extract] PASS: modular and complete VM extraction are byte-identical"
...
# Re-derive the full-corpus Print Assumptions receipt from source.
#
# This is the check behind the "0 project-local axioms" badge. Until this
# target existed the receipt was regenerated by nothing: the probe file is
# excluded from _CoqProject, no CI step invoked the probe scripts, and both
# generators hardcoded an absolute path to a single developer machine. The
# test that "checked" it compared committed JSON against committed README
# prose -- two files written together, not a re-derivation.
#
